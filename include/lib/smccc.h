...
#define SOC_ID_SET_JEP_106(bkid, mfid)                \
	((((bkid) << SOC_ID_JEP_106_BANK_IDX_SHIFT) & \
...
#define is_sve_hint_set(_f) \
	(((_f) & (FUNCID_SVE_HINT_MASK << FUNCID_SVE_HINT_SHIFT)) != U(0))
...
#define is_valid_fast_smc(_fid)           \
	((!(((_fid) >> 16) & U(0xff))) && \
	 (GET_SMC_TYPE(_fid) == (uint32_t)SMC_TYPE_FAST))
 ...
 */
#define DEFINE_SVC_UUID2(_name, _tl, _tm, _th, _cl, _ch, _n0, _n1, _n2, _n3, \
			 _n4, _n5)                                           \
	CASSERT((uint32_t)(_tl) != (uint32_t)SMC_UNK,                        \
		invalid_svc_uuid_##_name);                                   \
	static const uuid_t _name = {                                        \
		{ ((_tl) >> 24) & 0xFF, ((_tl) >> 16) & 0xFF,                \
		  ((_tl) >> 8) & 0xFF, ((_tl)&0xFF) },                       \
		{ ((_tm) >> 8) & 0xFF, ((_tm)&0xFF) },                       \
		{ ((_th) >> 8) & 0xFF, ((_th)&0xFF) },                       \
		(_cl),                                                       \
 ...
 */
static inline uint32_t smc_uuid_word(uint8_t b0, uint8_t b1, uint8_t b2,
				     uint8_t b3)
{
