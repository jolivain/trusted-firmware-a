...
#define PMF_DECLARE_CAPTURE_TIMESTAMP(_name)                 \
	void pmf_capture_timestamp_with_cache_maint_##_name( \
		unsigned int tid, unsigned long long ts);    \
	void pmf_capture_timestamp_##_name(unsigned int tid, \
...
#define PMF_CAPTURE_TIMESTAMP(_name, _tid, _flags)                             \
	do {                                                                   \
		...
		if (((_flags)&PMF_CACHE_MAINT) != 0U)                          \
			pmf_capture_timestamp_with_cache_maint_##_name((_tid), \
								       ts);    \
		else                                                           \
...
#define PMF_CAPTURE_AND_GET_TIMESTAMP(_name, _tid, _flags, _tsval)       \
	do {                                                             \
		...
		CASSERT(sizeof(_tsval) == sizeof(unsigned long long),    \
			invalid_tsval_size);                             \
		if (((_flags)&PMF_CACHE_MAINT) != 0U)                    \
			pmf_capture_timestamp_with_cache_maint_##_name(  \
				(_tid), (_tsval));                       \
		else                                                     \
...
#define PMF_WRITE_TIMESTAMP(_name, _tid, _flags, _wrval)                 \
	do {                                                             \
		CASSERT(sizeof(_wrval) == sizeof(unsigned long long),    \
			invalid_wrval_size);                             \
		if (((_flags)&PMF_CACHE_MAINT) != 0U)                    \
			pmf_capture_timestamp_with_cache_maint_##_name(  \
				(_tid), (_wrval));                       \
...
#define PMF_REGISTER_SERVICE_SMC(_name, _svcid, _totalid, _flags)             \
	...
	PMF_DEFINE_SERVICE_DESC(_name, PMF_ARM_TIF_IMPL_ID, _svcid, _totalid, \
				NULL, pmf_get_timestamp_by_mpidr_##_name)
 ...
 */
#define PMF_REGISTER_SERVICE_SMC_OWN(_name, _implid, _svcid, _totalid, _init, \
				     _getts)                                  \
	PMF_DEFINE_SERVICE_DESC(_name, _implid, _svcid, _totalid, _init, _getts)
...
#define PMF_ALLOCATE_TIMESTAMP_MEMORY(_name, _total_id)                    \
	...
	unsigned long long pmf_ts_mem_##_name[_total_id] __aligned(        \
		CACHE_WRITEBACK_GRANULE) __section(".pmf_timestamp_array") \
		__used;
...
#define PMF_DEFINE_CAPTURE_TIMESTAMP(_name, _flags)                          \
	...
	{                                                                    \
		...
		if (((_flags)&PMF_STORE_ENABLE) != 0)                        \
			__pmf_store_timestamp(base_addr, (uint64_t)tid, ts); \
...
#define PMF_DEFINE_GET_TIMESTAMP(_name)                                        \
	...
	{                                                                      \
		PMF_VALIDATE_TID(_name, tid);                                  \
		uintptr_t base_addr = (uintptr_t)pmf_ts_mem_##_name;           \
		return __pmf_get_timestamp(                                    \
			base_addr, tid, plat_core_pos_by_mpidr(mpidr), flags); \
	}
 ...
 */
#define PMF_DEFINE_SERVICE_DESC(_name, _implid, _svcid, _totalid, _init,      \
				_getts_by_mpidr)                              \
	static const pmf_svc_desc_t __pmf_desc_##_name __section(             \
		".pmf_svc_descs") __used = {                                  \
		.h.type = PARAM_EP,                                           \
		.h.version = VERSION_1,                                       \
		...
		.name = #_name,                                               \
		.svc_config =                                                 \
			((((_implid) << PMF_IMPL_ID_SHIFT) &                  \
			  PMF_IMPL_ID_MASK) |                                 \
			 (((_svcid) << PMF_SVC_ID_SHIFT) & PMF_SVC_ID_MASK) | \
			 (((_totalid) << PMF_TID_SHIFT) & PMF_TID_MASK)),     \
		.init = _init,                                                \
