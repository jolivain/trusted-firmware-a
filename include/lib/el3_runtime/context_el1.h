...
typedef struct el1_aarch32_regs {
	...
	uint64_t spsr_fiq;
	uint64_t dacr32_el2;
	uint64_t ifscr32_el2;
} el1_aarch32_regs_t;

typedef struct el1_ns_timer_regs {
...
#if NS_TIMER_SWITCH
#define read_el1_ctx_ns_timer(ctx, reg)		(((ctx)->ns_timer).reg)
#define write_el1_ctx_ns_timer(ctx, reg, val)	((((ctx)->ns_timer).reg)	\
							= (uint64_t) (val))
#else
#define read_el1_ctx_ns_timer(ctx, reg)		ULL(0)
#define write_el1_ctx_ns_timer(ctx, reg, val)
