...
#define CTX_TIMER_SYSREGS_END \
	(CTX_AARCH32_END + U(0x30)) /* Align to the next 16 byte boundary */
#else
...
#define write_ctx_reg(ctx, offset, val) \
	(((ctx)->ctx_regs[(offset) >> DWORD_SHIFT]) = (uint64_t)(val))
...
CASSERT(CTX_EL1_SYSREGS_OFFSET ==
		__builtin_offsetof(cpu_context_t, el1_sysregs_ctx),
	assert_core_context_el1_sys_offset_mismatch);
#if CTX_INCLUDE_EL2_REGS
CASSERT(CTX_EL2_SYSREGS_OFFSET ==
		__builtin_offsetof(cpu_context_t, el2_sysregs_ctx),
...
CASSERT(CTX_CVE_2018_3639_OFFSET ==
		__builtin_offsetof(cpu_context_t, cve_2018_3639_ctx),
