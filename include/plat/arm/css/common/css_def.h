...
#define CSS_G1S_IRQ_PROPS(grp)                                                 \
	INTR_PROP_DESC(CSS_IRQ_MHU, GIC_HIGHEST_SEC_PRIORITY, grp,             \
		       GIC_INTR_CFG_LEVEL),                                    \
		INTR_PROP_DESC(CSS_IRQ_GPU_SMMU_0, GIC_HIGHEST_SEC_PRIORITY,   \
 ...
 */
#define CSS_SCMI_PAYLOAD_BASE \
	(PLAT_ARM_SDS_MEM_BASE + PLAT_ARM_SDS_MEM_SIZE_MAX)
#define CSS_SCMI_MHU_DB_REG_OFF MHU_CPU_INTR_S_SET_OFFSET

/* Trusted mailbox base address common to all CSS */
/* If SDS is present, then mailbox is at top of SRAM */
#define PLAT_ARM_TRUSTED_MAILBOX_BASE \
	(ARM_SHARED_RAM_BASE + ARM_SHARED_RAM_SIZE - 0x8)
...
#define CSS_MAP_DEVICE                                    \
	MAP_REGION_FLAT(CSS_DEVICE_BASE, CSS_DEVICE_SIZE, \
			MT_DEVICE | MT_RW | MT_SECURE)

#define CSS_MAP_NSRAM \
	MAP_REGION_FLAT(NSRAM_BASE, NSRAM_SIZE, MT_DEVICE | MT_RW | MT_NS)

#if defined(IMAGE_BL2U)
#define CSS_MAP_SCP_BL2U                                               \
	MAP_REGION_FLAT(SCP_BL2U_BASE, SCP_BL2U_LIMIT - SCP_BL2U_BASE, \
...
#define GET_SSC_VERSION_PART_NUM(val) \
	(((val) >> SSC_VERSION_PART_NUM_SHIFT) & SSC_VERSION_PART_NUM_MASK)

/* Returns the configuration number of the platform */
#define GET_SSC_VERSION_CONFIG(val) \
	(((val) >> SSC_VERSION_CONFIG_SHIFT) & SSC_VERSION_CONFIG_MASK)
