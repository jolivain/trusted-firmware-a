...
{
	if (ENABLE_FEAT_PMUV3 == FEAT_STATE_DISABLED) {
		return false;
	}

	if (ENABLE_FEAT_PMUV3 == FEAT_STATE_ALWAYS) {
		return true;
	}

	unsigned int pmu = read_feat_pmuv3_id_field();

	return pmu != 0U && pmu != ID_AA64DFR0_PMUVER_IMP_DEF;
}
