...
#define MM_VERSION_FORM(major, minor) \
	((major << MM_VERSION_MAJOR_SHIFT) | (minor))
...
#define SPM_MM_VERSION_FORM(major, minor) \
	((major << SPM_MM_VERSION_MAJOR_SHIFT) | (minor))
#define SPM_MM_VERSION_COMPILED \
	SPM_MM_VERSION_FORM(SPM_MM_VERSION_MAJOR, SPM_MM_VERSION_MINOR)
...
uint64_t spm_mm_sp_call(uint32_t smc_fid, uint64_t x1, uint64_t x2,
			uint64_t x3);
