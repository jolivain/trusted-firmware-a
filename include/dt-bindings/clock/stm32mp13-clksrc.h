...
#define DIV(div_id, div) \
	((CMD_DIV << CMD_SHIFT) | ((div_id) << DIV_ID_SHIFT | (div)))

#define CLKSRC(mux_id, sel) \
	((CMD_MUX << CMD_SHIFT) | ((mux_id) << MUX_ID_SHIFT | (sel)))

/* MCO output is enable */
#define MCO_SRC(mco_id, sel)                                             \
	((CMD_CLK << CMD_SHIFT) | (((mco_id) << CLK_ID_SHIFT) | (sel)) | \
	 CLK_ON_MASK)

#define MCO_DISABLED(mco_id) \
	((CMD_CLK << CMD_SHIFT) | ((mco_id) << CLK_ID_SHIFT))

/* CLK output is enable */
#define CLK_SRC(clk_id, sel)                                             \
	((CMD_CLK << CMD_SHIFT) | (((clk_id) << CLK_ID_SHIFT) | (sel)) | \
	 CLK_ON_MASK)

#define CLK_DISABLED(clk_id) \
	((CMD_CLK << CMD_SHIFT) | ((clk_id) << CLK_ID_SHIFT))
