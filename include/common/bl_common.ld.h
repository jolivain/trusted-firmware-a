...
#define BASE_XLAT_TABLE               \
	...
	*(.base_xlat_table)__BASE_XLAT_TABLE_END__ =.;
...
#define RODATA_COMMON    \
	...
	PARSER_LIB_DESCS \
	CPU_OPS          \
	GOT BASE_XLAT_TABLE_RO EL3_LP_DESCS
...
#define STACK_SECTION                                    \
	...
	{                                                \
		__STACKS_START__ =.;                     \
		*(.tzfw_normal_stacks)__STACKS_END__ =.; \
	}
#endif
...
#define BAKERY_LOCK_NORMAL                                                    \
	...
	*(.bakery_lock).= ALIGN(CACHE_WRITEBACK_GRANULE);                     \
	__PERCPU_BAKERY_LOCK_END__ =.;                                        \
	__PERCPU_BAKERY_LOCK_SIZE__ = ABSOLUTE(__PERCPU_BAKERY_LOCK_END__ -   \
					       __PERCPU_BAKERY_LOCK_START__); \
...
#define PMF_TIMESTAMP                                                     \
	...
	KEEP(*(.pmf_timestamp_array)).= ALIGN(CACHE_WRITEBACK_GRANULE);   \
...
#define BSS_SECTION                                                 \
	...
	{                                                           \
		...
		*(SORT_BY_ALIGNMENT(.bss *)) *                      \
			(COMMON)BAKERY_LOCK_NORMAL PMF_TIMESTAMP    \
				BASE_XLAT_TABLE_BSS __BSS_END__ =.; \
	}
