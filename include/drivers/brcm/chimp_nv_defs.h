...
#define BYTE_SWAP_16(x) \
	((((u16_t)(x)&0xff00) >> 8) | (((u16_t)(x)&0x00ff) << 8))
#define BYTE_SWAP_32(x)                                                     \
	((((u32_t)(x)&0xff000000) >> 24) | (((u32_t)(x)&0x00ff0000) >> 8) | \
	 (((u32_t)(x)&0x0000ff00) << 8) | (((u32_t)(x)&0x000000ff) << 24))

/* auto-detect integer size */
#define BYTE_SWAP_INT(x)                             \
	(SIZEOF_IN_BITS(x) == 16 ? BYTE_SWAP_16(x) : \
	 SIZEOF_IN_BITS(x) == 32 ? BYTE_SWAP_32(x) : \
				   (x))
...
#define BNX_DIR_TYPE_IS_APE_BIN_FMT(type)                                      \
	((type) == BNX_DIR_TYPE_CHIMP_PATCH ||                                 \
	 ...
	 (type) == BNX_DIR_TYPE_BOOTCODE_2 || (type) == BNX_DIR_TYPE_APE_FW || \
	 (type) == BNX_DIR_TYPE_APE_PATCH || (type) == BNX_DIR_TYPE_TANG_FW || \
	 (type) == BNX_DIR_TYPE_TANG_PATCH ||                                  \
	 (type) == BNX_DIR_TYPE_KONG_FW ||                                     \
	 (type) == BNX_DIR_TYPE_KONG_PATCH ||                                  \
	 (type) == BNX_DIR_TYPE_BONO_FW || (type) == BNX_DIR_TYPE_BONO_PATCH)

/* Other (non APE BIN) executable NVM items are identified with
 * the following macro:
 */
#define BNX_DIR_TYPE_IS_OTHER_EXEC(type)                                     \
	((type) == BNX_DIR_TYPE_AVS || (type) == BNX_DIR_TYPE_EXP_ROM_MBA || \
	 (type) == BNX_DIR_TYPE_PCIE || (type) == BNX_DIR_TYPE_TSCF_UCODE || \
	 (type) == BNX_DIR_TYPE_EXT_PHY || (type) == BNX_DIR_TYPE_CCM ||     \
...
#define BNX_DIR_TYPE_IS_EXECUTABLE(type) \
	(BNX_DIR_TYPE_IS_APE_BIN_FMT(type) || BNX_DIR_TYPE_IS_OTHER_EXEC(type))
