...
struct sd_ctrl_info {
	...
	size_t sdRegBaseAddr;	/* sdio control registers */
	size_t hostRegBaseAddr;	/* SD Host control registers */
};
...
int32_t chal_sd_start(CHAL_HANDLE *sdHandle, int32_t mode,
...
int32_t chal_sd_stop(CHAL_HANDLE *sdHandle);
int32_t chal_sd_set_dma(CHAL_HANDLE *sdHandle, int32_t mode);
...
int32_t chal_sd_get_present_status(CHAL_HANDLE *sdHandle);
int32_t chal_sd_get_atuo12_error(CHAL_HANDLE *sdHandle);
void chal_sd_set_speed(CHAL_HANDLE *sdHandle, uint32_t speed);
uint32_t chal_sd_check_cap(CHAL_HANDLE *sdHandle, uint32_t cap);
