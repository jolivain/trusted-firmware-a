/*
 * Copyright (c) 22019, Arm Limited and Contributors. All rights reserved.
 *
...
/*******************************************************************************
 * GICv3.x miscellaneous definitions
 ******************************************************************************/
...
/* GICv3.x can only target up to 16 PEs with SGI */
#define GICV3X_MAX_SGI_TARGETS	U(16)

/* PPIs INTIDs 16-31 */
#define MAX_PPI_ID		U(31)

/* Extended PPIs INTIDs 1056-1119 */
#define MIN_EPPI_ID		U(1056)
#define MAX_EPPI_ID		U(1119)
#define TOTAL_EPPI_INTR_NUM	(MAX_EPPI_ID - MIN_EPPI_ID + U(1))

/* Total number of local interrupts */
#define TOTAL_LOCAL_INTR_NUM	(TOTAL_PCPU_INTR_NUM + TOTAL_EPPI_INTR_NUM)

/* Extended SPIs INTIDs 4096 - 5119 */
#define MIN_ESPI_ID		U(4096)
#define MAX_ESPI_ID		U(5119)
#define TOTAL_ESPI_INTR_NUM	(MAX_ESPI_ID - MIN_ESPI_ID + U(1))

/* Total number of Shared Peripheral Interrupts SPIs and ESPIs */
...
#define GICR_NUM_REGS(reg_name)	\
	DIV_ROUND_UP_2EVAL(TOTAL_LOCAL_INTR_NUM, (1 << reg_name##_SHIFT))

/* Interrupt ID mask for HPPIR, AHPPIR, IAR and AIAR CPU Interface registers */
#define INT_ID_MASK	U(0xffffff)

/******************************************************************************
 * GICv3.x specific Distributor interface register offsets and constants
 *****************************************************************************/
...
/*
 * GICD_IROUTER<n> register is at 0x6000 + 8*n, where n is the interrupt ID
...
#define GICV3X_IRM_PE		U(0)
#define GICV3X_IRM_ANY		U(1)
...
/*******************************************************************************
 * GICv3.x Redistributor interface registers & constants
 ******************************************************************************/
#define GICR_PCPUBASE_SHIFT_V3	U(0x11)		/* 128k for GICv3 */
#define GICR_PCPUBASE_SHIFT_V4	U(0x12)		/* 256k for GICv4 */
...
#define GICR_IGROUPR		(GICR_SGIBASE_OFFSET + U(0x80))
#define GICR_ISENABLER		(GICR_SGIBASE_OFFSET + U(0x100))
#define GICR_ICENABLER		(GICR_SGIBASE_OFFSET + U(0x180))
#define GICR_ISPENDR		(GICR_SGIBASE_OFFSET + U(0x200))
#define GICR_ICPENDR		(GICR_SGIBASE_OFFSET + U(0x280))
#define GICR_ISACTIVER		(GICR_SGIBASE_OFFSET + U(0x300))
#define GICR_ICACTIVER		(GICR_SGIBASE_OFFSET + U(0x380))
#define GICR_IPRIORITYR		(GICR_SGIBASE_OFFSET + U(0x400))
#define GICR_ICFGR		(GICR_SGIBASE_OFFSET + U(0xc00))
#define GICR_IGRPMODR		(GICR_SGIBASE_OFFSET + U(0xd00))
...
/*******************************************************************************
 * GICv3.x CPU interface registers & constants
 ******************************************************************************/
...
#define GICV3X_SGIR_VALUE(_aff3, _aff2, _aff1, _intid, _irm, _tgt)	\
...
/******************************************************************************
 * GICv3.x ITS registers and constants
 *****************************************************************************/
...
#define GICD_READ_64(REG, base, id)			\
	do {						\
		GICD_OFFSET_64(REG, id);		\
		return mmio_read_64(base + offset);	\
	} while (false)

/* Read 32-bit GIC Distributor register corresponding to its number */
#define	GICD_READ_NUM(REG, base, reg_num)		\
	mmio_read_32(base + GICD_##REG + (reg_num << 2))

/* Write to 64-bit GIC Distributor register corresponding to its interrupt ID */
#define GICD_WRITE_64(REG, base, id, val)		\
	do {						\
		GICD_OFFSET_64(REG, id);		\
		mmio_write_64(base + offset, val);	\
	} while (false)
...
/* Write to 32-bit GIC Distributor register corresponding to its number */
#define	GICD_WRITE_NUM(REG, base, reg_num, val)				\
	mmio_write_32(base + GICD_##REG + (reg_num << 2), val)

/* Write to 8-bit GIC Distributor register corresponding to its interrupt ID */
#define GICD_WRITE_8(REG, base, id, val)		\
	do {						\
		GICD_OFFSET_8(REG, id);			\
		mmio_write_8(base + offset, val);	\
	} while (false)
...
#define GICD_GET_BIT(REG, base, id)					\
	do {								\
		CALC_BIT_NUM(REG, id);					\
		GICD_OFFSET(REG, id);					\
		return (mmio_read_32(base + offset) >> bit_num) & 1U;	\
	} while (false)
...
#define GICD_SET_BIT(REG, base, id)				\
	do {							\
		CALC_BIT_NUM(REG, id);				\
		GICD_OFFSET(REG, id);				\
		mmio_setbits_32(base + offset, 1U << bit_num);	\
	} while (false)
...
#define GICD_CLR_BIT(REG, base, id)				\
	do {							\
		CALC_BIT_NUM(REG, id);				\
		GICD_OFFSET(REG, id);				\
		mmio_clrbits_32(base + offset, 1U << bit_num);	\
	} while (false)

/* Clear and set bits in GIC Distributor register */
#define GICD_CLR_SET_BIT(REG, base, id, mask, val)		\
	do {							\
		CALC_BIT_NUM(REG, id);				\
		GICD_OFFSET(REG, id);				\
		unsigned int bit_shift = bit_num << 1;		\
		uint32_t clear = mask << bit_shift;		\
		uint32_t set = (val & mask) << bit_shift;	\
		mmio_clrsetbits_32(base + offset, clear, set);	\
	} while (false)
...
#define	GICD_WRITE_BIT(REG, base, id)				\
	do {							\
		CALC_BIT_NUM(REG, id);				\
		GICD_OFFSET(REG, id);				\
		mmio_write_32(base + offset, 1U << bit_num);	\
	} while (false)

/*
 * Calculate GICv3.x GICR register offset for (E)PPIs
 * by converting EPPI INTIDs from 1056-1119 to 32-95.
 */
#define GICR_OFFSET(REG, id)					\
	if (id >= MIN_EPPI_ID)					\
		id -= MIN_EPPI_ID - MIN_SPI_ID;			\
		unsigned int offset = GICR_##REG + ((id >> REG##_SHIFT) << 2)

/* Read GIC Redistributor register corresponding to its number */
#define	GICR_READ_NUM(REG, base, reg_num)			\
	return mmio_read_32(base + GICR_##REG + (reg_num << 2))

/* Write to GIC Redistributor register corresponding to its number */
#define	GICR_WRITE_NUM(REG, base, reg_num, val)			\
	mmio_write_32(base + GICR_##REG + (reg_num << 2), val)
...
/* Clear and set bits in GIC Redistributor register */
#define GICR_CLR_SET_BIT(REG, base, id, mask, val)		\
	do {							\
		CALC_BIT_NUM(REG, id);				\
		GICR_OFFSET(REG, id);				\
		unsigned int bit_shift = bit_num << 1;		\
...
static inline bool gicv3x_is_intr_id_special_identifier(unsigned int id)
{
	return (id >= PENDING_G1S_INTID) && (id <= GIC_SPURIOUS_INTERRUPT);
}

/*******************************************************************************
 * Helper GICv3.x macros for SEL1
 ******************************************************************************/
static inline uint32_t gicv3x_acknowledge_interrupt_sel1(void)
{
	return read_icc_iar1_el1() & IAR1_EL1_INTID_MASK;
}

static inline uint32_t gicv3x_get_pending_interrupt_id_sel1(void)
{
	return read_icc_hppir1_el1() & HPPIR1_EL1_INTID_MASK;
}

static inline void gicv3x_end_of_interrupt_sel1(unsigned int id)
{
 ...
 ******************************************************************************/
static inline uint32_t gicv3x_acknowledge_interrupt(void)
{
	return read_icc_iar0_el1() & IAR0_EL1_INTID_MASK;
}

static inline void gicv3x_end_of_interrupt(unsigned int id)
{
 ...
 * the GICv3.x IP. It is used by the platform port to specify these attributes
 * in order to initialise the GICv3.x driver. The attributes are described
 * below.
 *
 ...
 *
 * The 'rdistif_base_addrs' field is a pointer to an array that has an entry
 * for storing the base address of the Redistributor interface frame of each
 * CPU in the system. The size of the array = 'rdistif_num'. The base addresses
 * are detected during driver initialisation.
 *
 ...
 * optional field. A GICv3.x implementation maps each MPIDR to a linear core
 * index as well. This mapping can be found by reading the "Affinity Value" and
 ...
 * hash function. Otherwise, the "Processor Number" field will be used to
 * access the array elements.
 *****************************************************************************/
typedef unsigned int (*mpidr_hash_fn)(u_register_t mpidr);

typedef struct gicv3x_driver_data {
	uintptr_t gicd_base;
	uintptr_t gicr_base;
...
} gicv3x_driver_data_t;

typedef struct gicv3x_redist_ctx {
	/* 64 bits registers */
...
typedef struct gicv3x_dist_ctx {
	/* 64 bits registers */
...
/*******************************************************************************
 * GICv3.x EL3 driver API
 ******************************************************************************/
void gicv3x_driver_init(const gicv3x_driver_data_t *plat_driver_data);
void gicv3x_distif_init(void);
...
/*
 * gicv3x_distif_post_restore and gicv3x_distif_pre_save must be implemented
 * if gicv3x_distif_save and gicv3x_rdistif_init_restore are used.
 * If no implementation-defined sequence is needed at these steps,
 * an empty function can be provided.
 */
...
unsigned int gicv3x_get_interrupt_active(unsigned int id,
