...
typedef enum { CC_FALSE = 0, CC_TRUE = 1 } CCBool;
...
#define CALC_FULL_BYTES(numBits) \
	(((numBits) + (CC_BITS_IN_BYTE - 1)) / CC_BITS_IN_BYTE)
#define CALC_FULL_32BIT_WORDS(numBits) \
	(((numBits) + (CC_BITS_IN_32BIT_WORD - 1)) / CC_BITS_IN_32BIT_WRD)
#define CALC_32BIT_WORDS_FROM_BYTES(sizeBytes) \
	(((sizeBytes) + CC_32BIT_WORD_SIZE - 1) / CC_32BIT_WORD_SIZE)
