...
#define BSV_RSA_WORKSPACE_MIN_SIZE            \
	(4 * BSV_CERT_RSA_KEY_SIZE_IN_BYTES + \
...
CCError_t CC_BsvRsaPrimVerify(
	...
	CCBsvNpBuff_t
		NpBuff, /*!< [in] The barret tag buffer big endian format - optional. */
	...
	CCBsvSignature_t
		pOutBuff, /*!< [out] The encrypted buffer in big endian format. */
...
CCError_t CC_BsvRsaPssVerify(
	...
	CCBsvNpBuff_t
		NpBuff, /*!< [in] The barret tag buffer big endian format - optional. */
	CCBsvSignature_t
		signature, /*!< [in] The signature buffer to verify - big endian format. */
	CCHashResult_t
		hashedData, /*!< [in] The data-in buffer to be verified as sha256 digest. */
	uint32_t *pWorkSpace, /*!< [in] The pointer to user allocated buffer for internal use. */
	size_t workBufferSize, /*!< [in] The size in bytes of pWorkSpace, must be at-least BSV_RSA_WORKSPACE_MIN_SIZE. */
