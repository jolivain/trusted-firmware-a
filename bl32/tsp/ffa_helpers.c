...
smc_args_t ffa_msg_send_direct_req(ffa_endpoint_id16_t sender,
				   ffa_endpoint_id16_t receiver, uint32_t arg3,
				   ...
				   uint32_t arg7)
{
	...
	/* Send Direct Request. */
	return smc_helper(FFA_MSG_SEND_DIRECT_REQ_SMC64, src_dst_ids, 0, arg3,
			  arg4, arg5, arg6, arg7);
}
				     ...
				     uint32_t arg7)
{
	...
	return set_smc_args(FFA_MSG_SEND_DIRECT_RESP_SMC64, src_dst_ids, 0,
...
static void ffa_memory_region_init_header(
	...
	ffa_mem_attr16_t attributes, ffa_mtd_flag32_t flags, uint64_t handle,
	uint64_t tag, ffa_endpoint_id16_t *receivers, uint32_t receiver_count,
	ffa_mem_perm8_t permissions)
{
	...
	emad = (struct ffa_emad_v1_0 *)((uint8_t *)memory_region +
...
static uint32_t ffa_memory_retrieve_request_init(
	...
	ffa_endpoint_id16_t sender, ffa_endpoint_id16_t *receivers,
	uint32_t receiver_count, uint64_t tag, ffa_mtd_flag32_t flags,
...
{
	return smc_helper(FFA_MEM_RETRIEVE_REQ_SMC32, descriptor_length,
			  fragment_length, 0, 0, 0, 0, 0);
}

/* Retrieve the next memory descriptor fragment. */
smc_args_t ffa_mem_frag_rx(uint64_t handle, uint32_t recv_length)
{
	return smc_helper(FFA_MEM_FRAG_RX, FFA_MEM_HANDLE_LOW(handle),
			  FFA_MEM_HANDLE_HIGH(handle), recv_length, 0, 0, 0, 0);
}

bool memory_retrieve(struct mailbox *mb, struct ffa_mtd **retrieved,
		     ...
		     uint32_t *total_length)
{
	...
	descriptor_size = ffa_memory_retrieve_request_init(
		memory_region, handle, sender, receivers, receiver_count, 0,
		flags, FFA_MEM_PERM_RW | FFA_MEM_PERM_NX,
	...
	*frag_length = ret._regs[2];
	...
	if (*frag_length == 0U || *total_length == 0U ||
	    *frag_length > *total_length ||
	    *frag_length > (mb->rxtx_page_count * PAGE_SIZE)) {
		ERROR("Invalid parameters!\n");
		return false;
	}
	...
	*retrieved = (struct ffa_mtd *)mem_region_buffer;
	 ...
	 */
	if (((*retrieved)->memory_region_attributes & FFA_MEM_ATTR_NS_BIT) ==
	    0U) {
