...
#if SPMC_AT_EL3
tsp_args_t *send_ffa_pm_success(void)
{
	uint32_t linear_id = plat_my_core_pos();
	unsigned int tsp_id;
	tsp_args_t smc_args = {0};
...
/*******************************************************************************
 * Helper function to swap source and destination partition IDs.
 ******************************************************************************/
...
bool memory_retrieve(struct mailbox *mb,
		     ...
		     ffa_id_t sender, ffa_id_t *test_receivers,
		     int receiver_count, uint32_t flags, uint32_t *frag_length,
				     ...
				     uint64_t arg7)
{
	...
	} else if ((arg2 & FFA_PM_MSG_MASK) == FFA_PM_MSG_WB_REQ) {
