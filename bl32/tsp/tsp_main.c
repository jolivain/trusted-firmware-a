...
#endif
#if SPMC_AT_EL3
	return (uint64_t) set_smc_args(FFA_MSG_WAIT, 0, 0, 0, 0, 0, 0, 0);
#else
 ...
 ******************************************************************************/
static tsp_args_t *ffa_msg_send_direct_resp(uint16_t sender,
					    uint16_t receiver,
					    uint32_t arg3,
					    uint32_t arg4,
					    uint32_t arg5,
 ...
 ******************************************************************************/
static tsp_args_t ffa_msg_send_direct_req(uint16_t sender,
					  uint16_t receiver,
					  uint32_t arg3,
					  uint32_t arg4,
					  uint32_t arg5,
...
static bool memory_retrieve(struct mailbox *mb,
			    ...
			    ffa_id_t *test_receivers, int receiver_count,
		     	    ...
		     	    uint32_t *total_length)
{
	...
	descriptor_size = ffa_memory_retrieve_request_init(
	    mb->send, handle, sender, test_receivers, receiver_count, 0, flags,
	    FFA_DATA_ACCESS_RW,
 ...
 * it to echo the value back in order to validate direct messages functionlaity.
 ******************************************************************************/
 ...
 ******************************************************************************/
static int test_memory_send(uint16_t sender, uint64_t handle,
			    bool share, bool multi_endpoint)
{
	...
	ffa_id_t test_receivers[2] = { partition_id, 0x10 };
	...
	while (total_length != recv_length) {
		...
		memcpy(&mem_region_buffer[recv_length], mailbox.recv,
		       frag_length);
	...
	/* Only expecting to be sent memory from NWd so map accordinly. */
	mem_attrs |= MT_NS;
	...
	if (!memory_relinquish((struct ffa_mem_relinquish *)mailbox.send,
				m->handle, test_receivers, receiver_count)) {
			   ...
			   uint64_t arg7)
{
	...
	case FFA_MSG_SEND_DIRECT_REQ_SMC32:
		...
		if ((arg2 & FFA_FWK_MSG_BIT)) {
			return handle_framework_message(arg0, arg1, arg2, arg3,
							arg4, arg5, arg6, arg7);
		} else {
			return handle_partition_message(arg0, arg1, arg2, arg3,
							arg4, arg5, arg6, arg7);
		}
	default:
	...
	INFO("%s: Unsupported FF-A FID (0x%lx)\n", __func__, smc_fid);
	panic();
}
#endif /* SPMC_AT_EL3*/
