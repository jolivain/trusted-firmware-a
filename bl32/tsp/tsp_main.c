...
int test_memory_send(uint16_t sender, uint64_t handle, bool share, bool multi_endpoint)
{
	...
	/*
	 * In the case that we're testing multiple endpoints choose a partition ID
	 * that resides in the normal world so the SPMC won't detect it as invalid.
	 */
	int receiver_count = multi_endpoint ? 2 : 1;
	ffa_id_t receivers[2] = { partition_id, 0x10 };

	memory_retrieve(&mailbox, &m, handle, source, receivers, receiver_count, flags,
