 ...
 ******************************************************************************/
static int ffa_test_relay(uint64_t arg0, uint64_t arg1, uint64_t arg2,
			  ...
			  uint64_t arg6, uint64_t arg7)
{
	...
	ffa_forward_result =
		ffa_msg_send_direct_req(ffa_endpoint_source(arg1), receiver,
					FF_A_ECHO_MESSAGE, arg4, 0, 0, 0);
			    ...
			    ffa_mtd_flag32_t flags, bool multi_endpoint)
{
	...
	receivers = (struct ffa_emad_v1_0 *)((uint8_t *)m + m->emad_offset);
	...
	if (ffa_get_data_access_attr(
		    receivers[0].mapd.memory_access_permissions) !=
	    FFA_MEM_PERM_RW) {
	...
	for (int32_t i = 0; i < (int32_t)composite->address_range_count; i++) {
		size_t size = composite->address_range_array[i].page_count *
			      PAGE_SIZE;

		ptr = (char *)composite->address_range_array[i].address;
		ret = mmap_add_dynamic_region((uint64_t)ptr, (uint64_t)ptr,
					      size, mem_attrs);

		if (ret != 0) {
			ERROR("Failed [%d] mmap_add_dynamic_region %u (%lx) (%lx) (%x)!\n",
			      i, ret,
			      (uint64_t)composite->address_range_array[i]
				      .address,
			      size, mem_attrs);
			...
			for (i--; i >= 0; i--) {
				ret = mmap_remove_dynamic_region(
					(uint64_t)composite
						->address_range_array[i]
						.address,
					composite->address_range_array[i]
							.page_count *
						PAGE_SIZE);
	...
	for (uint32_t i = 0U; i < composite->address_range_count; i++) {
		ret = mmap_remove_dynamic_region(
			...
			composite->address_range_array[i].page_count *
				PAGE_SIZE);
	...
	if (!memory_relinquish(
		    (struct ffa_mem_relinquish_descriptor *)mailbox.tx_buffer,
		    m->handle, tsp_id)) {
 ...
 ******************************************************************************/
smc_args_t *tsp_cpu_off_main(uint64_t arg0, uint64_t arg1, uint64_t arg2,
			     uint64_t arg3, uint64_t arg4, uint64_t arg5,
			     uint64_t arg6, uint64_t arg7)
{
	...
	VERBOSE("TSP: cpu 0x%lx: %d smcs, %d erets %d cpu off requests\n",
		read_mpidr(), tsp_stats[linear_id].smc_count,
		tsp_stats[linear_id].eret_count,
		tsp_stats[linear_id].cpu_off_count);
 ...
 * cpu_suspend request.
 ******************************************************************************/
smc_args_t *tsp_cpu_suspend_main(uint64_t arg0, uint64_t arg1, uint64_t arg2,
				 uint64_t arg3, uint64_t arg4, uint64_t arg5,
				 uint64_t arg6, uint64_t arg7)
{
	uint32_t linear_id = plat_my_core_pos();
 ...
 * cpu_suspend request.
 ******************************************************************************/
smc_args_t *tsp_cpu_resume_main(uint64_t max_off_pwrlvl, uint64_t arg1,
 ...
 ******************************************************************************/
static smc_args_t *handle_framework_message(uint64_t arg0, uint64_t arg1,
					    ...
					    uint64_t arg6, uint64_t arg7)
{
	...
	if ((arg2 & FFA_FWK_MSG_MASK) == FFA_FWK_MSG_PSCI) {
		...
		if (arg3 == PSCI_CPU_OFF) {
			return tsp_cpu_off_main(arg0, arg1, arg2, arg3, arg4,
						arg5, arg6, arg7);
		...
		if (arg3 == FFA_WB_TYPE_NOTS2RAM) {
			return tsp_cpu_resume_main(arg0, arg1, arg2, arg3, arg4,
						   arg5, arg6, arg7);
		}
	}
 ...
 ******************************************************************************/
static smc_args_t *handle_partition_message(uint64_t arg0, uint64_t arg1,
					    uint64_t arg2, uint64_t arg3,
					    uint64_t arg4, uint64_t arg5,
					    uint64_t arg6, uint64_t arg7)
{
	...
	case FF_A_MEMORY_SHARE:
		...
		status = test_memory_send(sender, arg4, FFA_FLAG_SHARE_MEMORY,
					  !multi_endpoint);
		break;

	case FF_A_MEMORY_LEND:
		INFO("TSP Tests: Memory Lend Request--\n");
		status = test_memory_send(sender, arg4, FFA_FLAG_LEND_MEMORY,
					  !multi_endpoint);
		break;
	...
	case FF_A_RELAY_MESSAGE:
		...
		status = ffa_test_relay(arg0, arg1, arg2, arg3, arg4, arg5,
					arg6, arg7);
		break;
 ...
 ******************************************************************************/
static smc_args_t *tsp_event_loop(uint64_t smc_fid, uint64_t arg1,
				  uint64_t arg2, uint64_t arg3, uint64_t arg4,
				  uint64_t arg5, uint64_t arg6, uint64_t arg7)
{
	...
	case FFA_MSG_SEND_DIRECT_REQ_SMC32:
		...
		return handle_partition_message(smc_fid, arg1, arg2, arg3, arg4,
						arg5, arg6, arg7);
	}
...
uint64_t tsp_main(void)
{
	...
	smc_args = smc_helper(FFA_SECONDARY_EP_REGISTER_SMC64,
			      (uint64_t)tsp_cpu_on_entry, 0, 0, 0, 0, 0, 0);
