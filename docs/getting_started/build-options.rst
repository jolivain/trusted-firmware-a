...
-  ``SPD``: Choose a Secure Payload Dispatcher component to be built into TF-A.
   ...
   services/std_svc/spmd and enabled by ``SPD=spmd``. It is exclusive to the
   usage of the ``SPM_MM`` option.
...
-  ``SPM_MM`` : Boolean option to enable the Management Mode (MM)-based Secure
   Partition Manager (SPM) implementation. The default value is ``0``. This
   option is exclusive to the usage of the SPM Dispatcher (``SPD=spmd``).
...
-  ``SP_LAYOUT_FILE``: Platform provided path to JSON file containing the
   ...
   part of TF-A tree. Only available when ``SPD=spmd``.
