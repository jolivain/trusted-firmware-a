...
-  ``E``: Boolean option to make warnings into errors. Default is 1.
   ...
   defaults to 0. This is done to encourage contributors to enable them, as they
   are expected to produce (quite a few for some) warnings that would otherwise
   fail the build. New contributions are expected to still build with ``W=0``
   and ``E=1`` (the default).
...
-  ``W``: Warning level. Some compiler warning options of interest have been
   ...
   - ``W=1``
     ...
     consuming to fix at the moment. It re-enable warnings taken out for ``W=0``
     builds (a few of the ``-Wextra`` additions). This level is expected to
     eventually be merged into ``W=0``. Some warnings are expected on some
   ...
   refer to the GCC or Clang documentation for more information on the
   individual flags: https://gcc.gnu.org/onlinedocs/gcc/Warning-Options.html and
