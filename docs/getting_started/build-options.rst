...
- ``ENABLE_TRBE_FOR_NS``: This flag is used to enable access of trace buffer
  control registers from NS ELs, NS-EL2 and NS-EL1(when NS-EL2 is implemented
...
- ``ENABLE_SYS_REG_TRACE_FOR_NS``: Boolean option to enable trace system
  registers access from NS ELs, NS-EL2 and NS-EL1 (when NS-EL2 is implemented
...
- ``ENABLE_TRF_FOR_NS``: Boolean option to enable trace filter control registers
  access from NS ELs, NS-EL2 and NS-EL1 (when NS-EL2 is implemented but unused),
  if FEAT_TRF is implemented. This flag is enabled by default.
