...
-  ``ENABLE_FEAT_HCX``: This option sets the bit SCR_EL3.HXEn in EL3 to allow
   ...
   adding HCRX_EL2 to the EL2 context save/restore operations. Warning: if this
   option is enabled on a system that does not support this feature, it will
   hang during el2 context save or restore due to trying to access a register
   that does not exist.
