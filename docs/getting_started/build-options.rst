...
- ``OPENSSL_DIR``: This option is used to provide the path to directory on the
  host machine where a custom installation of OpenSSL is located, which is used
  to build certificate generation and firmware encryption tools. If this option
  is not set, the default OS installation will be used.
