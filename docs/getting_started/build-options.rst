...
-  ``DRTM_SUPPORT``: Boolean to enable support for Dynamic Root of Trust for
   ...
   order to trust on the components involved in DRTM process. Mainly, to trust
   on the data provided by the DCE preamble, and on the measurement of various
   entities taken by DCE and D-CRTM as per the `PSA DRTM specification`_.
