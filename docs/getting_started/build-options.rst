...
- ``ERRATA_ABI_SUPPORT``: Boolean option to enable support for Errata management firmware
  interface for the BL31 image. By default its disabled (``0``).

- ``ERRATA_NON_ARM_INTERCONNECT``: Boolean option to enable support for errata in Errata
  management firmware interface for platforms with a non-arm interconnect. By default its disabled
  (``0``).
