...
-  ``DISABLE_MTPMU``: Numeric option to disable ``FEAT_MTPMU`` (Multi Threaded
   ...
   This flag requires ``ENABLE_FEAT_PMUV3_FOR_NS`` to be set.This flag can take
   values 0 to 2, to align with the ``FEATURE_DETECTION`` mechanism. Default is
   ``0``.
...
- ``ENABLE_FEAT_PMUV3_FOR_NS``: Numeric value to enable the ``FEAT_PMU``
...
-  ``ENABLE_SME_FOR_NS``: Numeric value to enable Scalable Matrix Extension
   ...
   world to trap to EL3. Requires ``ENABLE_SVE_FOR_NS`` to be set
   SME is an optional architectural feature for AArch64
...
-  ``ENABLE_SME_FOR_SWD``: Boolean option to enable the Scalable Matrix
   Extension for secure world use along with SVE and FPU/SIMD,
   ENABLE_SME_FOR_NS, and ENABLE_SVE_FOR_SWD must also be set to use this. If
   enabling this, the secure world MUST handle context switching for SME, SVE,
   and FPU/SIMD registers to ensure that no data is leaked to non-secure world.
...
-  ``ENABLE_SVE_FOR_SWD``: Boolean option to enable SVE for the Secure world.
   ...
   requires ENABLE_SVE_FOR_NS to be enabled. It will force ENABLE_SME_FOR_SWD
   off. The default is 0 and it is automatically disabled when the target
   architecture is AArch32.
