...
-  ``ENABLE_SVE_FOR_NS``: Numeric value to enable Scalable Vector Extension
   ...
   systems that have SPM_MM enabled. The default is 1.
...
-  ``ENABLE_SVE_FOR_SWD``: Boolean option to enable SVE and FPU/SIMD functionality
   ...
   Note that this build flag requires ``ENABLE_SVE_FOR_NS`` to be enabled. When
   enabling ``ENABLE_SVE_FOR_SWD``, a developer must carefully consider whether
