...
-  ``FEATURE_DETECTION``: Boolean option to enable the architectural features
   ...
   through feature specifc build flags are supported by the PE or not by
   validating them either at boot phase or at runtime based on the value possesed
   by the feature flag(0 to 2) and report error messages at an early stage.
   ...
   The mechanism permits the build flags to take values between 0 to 2 and
   ...
   In the above example, if the feature flag, ``ENABLE_FEAT_HCX`` is defined
   to 0, flag is not validated. If it is defined as 1, feature flag is validated
   at boot time, and a hard panic is generated, if feature is not detected in the
   PE. Finally, if the flag is set to 2, feature is validated at runtime.

   This flag is disabled by default, and is currently an experimental procedure.
   Platforms can explicitly make use of this by mechanism, by enabling it to
   validate whether they have set their build flags properly at an early phase.
