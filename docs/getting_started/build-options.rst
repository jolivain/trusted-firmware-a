...
-  ``RAS_FFH_SUPPORT_NS``: Support to enable Firmware first handling of RAS errors
   originating from NS world. When ``RAS_FFH_SUPPORT_NS`` is set to ``1``,
   ``HANDLE_EA_EL3_FIRST_NS`` must also be set to ``1``.

-  ``RAS_KFH_SUPPORT``: If ``FEAT_RAS_ENABLE`` is set, this is also set. KFH stands
   for kernel first handling which from EL3 point of view means that non-EL3 component
   will handle the RAS error.
