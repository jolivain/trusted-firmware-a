...
- ``ENABLE_FEAT_PMUV3``: Numeric value to enable the ``FEAT_PMU`` (Performance
  Monitor Unit) extension. Will also enable PMU < v3 on Aarch32.  ``FEAT_PMU``
  is an optional feature.  This flag can take the values 0 to 2, to align with
  the ``FEATURE_DETECTION`` mechanism. Default value is ``1``.
