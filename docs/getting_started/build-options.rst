...
-  ``ENABLE_FEAT_VHE``: Numeric value to enable the ``FEAT_VHE`` (Virtualization
   ...
   feature and is enabled from v8.1 and upwards. It can take values 0 to 2,
...
-  ``ENABLE_SPE_FOR_LOWER_ELS`` : Numeric value to enable Statistical Profiling
   extensions. This is an optional architectural feature for AArch64. This flag
...
-  ``ENABLE_SVE_FOR_NS``: Numeric value to enable Scalable Vector Extension
   ...
   assert on platforms where SVE is implemented and ``ENABLE_SVE_FOR_NS`` set
   to 1. This flag can take the values 0 to 2, to align with ``FEATURE_DETECTION``
   mechanism. The default is 0. When set, it is automatically disabled when
...
-  ``ENABLE_SVE_FOR_SWD``: Numeric value to enable SVE for the Secure world.
   ...
   requires ENABLE_SVE_FOR_NS to be enabled. This flag can take values 0 to 2,
   to align with the ``FEATURE_DETECTION`` mechanism.  The default is 0 and it
...
-  ``FEATURE_DETECTION``: The mechanism to detect whether the Architectural
   features enabled through feature specifc build flags are supported by the PE
   or not by validating the either at boot phase or at runtime and report error
   messages at an early stage. This prevents and benefits us from EL3 runtime
   exceptions during context save and restore routines guarded by these build
   flags. Henceforth validating them before their usage provides more control
