...
The build process is supported and tested on both operating systems, however there
are limitations to the Windows build that are discussed later; we recommend
building |TF-A| under Linux for best results.
...
A relatively recent Linux distribution is required for building |TF-A|. We
...
- GNU GCC (Native, for the host architecture)
- GNU Make
- Git
...
- OpenSSL (libssl-dev). Required to build the cert_create tool.
