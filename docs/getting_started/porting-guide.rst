...
reasons like Authentication failure, or a watchdog reset while booting from the
active bank, the platform can then switch to boot from a different bank. This
function then returns the bank that the platform has booted it's image from.
