...
gives the flexibility to perform any platform-specific actions safely, such as
initialization or modification of shared data structures, without the overhead of
explicit cache maintainace operations.
...
In future, when every Arm based CPU supports ``HW_ASSISTED_COHERENCY``, we believe the
``pwr_domain_on_finish()`` function will become redundant and is expected to be replaced
by ``pwr_domain_on_finish_late()`` function.
