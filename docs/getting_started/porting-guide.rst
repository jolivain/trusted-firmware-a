...
The :term:`SDEI` dispatcher requires the platform to provide the following macros
...
Normal :term:`SDEI` events on the platform. This must have a higher value
...
This macro must be defined to the EL3 exception priority level associated with
Critical :term:`SDEI` events on the platform. This must have a lower value
(therefore of higher priority) than ``PLAT_SDEI_NORMAL_PRI``.

**Note**: :term:`SDEI` exception priorities must be the lowest among Secure
priorities. Among the :term:`SDEI` exceptions, Critical :term:`SDEI` priority must
be higher than Normal :term:`SDEI` priority.
...
event registration and *Complete and Resume* :term:`SDEI` calls. The function
takes one argument, which is the address of the handler the :term:`SDEI` client
...
:term:`SDEI` specification requires that a PE comes out of reset with the events
masked. The client therefore is expected to call ``PE_UNMASK`` to unmask
:term:`SDEI` events on the PE. No :term:`SDEI` events can be dispatched until such
time.

Should a PE receive an interrupt that was bound to an :term:`SDEI` event while the
