...
Note -
If the platform has peripherals that are not managed by the SMMU. The platform
should investigate such peripherals to determine whether they can be trusted,
and such peripherals should be moved under "Non-host platforms" if they can be
trusted.
...
Secure region of DRAM. This function takes DLME region start and 
size an input arguments, and returns 0 on success and -1 on failure.
