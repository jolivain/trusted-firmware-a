...
This API is called from assembley files when encountering a critical failure that
cannont be recovered from. It also invokes elx_panic() which allows to report a
crash from lower exception level. This function assumes that it is invoked from a C
