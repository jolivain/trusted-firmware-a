...
1. Determine architecture extension support in TF-A build:
   All the mandatory architectural features upto ``ARM_ARCH_MAJOR.ARM_ARCH_MINOR`` are
   included and enabled by TF-A build system.
2. Generate binary target, passed to compiler via "-march" option
