...
CPU functions are asked to follow a modified version of the Procedure Call
Standard (PCS) in their internals. This is done to ensure calling these function
from outside the file doesn't unexpectedly corrupt registers in the very early
environment and to help the internals to be easier to understand.
...
Each errata has a build flag in ``lib/cpus/cpu-ops.mk`` of the form:
...
The errata framework is a convention and a small library to allow errata to
automatically be discovered. It enables compliant errata to automatically be
applied and reported at runtime (either by status reporting or the errata ABI).

To write a compliant mitigation for erratum number ``erratum_id`` on cpu that
...
#. A CPU revision checker function: ``check_erratum_<cpu_name>_<erratum_id>``
   ...
   Its should check whether this erratum applies on this revision of this CPU.
...
.. note::
 ...
 ``number`` part of its name and ``ERRATA()`` part of the registration should
 instead be ``CVE(<year>, <number>)``. In the extremely unlikely scenarion where
 a CVE and an erratum numbers clash, the CVE number should be prefixed with a
 zero.
...
.. note::
 ...
 Aarch32 uses the legacy convention. The checker funciton has the format
...
Aarch64 there are helpers to do omit most of this. They are located in
