...
- ``ARM_ARCH_MAJOR`` and ``ARM_ARCH_MINOR`` are passed to a march build utility
   this will try to come up with an appropriate march value to be passed to compiler
   by probing the compiler and checking whats supported by compiler and what best can
   be used. But if platform provides a ``MARCH_DIRECTIVE`` then it will used directly and
   compiler probing will be skipped.
