...
tables. The details of this library can be found in :ref:`Translation (XLAT) Tables Library`.
...
``plat_get_my_entrypoint()`` function in the :ref:`Porting Guide`). In the case of a
...
the :ref:`Porting Guide`) while the primary CPU executes the remaining cold boot path
...
-  Exception vectors
   ...
   ``plat_report_exception()`` function (see the :ref:`Porting Guide`). The status
   code is one of:
...
``bl1_plat_get_next_image_id()`` to determine if :ref:`Firmware Update (FWU)` is required or
...
next section, else BL1 assumes that :ref:`Firmware Update (FWU)` is required and execution
passes to the first image in the :ref:`Firmware Update (FWU)` process. In either case, BL1
...
the pre-requisites mentioned in the :ref:`PSCI Library Integration guide for Armv8-A AArch32 systems`.
...
region (refer to section 2.1 of :ref:`Porting Guide`). The coherent memory region
