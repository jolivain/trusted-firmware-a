...
-  SError : "plat_handle_el3_ea" Except for special handling of lower EL's SError exception
   which gets triggered in EL3 when PSTATE.A is unmasked. Its only applicable when NS lower
   EL's EA is routed to EL3 (HANDLE_EA_EL3_FIRST_NS=1).
...
current EL) any time after PSTATE.A is unmasked. This is wrong becasue the error cause
was in lower EL but exception happened in EL3.
...
-  FFH : Handle the syncrhonized error first using **handle_pending_async_ea()** after
