...
.. note::
   Some platforms enabling ``RESET_TO_BL31`` might still be able to receive
   parameters in registers depending on their actual boot sequence. On those
   occasions, they could define  ``RESET_TO_BL31_WITH_PARAMS`` in
   ``platform.mk`` to  avoid the input registers from being zeroed before
   entering BL31.
