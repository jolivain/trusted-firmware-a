...
#. To build the FIP image, ensure the following command line variables are set
   ...
   specified at build time. Three locations are currently supported (see
   ...
   -  ``ARM_ROTPK_LOCATION=regs``: the ROTPK hash is obtained from the Trusted
      ...
      On Juno model, the default value corresponds to an ECDSA-SECP256R1 public
      key hash, whose private part is not currently available.

   -  ``ARM_ROTPK_LOCATION=devel_rsa``: use the ROTPK hash that is generated
      from ROT_KEY location.

   -  ``ARM_ROTPK_LOCATION=devel_ecdsa``: use the ROTPK hash that is generated
      from ROT_KEY location.
