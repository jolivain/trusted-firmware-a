...
-  ``ARM_ROTPK_LOCATION``: used when ``TRUSTED_BOARD_BOOT=1``. It specifies the
   ...
   -  ``devel_full_dev_key`` : returns a development public key embedded in the
      BL1 and BL2 binaries. This key has been obtained from the RSA public key
      ``arm_rotpk_rsa.der``, located in ``plat/arm/board/common/rotpk``.

-  ``ARM_ROTPK_HASH``: used when ``ARM_ROTPK_LOCATION=devel_*``, excluding
   ``devel_full_dev_key``. Specifies the location of the ROTPK hash. Not
