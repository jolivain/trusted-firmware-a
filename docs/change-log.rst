...
- Arm Architecture
   ...
   - smccc: Implement SMCCC_ARCH_SOC_ID SMC call, Create separate header for
     ARM specific SMCCC defines, Use consistent SMCCC error code, add get smc
     function id num macro, Changes to support updated register usage in SMCCC
     v1.2
...
- Build System
   ...
   - Introduce per-BL CPPFLAGS and ASFLAGS

   - Define IMAGE_AT_EL1 or IMAGE_AT_EL3 globally for C files

   - Support per-BL LDFLAGS
...
- Drivers
   ...
   - auth: Measured Boot: add function for hash calculation
...
- Libraries
   ...
   - fconf: Add Firmware Configuration Framework (fconf).
...
- Platforms
   ...
   - brcm: Add BL2 and BL31 support for Broadcom stingray platform
   ...
   - nvidia: Tegra194: Enable system suspend and fake system suspend, and
     initialize smmu on system suspend exit
   - nvidia: Tegra194: Add macros to read GPU reset status, security carveout
     configuration registers, and 'TEGRA_TMRUS_SIZE'
   - nvidia: Tegra194: Add support for 64-bit TZDRAM base, SMMU and mc_sid,
     multiple SMMU devices, SMC64 encoding for MCE calls, cpu suspend, smmu ISO,
     memctrl/smmu drivers, MCE driver, and handler to retrieve power domain tree
...
- BL-Specific
   ...
   - BL2: Print ID of images we fail

   - linker_script: Move bss section to bl_common.ld.h, replace common
     read-only data with RODATA_COMMON, and move more common code to
     bl_common.ld.h
...
- Library Code
   ...
   - xlat_tables_v2: Read-only xlat tables for BL31 memory,
     use ARRAY_SIZE in REGISTER_XLAT_CONTEXT_FULL_SPEC,
...
- Platforms
   ...
   - nvidia: Tegra194: Update cpu core-id calculation, convert 'target_cpu' and
     'target_cluster' to 32-bits, remove L2 ECC parity protection setting, skip
     notifying MCE in fake system suspend, don't run MCE firmware on Emulation,
     and remove GPU, MPCORE and PTC registers from streamid list
...
- Tools
   - cert_create: Remove references to TBBR in common code and some unused
...
- Code Quality
   - coverity: Fix various MISRA violations including null pointer violations,
     C issues in BL1/BL2/BL31 and FDT helper functions, Using boolean essential type
   - coverity: Remove unnecessary header file, remove comparisons to LONG_MAX
     in debugfs devfip
...
- Platforms
   ...
   - nvidia: Tegra194: Fix smmu mask for board revision id, mce function
     declaration conflicts, and TEGRA186_SMMU_CTX_SIZE
