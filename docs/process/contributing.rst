...
-  TF-A uses Jenkins tool for Continuous Integration and testing activities.
   ...
   merged. So each of your patches goes through series of checks before they
   get merged on to the master branch.
-  ``Coverity Scan analysis`` is one of the testing we perform on our source code
   at regular intervals.
   We maintain a build script ``tf-cov-make`` to place the build configurations
   of various platforms in order to cover the entire source code being analysed by Coverity.
...
#. We maintain a separate repository named `Platform-Ci repository`_
   for placing all the test scripts which will be executed by the CI Jobs.
#. In this repository, ``tf-cov-make`` script is located at ``platform-ci/script/tf-coverity/tf-cov-make``
#. Edit this file by appending all the possible build configurations with
   the specific ``build-flags`` relevant to your platform, so that newly added source
   files gets build and analysed by Coverity.
...
-  In the above example there are two different SoCs ``stratix`` and ``agilex`` under
   the Intel platform and the build configurations has been added suitably to
   include most of their source files.
...
-  Similar to this you can still find many other different build configurations of
   various other platforms listed in the ``tf-cov-make`` script. Kindly refer them
   and append your build configurations respectively.
...
.. _platform-ci repository: https://review.trustedfirmware.org/admin/repos/ci/tf-a-ci-scripts
