...
security state can be attested to latter.
...
for convenience, as the fTPM service used to process and measure the Event Log
...
Measured Boot/fTPM functionality are:
   ...
   - **OP-TEE**: As stated earlier, the fTPM service used on this PoC is built as an
...
Following up with the fTPM starting-up process, we can see that all the
...
We are only interested in PCR0, as this is the one where the boot process
recorded all the images (see field ``PCRIndex`` on the Event Log above).
The rest of the records should be 0 at this point, but PCR0 should not be
0.
...
.. note::
   ...
   0, meaning that the fTPM was able to read the Event Log generated by TF-A
   and that all were correct in the sense that none of them were recorded in
   a PCR other than 0 and that the fTPM was able to properly parse all the
