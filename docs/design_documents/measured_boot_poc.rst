...
solely intention of showing how Measured Boot can be used in conjunction with
...
.. note::
   ...
   and therefore the instuctions given here might not apply in such scenarions.
...
In this PoC we are only interested in PCR0, which must not be non-null. This is
becasuse the boot process records all the images in this PCR (see field ``PCRIndex``
...
As stated before, the OP-TEE Toolkit includes support to build a third party fTPM
...
The most relevant options for Measured Boot support are:
   ...
   - **CFG_TA_DEBUG**: Enable debug logs in the Terminal_1 console.
   - **CFG_TEE_TA_LOG_LEVEL**: Define the log level used for the debug messages.
   - **CFG_TA_MEASURED_BOOT**: Enable support for measured boot on the fTPM.
