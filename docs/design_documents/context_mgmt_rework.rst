...
is enabled for a particular world, this feature will be enabled for Root.
The firmware at EL3 may not expecting this feature to be enabled and may cause
unwanted side-effects which may be problematic. Thus it would be more robust
if Root world is not subject EL3 sysreg variation of other worlds but can
maintain its own values which remain stable throughout root world execution.
...
programming of lower EL sysregs. For eg, if EL3 needs to make use of Pointer
Authentication feature, it needs to program its own PAuth Keys during
execution at EL3. Hence EL3 needs its own copy of PAuth registers which
needs to be restored on every entry to EL3. A similar case can be made
for DIT bit in PSTATE, or use of SP_EL0 for C Runtime Stack at EL3.
