Runtime Security Subsystem (RSS) backend
========================================
...
own boot process, which is the same as on non-RSS systems.  The last stage of
the RSS firmware is a persistent, runtime component. Much like AP_BL31, this is
a passive entity which has no periodical task to do and just waits for external
...
- ``Pointer-access messaging``: Default behavior. The message header and the
  payload are separated and they are conveyed in different ways. The header is
  sent over the channels, similar to the embedded messaging but the payload is
...
.. code-block:: c
    ...
    struct rss_comms_msg_t {
        ...
        struct rss_comms_header_t {
            uint8_t protocol_ver; /* embedded OR pointer-access */
            uint8_t seq_num; /* Currently unused on AP side. */
            uint16_t client_id;
        };
        /* Message body */
        union {
...
- ``Measured boot``: Securely store the firmware measurements which were
  ...
  in RSS can be found in the ``measured_boot_integration_guide`` [1]_ .
...
- ``"OTP assets management``: RSS provides access for AP to assets in OTP.
  ...
  keys used by AP during the trusted boot process can be requested from RSS and
  AP can request RSS to increase a non-volatile counter.
...
The entity at the end of the chain is responsible of composing the final
