...
named ``signer_id``.
In order to call this function, the caller must explicitly make the
``signer_id`` field NULL.
...
- Second parameter is the pointer to the key-OID of ther public key.
