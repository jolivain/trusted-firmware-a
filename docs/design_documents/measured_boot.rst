...
Measured Boot is the process of computing and securely records hashes of code
...
#. **Function : plat_mboot_measure_image()**
   ...
   - Record the measurement in the corresponding backend -
     ...
     - If it is Event Log backend, then record the measurement in TCG Event Log
       format.
     - If it is hardware TPM (like RSS), then extend the designated PCR
       (or slot) with the given measurement.
