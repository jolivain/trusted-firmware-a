...
by means of a starting address and a size. Hence, each BL stage that utilises
Mbed TLS must be able to provide a heap to it. The heap is requested from the
current BL stage, i.e. the current BL image inside which Mbed TLS is used.
