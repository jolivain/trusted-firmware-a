...
RMM-EL3 runtime specification.

The Boot Manifest, discussed later in section :ref:`rmm_el3_boot_manifest`,
uses a separate version number but with the same scheme. However, the manifest
version is isolated to the manifest is independent of the global RMM-EL3
...
The fields are sequentially increased and the rules for updating them are
explained below:
  ...
  - ``RES0``: Bit 31 of the version number is reserved 0 as to maintain
    consistency with the versioning schemes used in other submodules on
    RMM.
...
variations. It also allows RMM to be decoupled from BL2/BL31.
...
platform information passed on the boot manifest is generated by EL3 Firmware
and there is no imposition on how the data can be obtained (e.g by DTB, FCONF,
or other).
...
.. csv-table::
   ...
   x2,Maximum number of CPUs supported by EL3 Firmware.
...
Normal memory attributes (G-R-E) at EL3. At boot, this memory will be used to populate
...
RMMM-EL3 Runtime Services Interface
...
is allowed to an error code corresponding to any of the failure conditions.
...
.. csv-table::
   ...
   buf_pa,x1,[63:0],Address,PA where the RAK must be stored by EL3. The PA must belong to the shared buffer
