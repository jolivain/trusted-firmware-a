...
secondary Bootloader after FWU being done by update client and agent.
...
Bootloader (BL2) performs the following actions:
...
Secondary Bootloader (BL2) avoids upgrading the platform NV-counter until it's
...
not boot successfully from ``previous_active_index`` then the platform can choose
