...
In a trusted system at any instance, a given CPU can be executing in any world
(Non-Secure, Secure, Realm). Each world must have its configuration of system
registers independent of other security states to access and execute any of the
architectural features.

This covers a range of settings, beginning with register fields to configure the
security state for the individual world (SCR_EL3.NS and SCR_EL3.NSE), feature
enablement for each security state, saving and restoring register contents
(EL1, EL2, vector) and other registers which are likely to be overwritten across
the worlds and some errata bits etc.

If the CPU switches across security states (for eg: from non-secure to secure
...
the architectural features enabled in one security state will be unconditionally
accessible in the other security state as well. This can be a major concern when
...
for each CPU, we need to maintain world-specific context to ensure that register
...
responsible for their respective world context management.
...
Each feature is supported with a helper function (is_feature_supported( )), to
detect its presence at runtime. This helps dispatchers to select the desired
...
Note: The figure above illustrates a scenario on FVP for one of the build
