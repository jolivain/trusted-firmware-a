...
- Provides support for EL3 Logical Partition to support easy migration from EL3 to S-EL1.
...
- The SPMC provides support for statically allocated EL3 Logical Secure Partition
...
The following additional interfaces are supported at the Secure physical instance:
...
- An SP can send a direct request to another LSP.
- An SP can receive a direct request from another LSP.
...
FFA_MEM_DONATE is supported only from Nwld to Swld.
...
Multiple borrowers and fragmented memory sharing are supported.
...
FFA_MEM_FRAG_RX is to be used by SP if FFA_MEM_RETRIEVE_RESP returned descriptor
with fragment length less than total length.
