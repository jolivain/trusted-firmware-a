...
- Three-world execution:  This is the configuration to use if Secure
  ...
  - NS Host (RME capable Linux or TF-A Test),
  - Root (TF-A)
  - Realm (RMM or TRP)
...
kernel image and associated root filesystem is available.
...
.. note::
 ...
 If the ``RMM`` option is not specified, it build the Test R-EL2 payload (TRP)
 and this is only meant to be used for testing with TF-A-Tests.
...
``BL33`` can point to a Non Secure Bootloader like UEFI/Uboot or
...
`preloaded` to a suitable memory location and this option needs to be specified
via ``PRELOADED_BL33_BASE`` option. Also TF-A should implement the Linux kernel
register conventions for boot and this can be set using the
...
The command presents the rootfs as a virtio block device to linux kernel.
The `bp.virtioblockdevice.image_path` option can be ignored if NS Host is
TF-A-Tests.

If the TF-A was built to expect a preloaded Linux kernel and boot it directly,
then use the following argument to load the kernel image at the expected address.
...
Use the ``sp_layout.json`` previously generated in tf-a-test
...
`4. Running FVP for 3 world setup`_ to run tests for 4 world setup.
