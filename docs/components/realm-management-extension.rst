...
More information about the GPT library can be found in the GPT library chapter.
...
Management Interface (RMI). Current functions includes returning RMM version
and transitioning granules from Non-secure to Realm world and vice versa.

When building TF-A with RME enabled, if a path to RMM image is not provided,
TF-A builds TRP by default and uses it as RMM image.
