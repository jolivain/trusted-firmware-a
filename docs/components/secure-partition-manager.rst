...
In `[1]`_ , the section  "Boot information protocol" section defines a method
for passing data to the SPs at boot. It defines the format for the structures
boot information descriptor and boot information header, which describe the data
to be exchanged between SPMC and SP.
...
process the boot information header and descriptors, and access access its own
manifest DTB blob and extract its partition manifest properties.
