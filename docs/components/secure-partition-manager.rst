...
CPU cycles by transitioning to EL3 and S-SEL2.
...
- Notifications Pending Interrupt: virtual interrupt to be handled by the
  ...
  given secure partition. For per-vCPU notifications, the NPI is pended at the
  handling of FFA_NOTIFICATION_SET interface.
...
In the case of per-vCPU notifications, the NPI interrupt is set as pending
for a given receiver partition.

The FFA_NOTIFICATION_GET retrieves all pending global notifications and
...
Hafnium keeps the global counting of pending notifications whose information has
been retrieved by this interface. The counting is incremented and decremented at
the handling of FFA_NOTIFICATION_SET and FFA_NOTIFICATION_GET, respectively.
It also tracks the notifications whose information has been retrieved
individually, such that it avoids duplicating returned information for subsequent
calls to FFA_NOTIFICATION_INFO_GET. For each notification, this state information
is reset when receiver called FFA_NOTIFICATION_GET to retrieve them.
