...
following scenarios:
 - When it was targetted by a message send operation from another endpoint.
 - Return the result of calling ``FFA_PARTITION_INFO_GET``.
 - In a memory share operations, as part of the ``FFA_MEM_RETRIEVE_RESP``,
   with the memory descriptor of the shared memory.

If a normal world VM is expected to exchange messages with secure world,
its RXTX buffers should have been forwarded to the SPMC via FFA_RXTX_MAP ABI.
In this scenario, the SPMC is the owner of the VM's RX buffer.
...
The FFA_RX_RELEASE is used after the FF-A endpoint is done with processing the
data received in its RX buffer. If the RX buffer has been acquired by the
hypervisor, the FFA_RX_RELEASE call must be forwarded to the SPMC to
reestablish SPMC's RX ownership.

An SP attempt to message a normal world VM whose RX buffer was acquired by the
hypervisor fails with error code FFA_BUSY, to preserve the RX buffer integrity.
The operation could then be conducted after FFA_RX_RELEASE.

FFA_MSG_SEND2
~~~~~~~~~~~~~

Hafnium copies a message from the sender TX buffer, and copies it into
receiver's RX buffer. For messages from SPs to VMs, operation is only
possible if the SPMC owns the receiver's RX buffer.
...
On a successful message send, hafnium pends a framework notification
for the receiver, to inform it about a message in the RX buffer.
...
Hafnium implements the following memory sharing interfaces:
 - ``FFA_MEM_SHARE`` - for concurrent access between lender and borrower.
 - ``FFA_MEM_LEND`` - borrower to obtain exclusive access, though lender
   retains ownership of the memory.
 - ``FFA_MEM_DONATE`` - lender permamently relinquishes ownership of memory
   to the borrower.

The ``FFA_MEM_RETRIEVE_REQ`` interface is for the lender to request the
memory to be mapped into its S2 translation, and configure some of the
terms in which the memory is to be shared. On a successful call, the
SPMC responds back with ``FFA_MEM_RETRIEVE_RESP``.

The ``FFA_MEM_RELINQUISH`` is for when the borrower is done with using
a memory region.

The ``FFA_MEM_RECLAIM`` is for the lender and owner of memory to reestablish
its ownership and exclusive to the memory shared.

Hafnium also supports memory sharing operations from one lender to multiple
borrowers. The exception is ``FFA_MEM_DONATE`` whose semantics implies ownership
transmission, which should target only one partition.

The memory transaction descriptors are transmitted via RX/TX buffers. In
situations where the size of the memory transaction descriptor surpasses the
size if the RX/TX buffers, hafnium provides support for fragmented transmission of
the full transaction descriptor. The ``FFA_MEM_FRAG_RX`` and ``FFA_MEM_FRAG_TX``
...
It is also the designated allocator for the memory handle. The hypervisor or
OS kernel has the possibility to rely on the SPMC to maintain the state
...
EAC0 specification, in section 16.4.3. The intent is to aid with operations
...
that state information and do the necessary validations and update S2 memory
translation.
...
at least one borrower VM, the hypervisor is in charge of managing its S2
translation on a successful memory retrieve.
...
for hafnium's internal processing of the operation. If the FF-A version of a
