...
- The *memory* nodes provide platform information on the ranges of memory
  ...
  If the field specifies "memory" range is secure, else if it specifies
  ...
  the SPMC itself, the OS Kernel/Hypervisor, or other NWd VMs. The SPMC limits
  the SP's address space such that they do not access memory outside of those
  ranges.
...
- Memory regions are mapped in the SP EL1&0 Stage-2 translation regime at
  ...
  NS should be set consistently with the security state of the range that
  encompasses it. I.e. non-secure memory shall be part of a non-secure memory
  range, and secure memory shall be contained in a secure memory range.
