...
-  Translation: Incoming DMA requests are translated from bus address space to
   system physical address space using ARMv7/ARMv8 translation tables.
...
several I/O devices along with Internconnect and Memory system.
...
The primary focus of SMMUv3 driver development in Hafnium is to support secure
streams.

-  Currently, the driver only supports Stage2 translations. No support for Stage1
   or Nested translations.
-  Supports only AArch64 translation format (coherent with Hafnium project).
-  No support for features such as PCI Express (PASIDs, ATS, PRI), MSI, RAS,
   Fault handling, Perforamce Monitor Extensions, Event Handling, MPAM.
