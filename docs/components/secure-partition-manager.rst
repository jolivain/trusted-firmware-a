...
following scenarios:
 ...
 - In a memory share operation, at a success FFA_MEM_RETRIEVE_REQ providing
   the memory transaction descriptor containing information about the shared
   memory.
...
hypervisor fails, to preserve the RX buffer integraty. The operation could then
be conducted after FFA_RX_RELEASE.
...
Both receiver and sender need to enable support to indirect messaging,
in theirs respective partition manifest. The discovery of support
