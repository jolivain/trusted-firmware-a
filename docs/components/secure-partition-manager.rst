...
- Hypervisor refers to the NS-EL2 component managing virtual machines
  in the normal world.
- SPMC refers to the S-EL2 component managing virtual machines (or Secure
  Partitions) in the secure world.
...
time. It is re-using the description from `[1]`_ section
...
The primary SP's Execution Context exits through ``FFA_MSG_WAIT`` to indicate
...
In the secure world the primary EC passes the secondary EC entry point and
context. The SMC service call is trapped into the SPMC.
This can be either *(under discussion)*:
...
including a mailbox used for FF-A indirect messaging.
This same mailbox structure is re-used in the SPMC. In `[1]`_ only direct messaging
is allowed to SPs, so RX/TX buffers are not immediately necessary for messaging but
to allow implementation of `FFA_PARTITION_INFO_GET`_.
...
use a secure FF-A id as origin world through spoofing. A VM-to-SP messaging
passing shall have an origin world being non-secure (id bit 15 clear) and
destination world being secure (FF-A id bit 15 set). Similarly, an SP-to-SP
message shall have FF-A id bit 15 set for both origin and destination.

Direct messaging
----------------

VM-to-SP and SP-to-SP
~~~~~~~~~~~~~~~~~~~~~
