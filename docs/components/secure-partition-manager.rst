...
effective action for NS interrupts is set to QUEUED. Please refer to FF-A v1.1
EAC0 section 8.3.1 for further explanation.
...
A brief description of the events:
  ...
  - 5) Assuming SP1 vCPU is in WAITING state, SPMC signals virtual interrupt
       ...
       vCPU using ERET in SPMC schedule mode.
...
The following figure describes interrupt handling flow when a secure interrupt
triggers while execution is in secure world:

.. image:: ../resources/diagrams/ffa-secure-interrupt-handling-swd.png

A brief description of the events:

  - 1) OS kernel sends a direct request message to SP1. Further, SP1 sends a
       direct request message to SP2. SP1 enters BLOCKED state and SPMC resumes
       SP2. Secure interrupt triggers while SP2 is running.
  - 2) SP2 gets preempted and execution gets trapped to SPMC as IRQ.
  ...
  - 4) SPMC pends vIRQ for SP1 and signals through FFA_INTERRUPT interface.
       SPMC further resumes SP1 through ERET conduit. Note that SP1 continues
       to remain in Normal world schedule mode.
  - 6) Execution traps to vIRQ handler in SP1 provided that the virtual
       interrupt is not masked i.e., PSTATE.I = 0
  - 7) SP1 queries for the pending virtual interrupt id using a paravirtualized
       HVC call. SPMC clears the pending virtual interrupt state management
