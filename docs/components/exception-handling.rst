...
-  The Arm `SDEI specification`_ defines interfaces through which Normal world
   ...
   system events. The :term:`SDEI` specification requires that these events are
...
-  Receive exceptions, but handle part of the exception in EL3, and delegate
   ...
   an :term:`SDEI` event). In this scheme, exception handling potentially and
...
others (:term:`SDEI`, for example); and within :term:`SDEI`, Critical priority
:term:`SDEI` should be assigned higher priority than Normal ones.
