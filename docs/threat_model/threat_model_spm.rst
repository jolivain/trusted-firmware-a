...
+------------------------+------------------+-----------------+---------------+
|                        | In context of FF-A v1.0 this is the case of sharing|
|                        | the RX/TX buffer pair and usage in the             |
|                        | PARTITION_INFO_GET or mem sharing primitives.      |
| ``Mitigations``        | The SPMC must copy the contents of the TX buffer   |
...
|                        | standard. Additionally it is recommended the SPMC  |
|                        | complies with the Arm provided `FF-A ACS`_ test    |
|                        | suite.                                             |
+------------------------+----------------------------------------------------+
...
+------------------------+------------------+---------------+-----------------+
| ``Mitigations``        | The TF-A implementation does not provide           |
|                        | mitigations to this type of attack. It can be      |
|                        | addressed by the use of special HW circuity or     |
|                        | hardening at the chipset or platform level.        |
|                        |                                                    |
|                        |                                                    |
|                        |                                                    |
+------------------------+----------------------------------------------------+
...
|                        | configuration and page tables. A model where       |
|                        | regions can be run-time dynamically allocated can  |
|                        | be more sensitive to manage. Such threat may be    |
|                        | exercised by a chain of defects where a malicious  |
|                        | or corrupted driver may attempt to build on.       |
+------------------------+----------------------------------------------------+
...
|                        | region which which does not belong to this agent.  |
+------------------------+----------------------------------------------------+
...
+------------------------+------------------+---------------+-----------------+
| ``Mitigations``        | The SPMC may implement a specific policy for handle| 
|                        | allocation in memory sharing operations.           |
|                        |                                                    |
|                        |                                                    |
|                        |                                                    |
|                        |                                                    |
...
| ``Total Risk Rating``  | Medium (6)       | Medium (6)    |                 |
+------------------------+------------------+---------------+-----------------+
| ``Mitigations``        | In particular the SPMC must be hardened to prevent |
|                        | revealing its internal state or the state of an SP |
|                        | through a direct message response.                 |
|                        |                                                    |
|                        |                                                    |
|                        |                                                    |
|                        |                                                    |
...
|                        | The SW implementation does not provide specific    |
|                        | mitigations to this type of attack.                |
|                        |                                                    |
|                        |                                                    |
|                        |                                                    |
+------------------------+----------------------------------------------------+
...
| ``Total Risk Rating``  | Medium (6)       | Medium (6)      |               |
+------------------------+------------------+-----------------+---------------+
| ``Mitigations``        | It is assumed platforms integrating the SPMC at    |
|                        | S-EL2 (hence implementing the Armv8.4 FEAT_SEL2    |
|                        | architecture extension) implement a number of      |
|                        | mitigations fixing Spectre, Meltdown or other cache|
|                        | timing side-channel type of attacks. The SPMC may  |
|                        | be hardened further with SW mitigations            |
|                        | (e.g. speculation barriers) for the cases not      |
|                        | covered in HW. Usage of hardened compilers and     |
|                        | appropriate options, code inspection are           |
|                        | recommended ways to mitigate Spectre types of      |
