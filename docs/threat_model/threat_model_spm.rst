...
|                        | resume or turn on other endpoint excution contexts,|
|                        | thereby altering the internal state of SPMC and SPs|
|                        | , potentially leading to illegal state transitions |
|                        | and deadlocks.**                                   |
+------------------------+----------------------------------------------------+
...
| ``Total Risk Rating``  | Medium (9)       | Medium (9)      |               |
+------------------------+------------------+-----------------+---------------+
| ``Mitigations``        | TF-A SPMC provides mitigation against such actions |
|                        | by following the guidance for Partition runtime    |
|                        | models as described in FF-A v1.1 specification.    |
|                        | SPMC performs numerous checks in runtime to prevent|
|                        | illegal state transitions by adhering to the       |
|                        | partition runtime model. Only Primary VM is        |
|                        | allowed to turn on execution contexts of other     |
|                        | partitions and secondary partitions have limited   |
|                        | ability to resume endpoint execution contexts based|
...
|                        | call that could cause the system to crash or enter |
|                        | into an unknown state as no physical interrupt     |
|                        | could be pending for it to be handled in SPMC.**   |
+------------------------+----------------------------------------------------+
...
| ``Total Risk Rating``  | Medium (9)       | Medium (9)      |               |
+------------------------+------------------+-----------------+---------------+
| ``Mitigations``        | TF-A SPMC provides mitigation against such attack  |
|                        | by detecting such invocations from partitions and  |
|                        | simply returning FFA_ERROR code. SPMC only allows  |
|                        | SPMD to use FFA_INTERRUPT ABI to communicate a     |
|                        | pending secure interrupt triggered in normal world.|
+------------------------+----------------------------------------------------+
...
|                        | signaled by SPMC, thereby altering the state of the|
|                        | SPMC and potentially lead to system crash.**       |
|                        | Similarly, a malicious secure endpoint might invoke|
|                        | the deactivation ABI more than once for a virtual  |
|                        | interrupt. Moreover, a malicious secure endpoint   |
|                        | might deactivate a virtual secure interrupt that   |
|                        | was signaled to another endpoint execution context |
|                        | by the SPMC even before secure interrupt was       |
|                        | handled.                                           |
+------------------------+----------------------------------------------------+
...
| ``Total Risk Rating``  | Medium (9)       | Medium (9)      |               |
+------------------------+------------------+-----------------+---------------+
| ``Mitigations``        | During boot time, SPMC parses the partition        |
|                        | manifests to find the target execution context of a|
|                        | secure partition responsible for handling the      |
|                        | various secure physical interrupts. TF-A SPMC      |
|                        | provides mitigation against above mentioned        |
|                        | threats by keeping track of each pending virtual   |
|                        | interrupt signaled to an execution context of a    |
|                        | secure partition and denying any deactivation call |
|                        | from SP if there is no pending physical interrupt  |
|                        | mapped to the given virtual interrupt or if the    |
...
|                        | could corrupt the internal state of SPMC leading to|
|                        | an unknown state and further lead to system crash  |
...
|                        | SPMC can only process one secure interrut at a time|
|                        | , this could choke the system where all interrupt  |
|                        | are indefintely masked which could potentially lead|
|                        | to system crash or reboot.                         |
+------------------------+----------------------------------------------------+
| ``Diagram Elements``   | DF1, DF5                                           |
+------------------------+----------------------------------------------------+
| ``Affected TF-A        | SPMC                                               |
...
| ``Total Risk Rating``  | Medium (9)       | Medium (9)      |               |
+------------------------+------------------+-----------------+---------------+
| ``Mitigations``        | TF-A SPMC does not provide mitigation against such |
|                        | actions. Bounding the time taken for a SP to       |
|                        | perform signal completion can be achieved by using |
|                        | generic timer. Theoeritically, SPMC could disable  |
|                        | the secure interrupts mapped to such secure        |
|                        | partitions by using the interrupt controller driver|
+------------------------+----------------------------------------------------+

+------------------------+----------------------------------------------------+
| ID                     | 23                                                 |
+========================+====================================================+
| ``Threat``             | **An endpoint can call into another endpoint       |
...
|                        | by following the guidance for Partition runtime    |
|                        | models as described in FF-A v1.1 specification.    |
|                        | SPMC checks if the receiver endpoint is a          |
|                        | predecessor of the current endpoint in the present |
|                        | call chain and returns FFA_DENIED error code upon  |
|                        | such attempts.                                     |
+------------------------+----------------------------------------------------+
...
|                        | rendering it unable to handle a secure virtual     |
|                        | interrupt targetted for it. Secure interrupts, if  |
|                        | not handled timely, could compromise the state of  |
|                        | SP and SPMC thereby rendering the system           |
|                        | unresponsive.**                                    |
+------------------------+----------------------------------------------------+
...
| ``Total Risk Rating``  | Medium (9)       | Medium (9)      |               |
+------------------------+----------------------------------------------------+
| ``Mitigations``        | TF-A SPMC does not provide full mitigation against |
|                        | such actions. As the secure endpoint was preempted |
|                        | by a nonsecure interrupt, SPMC waits for Normal    |
|                        | world to resume the preempted secure endpoint in   |
|                        | order to signal the secure virtual interrupt to the|
|                        | execution context of target endpoint.              |
+------------------------+------------------+-----------------+---------------+
...
|                        | the secure endpoint from being scheduled. This     |
|                        | could lead to priority inversion as secure virtual |
|                        | interrupts are kept pending while non-secure       |
|                        | interrupts are handled by normal world VMs.**      |
+------------------------+----------------------------------------------------+
| ``Diagram Elements``   | DF1, DF2, DF3, DF5                                 |
+------------------------+----------------------------------------------------+
| ``Affected TF-A        | SPMC, SPMD                                         |
...
+------------------------+------------------+-----------------+---------------+
| ``Mitigations``        | TF-A SPMC does not provide full mitigation against |
|                        | such actions. However, based on guidance provided  |
|                        | in FF-A v1.1 EAC0 spec, SPMC implementation        |
