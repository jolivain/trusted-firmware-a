...
#define LSD_STQ_UNCORR_RAS_ERROR_LIST(X)                                       \
	...
	X(LSD_STQ, 40, 0x38,                                                   \
	  "Coherent Cache Data Store Uncorrectable ECC Error")                 \
	X(LSD_STQ, 38, 0x36,                                                   \
	  "Coherent Cache Data Load Uncorrectable ECC Error")                  \
...
#define DEFINE_ONE_RAS_NODE(node)                                             \
	static const struct ras_error node##_uncorr_ras_errors[] = {          \
		node##_UNCORR_RAS_ERROR_LIST(DEFINE_ONE_RAS_ERROR_MSG){ NULL, \
									0U }, \
	};                                                                    \
	static inline uint64_t node##_err_ctrl(void)                          \
	{                                                                     \
		uint64_t val = 0ULL;                                          \
		node##_UNCORR_RAS_ERROR_LIST(                                 \
			DEFINE_ENABLE_RAS_BIT) return val;                    \
	}
...
#define PER_CLUSTER_RAS_GROUP_NODES \
	PER_CLUSTER_RAS_NODE_LIST(DEFINE_ONE_RAS_AUX_DATA)
