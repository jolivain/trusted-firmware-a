				  ...
				  uint32_t time)
{
	...
	if (type > TEGRA_ARI_CROSSOVER_CCP3_SC1) {
		...
		nvg_set_request_data(
			(TEGRA_NVG_CHANNEL_CROSSOVER_C1_C6 + (uint64_t)type),
			(uint64_t)time);
	}
			       ...
			       uint32_t stats)
{
	 ...
	 */
	nvg_set_request_data(
		(TEGRA_NVG_CHANNEL_CSTATE_STATS_CLEAR + (uint64_t)state), val);
			   ...
			   uint32_t wake_time)
{
	...
	if ((state != TEGRA_ARI_CORE_C0) && (state != TEGRA_ARI_CORE_C1) &&
		...
		nvg_set_request_data((uint64_t)TEGRA_NVG_CHANNEL_IS_SC7_ALLOWED,
				     val);
...
int32_t nvg_online_core(uint32_t ari_base, uint32_t core)
{
	...
	if ((core >= MCE_CORE_ID_MAX) || (cpu == core)) {
		...
		if ((impl == DENVER_IMPL) && ((core == 2U) || (core == 3U))) {
			...
			/* get a core online */
			nvg_set_request_data(
				(uint64_t)TEGRA_NVG_CHANNEL_ONLINE_CORE,
		     ...
		     uint8_t enable)
{
	...
	nvg_set_request_data((uint64_t)TEGRA_NVG_CHANNEL_CC3_CTRL,
			     (uint64_t)val);
