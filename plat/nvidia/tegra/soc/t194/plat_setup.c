...
static inline bool tegra194_is_scr_valid(void)
{
	...
	for (uint8_t i = 0U; i < ARRAY_SIZE(t194_scr_settings); i++) {
		scr_val =
			mmio_read_32((uintptr_t)t194_scr_settings[i].scr_addr);
...
void plat_early_platform_setup(void)
{
	...
	if (enable_ccplex_lock_step != 0U) {
		...
		/* check if the bit is actually set */
		assert((read_actlr_el2() & DENVER_CPU_ENABLE_DUAL_EXEC_EL2) !=
		       0ULL);
		...
		/* check if the bit is actually set */
		assert((read_actlr_el1() & DENVER_CPU_ENABLE_DUAL_EXEC_EL1) !=
		       0ULL);
	}
}
...
{
	tegra_gic_setup(tegra194_interrupt_props,
			ARRAY_SIZE(tegra194_interrupt_props));
	tegra_gic_init();
...
struct tegra_bl31_params *plat_get_bl31_params(void)
{
	...
	val = (mmio_read_32(TEGRA_SCRATCH_BASE + SCRATCH_BL31_PARAMS_HI_ADDR) &
	       SCRATCH_BL31_PARAMS_HI_ADDR_MASK) >>
...
plat_params_from_bl2_t *plat_get_bl31_plat_params(void)
{
	...
	val = (mmio_read_32(TEGRA_SCRATCH_BASE +
			    SCRATCH_BL31_PLAT_PARAMS_HI_ADDR) &
	       SCRATCH_BL31_PLAT_PARAMS_HI_ADDR_MASK) >>
	      SCRATCH_BL31_PLAT_PARAMS_HI_ADDR_SHIFT;
	val <<= 32;
	val |= mmio_read_32(TEGRA_SCRATCH_BASE +
			    SCRATCH_BL31_PLAT_PARAMS_LO_ADDR);
