...
int UART_Init (UART_DEV_T devNum,  UART_BAUDRATE_T baudRate)
{
	...
	volatile struct npcm845x_gcr *gcr_ctl_obj = (struct npcm845x_gcr *)gcr_base;

	/* Clear bits 8/9 */
#ifdef NPCM845X_DEBUG
	mmio_write_32(clk_ctl_obj->clksel, (mmio_read_32(clk_ctl_obj->clksel) & ~(0x3 << 8)));
#endif /* NPCM845X_DEBUG */
...
#ifdef NPCM845X_DEBUG
	mmio_write_32(clk_ctl_obj->clksel ,(mmio_read_32((clk_ctl_obj->clksel) | (0x2 << 8)));
#endif /* NPCM845X_DEBUG */
	...
	/* clear bits 16-20 */
#ifdef NPCM845X_DEBUG
	mmio_write_32(clk_ctl_obj->clkdiv1 , (mmio_read_32(clk_ctl_obj->clkdiv1) & ~(0x1F << 16)));
#endif /* NPCM845X_DEBUG */

	clk_ctl_obj->clkdiv1 = (unsigned int)(clk_ctl_obj->clkdiv1 & ~(0x1F << 16));

	/* clear bits 11-15 - set value 0 */
	if (devNum == UART3_DEV) {
		clk_ctl_obj->clkdiv2 = (unsigned int)(clk_ctl_obj->clkdiv2 & ~(0x1F << 11));
	}
	...
	*/
	...
	mmio_write_8((uintptr_t)(&uart->fcr), (uint8_t)val); /* reset TX and RX FIFO */
	...
	*/
	...
	if (devNum == UART0_DEV) {
		...
		val &= ~(1 << 1);	/* BSPASEL (BMC Serial Port Alternate Port Select). GPO48/TXD2 and GPIO49/RXD2 are selected (default). */
		mmio_write_32((uintptr_t)(gcr_ctl_obj->mfsel4), val);
		
		/* Set BU0 Mux pins */
		val = mmio_read_32((uintptr_t)(gcr_ctl_obj->mfsel1));
		val |= 1 << 9;	/* Selects Core Serial Port 0 or GPIO option. If this bit is set, MFSEL6 bits 1 and 31 must be set to 0. 0: GPIO41 and GPIO42 selected */
		...
		/* Set SI2 Mux pins */
		val = mmio_read_32((uintptr_t)(gcr_ctl_obj->mfsel1));
		val |= 1<<11; /* HSI2SEL (Host Serial Interface 2 Select). TXD2, RXD2, nCTS2, nRTS2, nDCD2, nDTR2_BOUT2, nDSR2 and nRI2 selected. */
		mmio_write_32((uintptr_t)(gcr_ctl_obj->mfsel1), val);

		/* Set UART3 output to SI2 */
		val = mmio_read_32((uintptr_t)(gcr_ctl_obj->spswc));
		val &= (7 << 0);	/* clear bits 2-0 */
		val |= (2 << 0);	/* Mode uart_n - Host SP1 connected to BMC UART1, Host SP2 connected to BMC UART2, BMC UART3 connected to Serial Interface 2 */
