...
static int32_t zynqmp_nopmu_pwr_domain_on(u_register_t mpidr)
{
	...
	apu_pcli_cluster = APU_PCLI + APU_PCLI_CLUSTER_OFFSET +
			   (cluster * APU_PCLI_CLUSTER_STEP);
	apu_cluster_base = APU_CLUSTER0 + (cluster * APU_CLUSTER_STEP);

	/* Enable clock */
	mmio_setbits_32(PSX_CRF + ACPU0_CLK_CTRL + (cluster * 0x4),
			ACPU_CLK_CTRL_CLKACT);

	/* Enable cluster states */
	mmio_setbits_32(apu_pcli_cluster + PCLI_PSTATE_OFFSET,
			PCLI_PSTATE_VAL_SET);
	mmio_setbits_32(apu_pcli_cluster + PCLI_PREQ_OFFSET,
			PREQ_CHANGE_REQUEST);

	/* assert core reset */
	mmio_setbits_32(rst_apu_cluster,
			((RST_APU_COLD_RESET | RST_APU_WARN_RESET) << cpu));
	...
	mmio_clrbits_32(rst_apu_cluster,
			((RST_APU_COLD_RESET | RST_APU_WARN_RESET) << cpu));
	...
	mmio_write_32(apu_pcli_base + PCLI_PSTATE_OFFSET,
		      PCLI_PSTATE_VAL_CLEAR);
...
static int32_t no_pm_ioctl(uint32_t device_id, uint32_t ioctl_id, uint32_t arg1,
			   uint32_t arg2)
{
...
static uint64_t no_pm_handler(uint32_t smc_fid, uint64_t x1, uint64_t x2,
