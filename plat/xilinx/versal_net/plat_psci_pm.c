...
static int32_t versal_net_pwr_domain_on(u_register_t mpidr)
{
	...
	pm_req_wakeup(proc->node_id,
		      (versal_net_sec_entry & 0xFFFFFFFFU) | 0x1U,
...
static void __dead2 versal_net_system_reset(void)
{
	...
	pm_system_shutdown(XPM_SHUTDOWN_TYPE_RESET, pm_get_shutdown_scope(),
			   SECURE_FLAG);
 ...
 */
static void
versal_net_pwr_domain_suspend(const psci_power_state_t *target_state)
{
	uint32_t state;
	uint32_t cpu_id = plat_my_core_pos();
	const struct pm_proc *proc = pm_get_proc(cpu_id);
...
static void
versal_net_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
	(void)target_state;
 ...
 */
static void
versal_net_pwr_domain_suspend_finish(const psci_power_state_t *target_state)
{
	uint32_t cpu_id = plat_my_core_pos();
	const struct pm_proc *proc = pm_get_proc(cpu_id);
...
static void __dead2 versal_net_system_off(void)
{
	...
	pm_system_shutdown(XPM_SHUTDOWN_TYPE_SHUTDOWN, pm_get_shutdown_scope(),
			   SECURE_FLAG);
 ...
 */
static void
versal_net_get_sys_suspend_power_state(psci_power_state_t *req_state)
{
	uint64_t i;
