				...
				u_register_t arg2, u_register_t arg3)
{
	...
	uint32_t payload[PAYLOAD_ARG_CNT],
		max_size = ATF_HANDOFF_PARAMS_MAX_SIZE;
	...
	if (VERSAL_CONSOLE_IS(pl011) || (VERSAL_CONSOLE_IS(pl011_1))) {
		...
		int32_t rc =
			console_pl011_register((uintptr_t)VERSAL_UART_BASE,
		...
		console_set_scope(&versal_runtime_console,
				  (uint32_t)(CONSOLE_FLAG_BOOT |
	...
	PM_PACK_PAYLOAD4(payload, LOADER_MODULE_ID, 1,
			 PM_LOAD_GET_HANDOFF_PARAMS, (uintptr_t)addr >> 32U,
			 (uintptr_t)addr, max_size);
	...
	enum fsbl_handoff ret = fsbl_atf_handover(
		&bl32_image_ep_info, &bl33_image_ep_info, atf_handoff_addr);
