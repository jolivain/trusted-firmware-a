...
static void request_cpu_idle(void)
{
	...
	if (idle_requests < active_cores) {
		...
		for (i = 0; i < PLATFORM_CORE_COUNT; i++) {
			...
			plat_ic_raise_el3_sgi(VERSAL_CPU_IDLE_SGI,
					      (u_register_t)i);
		}
	}
}
...
static uint64_t ipi_fiq_handler(uint32_t id, uint32_t flags, void *handle,
				void *cookie)
{
	uint32_t payload[4] = {0U};

	VERBOSE("Received IPI FIQ from firmware\r\n");

	(void)plat_ic_acknowledge_interrupt();

	pm_get_callbackdata(payload, ARRAY_SIZE(payload), 0U, 0U);
	switch (payload[0]) {
...
int pm_setup(void)
{
	...
	ret = pm_register_notifier(XPM_DEVID_ACPU_0,
				   EVENT_CPU_IDLE_FORCE_PWRDWN, 0U, 1U, 0U);
	if (ret != 0) {
