...
int32_t pm_setup(void)
{
	...
	if (pm_ctx.api_version < PM_VERSION) {
		ERROR("BL31: Platform Management API version error. Expected: "
		      ...
		      pm_ctx.api_version >> 16, pm_ctx.api_version & 0xFFFFU);
		return -EINVAL;
	}
			...
			uint64_t flags)
{
	...
	case PM_REQ_WAKEUP: {
		...
		ret = pm_req_wakeup(pm_arg[0], set_addr, address, pm_arg[3]);
		SMC_RET1(handle, (uint64_t)ret);
	}
	...
	case PM_IOCTL: {
		...
		ret = pm_ioctl(pm_arg[0], pm_arg[1], pm_arg[2], pm_arg[3],
			       &value);
		SMC_RET1(handle, (uint64_t)ret | ((uint64_t)value) << 32);
	}
	...
	case PM_QUERY_DATA: {
		...
		pm_query_data(pm_arg[0], pm_arg[1], pm_arg[2], pm_arg[3], data);
	...
	case PM_SECURE_SHA:
		ret = pm_sha_hash(pm_arg[0], pm_arg[1], pm_arg[2], pm_arg[3]);
		SMC_RET1(handle, (uint64_t)ret);

	case PM_SECURE_RSA:
		ret = pm_rsa_core(pm_arg[0], pm_arg[1], pm_arg[2], pm_arg[3]);
		SMC_RET1(handle, (uint64_t)ret);
	...
	case PM_FPGA_GET_FEATURE_LIST: {
		...
		PM_PACK_PAYLOAD5(payload, smc_fid & FUNCID_NUM_MASK, pm_arg[0],
				 pm_arg[1], pm_arg[2], pm_arg[3]);
		ret = pm_ipi_send_sync(primary_proc, payload, ret_payload, 3U);
		SMC_RET2(handle,
			 (uint64_t)ret | (uint64_t)ret_payload[0] << 32U,
			 (uint64_t)ret_payload[1] | (uint64_t)ret_payload[2]
							    << 32U);
	}
