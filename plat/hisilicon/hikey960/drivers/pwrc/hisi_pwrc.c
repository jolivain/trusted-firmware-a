...
enum pdc_gic_mask { PDC_MASK_GIC_WAKE_IRQ, PDC_UNMASK_GIC_WAKE_IRQ };

enum pdc_finish_int_mask { PDC_DISABLE_FINISH_INT, PDC_ENABLE_FINISH_INT };
...
void hisi_set_cluster_pwdn_flag(unsigned int cluster, unsigned int core,
				unsigned int value)
{
	unsigned int val;
...
static int check_hotplug(unsigned int cluster, unsigned int boot_flag)
{
	...
	if (hisi_test_ap_suspend_flag() || ((boot_flag & mask) == mask))
		return 0;
...
void hisi_pdc_set_intmask(void *pdc_base_addr, unsigned int core,
...
static inline void hisi_pdc_set_gicmask(void *pdc_base_addr, unsigned int core,
