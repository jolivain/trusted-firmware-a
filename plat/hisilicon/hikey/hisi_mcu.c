...
{
	/* invalid cases */
	if ((head->image_size == 0) || (head->image_size > length) ||
	    (head->secs_num > MCU_SECTION_MAX) || (head->secs_num == 0))
		return 1;
				     ...
				     struct mcu_image_head *head)
{
	...
	if ((ap_dst_offset >= SOC_SRAM_M3_BASE_ADDR) &&
		...
		return 0;
		 ...
		 (ap_dst_offset <
		  MCU_SYS_MEM_ADDR + MCU_SYS_MEM_SIZE - sec->size))
		return 0;
...
int hisi_mcu_load_image(uintptr_t image_base, uint32_t image_size)
{
	...
	for (i = 0; i < head->secs_num; i++) {
		...
		INFO("%s:  [SRC 0x%x] 0x%x 0x%x 0x%x 0x%x\n", __func__,
		     (unsigned int)(uintptr_t)src, src[0], src[1], src[2],
		     src[3]);
