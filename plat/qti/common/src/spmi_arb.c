...
/*
 * THIS DRIVER DOES NOT ARBITRATE ACCESS WITH THE KERNEL! ONLY USE WHEN KERNEL
 * IS KNOWN TO BE QUIESCED (e.g. before boot or during shutdown)!
 */
...
static int addr_to_apid(uint32_t addr)
{
	...
	for (i = 0; i < NUM_APID; i++) {
		...
		if (reg != 0 && (addr & PPID_MASK) == (reg & PPID_MASK))
			return i;
	}
...
static int wait_for_done(uint16_t apid)
{
	...
	while (timeout--) {
		...
		if (status & ARB_STATUS_DONE) {
			if (status & ARB_STATUS_FAILURE ||
			    status & ARB_STATUS_DENIED ||
			    status & ARB_STATUS_DROPPED)
				return status & 0xff;
			return 0;
		}
		mdelay(1);
...
static void arb_command(uint16_t apid, uint8_t opcode, uint32_t addr,
			uint8_t bytes)
{
	mmio_write_32(REG_ARB_CMD(apid),
		      opcode << 27 | (addr & 0xff) << 4 | (bytes - 1));
}
