...
#ifdef USE_AUTH_EC
static int xor_bytes(uint8_t *output, uint8_t *input1,
			const uint8_t *input2, size_t bytes)
{
	size_t i;

	if ((output == NULL) || (input1 == NULL) || (input2 == NULL)) {
		ERROR("XOR: parameter NULL!\n");
		return -1;
	}
	for (i = 0; i < bytes; i++)
...
int plat_ec_decryption_enabled(void)
{
	uint64_t row_data;
	int i;

	/* SOTP.EC_EN bit must be set */
	if ((sotp_mem_read(SOTP_ATF2_CFG_ROW_ID, SOTP_ROW_NO_ECC) &
			SOTP_EC_EN_MASK) == 0)
		return -1;

	/* If there is a valid key, it is ENABLED */
	if (!(mmio_read_32(SOTP_STATUS_1) & SOTP_K_AES_ECC_ERROR_MASK))
		return 0;

	/* if the entire section is 0x0, it is not ENABLED */
	for (i = 0; i < (SOTP_NVCOUNTER_ROW - SOTP_K_AES_ROW); i++) {
		row_data = sotp_mem_read(SOTP_K_AES_ROW + i, SOTP_ROW_NO_ECC);
		if (row_data != 0)
			break;
	}

	/* If KEAS section is empty, it is DISABLED */
	if (i == (SOTP_NVCOUNTER_ROW - SOTP_K_AES_ROW)) {
		INFO("EC: SOTP not programmed.\n");
		return -1;
	}

	/* Let user know if CRC does not match with what is in KAES rows */
	if ((mmio_read_32(SOTP_STATUS_1) & SOTP_K_AES_ECC_ERROR_MASK))
		WARN("EC: CRC error\n");

	/* Decryption will be ENABLED */
	return 0;
}

/* K_ECC = SOTP.slice XOR CERT.slice XOR ROM.slice */
int plat_generate_key_from_slices(unsigned char *private_key,
				const unsigned char *cert_slice)
{
	uint8_t sotp_slice[SHA256_BYTES];
	uint8_t tmp_slice[SHA256_BYTES];
	int ret;

	if (private_key == NULL)
		return -1;

	memset(private_key, 0x0, SHA256_BYTES);

	/* Get SOTP.slice */
	ret = sotp_read_key(sotp_slice,
			SHA256_BYTES,
			SOTP_K_AES_ROW,
			SOTP_NVCOUNTER_ROW-1);

	if (ret != 0) {
		ERROR("Could not retrieve the SOTP.slice\n");
		return -1;
	}

	ret = xor_bytes(tmp_slice, sotp_slice, cert_slice, SHA256_BYTES);
	if (ret != 0)
		return -1;

	ret = xor_bytes(private_key, tmp_slice, rom_slice, SHA256_BYTES);
	if (ret != 0)
		return -1;
