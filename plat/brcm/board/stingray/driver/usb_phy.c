...
static usb_phy_port_t usb_phy_port[2][MAX_NR_PORTS];

static usb_phy_t usb_phy_info[2] = {
	{DRDU2_U2PLL_NDIV_FRAC, USB3H_PIPE_CTRL, 0, USB3H_DRDU2_PHY},
	{0, 0, DRDU3_PIPE_CTRL, DRDU3_PHY}
};
...
int32_t u3h_u2drd_phy_reset(usb_phy_port_t *phy_port)
{
...
int32_t u3drd_phy_reset(usb_phy_port_t *phy_port)
{
	usb_phy_t *phy = phy_port->p;
...
static int32_t u3h_u2drd_phy_power_off(usb_phy_port_t *phy_port)
{
...
static int32_t u3drd_phy_power_off(usb_phy_port_t *phy_port)
{
	usb_phy_t *p = phy_port->p;
...
int32_t usb_info_fill(usb_phy_t *phy_info)
{
	...
	if (phy_info->initialized)
		return USB_PHY_ALREADY_STARTED;

	if (phy_info->phy_id == USB3H_DRDU2_PHY) {
		phy_info->phy_port = usb_phy_port[USB3H_DRDU2_PHY - 1];
		phy_info->ports_enabled = 0x7;
	} else {
		phy_info->phy_port = usb_phy_port[DRDU3_PHY - 1];
		phy_info->ports_enabled = 0x3;
	}

	for (index = MAX_NR_PORTS - 1; index > -1; index--) {
...
int32_t usb_phy_init(usb_platform_dev *device)
{
	...
	for (index = 0; index < MAX_NR_PORTS; index++) {
		if (phy_info->phy_port[index].enabled) {
	...
	for (index = 0; index < MAX_NR_PORTS; index++) {
		if (phy_info->phy_port[index].enabled) {
			...
			case DRDU3_PHY:
				...
				break;
			default:
				INFO("usb_phy_shutdown: invalid phy id 0x%x\n",
				     phy_info->phy_id);
			}
		}
	}
