				      ...
				      unsigned int reset_type)
{
	...
	if (reset_type & PLL_RESET_TYPE_POST) {
		pll_rst_ctrl &=
			~(0x1 << A72_CRM_PLL_PWR_ON__PLL0_POST_RESETB_R);
	}
...
static void ARMCOE_crm_pllSetMode(unsigned int cluster_num, unsigned int mode)
{
	...
	ihostx_config_root = ARMCOE_crm_getBaseAddress(cluster_num);
	pll_byp_ctrl =
		mmio_read_32(ihostx_config_root + A72_CRM_PLL_CHNL_BYPS_EN);
				  ...
				  unsigned int pdiv)
{
	...
	mmio_write_32(ihostx_config_root + A72_CRM_PLL0_CTRL3,
		      ndiv_int | ((pdiv << A72_CRM_PLL0_CTRL3__PLL0_PDIV_R &
				   0xF000)));
...
static unsigned int ARMCOE_crm_pllIsLocked(unsigned int cluster_num)
{
	...
	for (i = 0; i < 1500; i++) {
		insert_delay(256);
		lock_status =
			mmio_read_32(ihostx_config_root + A72_CRM_PLL_STATUS);
...
uint32_t bcm_get_ihost_pll_freq(uint32_t cluster_num)
{
	...
	ihostx_config_root = ARMCOE_crm_getBaseAddress(cluster_num);
	ndiv_int = mmio_read_32(ihostx_config_root + A72_CRM_PLL0_CTRL3) &
		   0x3FF;
