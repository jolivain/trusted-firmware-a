...
#define FSX_RINGx_BASE(__b, __i) ((__b) + (__i)*0x10000)

#define FSX_RINGx_VERSION_NUMBER(__b, __i) (FSX_RINGx_BASE(__b, __i) + 0x0)

#define FSX_RINGx_MSI_DEV_ID(__b, __i) (FSX_RINGx_BASE(__b, __i) + 0x44)

#define FSX_COMM_RINGx_BASE(__b, __i) ((__b) + 0x200000 + (__i)*0x100)

#define FSX_COMM_RINGx_CONTROL(__b, __i) (FSX_COMM_RINGx_BASE(__b, __i) + 0x0)
...
#define FSX_AEx_BASE(__b, __i) ((__b) + 0x202000 + (__i)*0x100)

#define FSX_AEx_CONTROL_REGISTER(__b, __i) (FSX_AEx_BASE(__b, __i) + 0x0)
...
#define FSX_DMEx_BASE(__b, __i) ((__b) + (__i)*0x1000)

#define FSX_DMEx_AXI_CONTROL(__b, __i) (FSX_DMEx_BASE(__b, __i) + 0x4)
...
#define FSX_DMEx_WR_FIFO_THRESHOLD(__b, __i) (FSX_DMEx_BASE(__b, __i) + 0xc)
...
#define FSX_DMEx_RD_FIFO_THRESHOLD(__b, __i) (FSX_DMEx_BASE(__b, __i) + 0x14)
...
void fsx_init(eFSX_TYPE fsx_type, unsigned int ring_count,
	      unsigned int dme_count, unsigned int ae_count,
	      unsigned int start_stream_id, unsigned int msi_dev_id,
	      uintptr_t idm_io_control_direct, uintptr_t idm_reset_control,
	      uintptr_t base, uintptr_t dme_base)
{
	int try;
	...
	VERBOSE(" - set AXI read burst threshold = 0x%x\n",
		mmio_read_32(fsx_comm_rm +
			     FSX_COMM_RM_AXI_READ_BURST_THRESHOLD));
	 ...
	 */
	mmio_write_32(fsx_comm_rm + FSX_COMM_RM_CONFIG_INTERRUPT_STATUS_CLEAR,
		      0xffffffff);
	...
	for (i = 0; i < ring_count; i++) {
		...
		mmio_write_32(FSX_COMM_RINGx_MSI_DEV_ID(base, i), msi_dev_id);
		v = 0;
...
void fsx_meminit(const char *name, uintptr_t idm_io_control_direct,
...
void fs4_disable_clocks(bool disable_sram, bool disable_crypto,
			bool disable_raid)
{
