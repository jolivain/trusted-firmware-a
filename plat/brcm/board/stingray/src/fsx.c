		 ...
		 uintptr_t idm_io_status)
{
	...
	mmio_setbits_32(idm_io_control_direct,
			(1 << FS4_IDM_IO_CONTROL_DIRECT__MEM_ARRPOWERON));
	while (!(mmio_read_32(idm_io_status) &
		 (1 << FS4_IDM_IO_STATUS__MEM_ARRPOWERON)))
	;
	...
	while (!(mmio_read_32(idm_io_status) &
		 (1 << FS4_IDM_IO_STATUS__MEM_ARRPOWEROK)))
	;
	...
	while (!(mmio_read_32(idm_io_status) &
		 (1 << FS4_IDM_IO_STATUS__MEM_POWERON)))
	;
	...
	while (!(mmio_read_32(idm_io_status) &
		 (1 << FS4_IDM_IO_STATUS__MEM_POWEROK)))
	;
