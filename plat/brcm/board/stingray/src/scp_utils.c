...
{
	return (scp_send_cmd(MCU_IPC_MCU_CMD_NOP, 0,
...
int plat_bcm_bl2_plat_handle_scp_bl2(image_info_t *scp_bl2_image_info)
{
	 ...
	 */
	scp_patch_activated = (scp_send_cmd(MCU_IPC_MCU_CMD_NOP, 0,
					    SCP_CMD_DEFAULT_TIMEOUT_US) == 0);
	if (scp_patch_activated) {
		...
		if (mcu_ap_init_param & MCU_PATCH_LOADED_BY_NITRO) {
			...
			SCP_WRITE_CFG(board_cfg.mcu_init_param,
				      mcu_ap_init_param &
		...
		ret = scp_send_cmd(MCU_IPC_MCU_CMD_INIT, MCU_PATCH_LOADED_BY_AP,
