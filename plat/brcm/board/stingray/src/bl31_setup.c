...
static void brcm_stingray_gain_qspi_control(void)
{
	if (boot_source_get() != BOOT_SOURCE_QSPI) {
		if (bcm_chimp_is_nic_mode() && (!bcm_chimp_handshake_done())) {
			/*
...
#define CDRU_SATA_RESET_N BIT(CDRU_MISC_RESET_CONTROL__CDRU_SATA_RESET_N_R)
#define CDRU_MISC_CLK_SATA BIT(CDRU_MISC_CLK_ENABLE_CONTROL__CDRU_SATA_CLK_EN_R)
...
#define SATA_APBT_IDM_PORT_REG(port, reg) (((port / 4) << 12) + reg)

#define SATA_IDM_PORT_REG(port, reg) ((port << 12) + reg)

#define SATA_PORT_REG(port, reg) (((port % 4) << 16) + ((port / 4) << 20) + reg)
...
static void brcm_stingray_sata_init(void)
{
	...
	mmio_setbits_32(CDRU_MISC_CLK_ENABLE_CONTROL, CDRU_MISC_CLK_SATA);
	...
	for (port = 0; port < USE_SATA_PORTS; port++) {
		...
		mmio_write_32(SATA_APBT_IDM_PORT_REG(
				      sata_port, SATA_APBT0_IDM_RESET_CONTROL),
			      0x0);
		mmio_setbits_32(SATA_APBT_IDM_PORT_REG(
					sata_port,
...
static void poweroff_sata_pll(void)
{
	...
	/* enable isolation */
	mmio_setbits_32(CRMU_AON_CTRL1, BIT(CRMU_AON_CTRL1__LCPLL0_ISO_IN));

	/* Power off the SATA PLL/LDO */
	mmio_clrbits_32(CRMU_AON_CTRL1, (BIT(CRMU_AON_CTRL1__LCPLL0_PWRON_LDO) |
...
static void brcm_stingray_pka_meminit(void)
{
	...
	VERBOSE(" - arrpoweron\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_PKA_MEM_PWR_CTRL__ARRPOWERONIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_PKA_MEM_PWR_CTRL__ARRPOWERONOUT))
		;

	VERBOSE(" - arrpowerok\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_PKA_MEM_PWR_CTRL__ARRPOWEROKIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_PKA_MEM_PWR_CTRL__ARRPOWEROKOUT))
		;

	VERBOSE(" - poweron\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_PKA_MEM_PWR_CTRL__POWERONIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_PKA_MEM_PWR_CTRL__POWERONOUT))
		;

	VERBOSE(" - powerok\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_PKA_MEM_PWR_CTRL__POWEROKIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
...
static void brcm_stingray_smmu_init(void)
{
	...
	VERBOSE(" - scr0=0x%x scr1=0x%x scr2=0x%x\n",
		mmio_read_32(smmu_base + 0x0), mmio_read_32(smmu_base + 0x4),
		mmio_read_32(smmu_base + 0x8));

	VERBOSE(" - idr0=0x%x idr1=0x%x idr2=0x%x\n",
		mmio_read_32(smmu_base + 0x20), mmio_read_32(smmu_base + 0x24),
		mmio_read_32(smmu_base + 0x28));

	VERBOSE(" - idr3=0x%x idr4=0x%x idr5=0x%x\n",
		mmio_read_32(smmu_base + 0x2c), mmio_read_32(smmu_base + 0x30),
...
static void brcm_stingray_dma_pl330_meminit(void)
{
	...
	VERBOSE(" - arrpoweron\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_DMAC_MEM_PWR_CTRL__ARRPOWERONIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_DMAC_MEM_PWR_CTRL__ARRPOWERONOUT))
		;

	VERBOSE(" - arrpowerok\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_DMAC_MEM_PWR_CTRL__ARRPOWEROKIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_DMAC_MEM_PWR_CTRL__ARRPOWEROKOUT))
		;

	VERBOSE(" - poweron\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_DMAC_MEM_PWR_CTRL__POWERONIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_DMAC_MEM_PWR_CTRL__POWERONOUT))
		;

	VERBOSE(" - powerok\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_DMAC_MEM_PWR_CTRL__POWEROKIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
...
static void brcm_stingray_audio_init(void)
{
	...
	VERBOSE(" - arrpoweron\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_AUDIO_POWER_CTRL__ARRPOWERONIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_AUDIO_POWER_CTRL__ARRPOWERONOUT))
		;

	VERBOSE(" - arrpowerok\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_AUDIO_POWER_CTRL__ARRPOWEROKIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_AUDIO_POWER_CTRL__ARRPOWEROKOUT))
		;

	VERBOSE(" - poweron\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_AUDIO_POWER_CTRL__POWERONIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
		 ICFG_AUDIO_POWER_CTRL__POWERONOUT))
		;

	VERBOSE(" - powerok\n");
	mmio_setbits_32(icfg_mem_ctrl, ICFG_AUDIO_POWER_CTRL__POWEROKIN);
	while (!(mmio_read_32(icfg_mem_ctrl) &
...
#error "USE_FS4 and USE_FS6 should not be used together"
	...
	fsx_init(eFS4_RAID, FS4_RAID_RING_COUNT, FS4_RAID_DME_COUNT,
		 ...
		 FS4_RAID_IDM_RESET_CONTROL, FS4_RAID_BASE, FS4_RAID_DME_BASE);

	fsx_meminit("raid", FS4_RAID_IDM_IO_CONTROL_DIRECT,
					...
					bl_params_t *plat_params_from_bl2)
{
	 ...
	 */
	if ((boot_source_get() != BOOT_SOURCE_QSPI) &&
	    (!bcm_chimp_is_nic_mode()) && (!bcm_chimp_wait_handshake())) {
	...
	flush_dcache_range(PLAT_BRCM_TRUSTED_SRAM_BASE,
