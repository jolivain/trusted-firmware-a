...
#define ARM_SMMU_NSNUMCBO \
	(ARM_SMMU_MAX_NUM_CNTXT_BANK - ARM_SMMU_NUM_SECURE_MASTER)
#define ARM_SMMU_NSNUMSMRGO \
	(ARM_SMMU_MAX_NUM_CNTXT_BANK - ARM_SMMU_NUM_SECURE_MASTER)
/* Reserved Banks. */
#define SMMU_CTX_BANK_IDX \
	(SMMU_CTX_BANK_IDX_SECURE_CRMU - ARM_SMMU_NUM_SECURE_MASTER)
...
static void arm_smmu_smr_cfg(struct arm_smmu_device *smmu, uint32_t index)
{
	...
	mmio_write_32((uintptr_t)(ARM_SMMU_GR0(smmu) + ARM_SMMU_GR0_SMR(idx)),
		      reg);
}
...
static void arm_smmu_s2cr_cfg(struct arm_smmu_device *smmu, uint32_t index)
{
	...
	mmio_write_32((uintptr_t)(ARM_SMMU_GR0(smmu) + ARM_SMMU_GR0_S2CR(idx)),
		      reg);
}

static void smmu_set_pgtbl(struct arm_smmu_device *smmu, enum iommu_domain dom,
...
void arm_smmu_create_identity_map(enum iommu_domain dom)
{
	...
	for (idx = 0; idx < smmu->streams; idx++) {
		...
		mmio_write_32(
			(uintptr_t)(ARM_SMMU_CB_BASE(smmu) +
		...
		reg = (ARM_LPAE_MAIR_ATTR_NC
		      ...
		      (ARM_LPAE_MAIR_ATTR_WBRWA << ARM_LPAE_MAIR_ATTR_SHIFT(
