...
static bl31_gpio_info_t rst_gpio;
static bl31_gpio_info_t poweroff_gpio;
static bl31_gpio_info_t suspend_gpio[10];
uint32_t suspend_gpio_cnt;
static bl31_rk_apio_info_t suspend_apio;
...
bl31_gpio_info_t *plat_get_rockchip_gpio_reset(void)
{
	return &rst_gpio;
}

bl31_gpio_info_t *plat_get_rockchip_gpio_poweroff(void)
{
	return &poweroff_gpio;
}

bl31_gpio_info_t *plat_get_rockchip_suspend_gpio(uint32_t *count)
{
...
bl31_rk_apio_info_t *plat_get_rockchip_suspend_apio(void)
{
	return &suspend_apio;
}

static bool rk_plat_param_handler(bl31_plat_param_header_t *param)
{
	/* Store platform parameters for later processing if needed. */
	switch (param->type) {
	case BL31_PLAT_PARAM_RK_RESET_GPIO:
		rst_gpio = ((bl31_plat_param_gpio_t *)param)->gpio;
		return true;
	case BL31_PLAT_PARAM_RK_POWEROFF_GPIO:
		poweroff_gpio = ((bl31_plat_param_gpio_t *)param)->gpio;
		return true;
	case BL31_PLAT_PARAM_RK_SUSPEND_GPIO:
		if (suspend_gpio_cnt >= ARRAY_SIZE(suspend_gpio)) {
			ERROR("Exceeded the supported suspend GPIO number.\n");
			return true;
		}
		suspend_gpio[suspend_gpio_cnt++] =
			((bl31_plat_param_gpio_t *)param)->gpio;
		return true;
	case BL31_PLAT_PARAM_RK_SUSPEND_APIO:
		suspend_apio = ((bl31_plat_param_rk_apio_t *)param)->apio;
		return true;
	}
...
void params_early_setup(u_register_t plat_param_from_bl2)
{
	...
	bl31_plat_params_parse(plat_param_from_bl2, rk_plat_param_handler);
}
