...
static bl_aux_gpio_info_t rst_gpio;
static bl_aux_gpio_info_t poweroff_gpio;
static bl_aux_gpio_info_t suspend_gpio[10];
uint32_t suspend_gpio_cnt;
static bl_aux_rk_apio_info_t suspend_apio;
...
bl_aux_gpio_info_t *plat_get_rockchip_gpio_reset(void)
{
	return &rst_gpio;
}

bl_aux_gpio_info_t *plat_get_rockchip_gpio_poweroff(void)
{
	return &poweroff_gpio;
}

bl_aux_gpio_info_t *plat_get_rockchip_suspend_gpio(uint32_t *count)
{
...
bl_aux_rk_apio_info_t *plat_get_rockchip_suspend_apio(void)
{
	return &suspend_apio;
}

static bool rk_aux_param_handler(bl_aux_param_header_t *param)
{
	/* Store platform parameters for later processing if needed. */
	switch (param->type) {
	case BL_AUX_PARAM_RK_RESET_GPIO:
		rst_gpio = ((bl_aux_param_gpio_t *)param)->gpio;
		return true;
	case BL_AUX_PARAM_RK_POWEROFF_GPIO:
		poweroff_gpio = ((bl_aux_param_gpio_t *)param)->gpio;
		return true;
	case BL_AUX_PARAM_RK_SUSPEND_GPIO:
		...
		suspend_gpio[suspend_gpio_cnt++] =
			((bl_aux_param_gpio_t *)param)->gpio;
		return true;
	case BL_AUX_PARAM_RK_SUSPEND_APIO:
		suspend_apio = ((bl_aux_param_rk_apio_t *)param)->apio;
		return true;
	}
