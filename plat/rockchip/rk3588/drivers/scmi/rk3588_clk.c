...
{
	static bool is_b0pll_disabled = false;

	if (is_b0pll_disabled)
		return;
...
{
	static bool is_b1pll_disabled = false;

	if (is_b1pll_disabled)
		return;
...
{
	static bool is_lpll_disabled = false;
...
static unsigned long clk_scmi_gpu_get_rate(rk_scmi_clock_t *clock)
{
	...
	if (mmio_read_32(CRU_BASE + CRU_CLKSEL_CON(158)) & 0x4000) {
...
static unsigned long clk_scmi_npu_get_rate(rk_scmi_clock_t *clock)
{
	...
	if (mmio_read_32(CRU_BASE + CRU_CLKSEL_CON(74)) & 0x1) {
...
static unsigned long clk_scmi_sbus_get_rate(rk_scmi_clock_t *clock)
{
	...
	if (mmio_read_32(BUSSCRU_BASE + CRU_CLKSEL_CON(0)) & 0x0800) {
