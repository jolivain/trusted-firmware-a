...
static void pmu_power_domains_suspend(void)
{
	...
	if (!(ddr_data.pmu_pd_st0 & BIT(PD_PHP)))
		pd_php_save();

	if (!(ddr_data.pmu_pd_st0 & BIT(PD_CRYPTO)))
		pd_crypto_save();
...
static void ddr_sleep_config(void)
{
	...
	if (!pmu_power_domain_st(PD_DDR01)) {
	...
	if (!pmu_power_domain_st(PD_DDR23)) {
...
void pm_pll_wait_lock(uint32_t pll_base)
{
	...
	if (mmio_read_32(pll_base + CRU_PLL_CON(1)) & CRU_PLLCON1_PWRDOWN)
		return;
