...
static uint32_t gpio_get_clock(uint32_t gpio_number)
{
	...
	if ((mmio_read_32(info->clkgate_reg) & 1 << info->clkgate_bit) == 0) {
		return 0U;
	}
	mmio_write_32(
		info->clkgate_reg,
...
/* restores old state of gpio clock, assuming it is running now */
...
static int get_value(int gpio)
{
	...
	clock_state = gpio_get_clock(gpio);
	value = mmio_read_32(port_info[port].port_base + EXT_PORTA) >> num & 0x1;
	gpio_put_clock(gpio, clock_state);
