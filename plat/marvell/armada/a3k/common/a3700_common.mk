...
TIM_CFG		:= $(abspath $(BUILD_PLAT)/atf-tim.txt)
TIM_UART_CFG	:= $(abspath $(BUILD_PLAT)/$(BUILD_UART)/atf-tim.txt)
IMAGESPATH	:= $(WTP)/tim/trusted
TIMN_CFG	:= $(abspath $(BUILD_PLAT)/atf-timN.txt)
TIMN_UART_CFG	:= $(abspath $(BUILD_PLAT)/$(BUILD_UART)/atf-timN.txt)
...
TIM_CFG		:= $(abspath $(BUILD_PLAT)/atf-ntim.txt)
TIM_UART_CFG	:= $(abspath $(BUILD_PLAT)/$(BUILD_UART)/atf-ntim.txt)
...
ifeq ($(MARVELL_SECURE_BOOT),1)
	...
	$(Q)cd $(BUILD_PLAT) && $(TIM2IMG) $(TIM2IMGARGS) -o $(abspath $(BUILD_PLAT)/$(FLASH_IMAGE))
	@$(ECHO_BLANK_LINE)
	@echo "Built $@ successfully"
	@$(ECHO_BLANK_LINE)
