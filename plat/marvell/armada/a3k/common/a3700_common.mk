...
$(DOIMAGETOOL): FORCE
	...
	$(if $(wildcard $(CRYPTOPP_LIBDIR)/*),,$(error "'CRYPTOPP_LIBDIR=$(CRYPTOPP_LIBDIR)' was specified, but '$(CRYPTOPP_LIBDIR)' directory does not exist"))
	$(if $(wildcard $(CRYPTOPP_INCDIR)/*),,$(error "'CRYPTOPP_INCDIR=$(CRYPTOPP_INCDIR)' was specified, but '$(CRYPTOPP_INCDIR)' directory does not exist"))
ifdef CRYPTOPP_PATH
...
$(BUILD_PLAT)/$(BUILD_UART).tgz.bin: $(BUILD_PLAT)/$(BOOT_IMAGE) $(BUILD_PLAT)/wtmi.bin $(DOIMAGETOOL) $(TIMBUILD)
	@$(ECHO_BLANK_LINE)
	@echo "Building uart images"
	$(Q)mkdir -p $(BUILD_PLAT)/$(BUILD_UART)
	$(Q)cp -a $(BUILD_PLAT)/wtmi.bin $(BUILD_PLAT)/$(BUILD_UART)/wtmi.bin
	$(Q)cp -a $(BUILD_PLAT)/$(BOOT_IMAGE) $(BUILD_PLAT)/$(BUILD_UART)/boot-image.bin
...
ifeq ($(MARVELL_SECURE_BOOT),1)
	$(Q)sed -i 's|WTMI_IMG|wtmi_h.bin|1' $(TIMNUARTCFG)
...
ifeq ($(MARVELL_SECURE_BOOT),1)
	...
	$(Q)tar czf $(BUILD_PLAT)/$(BUILD_UART).tgz.bin -C $(BUILD_PLAT) $(BUILD_UART)/$(TIM_IMAGE) $(BUILD_UART)/wtmi_h.bin $(BUILD_UART)/boot-image_h.bin
	@$(ECHO_BLANK_LINE)
	@echo "Built $@ successfully"
	@$(ECHO_BLANK_LINE)

$(BUILD_PLAT)/$(FLASH_IMAGE): $(BUILD_PLAT)/$(BOOT_IMAGE) $(BUILD_PLAT)/wtmi.bin $(DOIMAGETOOL) $(TIMBUILD) $(TIM2IMG)
	@$(ECHO_BLANK_LINE)
...
$(BUILD_PLAT)/$(BUILD_UART).tgz.bin $(BUILD_PLAT)/$(FLASH_IMAGE):
