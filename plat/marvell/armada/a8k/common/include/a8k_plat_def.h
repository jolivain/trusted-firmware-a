...
#define MVEBU_CP_REGS_BASE(cp_index) \
	(MVEBU_AP_IO_BASE(0) + (cp_index)*MVEBU_CP_OFFSET)
...
#define MVEBU_CP_MPP_REGS(cp_index, n) \
	(MVEBU_CP_REGS_BASE(cp_index) + 0x440000 + ((n) << 2))
#define MVEBU_PM_MPP_REGS(cp_index, n) \
	(MVEBU_CP_REGS_BASE(cp_index) + 0x440000 + ((n / 8) << 2))
#define MVEBU_CP_GPIO_DATA_OUT(cp_index, n) \
	(MVEBU_CP_REGS_BASE(cp_index) + 0x440100 + ((n > 31) ? 0x40 : 0x00))
#define MVEBU_CP_GPIO_DATA_OUT_EN(cp_index, n) \
	(MVEBU_CP_REGS_BASE(cp_index) + 0x440104 + ((n > 31) ? 0x40 : 0x00))
#define MVEBU_CP_GPIO_DATA_IN(cp_index, n) \
	(MVEBU_CP_REGS_BASE(cp_index) + 0x440110 + ((n > 31) ? 0x40 : 0x00))
...
#define MVEBU_AP_MC_TRUSTZONE_REG_LOW(ap, win) \
	(MVEBU_REGS_BASE_AP(ap) + 0x20080 + ((win)*0x8))
#define MVEBU_AP_MC_TRUSTZONE_REG_HIGH(ap, win) \
	(MVEBU_REGS_BASE_AP(ap) + 0x20084 + ((win)*0x8))

/* MCI indirect access definitions */
#define MCI_MAX_UNIT_ID 2
/* SoC RFU / IHBx4 Control */
#define MCIX4_REG_START_ADDRESS_REG(unit_id) \
	(MVEBU_RFU_BASE + 0x4218 + (unit_id * 0x20))
...
#define MVEBU_SATA_M2A_AXI_ARCACHE_MASK \
	(0xF << MVEBU_SATA_M2A_AXI_ARCACHE_OFFSET)
#define MVEBU_SATA_M2A_AXI_AWCACHE_OFFSET 5
#define MVEBU_SATA_M2A_AXI_AWCACHE_MASK \
	(0xF << MVEBU_SATA_M2A_AXI_AWCACHE_OFFSET)
