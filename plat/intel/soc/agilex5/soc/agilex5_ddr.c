...
/* DDR banks info set */
static ddr_info ddr_info_set[CONFIG_NR_DRAM_BANKS];
...
static inline bool is_ddr_init_hang(void)
{
	...
	if (sys_reg & SYSMGR_BS_POR0_DDR_PROGRESS_MASK) {
...
static inline bool hps_ocram_dbe_status(void)
{
	...
	if (sys_reg & SYSMGR_BS_COLD3_OCRAM_DBE_MASK)
		return true;
...
static inline bool ddr_ecc_dbe_status(void)
{
	...
	if (sys_reg & SYSMGR_BS_COLD3_DDR_DBE_MASK)
		return true;
