					...
					uint32_t *count)
{
	...
	*count = 0;
	...
	resp_len = mailbox_read_response(rcv_id % MBOX_MAX_JOB_ID, resp, 5);
...
static int is_fpga_config_buffer_full(void)
{
	for (int i = 0; i < FPGA_CONFIG_BUFFER_SIZE; i++)
...
static int is_address_in_ddr_range(uint64_t addr)
{
