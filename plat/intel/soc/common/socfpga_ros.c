...
uint32_t ros_qspi_get_ssbl_offset(unsigned long *const offset)
{
	...
	if (strncmp(spt.partition[img_index].name, FACTORY_IMAGE, SPT_PARTITION_NAME_LENGTH) == 0U) {
	...
	for (uint32_t index = 0U; index < spt.partitions; index++) {
		if (strncmp(spt.partition[index].name, ssbl_name, SPT_PARTITION_NAME_LENGTH) == 0U) {
