...
static uint32_t get_spt(spt_table_t *const spt_buf);
static uint32_t load_and_check_spt(spt_table_t *const spt_buf, size_t offset);
static uint32_t get_current_image_index(const spt_table_t *spt_buf, uint32_t *const index);
static void swap_bits(char *const data, uint32_t len);

uint32_t ros_qspi_get_ssbl_offset(unsigned long *const offset)
{
	if (offset == NULL) {
...
static uint32_t get_current_image_index(const spt_table_t *spt_buf, uint32_t *const img_index)
{
	...
	*img_index = 0;
	for (uint32_t index = 0 ; index < spt_buf->partitions; index++) {
...
static uint32_t load_and_check_spt(spt_table_t * const spt_ptr, size_t offset)
{
