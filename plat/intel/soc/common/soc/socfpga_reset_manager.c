...
int socfpga_bridges_enable(uint32_t mask)
{
	...
	if (brg_mask & RSTMGR_BRGMODRSTMASK_FPGA2SOC) {
		...
		if (ret < 0) {
			ERROR("FPGA bridge fpga handshake fpgahsreq: Timeout handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2S bridge fpga handshake f2sdram_flush_req: Timeout handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2S bridge fpga handshake f2s_flush_ack: Timeout handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2S bridge fpga handshake fpgahsack: Timeout handshake ack\n");
		}
	...
	if (brg_mask & RSTMGR_BRGMODRSTMASK_F2SDRAM0) {
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake fpgahsreq: Timeout handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake f2sdram_flush_req: Timeout handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake f2sdram_flush_ack: Timeout handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake fpgahsack: Timeout handshake ack\n");
		}
