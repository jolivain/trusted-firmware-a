...
#if PLATFORM_MODEL == PLAT_SOCFPGA_AGILEX5
	//unsigned int fused_cpuid = 0;
	//fused_cpuid = (mmio_read_32(AGX5_PWRMGR(MPU_BOOTCONFIG)) &
	//AGX5_PWRMGR_CPU_POWER_STATE_MASK) >> 1;

	//VERBOSE("%s: mpidr: 0x%lx\n", __func__, mpidr);

	// check cpu_id validity (0-3) and if the cpu_id is being fused by SDM,
	// return INTERN_FAIL
