...
static uintptr_t imx_sip_handler(unsigned int smc_fid, u_register_t x1,
				 ...
				 u_register_t flags)
{
	...
	case IMX_SIP_AARCH32:
		SMC_RET1(handle,
			 imx_kernel_entry_handler(smc_fid, x1, x2, x3, x4));
		break;
	...
	case IMX_SIP_MISC_SET_TEMP:
		SMC_RET1(handle,
			 imx_misc_set_temp_handler(smc_fid, x1, x2, x3, x4));
#endif
	...
	case IMX_SIP_BUILDINFO:
		SMC_RET1(handle,
			 imx_buildinfo_handler(smc_fid, x1, x2, x3, x4));
	default:
...
DECLARE_RT_SVC(imx_sip_svc, OEN_SIP_START, OEN_SIP_END, SMC_TYPE_FAST,
	       imx_sip_setup, imx_sip_handler);
