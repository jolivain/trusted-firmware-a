			...
			bool sec_access, uint32_t glbac_id)
{
	...
	if (mrc_base == NULL || glbac_id >= 8U || rgn_id >= 16U) {
		return -EINVAL;
	}
...
{
	return (!!(mmio_read_32(mrc_base) & 0x8000));
}

bool trdc_mbc_enabled(unsigned long mbc_base)
{
	return (!!(mmio_read_32(mbc_base) & 0x4000));
}
