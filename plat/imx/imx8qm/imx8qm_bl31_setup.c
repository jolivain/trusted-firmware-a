...
static const int imx8qm_cci_map[] = { CLUSTER0_CCI_SLVAE_IFACE,
...
void mx8_partition_resources(void)
{
	...
	if (owned) {
		...
		if (err)
			ERROR("sc_rm_set_resource_movable: rsrc %u, ret %u\n",
			      SC_R_M4_0_PID0, err);
		err = sc_rm_assign_resource(ipc_handle, os_part,
					    SC_R_M4_0_PID0);
		if (err)
				...
				u_register_t arg2, u_register_t arg3)
{
	...
	mmio_write_32(IMX_GPT_LPCG_BASE,
		      mmio_read_32(IMX_GPT_LPCG_BASE) | (1 << 25));
