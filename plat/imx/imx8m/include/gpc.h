...
#define LPM_MODE(local_state) \
	((local_state) == PLAT_WAIT_RET_STATE ? A53_LPM_WAIT : A53_LPM_STOP)

#define DSM_MODE_MASK BIT(31)
#define CORE_WKUP_FROM_GIC (IRQ_SRC_C0 | IRQ_SRC_C1 | IRQ_SRC_C2 | IRQ_SRC_C3)
#define A53_CORE_WUP_SRC(core_id) \
	(1 << ((core_id) < 2 ? 28 + (core_id) : 22 + (core_id)-2))
#define COREx_PGC_PCR(core_id) (0x800 + (core_id)*0x40)
#define COREx_WFI_PDN(core_id) \
	(1 << ((core_id) < 2 ? (core_id)*2 : ((core_id)-2) * 2 + 16))
#define COREx_IRQ_WUP(core_id) \
	((core_id) < 2 ? (1 << ((core_id)*2 + 8)) : (1 << ((core_id)*2 + 20)))
#define COREx_LPM_PUP(core_id) \
	((core_id) < 2 ? (1 << ((core_id)*2 + 9)) : (1 << ((core_id)*2 + 21)))
...
#define IMX_PD_DOMAIN(name, on)                                      \
	{                                                            \
		.pwr_req = name##_PWR_REQ, .pgc_offset = name##_PGC, \
		.need_sync = false, .always_on = (on),               \
	}
...
#define IMX_MIX_DOMAIN(name, on)                                     \
	{                                                            \
		...
		.adb400_ack = name##_ADB400_ACK, .need_sync = true,  \
		.always_on = (on),                                   \
	}
...
void imx_set_cpu_secure_entry(unsigned int core_index,
