...
void imx_csu_init(const struct imx_csu_cfg *csu_cfg)
{
	...
	while (csu->type != CSU_INVALID) {
		...
		case CSU_CSL:
			...
			mmio_clrsetbits_32(
				CSLx_REG(csu->idx), CSLx_CFG(0xff, csu->idx),
				CSLx_CFG(csu->csl_level | (csu->lock << 8),
					 csu->idx));
			break;
		case CSU_HP:
			...
			mmio_clrsetbits_32(
				CSU_HP_REG(csu->idx), CSU_HP_CFG(0x1, csu->idx),
				CSU_HP_CFG(csu->hp | (csu->lock << 0x1),
					   csu->idx));
			break;
		case CSU_SA:
			...
			mmio_clrsetbits_32(
				CSU_SA_REG(csu->idx), CSU_SA_CFG(0x1, csu->idx),
				CSU_SA_CFG(csu->sa | (csu->lock << 0x1),
					   csu->idx));
			break;
		case CSU_HPCONTROL:
			...
			mmio_clrsetbits_32(
				CSU_HPCONTROL_REG(csu->idx),
				CSU_HPCONTROL_CFG(0x1, csu->idx),
				CSU_HPCONTROL_CFG(csu->hpctrl |
							  (csu->lock << 0x1),
						  csu->idx));
			break;
		default:
