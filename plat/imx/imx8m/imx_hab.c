...
typedef enum hab_status hab_loader_callback_f_t(void **, size_t *,
						const void *);
...
struct hab_rvt_api {
	...
	enum hab_status (*check_target)(enum hab_target type, const void *start,
					size_t bytes);
	void *(*authenticate_image)(uint8_t cid, long ivt_offset, void **start,
				    size_t *bytes,
				    hab_loader_callback_f_t loader);
	enum hab_status (*run_dcd)(const uint8_t *dcd);
	enum hab_status (*run_csf)(const uint8_t *csf, uint8_t cid,
				   uint32_t srkmask);
	...
	enum hab_status (*report_status)(enum hab_config *config,
					 enum hab_state *state);
	void (*failsafe)(void);
	void *(*authenticate_image_no_dcd)(uint8_t cid, long ivt_offset,
					   void **start, size_t *bytes,
					   hab_loader_callback_f_t loader);
	uint32_t (*get_version)(void);
	enum hab_status (*authenticate_container)(
		uint8_t cid, long ivt_offset, void **start, size_t *bytes,
		hab_loader_callback_f_t loader, uint32_t srkmask, int skip_dcd);
};
		    ...
		    u_register_t x3, u_register_t x4)
{
	...
	case IMX_SIP_HAB_CHECK_TARGET:
		return g_hab_rvt_api->check_target(
			(enum hab_target)x2, (const void *)x3, (size_t)x4);
	case IMX_SIP_HAB_AUTH_IMG:
		return (unsigned long)g_hab_rvt_api->authenticate_image(
			HAB_CID_ATF, x2, (void **)x3, (size_t *)x4, NULL);
	case IMX_SIP_HAB_REPORT_EVENT:
		return g_hab_rvt_api->report_event(HAB_FAILURE, (uint32_t)x2,
						   (uint8_t *)x3, (size_t *)x4);
