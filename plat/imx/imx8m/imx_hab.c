 ...
 * Functions are odered in the structure below based on the offsets in ROM
 * image, and shall not be changed!
 * Details on API allocation offsets and function descrioption could be
...
struct hab_rvt_api {
	...
	enum hab_status (*check_target )(enum hab_target, const void *, size_t);
	void* (*authenticate_image)(uint8_t, long, void **, size_t *,
		hab_loader_callback_f_t);
	enum hab_status (*run_dcd)(const uint8_t*);
	enum hab_status (*run_csf)(const uint8_t*, uint8_t);
	enum hab_status (*assert)(long, const void *, uint32_t);
	enum hab_status (*report_event)(enum hab_status, uint32_t, uint8_t* , size_t*);
	enum hab_status (*report_status)(enum hab_config *, enum hab_state *);
	void (*failsafe)(void);
	void* (*authenticate_image_no_dcd)(uint8_t, long, void **, size_t *,
		hab_loader_callback_f_t);
	uint32_t (*get_version)(void);
	enum hab_status(* authenticate_container)(uint8_t, long, void **, size_t*,
		hab_loader_callback_f_t, uint32_t, int);
};
