...
static void imx_noc_qos(uint32_t domain_id)
{
	int i;
	uint32_t hurry;

	if (domain_id == HDMIMIX) {
		mmio_write_32(IMX_HDMI_CTL_BASE + 0x220, 0x22018);
		mmio_write_32(IMX_HDMI_CTL_BASE + 0x220, 0x22010);

		/* set GPR to make lcdif read hurry level 0x7 */
		hurry = mmio_read_32(IMX_HDMI_CTL_BASE + 0x200);
		hurry |= 0x00077000;
		mmio_write_32(IMX_HDMI_CTL_BASE + 0x200, hurry);
	}

	if (domain_id == MEDIAMIX) {
		/* handle mediamix special */
		mmio_write_32(IMX_MEDIAMIX_CTL_BASE + 0x0, 0x1FFFFFF);
		mmio_write_32(IMX_MEDIAMIX_CTL_BASE + 0x4, 0x1FFFFFF);
		mmio_write_32(IMX_MEDIAMIX_CTL_BASE + 0x8, 0x40030000);

		/* set GPR to make lcdif read hurry level 0x7 */
		hurry = mmio_read_32(IMX_MEDIAMIX_CTL_BASE + 0x4c);
		hurry |= 0xfc00;
		mmio_write_32(IMX_MEDIAMIX_CTL_BASE + 0x4c, hurry);
		/* set GPR to make isi write hurry level 0x7 */
		hurry = mmio_read_32(IMX_MEDIAMIX_CTL_BASE + 0x50);
		hurry |= 0x1ff00000;
		mmio_write_32(IMX_MEDIAMIX_CTL_BASE + 0x50, hurry);
	}
...
static void imx_gpc_pm_domain_enable(uint32_t domain_id, bool on)
{
	...
	if (on) {
		...
		if (domain_id == HDMIMIX) {
			/* assert the reset */
			mmio_write_32(IMX_HDMI_CTL_BASE + 0x20, 0x0);
			/* enable all th function clock */
			mmio_write_32(IMX_HDMI_CTL_BASE + 0x40, 0xFFFFFFFF);
			mmio_write_32(IMX_HDMI_CTL_BASE + 0x50, 0x7ffff87e);
		}
		...
		if (domain_id == HDMIMIX) {
			...
			/* deassert the reset */
			mmio_write_32(IMX_HDMI_CTL_BASE + 0x20, 0xffffffff);
