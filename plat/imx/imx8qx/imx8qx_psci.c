...
static const int ap_core_index[PLATFORM_CORE_COUNT] = { SC_R_A35_0, SC_R_A35_1,
							SC_R_A35_2,
							SC_R_A35_3 };
...
int imx_pwr_domain_on(u_register_t mpidr)
{
	...
	if (sc_pm_cpu_start(ipc_handle, ap_core_index[cpu_id], true,
			    BL31_BASE) != SC_ERR_NONE) {
...
void imx_domain_suspend(const psci_power_state_t *target_state)
{
	...
	if (is_local_state_off(target_state->pwr_domain_state[MPIDR_AFFLVL0])) {
		plat_gic_cpuif_disable();
		sc_pm_set_cpu_resume(ipc_handle, ap_core_index[cpu_id], true,
				     BL31_BASE);
		sc_pm_req_cpu_low_power_mode(ipc_handle, ap_core_index[cpu_id],
					     SC_PM_PW_MODE_OFF,
					     SC_PM_WAKE_SRC_GIC);
	...
	if (is_local_state_off(target_state->pwr_domain_state[MPIDR_AFFLVL1]))
		sc_pm_req_low_power_mode(ipc_handle, SC_R_A35,
					 SC_PM_PW_MODE_OFF);

	if (is_local_state_retn(
