#
# Copyright (c) 2019-2021, Arm Limited. All rights reserved.
#
# SPDX-License-Identifier: BSD-3-Clause
#

# Include framework files
include(Common/STGT)

# TODO: revise config in this file, compare with .mk files

# plat_bl_common
stgt_add_src(
    NAME
    bl1
    bl2
    bl31
    SRC
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/${TFA_ARCH}/juno_helpers.S
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_common.c)

# plat_bl1
stgt_add_src(
    NAME bl1 SRC ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_err.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_bl1_setup.c
    ${PROJECT_SOURCE_DIR}/drivers/arm/sp805/sp805.c)

stgt_add_src_cond(
    NAME
    bl1
    KEY
    CSS_USE_SCMI_SDS_DRIVER
    VAL
    1
    SRC
    ${PROJECT_SOURCE_DIR}/drivers/arm/css/sds/sds.c)

# plat_bl2
stgt_add_src(
    NAME
    bl2
    SRC
    ${PROJECT_SOURCE_DIR}/drivers/arm/sp805/sp805.c
    ${PROJECT_SOURCE_DIR}/lib/utils/mem_region.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_err.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_bl2_setup.c
    ${PROJECT_SOURCE_DIR}/plat/arm/common/arm_nor_psci_mem_protect.c)

# plat_bl31
stgt_add_src(
    NAME
    bl31
    SRC
    ${PROJECT_SOURCE_DIR}/lib/utils/mem_region.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_pm.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_topology.c
    ${PROJECT_SOURCE_DIR}/plat/arm/common/arm_nor_psci_mem_protect.c
    # Juno GIC sources
    ${PROJECT_SOURCE_DIR}/drivers/arm/gic/common/gic_common.c
    ${PROJECT_SOURCE_DIR}/drivers/arm/gic/v2/gicv2_main.c
    ${PROJECT_SOURCE_DIR}/drivers/arm/gic/v2/gicv2_helpers.c
    ${PROJECT_SOURCE_DIR}/plat/common/plat_gicv2.c
    ${PROJECT_SOURCE_DIR}/plat/arm/common/arm_gicv2.c)

stgt_add_src(
    NAME
    bl1
    bl31
    SRC
    # Juno interconnect sources
    ${PROJECT_SOURCE_DIR}/drivers/arm/cci/cci.c
    ${PROJECT_SOURCE_DIR}/plat/arm/common/arm_cci.c)

stgt_add_src(
    NAME
    bl1
    bl2
    bl31
    SRC
    # Juno security sources
    ${PROJECT_SOURCE_DIR}/drivers/arm/tzc/tzc400.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_security.c
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_trng.c
    ${PROJECT_SOURCE_DIR}/plat/arm/common/arm_tzc400.c)

stgt_add_src_cond(
    NAME
    bl1
    bl2
    KEY
    ENABLE_STACK_PROTECTOR
    VAL
    1
    SRC
    # Juno security sources
    ${PROJECT_SOURCE_DIR}/plat/arm/board/juno/juno_stack_protector.c)

stgt_add_src(
    NAME
    bl1
    bl31
    SRC
    # CPU libs
    ${PROJECT_SOURCE_DIR}/lib/cpus/aarch64/cortex_a53.S
    ${PROJECT_SOURCE_DIR}/lib/cpus/aarch64/cortex_a57.S
    ${PROJECT_SOURCE_DIR}/lib/cpus/aarch64/cortex_a72.S)

include(${PROJECT_SOURCE_DIR}/plat/arm/board/common/board_common.cmake)
include(${PROJECT_SOURCE_DIR}/plat/arm/common/arm_common.cmake)
include(${PROJECT_SOURCE_DIR}/plat/arm/soc/common/soc_css.cmake)
include(${PROJECT_SOURCE_DIR}/plat/arm/css/common/css_common.cmake)

# TODO: TRUSTED_BOARD_BOOT, ROMLIB
