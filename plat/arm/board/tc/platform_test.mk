...
ifeq (${PLATFORM_TEST},1)
    ...
    ifeq (${MEASUREMENT_VALUE_SIZE},)
        # Example: 32
        $(error Error: MEASUREMENT_VALUE_SIZE not set)
    endif
    ifeq (${MEASURED_BOOT_HASH_ALG},)
        # Example: "PSA_ALG_SHA_256"
        $(error Error: MEASURED_BOOT_HASH_ALG not set)
    endif
    ...
    PLATFORM_MBEDTLS_CONFIG_FILE := "<plat_tc_mbedtls_config.h>"
    $(eval $(call add_define,PLATFORM_MBEDTLS_CONFIG_FILE))
    ...
    # TODO: Created patches for warnings in tf-m-extras and tf-m-tests
