 ...
 * actually have any external RNG and this function is only needed during the
 * the execution TF-M testsuite during xporting the public part of the
 * delegated attestation key.
 *
 * Use a system counter to fill the output buffer, not ideal way though to
 * provide random number.
 * We can't use 'rand' library function as TF-A doesn't support and also,
 * can't use architectural 'rand' instruction as it is only supported for
 * the platform that with Armv8.5-a+ architecture.
 */
