...
CASSERT(ARM_DRAM_NUM_BANKS == 2UL, ARM_DRAM_BANKS_NUM_mismatch);

/* FVP DRAM banks */
const struct dram_bank fvp_dram_banks[ARM_DRAM_NUM_BANKS] = {
...
int plat_rmmd_load_manifest(struct rmm_manifest *manifest)
{
	...
	manifest->plat_dram.num_banks = ARM_DRAM_NUM_BANKS;
	...
	checksum = ARM_DRAM_NUM_BANKS + (uint64_t)bank_ptr;

	for (unsigned long i = 0UL; i < ARM_DRAM_NUM_BANKS; i++) {
		checksum += bank_ptr[i].base + bank_ptr[i].size;
	}
