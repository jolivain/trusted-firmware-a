 ...
 *         message to a dedicated S-EL0(or S-EL)1 RAS Partition managed by SPMC.
 ...
 *         the RAS error.
 * Step 4: RAS SP generates the Common Platform Error Record(CPER) and shares it
 *         with normal world firmware and/or OS kernel through a reserved buffer
 *         memory.
...
#if PLATFORM_TEST_FFH_LSP_RAS_SP
		...
		int probe_data, const struct err_handler_data *const data)
{
	...
	/*
	 * Initiate an EL3 direct message from LSP to RAS SP. Currently, the
	 * payload is being spoofed. The direct message response contains the
	 * SDEI event ID for the associated RAS error.
	 */
