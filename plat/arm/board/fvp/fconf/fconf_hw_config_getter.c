...
int fconf_populate_dram_layout(uintptr_t config)
{
	...
	case (4 * sizeof(uint32_t)):
		dram_layout.banks_num = 1UL;
		break;
	case (8 * sizeof(uint32_t)):
		dram_layout.banks_num = 2UL;
		break;
	default:
	...
	for (unsigned long i = 0UL; i < dram_layout.banks_num; i++) {
		err = fdt_get_reg_props_by_index(hw_config_dtb, node, (int)i,
					&dram_layout.dram_bank[i].base,
					&dram_layout.dram_bank[i].size);
		if (err < 0) {
