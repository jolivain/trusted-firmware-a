...
int fconf_populate_gicv3_config(uintptr_t config)
{
	...
	if (node < 0) {
		ERROR("Unrecognized hardware configuration property (%d)\n", node);
		panic();
	}
...
{
	int err, node, cluster_node, core_node, max_pwr_lvl;
	...
	if (node < 0) {
		ERROR("FCONF: Unrecognized hardware configuration property (%d)\n", node);
		panic();
	}
