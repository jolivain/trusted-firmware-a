...
int fconf_populate_uart_config(uintptr_t config)
{
	...
	err = fdt_get_reg_props_by_index(hw_config_dtb, uart_node, 0, &addr, NULL);
	if (err < 0) {
	 ...
	 * domain.
	 */
	uart_serial_config.uart_base = fdtw_translate_address(hw_config_dtb, uart_node,
								(uint64_t)addr);
	...
	/*
	 * The device tree node which captures the clock information of uart
