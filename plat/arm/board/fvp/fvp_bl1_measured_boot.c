...
struct rss_mboot_metadata fvp_rss_mboot_metadata[] = {
	{.id = FW_CONFIG_ID,    .slot = U(5), .sw_type = RSS_MBOOT_FW_CONFIG_STRING,     .lock_measurement = true },
	{.id = TB_FW_CONFIG_ID, .slot = U(6), .sw_type = RSS_MBOOT_TB_FW_CONFIG_STRING,  .lock_measurement = true },
	{.id = BL2_IMAGE_ID,    .slot = U(7), .sw_type = RSS_MBOOT_BL2_STRING,           .lock_measurement = true },
