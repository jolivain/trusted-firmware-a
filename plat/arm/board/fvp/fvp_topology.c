...
{
	int i, cluster_count, cpus_per_cluster;

	/*
	 * fconf framework is not supported for RESET_TO_SP_MIN systems.
	 */
#if !RESET_TO_SP_MIN
	...
	assert((cluster_count > 0) && (cluster_count <= FVP_CLUSTER_COUNT));
	assert(cpus_per_cluster > 0);
