...
void bl2_early_platform_setup2(u_register_t arg0, u_register_t arg1, u_register_t arg2, u_register_t arg3)
{
	...
	te = transfer_list_find(bl2_tl, TL_TAG_SRAM_LAYOUT64);
	assert(te != NULL);
...
void bl2_platform_setup(void)
{
	...
	bl31_tl = transfer_list_init((void *)PLAT_ARM_EL3_FW_HANDOFF_BASE,
				     PLAT_ARM_FW_HANDOFF_SIZE);
	assert(bl31_tl != NULL);
#endif
...
struct bl_params *plat_get_next_bl_params(void)
{
	...
	while ((next_exe_img_id = param_node->next_handoff_image_id) !=
	       INVALID_IMAGE_ID) {
		param_node = &bl_mem_params_desc_ptr[get_bl_params_node_index(
			next_exe_img_id)];

		te = transfer_list_add(bl31_tl, TL_TAG_EXEC_EP_INFO64,
		...
		if (next_exe_img_id == BL33_IMAGE_ID) {
			 ...
			 */
			ep->args.arg2 = param_node->image_info.image_base;
			ep->args.arg3 = param_node->image_info.image_size;
			ep->args.arg4 = param_node->image_info.image_base +
...
int bl2_plat_handle_pre_image_load(unsigned int image_id)
{
	...
	if (image_id == HW_CONFIG_ID) {
		 ...
		 */
		te = transfer_list_add(bl31_tl, TL_TAG_FDT, FVP_HW_CONFIG_SIZE,
				       NULL);

		assert(te != NULL);

		param_node->image_info.h.attr &= ~IMAGE_ATTRIB_SKIP_LOADING;
		param_node->image_info.image_max_size = FVP_HW_CONFIG_SIZE;
