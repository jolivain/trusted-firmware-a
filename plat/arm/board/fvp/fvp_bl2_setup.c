...
void bl2_platform_setup(void)
{
	...
	bl31_tl = transfer_list_init((void *)FW_BL31_HANDOFF_BASE, FW_HANDOFF_SIZE);
	assert(bl31_tl != NULL);
#endif
...
struct bl_params *plat_get_next_bl_params(void)
{
	...
	index = get_first_exe_img_index(bl_mem_params_desc_ptr, bl_mem_params_desc_num);

	arm_bl_params->head = &bl_mem_params_desc_ptr[index].params_node_mem;
	arm_bl_params->head->ep_info = &bl_mem_params_desc_ptr[index].ep_info;
	ep = transfer_list_set_handoff_args(bl31_tl, arm_bl_params->head->ep_info);
	assert(ep != NULL);

	unsigned int next_exe_img_id = bl_mem_params_desc_ptr[index].next_handoff_image_id;
	...
	while (next_exe_img_id != INVALID_IMAGE_ID) {
		desc_ptr = &bl_mem_params_desc_ptr[get_bl_params_node_index(next_exe_img_id)];

		ep_info[ep_index] = desc_ptr->ep_info;

		if (next_exe_img_id == BL33_IMAGE_ID) {
			ep = transfer_list_set_handoff_args(ns_tl, &ep_info[ep_index++]);
			assert(ep != NULL);
		}
	...
	te = transfer_list_add(bl31_tl, TL_TAG_EP_INFO, ep_index * sizeof(entry_point_info_t), &ep_info);
	assert(te != NULL);
#else
...
int bl2_plat_handle_pre_image_load(unsigned int image_id) {
	...
	if (image_id == HW_CONFIG_ID) {
		...
		te = transfer_list_add(bl31_tl, TL_TAG_FDT,
				...
				NULL);
		assert(te != NULL);
		param_node->image_info.image_base = (uintptr_t)transfer_list_entry_data(te);
	}
#endif /* TRANSFER_LIST */
	return 0;
}
