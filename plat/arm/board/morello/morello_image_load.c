...
static int plat_morello_append_config_node(struct morello_plat_info *plat_info)
{
	...
	if (nodeoffset < 0) {
		ERROR("Failed to get platform-info node offset\n");
		return -1;
	}
	...
	if (err < 0) {
		ERROR("Failed to set remote-ddr-size\n");
		return -1;
	}
	...
	if (err < 0) {
		ERROR("Failed to set remote-chip-count\n");
		return -1;
	}
	...
	if (err < 0) {
		ERROR("Failed to set multichip-mode\n");
		return -1;
	}
	...
	if (err < 0) {
		ERROR("Failed to set scc-config\n");
		return -1;
	}
	...
	err = fdt_setprop_u64(fdt, nodeoffset, "local-ddr-size",
			usable_mem_size);
	if (err < 0) {
		ERROR("Failed to set local-ddr-size\n");
		return -1;
	}

	flush_dcache_range((uintptr_t)fdt,
