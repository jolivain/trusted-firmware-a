...
extern unsigned char arm_rotpk_hash[], arm_rotpk_hash_end[];
extern unsigned char arm_rotpk_header[];

static unsigned char rotpk_hash_der[ROTPK_HEADER_LEN + ROTPK_HASH_LEN];
			...
			unsigned int *flags)
{
	...
	memcpy(rotpk_hash_der, arm_rotpk_header, ROTPK_HEADER_LEN);
	dst = (uint8_t *)&rotpk_hash_der[ROTPK_HEADER_LEN];

	words = ROTPK_HASH_LEN >> 2;
...
int arm_get_rotpk_info_dev(void **key_ptr, unsigned int *key_len,
	 unsigned int *flags)
{
	*key_ptr = arm_rotpk_hash;
	*key_len = arm_rotpk_hash_end - arm_rotpk_hash;
	*flags = ROTPK_IS_HASH;
	return 0;
}
#endif
			...
			unsigned int *flags)
{
	...
	/* Copy the DER header */
	memcpy(rotpk_hash_der, arm_rotpk_header, ROTPK_HEADER_LEN);
	dst = &rotpk_hash_der[ROTPK_HEADER_LEN];
	*key_ptr = rotpk_hash_der;
	*key_len = sizeof(rotpk_hash_der);
	return cc_get_rotpk_hash(dst, ROTPK_HASH_LEN, flags);
}
#endif
