 ...
 *    < 0 = error
 */
static int arm_set_event_log_info(uintptr_t config_base, const char *compatible,
#ifdef SPD_opteed
 ...
 *    < 0 = error
 */
int arm_set_tos_fw_info(uintptr_t config_base, uintptr_t log_addr, size_t log_size)
{
	int err;
			...
			size_t log_size, uintptr_t *ns_log_addr)
{
	...
	if ((ns_addr + log_size) > ARM_NS_DRAM1_END) {
		return -1;
	}
