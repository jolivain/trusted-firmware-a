				  ...
				  void *log_addr, size_t log_size)
{
	...
	int node = fdt_node_offset_by_compatible(dtb, -1, compatible);
	...
	if (sm_log_addr != NULL) {
		err = fdtw_write_inplace_cells(dtb, node,
			DTB_PROP_HW_SM_LOG_ADDR, 2, &log_addr);
		if (err < 0) {
			ERROR("Unable to write DTB property '%s'\n",
...
/*
 * This function writes the Event Log address and its size and
...
int arm_set_tos_fw_info(void *dtb, void *log_addr, size_t log_size)
{
	...
	/*
	 * Write the Event Log address, its size and number of events
	 * in the DTB.
	 */
...
/*
 * This function writes the Event Log address, its size and number of events
			...
			size_t log_size, void **ns_log_addr)
{
	...
	err = arm_set_event_log_info(dtb, "arm,nt_fw",
					...
					log_addr,
#endif
					(void *)ns_addr, log_size);

	/* Return Event Log address in Non-secure memory */
	*ns_log_addr = (err < 0) ? NULL : (void *)ns_addr;
	return err;
}
#endif /* MEASURED_BOOT */
