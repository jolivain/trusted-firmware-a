...
int fconf_populate_ethosn_config(uintptr_t config)
{
	...
	for (ethosn_node = fdt_node_offset_by_compatible(hw_conf_dtb, -1, "ethosn");
		ethosn_node >= 0;
		ethosn_node = fdt_node_offset_by_compatible(hw_conf_dtb, ethosn_node, "ethosn")) {
		int sub_node;
		uint8_t ethosn_status;
		uint32_t device_core_count = 0;
		...
		fdt_for_each_subnode(sub_node, hw_conf_dtb, ethosn_node) {
			...
			err = fdt_get_reg_props_by_index(
					hw_conf_dtb
					, ethosn_node
					, device_core_count, &core_addr
					, NULL);
			if (err < 0) {
				ERROR(
