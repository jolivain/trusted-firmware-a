...
const struct policies_load_info load_info[FCONF_ARM_IO_UUID_NUMBER] = {
...
int fconf_populate_arm_io_policies(void *dtb)
{
	int err;
	int node;
	int i;
	...
	for (i = 0; i < FCONF_ARM_IO_UUID_NUMBER; i++) {
		...
		VERBOSE("FCONF: arm_io_policies.%s cell found with value = 0x%x - %x - %x - %x\n",
			load_info[i].name,
