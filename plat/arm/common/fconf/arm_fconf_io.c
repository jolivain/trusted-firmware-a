...
int fconf_populate_arm_io_policies(uintptr_t config)
{
	...
	/* Assert the node offset point to "arm,io_fip_handle" compatible property */
	const char *compatible_str = "arm,io_fip_handle";
	...
	for (i = 0; i < FCONF_ARM_IO_UUID_NUMBER; i++) {
		...
		err = fdtw_read_array(dtb, node, load_info[i].name, 4, &uuid_helper.uuid_word.word);
		if (err < 0) {
		...
		VERBOSE("FCONF: arm_io_policies.%s cell found with value = 0x%x - %x - %x - %x\n",
			load_info[i].name,
			uuid_helper.uuid_word.word[0], uuid_helper.uuid_word.word[1],
			uuid_helper.uuid_word.word[2], uuid_helper.uuid_word.word[3]);
