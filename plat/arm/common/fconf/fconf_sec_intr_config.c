...
#define G0_INTR_NUM(i)		g0_intr_prop[3 * (i)]
#define G0_INTR_PRIORITY(i)	g0_intr_prop[3 * (i) + 1]
#define G0_INTR_CONFIG(i)	g0_intr_prop[3 * (i) + 2]

#define G1S_INTR_NUM(i)		g1s_intr_prop[3 * (i)]
#define G1S_INTR_PRIORITY(i)	g1s_intr_prop[3 * (i) + 1]
#define G1S_INTR_CONFIG(i)	g1s_intr_prop[3 * (i) + 2]
...
int fconf_populate_sec_intr_config(uintptr_t config)
{
	...
	err = fdt_read_uint32(hw_config_dtb, node, "g1s_intr_cnt", &g1s_intr_count);
	if(err < 0){
	...
	for (uint32_t i = 0; i < g0_intr_count; i++){
		...
		sec_intr_property.intr_grp = 1; //INTR_GROUP0
		sec_intr_prop.descriptor[i] = sec_intr_property;
		print_intr_prop(sec_intr_property);
	}

	/* Populate Group 1 secure interrupt descriptors into fconf based C struct */
	for (uint32_t i = 0; i < g1s_intr_count; i++){
		...
		sec_intr_property.intr_grp = 0; //"INTR_GROUP1S";
