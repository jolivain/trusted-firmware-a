...
#if ENABLE_RME
static void arm_bl2_gpt_setup()
{
	/*
	 * It is to be noted that any Arm platform that reuses arm_bl2_gpt_setup
	 * must implement plat_arm_get_gpccr_info within its platform code
	 */
	const arm_gpccr_info_t *arm_gpccr_info =
		plat_arm_get_gpccr_info();

	if (arm_gpccr_info == NULL) {
		ERROR("arm_gpccr_info not initialized!!\n");
		panic();
	}

	/* Initialize entire protected space to GPT_GPI_ANY. */
	if (gpt_init_l0_tables(arm_gpccr_info->pps, arm_gpccr_info->l0_base,
		arm_gpccr_info->l0_size) < 0) {
		ERROR("gpt_init_l0_tables() failed!\n");
		panic();
	}

	/* Carve out defined PAS ranges. */
	if (gpt_init_pas_l1_tables(arm_gpccr_info->pgs,
				   arm_gpccr_info->l1_base,
				   arm_gpccr_info->l1_size,
				   arm_gpccr_info->pas_region_base,
				   arm_gpccr_info->pas_region_count) < 0) {
