...
void arm_transfer_list_dyn_cfg_init(struct transfer_list_header *secure_tl)
{
	...
	bl_mem_params_node_t *next_param_node = get_bl_mem_params_node(HW_CONFIG_ID);
	 ...
	 */
	te = transfer_list_add(secure_tl, TL_TAG_FDT, PLAT_ARM_HW_CONFIG_SIZE, NULL);
	assert(te != NULL);
					...
					struct transfer_list_header *ns_tl)
{
	...
	while ((next_exe_img_id = next_param_node->next_handoff_image_id) !=
	       INVALID_IMAGE_ID) {
		next_param_node = &bl_mem_params_desc_ptr[get_bl_params_node_index(
			next_exe_img_id)];
		...
		if (next_exe_img_id == BL33_IMAGE_ID) {
			...
			ep->args.arg4 = next_param_node->image_info.image_base +
...
void arm_transfer_list_copy_hw_config(struct transfer_list_header *secure_tl,
				      struct transfer_list_header *ns_tl)
{
	struct transfer_list_entry *te = transfer_list_find(secure_tl, TL_TAG_FDT);
	assert(te != NULL);
