...
static fwu_metadata_t g_fwu_metadata;
...
bool arm_is_trial_run_state(void)
{
	...
	for (unsigned int i = 0; i < NR_OF_IMAGES_IN_FW_BANK; i++) {
		image_entry_t entry = g_fwu_metadata.img_entry[i];
		image_bank_info_t img_info =
...
static int arm_fwu_metadata_crc_check(void)
{
	...
	calc_crc = tf_crc32(calc_crc, &metadata[4],
			    (sizeof(fwu_metadata_t) - 4));
...
{
	/* ToDo: add more confitions for sanity check */
...
int arm_load_fwu_metadata(const char* part_name)
{
	...
	if (result != 0) {
		ERROR("failed to load FWU metadata\n");
		return result;
	}

	result = io_read(image_handle, (uintptr_t)&g_fwu_metadata,
			 sizeof(fwu_metadata_t), &bytes_read);
	if ((result != 0) || (sizeof(fwu_metadata_t) != bytes_read)) {
		goto exit;
	}
