...
#if TRANSFER_LIST
	te = transfer_list_add(secure_tl, TL_TAG_CRYPTO_INFO, sizeof(crypto_heap_info_t), NULL);
	assert(te != NULL);

	crypto_heap_info_t *heap_info = (crypto_heap_info_t *)transfer_list_entry_data(te);
	arm_transfer_list_get_heap_info(&heap_info->addr, &heap_info->size);
#else
