 ...
 */
void css_setup_cpu_pwr_down_intr()
{
#if CSS_SYSTEM_WARM_RESET
 ...
 * down sequence. On a PSCI shutdown/reboot request, only the primary CPU gets
 * an opportunity to do the powerdown sequence. To achieve graceul reset, of all
 * cores in the system, the primary CPU raise warm reboot SGI to secondary CPUs
 * which are online. Add handler for the reboot SGI where the secondary CPU will
 * execute the powerdown sequence.
 */
		...
		void *handle, void *cookie)
{
	...
	/* Processing of interrupt complete */
