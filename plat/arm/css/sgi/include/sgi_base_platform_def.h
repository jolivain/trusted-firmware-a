...
#define PLATFORM_CORE_COUNT                            \
	(CSS_SGI_CHIP_COUNT * PLAT_ARM_CLUSTER_COUNT * \
...
#define PLAT_ARM_MAX_BL2_SIZE (0x20000 + ((CSS_SGI_CHIP_COUNT - 1) * 0x2000))
#else
#define PLAT_ARM_MAX_BL2_SIZE (0x14000 + ((CSS_SGI_CHIP_COUNT - 1) * 0x2000))
#endif
...
#define PLAT_ARM_MAX_BL31_SIZE \
	(CSS_SGI_BL31_SIZE + PLAT_ARM_MAX_BL2_SIZE + PLAT_ARM_MAX_BL1_RW_SIZE)
...
#define CSS_SGI_MAP_DEVICE                                        \
	MAP_REGION_FLAT(CSS_SGI_DEVICE_BASE, CSS_SGI_DEVICE_SIZE, \
			MT_DEVICE | MT_RW | MT_SECURE)

#define ARM_MAP_SHARED_RAM_REMOTE_CHIP(n)                                \
	MAP_REGION_FLAT(                                                 \
		CSS_SGI_REMOTE_CHIP_MEM_OFFSET(n) + ARM_SHARED_RAM_BASE, \
		ARM_SHARED_RAM_SIZE, MT_NON_CACHEABLE | MT_RW | MT_SECURE)

#define CSS_SGI_MAP_DEVICE_REMOTE_CHIP(n)                   \
	MAP_REGION_FLAT(CSS_SGI_REMOTE_CHIP_MEM_OFFSET(n) + \
				CSS_SGI_DEVICE_BASE,        \
			CSS_SGI_DEVICE_SIZE, MT_DEVICE | MT_RW | MT_SECURE)

#define SOC_CSS_MAP_DEVICE_REMOTE_CHIP(n)                   \
	MAP_REGION_FLAT(CSS_SGI_REMOTE_CHIP_MEM_OFFSET(n) + \
				SOC_CSS_DEVICE_BASE,        \
			SOC_CSS_DEVICE_SIZE, MT_DEVICE | MT_RW | MT_SECURE)

/* Map the secure region for access from S-EL0 */
#define PLAT_ARM_SECURE_MAP_DEVICE                                \
	MAP_REGION_FLAT(SOC_CSS_DEVICE_BASE, SOC_CSS_DEVICE_SIZE, \
 ...
 */
#define CSS_SGI_SP_CPER_BUF_BASE \
	(PLAT_SP_IMAGE_NS_BUF_BASE + PLAT_SP_IMAGE_NS_BUF_SIZE)
#define CSS_SGI_SP_CPER_BUF_SIZE ULL(0x20000)
#define CSS_SGI_SP_CPER_BUF_MMAP                                            \
	MAP_REGION2(CSS_SGI_SP_CPER_BUF_BASE, CSS_SGI_SP_CPER_BUF_BASE,     \
		    CSS_SGI_SP_CPER_BUF_SIZE, MT_RW_DATA | MT_NS | MT_USER, \
		    PAGE_SIZE)
...
#define GET_SGI_PART_NUM GET_SSC_VERSION_PART_NUM(mmio_read_32(SSC_VERSION))
/* Returns the configuration number of the platform */
#define GET_SGI_CONFIG_NUM GET_SSC_VERSION_CONFIG(mmio_read_32(SSC_VERSION))
#endif /* __ASSEMBLER__ */
 ...
 */
#define PLAT_ARM_MEM_PROT_ADDR \
	(V2M_FLASH0_BASE + V2M_FLASH0_SIZE - V2M_FLASH_BLOCK_SIZE)
...
#define SOC_PLATFORM_SECURE_UART                                  \
	MAP_REGION_FLAT(SOC_CSS_SEC_UART_BASE, SOC_CSS_UART_SIZE, \
			MT_DEVICE | MT_RW | MT_SECURE | MT_USER)
