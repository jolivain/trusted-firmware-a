...
#define MAX_PE_COUNT (PLAT_ARM_CLUSTER_COUNT * CSS_SGI_MAX_CPUS_PER_CLUSTER)
...
struct isolated_cpu_mpid_list {
	...
	uint64_t mpid_list[MAX_PE_COUNT]; /* List of isolated CPU MPIDs */
};
...
void plat_arm_sgi_get_isolated_cpu_list(struct isolated_cpu_mpid_list *list)
{
	...
	if (list->num_entries > MAX_PE_COUNT) {
		ERROR("Isolated CPU list count %ld greater than max"
		      " number supported %d\n",
		      list->num_entries, MAX_PE_COUNT);
		panic();
