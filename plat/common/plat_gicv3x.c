...
extern uint32_t max_ppi_intid;

/* Maximum SPI INTID: 32*(ITLinesNumber + 1) - 1 */
extern uint32_t max_spi_intid;

/* Maximum ESPI INTID: 32*(ESPI_range + 1) + 4095 */
extern uint32_t max_espi_intid;
...
uint32_t plat_ic_get_pending_interrupt_id(void)
{
	...
	assert(IS_IN_EL3());
	irqnr = gicv3x_get_pending_interrupt_id();
	return gicv3x_is_intr_id_special_identifier(irqnr) ?
				INTR_ID_UNAVAILABLE : irqnr;
}

/*
 * This function returns the type of the highest priority pending interrupt
 * at the Interrupt controller. In the case of GICv3.x, the Highest Priority
...
uint32_t plat_ic_get_pending_interrupt_type(void)
{
	...
	assert(IS_IN_EL3());
	irqnr = gicv3x_get_pending_interrupt_type();
...
{
	assert(IS_IN_EL3());
	return gicv3x_acknowledge_interrupt();
}
...
{
	assert(IS_IN_EL3());
	return gicv3x_get_interrupt_type(id, plat_my_core_pos());
}
...
{
	return gicv3x_get_running_priority();
}

int plat_ic_is_spi(uint32_t id)
{
	return ((id >= MIN_SPI_ID) && (id <= max_spi_intid)) ||
	       ((id >= MIN_ESPI_ID) && (id <= max_espi_intid));
}

int plat_ic_is_ppi(uint32_t id)
{
	return (id >= MIN_PPI_ID) && (id <= max_ppi_intid);
}

int plat_ic_is_sgi(uint32_t id)
{
	return (id < MIN_PPI_ID);
}

unsigned int plat_ic_get_interrupt_active(uint32_t id)
{
	return gicv3x_get_interrupt_active(id, plat_my_core_pos());
}

void plat_ic_enable_interrupt(uint32_t id)
{
	gicv3x_enable_interrupt(id, plat_my_core_pos());
}

void plat_ic_disable_interrupt(uint32_t id)
{
	gicv3x_disable_interrupt(id, plat_my_core_pos());
}

void plat_ic_set_interrupt_priority(uint32_t id, unsigned int priority)
{
	gicv3x_set_interrupt_priority(id, plat_my_core_pos(), priority);
}
...
void plat_ic_set_interrupt_type(uint32_t id, unsigned int type)
{
	gicv3x_set_interrupt_type(id, plat_my_core_pos(), type);
}
...
void plat_ic_raise_el3_sgi(int sgi_num, u_register_t target)
{
	...
	gicv3x_raise_secure_g0_sgi((unsigned int)sgi_num, target);
}

void plat_ic_set_spi_routing(uint32_t id, unsigned int routing_mode,
				u_register_t mpidr)
{
	unsigned int irm;

	if (routing_mode == INTR_ROUTING_MODE_PE) {
		assert(plat_core_pos_by_mpidr(mpidr) >= 0);
		irm = GICV3X_IRM_PE;
	} else { /* INTR_ROUTING_MODE_ANY */
		irm = GICV3X_IRM_ANY;
	}

	gicv3x_set_spi_routing(id, irm, mpidr);
}

void plat_ic_set_interrupt_pending(uint32_t id)
{
	/* Disallow setting SGIs pending */
	assert(id >= MIN_PPI_ID);
	gicv3x_set_interrupt_pending(id, plat_my_core_pos());
}

void plat_ic_clear_interrupt_pending(uint32_t id)
{
	/* Disallow clearing SGIs pending */
	assert(id >= MIN_PPI_ID);
	gicv3x_clear_interrupt_pending(id, plat_my_core_pos());
}

unsigned int plat_ic_set_priority_mask(unsigned int mask)
{
	return gicv3x_set_pmr(mask);
}

unsigned int plat_ic_get_interrupt_id(unsigned int raw)
{
	uint32_t id = raw & INT_ID_MASK;

	return gicv3x_is_intr_id_special_identifier(id) ?
...
uint32_t plat_ic_get_pending_interrupt_id(void)
{
	...
	assert(IS_IN_EL1());
	irqnr = gicv3x_get_pending_interrupt_id_sel1();
...
{
	assert(IS_IN_EL1());
	return gicv3x_acknowledge_interrupt_sel1();
}
...
{
	assert(IS_IN_EL1());
	gicv3x_end_of_interrupt_sel1(id);
}
