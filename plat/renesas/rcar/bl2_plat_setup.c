...
static void bl2_add_kaslr_seed(void)
{
	...
	do {
		...
		if (isr & CC63_TRNG_ISR_REG_AUTOCORR_ERR)
			panic();
	} while (!(isr & CC63_TRNG_ISR_REG_EHR_VALID));

	mmio_write_32(RCAR_CC63_BASE + CC63_TRNG_ICR_REG_ADDR, 0xffffffffU);
	seed = mmio_read_64(RCAR_CC63_BASE + CC63_TRNG_EHR_DATA_ADDR_0_REG_ADDR);
	mmio_write_32(RCAR_CC63_BASE + CC63_TRNG_SOURCE_ENABLE_REG_ADDR, CC63_TRNG_SOURCE_ENABLE_REG_CLR);
