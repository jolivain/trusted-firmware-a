...
#define CPG_MSTPSR3		0x0048U
#define MSTP318			(1U << 18)
#define MSTP319			(1U << 19)
#define PMSR			0x5cU
#define PMSR_L1FAEG		(1U << 31)
#define PMSR_PMEL1RX		(1U << 23)
#define PMCTLR			0x60U
...
{
	uint32_t rcar_pcie_base[] = { 0xfe011000U, 0xee811000U };
	...
	if ((pmsr & PMSR_PMEL1RX) && ((pmsr & 0x70000U) != 0x30000U)) {
		/* Fix applicable */
...
void rcar_console_boot_init(void)
{
	...
	ret = console_rcar_register(0, 0U, 0U, &rcar_boot_console);
	if (ret == 0) {
		panic();
	}
...
void rcar_console_runtime_init(void)
{
	...
	ret = console_rcar_register(1, 0U, 0U, &rcar_runtime_console);
	if (ret == 0) {
		panic();
	}
