...
void read_cpuinfo_from_dt(void *dtb)
{
	...
	while (node > 0) {
		...
		sbsa_cpu_info.cpu[cpu].nodeid = nodeid;
		sbsa_cpu_info.cpu[cpu].mpidr = mpidr;
	...
	sbsa_cpu_info.num_cpus = cpu;
	INFO("Found %d cpus\n", sbsa_cpu_info.num_cpus);
}
			       ...
			       u_register_t flags)
{
	...
	case SIP_SVC_GET_CPU_COUNT:
		SMC_RET2(handle, NULL, sbsa_cpu_info.num_cpus);

	case SIP_SVC_GET_CPU_NODE:
		index = x1;
		if (index < PLATFORM_CORE_COUNT) {
			SMC_RET3(handle, NULL,
				sbsa_cpu_info.cpu[index].nodeid,
				sbsa_cpu_info.cpu[index].mpidr);
		} else {
