...
static int handoff_arch(uint32_t opteed_rw)
{
#if TRANSFER_LIST
	struct transfer_list_entry *te;

	te = transfer_list_add(bl2_tl, TL_TAG_OPTEED_AARCH,
			       sizeof(opteed_rw), &opteed_rw);
	if (!te) {
		INFO("Cannot add TE for opteed aarch\n");
		return -1;
	}
#endif
	return 0;
...
static int qemu_bl2_handle_post_image_load(unsigned int image_id)
{
	...
	case BL32_IMAGE_ID:
		...
		// add TL_TAG_OPTEE_PAGABLE_PART and
		// TL_TAG_OPTEED_AARCH entry to the TL
		if (handoff_pageable_part(bl_mem_params->ep_info.args.arg1) ||
			handoff_arch(GET_RW(bl_mem_params->ep_info.spsr))) {
			return -1;
		}
#endif
