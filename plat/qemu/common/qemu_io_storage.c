...
unsigned int qemu_io_register_sp_pkg(const char *name, const char *uuid,
				     uintptr_t load_addr)
{
	struct sp_pkg *pkg;
...
static int get_alt_image_source(unsigned int image_id, uintptr_t *dev_handle,
				  uintptr_t *image_spec)
{
	const io_file_spec_t *spec;
	int result;

	if (image_id >= SP_PKG1_ID && image_id <= SP_PKG8_ID)
		spec = &sp_pkgs[image_id - SP_PKG1_ID].sh_file_spec;
	else
		spec = &sh_file_spec[image_id];
			  ...
			  uintptr_t *image_spec)
{
	...
	if (image_id >= SP_PKG1_ID && image_id <= SP_PKG8_ID) {
		policy = &sp_pkgs[image_id - SP_PKG1_ID].policy;
	} else {
