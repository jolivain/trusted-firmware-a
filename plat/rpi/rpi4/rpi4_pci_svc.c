 ...
 * is returned in val. Invalid segment/BDF values return failure, but
 * reads to valid functions that don't exist return INVALID_PCI_ADDR
 * as is specified by PCI for requests that aren't competed by EPs.
 ...
 * segment/BDF values return failure, but write to valid functions
 ...
 * this is the last segment, then nseg=0
 *
