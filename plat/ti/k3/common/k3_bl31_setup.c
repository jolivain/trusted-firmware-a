...
void bl31_platform_setup(void)
{
	...
	ret = ti_sci_get_revision(firmware_description, &firmware_revision,
			&abi_major, &abi_minor, &sub_version, &patch_version);
	if (ret)
	{
	 ...
	 */
	if (firmware_revision > 9 ||
	    (firmware_revision == 9 && sub_version > 1) ||
	    (firmware_revision == 9 && sub_version == 1 && patch_version >= 7)) {
