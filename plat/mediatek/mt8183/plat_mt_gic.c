...
static void enable_sgi_fiq(uint32_t fiq_num)
{
	...
	result = gic_populate_rdist(&rdist_sgi_base);
	assert(result == 0);
...
static inline uint32_t get_irq_status(uint32_t irq, uint32_t reg_base,
				      uint32_t width)
{
	uint32_t bit_offset = (irq % (32 / width)) * width;
	uint32_t bit_mask = (1 << width) - 1;
	uint32_t reg_offset = irq / (32 / width) * 4;
	uint32_t raw_value = mmio_read_32(MT_GIC_BASE + reg_base + reg_offset);

	return (raw_value >> bit_offset) & bit_mask;
}
...
uint64_t mt_irq_dump_status(uint32_t irq)
{
	...
	/* get priority */
	rc |= get_irq_status(irq, GICD_IPRIORITYR, 8) << 2;
	...
	/* get router, occupy 16bit in rc [14:29] */
	{
		uint64_t route = mmio_read_64(
				 MT_GIC_BASE + GICD_V3_IROUTER + (irq * 8));
