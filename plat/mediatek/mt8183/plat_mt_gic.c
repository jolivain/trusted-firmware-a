...
void gic_sgi_save_all(void)
{
	...
	base = MT_GIC_RDIST_BASE + SZ_64K;
	for (cpuid = 0; cpuid < PLATFORM_CORE_COUNT; cpuid++) {
		cpu_offset = cpuid * SZ_64K * 2;
		gic_data[0].saved_sgi[cpuid] = mmio_read_32(
			base + GICD_V3_ISPENDR + cpu_offset) & SGI_MASK;
	}
}
...
void gic_sgi_restore_all(void)
{
	...
	for (cpuid = 0; cpuid < PLATFORM_CORE_COUNT; cpuid++) {
		...
		mmio_write_32(base + GICD_V3_ISPENDR + cpu_offset,
			      gic_data[0].saved_sgi[cpuid] & SGI_MASK);
	}
}
...
void gic_rdist_save(void)
{
	...
	/* handle SGI & PPI */
	gic_data[0].saved_conf[0] =
			mmio_read_32(rdist_base + GICD_ICFGR);
	gic_data[0].saved_priority[0] =
			mmio_read_32(rdist_base + GICD_IPRIORITYR);
...
void gic_dist_restore(void)
{
	...
	/* get the base of redistributor first */
	if (gic_populate_rdist(&rdist_sgi_base)) {
		return;
	}
	...
	/* handle SGI & PPI */
	mmio_write_32(rdist_sgi_base + GICD_ICFGR,
		      gic_data[0].saved_conf[0]);
...
void gic_rdist_restore_all(void)
{
	...
	for (cpuid = 0; cpuid < PLATFORM_CORE_COUNT; cpuid++) {
		cpu_offset = cpuid * SZ_64K * 2;
		mmio_write_32(base + GICD_ICFGR + cpu_offset,
			      gic_data[0].saved_conf[0]);
		mmio_write_32(base + GICD_IPRIORITYR + cpu_offset,
			      gic_data[0].saved_priority[0]);
		mmio_write_32(base + GICD_ISENABLER + cpu_offset,
			      gic_data[0].saved_enable[0]);
		mmio_write_32(base + GICD_IGROUPR + cpu_offset,
			      gic_data[0].saved_group[0]);
		mmio_write_32(base + GICE_V3_IGRPMOD0 + cpu_offset,
			      gic_data[0].saved_grpmod[0]);
	}
}
...
static uint16_t compute_target_list(uint8_t *cpu, unsigned int map,
				    uint64_t cluster_id)
{
	uint16_t tlist = 0;
	...
	for (target_cpu = *cpu; target_cpu < PLATFORM_CORE_COUNT; ++target_cpu) {
		...
		if ((mpidr & 0xff) > 16) {
			ERROR("%s:%d: wrong cpu[%u] mpidr(%lx)\n",
				__func__, __LINE__, (unsigned int) target_cpu,
				(long unsigned int) mpidr);
			break;
		}
...
static void enable_sgi_fiq(uint32_t fiq_num)
{
	...
	result = gic_populate_rdist((unsigned int *)&rdist_sgi_base);
	assert(result == 0);
	gicr_write_igroupr0(rdist_sgi_base, 0xffffffff);
}
...
int gic_cpuif_init(void)
{
	...
	if (gic_populate_rdist(&rdist_base))
		return -1;
	...
	/* before we can touch other ICC_* system registers,
	 * make sure this have effect
	 */
	isb();
...
static void gic_distif_init(unsigned int gicd_base)
{
	...
	for (i = 1; i < irq_set; i++) {
		mmio_write_32(gicd_base + GICE_V3_IGROUP0 + (i << 2),
			      0xffffffff);
...
void mt_bl31_spi_enable(uint32_t fiq_num, uint32_t int_group,
...
void setup_int_schedule_mode(enum irq_schedule_mode mode, unsigned int active_cpu)
{
	...
	if (mode == HW_MODE) {
		...
		mmio_write_32(GIC_INT_MASK,
			(mmio_read_32(GIC_INT_MASK) &
			 ~(GIC500_ACTIVE_SEL_MASK)));
		mmio_write_32(GIC_INT_MASK,
			(mmio_read_32(GIC_INT_MASK) &
			 ~(GIC500_ACTIVE_CPU_MASK)) |
			 (active_cpu << GIC500_ACTIVE_CPU_SHIFT));
	}
	return;
}
...
void gic_setup(void)
{
	...
	gic_distif_init((unsigned int)MT_GIC_BASE);
	gic_cpuif_init();
...
uint64_t mt_irq_dump_status(uint32_t irq)
{
	...
	bit = 1 << (irq % 32);
	result = ((mmio_read_32(
		   dist_base + GICD_ISENABLER + irq / 32 * 4) & bit) ? 1 : 0);
	rc |= result;
	...
	/* 0x1:irq,0x0:fiq */
	result = ((mmio_read_32(
		   dist_base + GICD_IGROUPR + irq / 32 * 4) & bit) ? 1 : 0);
	rc |=  result << 1;

	/* get priority */
	bit = 0xff << ((irq % 4) * 8);
	result = ((mmio_read_32(
		   dist_base + GICD_IPRIORITYR + irq / 4 * 4) & bit) >>
		   ((irq % 4) * 8));
	rc |= result << 2;
	...
	/* edge:0x2, level:0x1 */
	result = ((mmio_read_32(
		   dist_base + GICD_ICFGR + irq / 16 * 4) & bit) >>
		   ((irq % 16) * 2));
	rc |= (result>>1) << 10;
	...
	bit = 1 << (irq % 32);
	result = ((mmio_read_32(
		   dist_base + GICD_ISPENDR + irq / 32 * 4) & bit)?1:0);
	rc |= result << 11;

	/* get active status */
	bit = 1 << (irq % 32);
	result = ((mmio_read_32(
		   dist_base + GICD_ISACTIVER + irq / 32 * 4) & bit)?1:0);
	rc |= result << 12;
...
{
	uint32_t val = mmio_read_32(GIC_SYNC_DCM);
