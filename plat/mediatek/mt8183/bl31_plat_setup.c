...
void bl31_platform_setup(void)
{
	...
	/* Set permission */
	/* DOMAIN1: only can access 0x50000000 ~ 0x52900000(use region0) */
	emi_mpu_set_region_protection(
		0x50000000, 0x52900000, 0, (SEC_RW_NSEC_W << 3));
	/* DOMAIN1/2: forbidden all access to DRAM (use region1/region2) */
	emi_mpu_set_region_protection(
		0x40000000UL, 0x60000000UL, 1,
		(FORBIDDEN << 3 | FORBIDDEN << 6));
	emi_mpu_set_region_protection(
		0x60000000UL, 0x80000000UL, 2,
		(FORBIDDEN << 3 | FORBIDDEN << 6));
