...
int sspm_mbox_read(uint32_t slot, uint32_t *data, uint32_t len)
{
	...
	if (data)
		memcpy_from_sspm(data, MBOX3_BASE + slot * 4, len);
...
int sspm_mbox_write(uint32_t slot, uint32_t *data, uint32_t len)
{
	...
	if (data)
		memcpy_to_sspm(MBOX3_BASE + slot * 4, data, len);
...
int sspm_ipi_send_non_blocking(uint32_t id, uint32_t *data)
{
	...
	if (id == IPI_ID_PLATFORM) {
		memcpy_to_sspm(MBOX0_BASE + PINR_OFFSET_PLATFORM * 4, data,
			       PINR_SIZE_PLATFORM);
		dsb();
		mmio_write_32(MBOX0_BASE + MBOX_OUT_IRQ_OFS, 0x1);
	} else if (id == IPI_ID_SUSPEND) {
		memcpy_to_sspm(MBOX1_BASE + PINR_OFFSET_SUSPEND * 4, data,
			       PINR_SIZE_SUSPEND);
		dsb();
		mmio_write_32(MBOX1_BASE + MBOX_OUT_IRQ_OFS, 0x2);
	}
...
int sspm_ipi_recv_non_blocking(uint32_t id, uint32_t *data, uint32_t len)
{
	...
	if (ret == -EINPROGRESS) {
		if (id == IPI_ID_PLATFORM) {
			...
			dsb();
			/* clear interrupt bit*/
			mmio_write_32(MBOX0_BASE + MBOX_IN_IRQ_OFS, 0x1);
			ret = 0;
		} else if (id == IPI_ID_SUSPEND) {
			memcpy_from_sspm(data,
			...
			dsb();
			/* clear interrupt bit*/
			mmio_write_32(MBOX1_BASE + MBOX_IN_IRQ_OFS, 0x2);
			ret = 0;
		}
	} else if (ret == 0) {
		ret = -EBUSY;
...
int sspm_alive_show(void)
{
	...
	while (sspm_ipi_recv_non_blocking(IPI_ID_PLATFORM, &ipi_data,
					  sizeof(ipi_data) /
						  sizeof(uint32_t)) &&
	       count) {
		mdelay(100);
