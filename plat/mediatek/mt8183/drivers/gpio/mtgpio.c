...
int32_t gpio_get_pupd_iocfg(uint32_t pin)
{
	switch (pin) {
	case 29 ... 34:
		return IOCFG_1_BASE+0x0C0;
	case 35 ... 42:
		return IOCFG_2_BASE+0x0C0;
	case 91 ... 94:
		return IOCFG_5_BASE+0x0C0;
	case 122 ... 133:
		return IOCFG_7_BASE+0x0C0;
	default:
		return -1;
	}
}
...
void mt_set_gpio_pull_enable_chip(uint32_t pin, int en)
{
	...
	assert(!((PULL_offset[pin].offset == (int8_t)-1) && (pupd_offset == (int8_t)-1)));

	if (en == GPIO_PULL_DISABLE) {
		if (PULL_offset[pin].offset == (int8_t)-1)
			mmio_clrbits_32(pupd_addr, 3U << pupd_offset);
		else
			mmio_clrbits_32(pullen_addr, 1U << PULL_offset[pin].offset);
		...
		if (PULL_offset[pin].offset == (int8_t)-1) {
			...
			mmio_setbits_32(pullen_addr, 1U << PULL_offset[pin].offset);
		}
...
int mt_get_gpio_pull_select_chip(uint32_t pin)
{
	...
	if (PULL_offset[pin].offset == (int8_t)-1) {
		...
		if (reg & (3U << pupd_offset)) {
			...
			return ((reg & (1U << (pupd_offset + 2))) ? GPIO_PULL_DOWN : GPIO_PULL_UP);
		} else {
		...
		if ((reg & (1U << PULL_offset[pin].offset))) {
			...
			return ((reg & (1U << PULL_offset[pin].offset)) ? GPIO_PULL_UP : GPIO_PULL_DOWN);
		} else {
			return GPIO_PULL_NONE;
		}
	}
