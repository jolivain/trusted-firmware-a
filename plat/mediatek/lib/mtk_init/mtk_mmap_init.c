...
IMPORT_SYM(uintptr_t, __MTK_MMAP_POINTER_POOL_START__,
	   MTK_MMAP_POINTER_POOL_START);
IMPORT_SYM(uintptr_t, __MTK_MMAP_POINTER_POOL_END_UNALIGNED__,
...
#define MAP_MTK_SECTIONS                                 \
	MAP_REGION_FLAT(RW_START, DATA_START - RW_START, \
...
void mtk_xlat_init(const mmap_region_t *bl_regions)
{
	...
	print_mmap(regions);
	mmap_add(bl_regions);
	if (MTK_MMAP_POINTER_POOL_START !=
	    MTK_MMAP_POINTER_POOL_END_UNALIGNED) {
		for (iter = (struct mtk_mmap_descriptor *)
			     MTK_MMAP_POINTER_POOL_START;
