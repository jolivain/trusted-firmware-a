...
#define IS_AFFLV_PUBEVENT(_pstate) \
	(_pstate & (MT_CPUPM_PWR_DOMAIN_MCUSYS | MT_CPUPM_PWR_DOMAIN_CLUSTER))
...
#define IS_CPUIDLE_FN_ENABLE(x)	(mtk_cpu_pwr.ops && (mtk_cpu_pwr.fn_mask & x))
#define IS_CPUSMP_FN_ENABLE(x)	(mtk_cpu_pwr.smp && (mtk_cpu_pwr.fn_mask & x))
...
#define GET_MEDIATEK_PSTATE(_domain, _psci_state, _state) ({ \
	...
	if (IS_CPUIDLE_FN_ENABLE(MTK_CPUPM_FN_CPUPM_GET_PWR_STATE)) \
		mret = mtk_cpu_pwr.ops->get_pstate(_domain, _psci_state, _state); \
	mret; })

static inline unsigned int armv8_2_get_pwr_afflv(const psci_power_state_t *state)
{
	unsigned int i;

	for (i = (unsigned int)PLAT_MAX_PWR_LVL; i >= (unsigned int)PSCI_CPU_PWR_LVL; i--) {
		if (is_local_state_run(state->pwr_domain_state[i]) == 0) {
			return i;
		}
	}

	return (unsigned int)PSCI_INVALID_PWR_LVL;
}
...
static int armv8_2_validate_power_state(unsigned int power_state, psci_power_state_t *req_state)
{
	...
	if (IS_CPUIDLE_FN_ENABLE(MTK_CPUPM_FN_PWR_STATE_VALID)) {
		...
		ret = mtk_cpu_pwr.ops->pwr_state_valid(aff_lvl, pstate);
		if (ret) {
			return PSCI_E_INVALID_PARAMS;
		}
	}
...
static void armv8_2_get_sys_suspend_power_state(psci_power_state_t *req_state)
{
	...
	if (ret != MTK_CPUPM_E_OK) {
		...
		req_state->pwr_domain_state[PSCI_CPU_PWR_LVL] =
						PLAT_MAX_RET_STATE;
		for (lv = PSCI_CPU_PWR_LVL + 1; lv <= PLAT_MAX_PWR_LVL; lv++) {
			req_state->pwr_domain_state[lv] = PSCI_LOCAL_STATE_RUN;
		}
...
#define ISSUE_CPU_PM_REG_FAIL(_success) ({ _success = 0; assert(0); })
