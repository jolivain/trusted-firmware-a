...
/* per-cpu power state */
unsigned int armv8_2_power_state[PLATFORM_CORE_COUNT];
...
#define GET_MEDIATEK_PSTATE(_domain, _psci_state, _state) ({ \
	...
	if (IS_CPUIDLE_FN_ENABLE(MTK_CPUPM_FN_CPUPM_GET_PWR_STATE)) \
		mret = imtk_cpu_pwr.ops->get_pstate(_domain, _psci_state, _state); \
	mret; })

static inline
unsigned int armv8_2_get_pwr_afflv(const psci_power_state_t *state)
{
	int i;
...
static void armv8_2_mcusys_pwr_dwn_common(const struct mtk_cpupm_pwrstate *state)
{
	...
	/* Add code here that behavior before system enter mcusys'off */
...
static void armv8_2_get_sys_suspend_power_state(psci_power_state_t *req_state)
{
	...
	ret = imtk_cpu_pwr.ops->pwr_state_valid(PLAT_MAX_PWR_LVL,
