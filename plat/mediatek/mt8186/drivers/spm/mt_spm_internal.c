				       ...
				       const struct wake_status *wakesta)
{
	...
	if (wakesta != NULL) {
		...
		INFO("r12 = 0x%x, r12_ext = 0x%x, r13 = 0x%x, debug_flag = 0x%x 0x%x\n",
		     wakesta->r12, wakesta->r12_ext, wakesta->r13,
		...
		INFO("rt_req_sta0 = 0x%x, rt_req_sta1 = 0x%x, rt_req_sta2 = 0x%x\n",
		     wakesta->rt_req_sta0, wakesta->rt_req_sta1,
		     wakesta->rt_req_sta2);
		INFO("rt_req_sta3 = 0x%x, dram_sw_con_3 = 0x%x, raw_ext_sta = 0x%x\n",
		     wakesta->rt_req_sta3, wakesta->rt_req_sta4,
		     wakesta->raw_ext_sta);
		...
		if (wakesta->timer_out != 0U) {
			...
			spm_26m_off_pct =
				(100 * bk_vtcxo_dur) / wakesta->timer_out;
			  ...
			  unsigned int resource_usage)
{
	...
	uint8_t reg_sspm_srcclkena_mask_b =
		(pwrctrl->pcm_flags & SPM_FLAG_SSPM_INFRA_SLEEP_MODE) ?
			0U :
			pwrctrl->reg_sspm_srcclkena_mask_b;
	...
	mmio_write_32(
		...
		((pwrctrl->reg_md_0_srcclkena_mask_b & 0x1) << 0) |
			...
			((pwrctrl->reg_srcclkeni_srcclkena_mask_b & 0x7)
			 << 17) |
			((pwrctrl->reg_srcclkeni_infra_req_mask_b & 0x7)
			 << 20) |
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	mmio_write_32(
		...
		((pwrctrl->reg_ccif_event_infra_req_mask_b & 0xffff) << 0) |
			((pwrctrl->reg_ccif_event_apsrc_req_mask_b & 0xffff)
			 << 16));
	...
	mmio_write_32(
		...
		((pwrctrl->reg_dvfsrc_event_trigger_mask_b & 0x1) << 0) |
			...
			((pwrctrl->reg_spm_reserved_srcclkena_mask_b & 0x1)
			 << 12) |
			((pwrctrl->reg_spm_reserved_infra_req_mask_b & 0x1)
			 << 13) |
			((pwrctrl->reg_spm_reserved_apsrc_req_mask_b & 0x1)
			 << 14) |
			((pwrctrl->reg_spm_reserved_vrf18_req_mask_b & 0x1)
			 << 15) |
			((pwrctrl->reg_spm_reserved_ddren_req_mask_b & 0x1)
			 << 16) |
	...
	mmio_write_32(
		...
		((pwrctrl->reg_ccif_event_srcclkena_mask_b & 0xffff) << 0) |
			...
			((pwrctrl->reg_conn_srcclkenb2pwrap_mask_b & 0x1)
			 << 25) |
			((pwrctrl->reg_dramc_md32_apsrc_req_mask_b & 0x3)
			 << 26));
	...
	mmio_write_32(
		...
		((pwrctrl->reg_mcusys_merge_apsrc_req_mask_b & 0x1ff) << 0) |
			((pwrctrl->reg_mcusys_merge_ddren_req_mask_b & 0x1ff)
			 << 9) |
...
{
	if (en == 1) {
		mmio_clrsetbits_32(RC_M00_SRCLKEN_CFG, RC_SW_SRCCLKEN_FPM,
				   RC_SW_SRCCLKEN_RC);
		assert(mt_spm_bblpm_cnt == 0);
		mt_spm_bblpm_cnt += 1;
	} else {
		mmio_clrsetbits_32(RC_M00_SRCLKEN_CFG, RC_SW_SRCCLKEN_RC,
				   RC_SW_SRCCLKEN_FPM);
		mt_spm_bblpm_cnt -= 1;
	}
}
