...
static int mt_get_gpio_out_chip(uint32_t pin)
{
	...
	reg = mmio_read_32(DOUT_BASE + 0x10U * pos);
...
static void mt_set_gpio_mode_chip(uint32_t pin, int mode)
{
	...
	mask = (1U << GPIO_MODE_BITS) - 1U;

	mode = mode & mask;
	pos = pin / MAX_GPIO_MODE_PER_REG;
	bit = (pin % MAX_GPIO_MODE_PER_REG) * GPIO_MODE_BITS;

	data = mmio_read_32(MODE_BASE + 0x10U * pos);
	data &= (~(mask << bit));
	data |= (mode << bit);
	mmio_write_32(MODE_BASE + 0x10U * pos, data);
}
...
static int mt_get_gpio_out(int gpio)
{
	uint32_t pin;
...
static void mt_set_gpio_mode(int gpio, int mode)
{
	uint32_t pin;
...
static int mt_get_gpio_mode(int gpio)
{
	uint32_t pin;
