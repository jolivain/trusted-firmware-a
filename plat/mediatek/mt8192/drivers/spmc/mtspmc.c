...
void mcucfg_init_archstate(uint32_t cluster, uint32_t cpu, int arm64)
{
	uint32_t reg;
	...
	if (arm64) {
		mmio_setbits_32(reg, BIT(16 + cpu));
	} else {
		mmio_clrbits_32(reg, BIT(16 + cpu));
	}
}
...
int spm_get_cluster_powerstate(uint32_t cluster)
{
	assert(cluster == 0U);
...
int spm_get_cpu_powerstate(uint32_t cluster, uint32_t cpu)
{
