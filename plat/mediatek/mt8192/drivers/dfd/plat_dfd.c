		      ...
		      uint64_t cache_dump)
{
	...
	/* DFD3.0 */
	mmio_write_32(DFD_TEST_SI_0, 0x1E000202);
	mmio_write_32(DFD_TEST_SI_1, 0x20408100);
	mmio_write_32(DFD_TEST_SI_2, 0x10101000);
	mmio_write_32(DFD_TEST_SI_3, 0x00000010);

	/* for iLDO feature */
	sync_writel(DFD_POWER_CTL, 0xF9);

	/* set base address */
	mmio_write_32(0x10043034, base_addr >> 24);
	 ...
	 */
	mmio_clrbits_32(0x10001220, 1 << 8);
	mmio_clrbits_32(0x10001a3c, 1 << 2);
	...
	if ((cache_dump & DFD_CACHE_DUMP_ENABLE) != 0UL) {
		/* DFD3.5 */
		mmio_write_32(DFD_TEST_SI_0, 0x1e000002);
		sync_writel(DFD_V35_ENALBE, 0x1);
		sync_writel(DFD_V35_TAP_NUMBER, 0xB);
		sync_writel(DFD_V35_TAP_EN, 0x43FF);
