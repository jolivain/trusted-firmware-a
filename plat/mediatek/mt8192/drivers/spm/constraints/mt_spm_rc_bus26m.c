			       ...
			       unsigned int *resource_req)
{
	...
	spm_lp->pwrctrl->pcm_flags =
				(uint32_t)CONSTRAINT_BUS26M_PCM_FLAG;
	spm_lp->pwrctrl->pcm_flags1 =
				(uint32_t)CONSTRAINT_BUS26M_PCM_FLAG1;
...
int spm_update_rc_bus26m(int state_id, int type, const void *val)
{
	...
	if (type == PLAT_RC_UPDATE_CONDITION) {
		...
		ERROR("Roger, before check bus26m off condition\n");
		status.is_cond_block =
			mt_spm_cond_check(tlb, tlb_check,
					 (status.is_valid &
					 MT_SPM_RC_VALID_COND_LATCH) ?
					 &cond_bus26m_res : NULL);
		ERROR("Roger, after check bus26m off condition, status.is_cond_block = 0x%x\n", status.is_cond_block);
