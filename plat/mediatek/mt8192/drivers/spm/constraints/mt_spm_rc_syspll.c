...
#else
#define CONSTRAINT_SYSPLL_RESOURCE_REQ	(0)
#endif
...
int spm_update_rc_syspll(int state_id, int type, const void *val)
{
	...
	if (type == PLAT_RC_UPDATE_CONDITION) {
		...
		ERROR("Roger, before check syspll off condition\n");
		status.is_cond_block = mt_spm_cond_check(tlb, tlb_check,
						(status.is_valid &
						MT_SPM_RC_VALID_COND_LATCH) ?
						&cond_syspll_res : NULL);
		ERROR("Roger, after check syspll off condition, status.is_cond_block = 0x%x\n", status.is_cond_block);
...
int spm_run_rc_syspll(int cpu, int state_id)
{
	...
	MT_SPM_RC_TAG((unsigned int)cpu, state_id, MT_RM_CONSTRAINT_ID_SYSPLL);
