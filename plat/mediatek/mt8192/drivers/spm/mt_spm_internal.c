				       ...
				       const struct wake_status *wakesta)
{
	...
	if ((wakesta->tr.comm.r12 & R12_PCM_TIMER) &&
	    (wakesta->wake_misc & WAKE_MISC_PCM_TIMER_EVENT)) {
			wr = WR_PCM_TIMER;
	}
			  ...
			  unsigned int resource_usage)
{
	...
	uint8_t apsrc_req = (resource_usage & MT_SPM_DRAM_S0) ?
			    1 : pwrctrl->reg_spm_apsrc_req;
	uint8_t ddr_en_req = (resource_usage & MT_SPM_DRAM_S1) ?
			     1 : pwrctrl->reg_spm_ddr_en_req;
	uint8_t vrf18_req = (resource_usage & MT_SPM_SYSPLL) ?
			    1 : pwrctrl->reg_spm_vrf18_req;
	uint8_t infra_req = (resource_usage & MT_SPM_INFRA) ?
			    1 : pwrctrl->reg_spm_infra_req;
	uint8_t f26m_req  = (resource_usage & (MT_SPM_26M | MT_SPM_XO_FPM)) ?
...
void __spm_xo_soc_bblpm(int en)
{
	...
	if (en) {
		val = (val | RC_SW_SRCLKEN_RC) & ~RC_SW_SRCLKEN_FPM;
		assert(mt_spm_bblpm_cnt == 0);
		mt_spm_bblpm_cnt += 1;
	} else {
		val = (val | RC_SW_SRCLKEN_FPM) & ~RC_SW_SRCLKEN_RC;
		mt_spm_bblpm_cnt -= 1;
	}
...
void __spm_hw_s1_state_monitor(int en, unsigned int *status)
{
	...
	if (en) {
		...
		if ((reg & SPM_ACK_CHK_3_CON_RESULT) && status) {
