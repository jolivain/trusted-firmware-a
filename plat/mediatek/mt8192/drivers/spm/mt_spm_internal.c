			  ...
			  unsigned int resource_usage)
{
	...
	uint8_t f26m_req =
		((resource_usage & (MT_SPM_26M | MT_SPM_XO_FPM)) != 0U) ?
			1 :
			pwrctrl->reg_spm_f26m_req;

	mmio_write_32(
		SPM_SRC_REQ,
		((apsrc_req & 0x1) << 0) | ((f26m_req & 0x1) << 1) |
			((infra_req & 0x1) << 3) | ((vrf18_req & 0x1) << 4) |
			((ddr_en_req & 0x1) << 7) |
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	mmio_write_32(
		...
		((pwrctrl->reg_md_srcclkena_0_mask_b & 0x1) << 0) |
			((pwrctrl->reg_md_srcclkena2infra_req_0_mask_b & 0x1)
			 << 1) |
			((pwrctrl->reg_md_apsrc2infra_req_0_mask_b & 0x1)
			 << 2) |
			...
			((pwrctrl->reg_md_srcclkena2infra_req_1_mask_b & 0x1)
			 << 7) |
			((pwrctrl->reg_md_apsrc2infra_req_1_mask_b & 0x1)
			 << 8) |
			...
			((pwrctrl->reg_srcclkeni0_srcclkena_mask_b & 0x1)
			 << 19) |
			((pwrctrl->reg_srcclkeni0_infra_req_mask_b & 0x1)
			 << 20) |
			((pwrctrl->reg_srcclkeni1_srcclkena_mask_b & 0x1)
			 << 21) |
			((pwrctrl->reg_srcclkeni1_infra_req_mask_b & 0x1)
			 << 22) |
			((pwrctrl->reg_srcclkeni2_srcclkena_mask_b & 0x1)
			 << 23) |
			((pwrctrl->reg_srcclkeni2_infra_req_mask_b & 0x1)
			 << 24) |
	...
	mmio_write_32(
		...
		((pwrctrl->reg_dvfsrc_event_trigger_mask_b & 0x1) << 0) |
			...
			((pwrctrl->reg_spm_srcclkena_reserved_mask_b & 0x1)
			 << 12) |
			((pwrctrl->reg_spm_infra_req_reserved_mask_b & 0x1)
			 << 13) |
			((pwrctrl->reg_spm_apsrc_req_reserved_mask_b & 0x1)
			 << 14) |
			((pwrctrl->reg_spm_vrf18_req_reserved_mask_b & 0x1)
			 << 15) |
			((pwrctrl->reg_spm_ddr_en_reserved_mask_b & 0x1)
			 << 16) |
	...
	mmio_write_32(
		...
		((pwrctrl->ccif_event_mask_b & 0xffff) << 0) |
			...
			((pwrctrl->reg_dramc0_md32_infra_req_mask_b & 0x1)
			 << 21) |
			((pwrctrl->reg_dramc0_md32_vrf18_req_mask_b & 0x1)
			 << 22) |
			((pwrctrl->reg_dramc1_md32_infra_req_mask_b & 0x1)
			 << 23) |
			((pwrctrl->reg_dramc1_md32_vrf18_req_mask_b & 0x1)
			 << 24) |
			((pwrctrl->reg_conn_srcclkenb2pwrap_mask_b & 0x1)
			 << 25) |
	...
	mmio_write_32(
		...
		((pwrctrl->reg_mcusys_merge_apsrc_req_mask_b & 0x1ff) << 0) |
			((pwrctrl->reg_mcusys_merge_ddr_en_mask_b & 0x1ff)
			 << 9) |
...
{
	if (en == 1) {
		mmio_clrsetbits_32(RC_M00_SRCLKEN_CFG, RC_SW_SRCLKEN_FPM,
				   RC_SW_SRCLKEN_RC);
		assert(mt_spm_bblpm_cnt == 0);
		mt_spm_bblpm_cnt += 1;
	} else {
		mmio_clrsetbits_32(RC_M00_SRCLKEN_CFG, RC_SW_SRCLKEN_RC,
				   RC_SW_SRCLKEN_FPM);
		mt_spm_bblpm_cnt -= 1;
	}
}
