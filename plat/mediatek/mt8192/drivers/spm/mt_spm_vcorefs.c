...
static uint32_t opp_uv[] = {725000, 650000, 600000, 575000};
...
{
	uint32_t idx;
	uint32_t ptpod = spm_vcorefs_get_efuse_data();
...
static void spm_vcorefs_vcore_setting(uint64_t flag)
{
	...
	for (i = 0; i < ARRAY_SIZE(opp_uv); i++) {
		if (dvfs_v_mode == 3) {
			/* LV */
			opp_uv[i] = round_down((opp_uv[i] * 95) / 100,
					      PMIC_STEP_UV);
		} else if (dvfs_v_mode == 1) {
			/* HV */
			opp_uv[i] = round_up((opp_uv[i] * 105) / 100,
					    PMIC_STEP_UV);
		}
