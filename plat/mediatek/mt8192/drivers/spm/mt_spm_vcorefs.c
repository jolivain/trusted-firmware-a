					 ...
					 const struct pwr_ctrl *src_pwr_ctrl)
{
	...
	if (dest_pwr_ctrl->pcm_flags_cust > 0) {
...
{
	return ((spm_vcorefs_get_efuse_data() & 0xF) == 11) ? 1 : 0;
}

static void spm_vcorefs_vcore_setting(uint64_t flag)
{
	uint32_t dvfs_v_mode, dvfsrc_rsrv, i;
	uint32_t opp_uv[] = {725000, 650000, 600000, 575000};
	...
	for (i = 0; i < ARRAY_SIZE(opp_uv); i++) {
		if (dvfs_v_mode == 3) {
			/* LV */
			opp_uv[i] = round_down((opp_uv[i] * VCORE_LV) / 100,
					      PMIC_STEP_UV);
		} else if (dvfs_v_mode == 1) {
			/* HV */
			opp_uv[i] = round_up((opp_uv[i] * VCORE_HV) / 100,
					    PMIC_STEP_UV);
		}
...
uint64_t spm_vcorefs_args(uint64_t x1, uint64_t x2, uint64_t x3, uint64_t *x4)
{
	...
	case VCOREFS_SMC_CMD_INIT:
		...
		if ((mmio_read_32(DVFSRC_RSRV_4) & VCORE_CT_ENABLE) > 0) {
