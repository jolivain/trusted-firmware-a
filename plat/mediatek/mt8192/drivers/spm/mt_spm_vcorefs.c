					 ...
					 const struct pwr_ctrl *src_pwr_ctrl)
{
	...
	if (dest_pwr_ctrl->pcm_flags_cust)
		dest_pwr_ctrl->pcm_flags_cust = (dest_pwr_ctrl->pcm_flags_cust &
						(~dvfs_mask)) |
...
#if 0	/*Wait for PCIE & DRAM BW monitor */
...
static void spm_vcorefs_vcore_setting(uint64_t flag)
{
	...
	ERROR("%s: henry vcore in atf (%d/%d/%d/%d) done\n", __func__,
