...
#define SPM_COND_CHECK_BLOCKED_PLL	(1<<30L)
#define SPM_COND_CHECK_FAIL		(1U<<31L)
...
#define mt_spm_cond_dump_blk(st, res, _log) ({\
	if (st) {\
		int i = 0;\
		for (i = 0;  i < PLAT_SPM_COND_MAX; ++i) {\
			if (st & (1<<i))\
				_log("[BLOCKED  CG][%d] - val = 0x%x\n",\
						i, res->table_cg[i]);\
		} \
		if (st & SPM_COND_CHECK_BLOCKED_PLL)\
			_log("[BLOCKED PLL] - val = 0x%x\n", res->table_pll);\
	} })
#endif
