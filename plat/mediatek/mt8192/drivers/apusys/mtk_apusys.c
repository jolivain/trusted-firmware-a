							...
							uint32_t *ret1)
{
	...
	case MTK_SIP_APU_START_MCU:
		...
		mmio_write_32(REVISER_SECUREFW_CTXT, 0);
		mmio_write_32(REVISER_USDRFW_CTXT, 0);

		/* setup secure sideband */
		mmio_write_32(AO_SEC_FW,
				(SEC_FW_NON_SECURE << SEC_FW_SHIFT_NS) |
				(0 << SEC_FW_DOMAIN_SHIFT));

		/* setup boot address */
		mmio_write_32(AO_MD32_BOOT_CTRL, 0);
		...
		INFO("[APUSYS] reviser_ctxt=%x,%x, ,sec_fw=0x%08x"
			"boot_ctrl=08%08x, predefine=0x%08x, sys_ctrl=0x%08x\n",
