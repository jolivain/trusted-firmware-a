...
void gic_sgi_save_all(void)
{
	...
	for (proc_num = 0; proc_num < PLATFORM_CORE_COUNT; proc_num++) {
		gicr_base = gicv3_driver_data->rdistif_base_addrs[proc_num];
		gic_data.saved_sgi[proc_num] = mmio_read_32(gicr_base + GICR_ISPENDR0) & SGI_MASK;
	}
}
...
void gic_sgi_restore_all(void)
{
	...
	for (proc_num = 0; proc_num < PLATFORM_CORE_COUNT; proc_num++) {
		...
		mmio_write_32(gicr_base + GICR_ISPENDR0, gic_data.saved_sgi[proc_num] & SGI_MASK);
	}
}
...
uint32_t mt_irq_get_pending(uint32_t irq)
{
	...
	return (mmio_read_32(BASE_GICD_BASE + GICD_ISPENDR +
		irq / 32 * 4) & bit) ? 1 : 0;
}
