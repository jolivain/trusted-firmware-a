...
enum { WAIT_IDLE_POLLING_DELAY_US = 1, READ_POLLING_DELAY_US = 2 };
					    ...
					    uint32_t *read_reg)
{
	...
	do {
		...
		if (((reg_rdata >> RDATA_WACS_FSM_SHIFT) &
		     RDATA_WACS_FSM_MASK) == WACS_FSM_WFVLDCLR)
			break;
			   ...
			   uint32_t *rdata, uint32_t init_check)
{
	...
	if (write == 0) {
		...
		return_value = wait_for_state_ready(
			TIMEOUT_READ, &mtk_pwrap->wacs2_rdata, &reg_rdata);
		if (return_value != 0) {
