...
static unsigned int pll_cnt[APUPLL_MAX];
/**
 ...
 *
 */
static int vd2pllidx(enum DVFS_VOLTAGE_DOMAIN domain)
{
	int ret = -EEXIST;
 ...
 */
static const char *pllidx2name(int pll_idx)
{
 ...
 */
static int _fhctl_mon_done(uint32_t pll_idx, unsigned long tar_dds)
{
	...
	mon_dds = apupwr_readl(fhctl_mon_addr[pll_idx]) & DDS_MASK;
	if (tar_dds != mon_dds) {
		ERROR("retry = %d %s monitor DDS 0x%08lx != expect 0x%08lx\n",
		      retry, pllidx2name(pll_idx), mon_dds, tar_dds);
		return -ETIMEDOUT;
	}
...
{
	int pll_postdiv_reg = 0;
	uint32_t val;

	val = apupwr_readl(mixed_con1_addr[pll_idx]);
	pll_postdiv_reg = (val >> POSDIV_SHIFT) & 0x1F;
...
{
	apupwr_clrbits(0x7 << 24, mixed_con1_addr[pll_idx]);
	apupwr_setbits((post_div & 0x7) << POSDIV_SHIFT,
 ...
 */
static int _cal_pll_data(uint32_t *pd, uint32_t *dds, uint32_t freq)
{
 ...
 * Context: Any context.
 */
static void _pll_en(uint32_t pll_idx, uint32_t on)
{
	if (on)
		apupwr_setbits(1, mixed_con0_addr[pll_idx]);
	else
		apupwr_clrbits(1, mixed_con0_addr[pll_idx]);
}
 ...
 * Context: Any context.
 */
static void _pll_pwr(uint32_t pll_idx, uint32_t on)
{
	if (on)
		apupwr_setbits(1, mixed_con3_addr[pll_idx]);
	else
		apupwr_clrbits(1, mixed_con3_addr[pll_idx]);
}
 ...
 * Context: Any context.
 */
static void _pll_iso(uint32_t pll_idx, uint32_t enable)
{
	if (enable)
		apupwr_setbits(1 << 1, mixed_con3_addr[pll_idx]);
	else
		apupwr_clrbits(1 << 1, mixed_con3_addr[pll_idx]);
}
 ...
 */
static int _pll_switch(uint32_t pll_idx, int on, int fhctl_en)
{
 ...
 * * 0		- done for enable pll or fhctl as well.
 */
int32_t apu_pll_enable(int pll_idx, int enable, int fhctl_en)
{
	if (pll_idx >= APUPLL_MAX) {
		ERROR("%s wrong pll_idx: %d\n", __func__, pll_idx);
		return -EINVAL;
...
int32_t anpu_pll_set_rate(enum DVFS_VOLTAGE_DOMAIN domain,
			  enum PLL_SET_RATE_MODE mode, int freq)
{
	...
	switch (mode) {
	case CON0_PCW:
		pd = 1U << 31 | (pd & 0x7) << POSDIV_SHIFT | dds;
	...
	case FHCTL_SW:
		...
		apupwr_setbits(1U << 31, fhctl_dds_addr[pll_idx]);
		udelay(1);
		/* pll con0 enable */
		_pll_en(pll_idx, 1);
		udelay(20);
