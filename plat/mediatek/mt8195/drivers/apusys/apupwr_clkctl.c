...
uint32_t aacc_set[APUSYS_BUCK_DOMAIN_NUM] = {APU_ACC_CONFG_SET1,
	APU_ACC_CONFG_SET2, APU_ACC_CONFG_SET4, APU_ACC_CONFG_SET5,
	APU_ACC_CONFG_SET0, APU_ACC_CONFG_SET7};
uint32_t aacc_clr[APUSYS_BUCK_DOMAIN_NUM] = {APU_ACC_CONFG_CLR1,
	APU_ACC_CONFG_CLR2, APU_ACC_CONFG_CLR4, APU_ACC_CONFG_CLR5,
...
int32_t apupwr_smc_acc_set_parent(uint32_t freq, uint32_t domain)
{
	...
	if (freq > DVFS_FREQ_ACC_APUPLL) {
		...
		return APUPWR_CLKCTL_ERR_DVFS_FREQ_NOT_SUPPORTED;
	}
	...
	case V_TOP_IOMMU:
		...
		break;
	default:
		return APUPWR_CLKCTL_ERR_DOMAIN_NOT_SUPPORTED;
	}
...
{
	int ret = APUPWR_CLKCTL_OK;

	if (enable) {
		ret = apu_pll_enable(APUPLL, enable, 0);
		if (ret)
			return ret;

		ret = apu_pll_enable(NPUPLL, enable, 0);
		if (ret)
			return ret;

		ret = apu_pll_enable(APUPLL1, enable, 0);
		if (ret)
			return ret;

		ret = apu_pll_enable(APUPLL2, enable, 0);
		if (ret)
			return ret;
	} else {
		ret = apu_pll_enable(APUPLL2, enable, 0);
