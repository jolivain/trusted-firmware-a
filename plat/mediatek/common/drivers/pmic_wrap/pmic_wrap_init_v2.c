...
static unsigned int pwrap_check_idle(void *wacs_register, unsigned int timeout_us) {
...
static inline unsigned int pwrap_check_vldclr(void *wacs_register, unsigned int timeout_us) {
	unsigned int reg_rdata;
	uint32_t retry;
		    ...
		    uint32_t init_check)
{
	...
	return_value = pwrap_check_idle(&mtk_pwrap->wacs2_sta, PWRAP_WAIT_IDLE_US);
	if(return_value)
		return return_value;
	...
	if (write == 0) {
		 ...
		 */
		return_value = pwrap_check_vldclr(&mtk_pwrap->wacs2_sta, PWRAP_READ_US);
		if(return_value)
			return return_value;
