...
IMPORT_SYM(uintptr_t, __MTK_RODATA_START__, MTK_RODATA_START);
IMPORT_SYM(uintptr_t, __MTK_RODATA_END__, MTK_RODATA_END);
...

{
	...
	if (p_mtk_bl_param != NULL) {
		atfarg  = (struct atf_arg_t *)p_mtk_bl_param->atf_arg_addr;
	}
	if (atfarg != NULL) {
		memcpy((void *)&gatfarg, (void *)atfarg, sizeof(struct atf_arg_t));
	} else {
		NOTICE("atfarg is NULL!\n");
	}
