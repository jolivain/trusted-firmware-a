...
#define invoke_mt_lpm_dispatch(id, x2, x3, x4,cookie, handle, flags) ({	\
	uint64_t res = 0;						\
	if (IS_MT_LPM_SMC(id)) {					\
		unsigned int user = 0;					\
...
extern void mt_lpm_dispatcher_registry(unsigned int id, mt_lpm_dispatch_fn fn);
extern struct mt_dispatch_ctrl mt_dispatcher;
#endif /* __MT_LPM_DISPATCH_H__ */
