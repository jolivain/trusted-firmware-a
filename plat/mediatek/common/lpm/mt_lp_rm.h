...
struct mt_resource_constraint {
	int level;
	int (*init)(void);
	bool (*is_valid)(int cpu, int stateid);
...
extern int mt_lp_rm_find_and_run_constraint(int idx, int cpuid,
					    int stateid, void *priv);
extern int mt_lp_rm_reset_constraint(int constraint_id, int cpuid, int stateid);
