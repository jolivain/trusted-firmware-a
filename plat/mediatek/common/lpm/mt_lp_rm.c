...
int mt_lp_rm_register(struct mt_resource_manager *rm)
{
	...
	if (rm == NULL || rm->consts == NULL || plat_mt_rm.plat_rm != NULL) {
		return MT_RM_STATUS_BAD;
	}

	for (i = 0, rc = rm->consts; *rc != NULL; i++, rc++) {
		if ((*rc)->init != NULL) {
			(*rc)->init();
		}
	}
...
int mt_lp_rm_reset_constraint(int idx, int cpuid, int stateid)
{
	struct mt_resource_constraint const *rc = NULL;

	if (plat_mt_rm.plat_rm == NULL || idx < 0 || idx >= plat_mt_rm.count) {
		return MT_RM_STATUS_BAD;
	}
