/*
 * Copyright (c) 2020, MediaTek Inc. All rights reserved.
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include <common/debug.h>
#include <lib/spinlock.h>
#include <mt_lp_rqm.h>
#include <mtk_plat_common.h>
#include <plat_mtk_lpm.h>
#include <stdint.h>
#include <string.h>

spinlock_t mt_lp_rq_lock;

struct _mt_lp_res_req_m_ {
	unsigned int uname[MT_LP_RQ_USER_MAX];
	unsigned int user_num;
	unsigned int user_valid;
	unsigned int resource_num;
	unsigned int generic_resource_req;
	unsigned int flag;
	struct mt_resource_req_manager *plat_rqm;
};

static struct _mt_lp_res_req_m_ plat_mt_rqm;
struct mt_lp_resource_user spm_res_user;

static int mt_lp_resource_request(struct mt_lp_resource_user *ru,
				  unsigned int resource)
{
	int i = 0;
	struct mt_lp_res_req *const *rs;

	if (!ru || !resource || resource > MT_LP_RQ_ALL) {
		ERROR("invalid request(%x)\n", resource);
		return MT_LP_RQ_STA_BAD;
	}

	spin_lock(&mt_lp_rq_lock);
	for (i = 0, rs = (plat_mt_rqm.plat_rqm)->res;
	     i < plat_mt_rqm.resource_num; i++) {
		if (resource & rs[i]->res_id) {
			rs[i]->res_usage |= ru->umask;
		}
	}

	plat_mt_rqm.flag = MT_LP_RQ_FLAG_NEED_UPDATE;
	spin_unlock(&mt_lp_rq_lock);

	return MT_LP_RQ_STA_OK;
}

static int mt_lp_resource_release(struct mt_lp_resource_user *ru)
{
	int i = 0;
	struct mt_lp_res_req *const *rs;

	if (!ru) {
		return MT_LP_RQ_STA_BAD;
	}

	spin_lock(&mt_lp_rq_lock);
	for (i = 0, rs = (plat_mt_rqm.plat_rqm)->res;
	     i < plat_mt_rqm.resource_num; i++) {
		rs[i]->res_usage &= ~(ru->umask);
	}

	plat_mt_rqm.flag = MT_LP_RQ_FLAG_NEED_UPDATE;
	spin_unlock(&mt_lp_rq_lock);

	return MT_LP_RQ_STA_OK;
}

int
mt_lp_resource_request_manager_register(struct mt_resource_req_manager *rqm)
{
	unsigned int i;
	struct mt_lp_res_req *const *rs;

	if (!rqm || !rqm->res || plat_mt_rqm.plat_rqm) {
		return MT_LP_RQ_STA_BAD;
	}

	/* count resource num */
	for (i = 0, rs = rqm->res; *rs ; i++, rs++) {
		;
	}

	plat_mt_rqm.plat_rqm = rqm;
	plat_mt_rqm.resource_num = i;

	return MT_LP_RQ_STA_OK;
}

int mt_lp_resource_user_register(char *user, struct mt_lp_resource_user *ru)
{
	int i, len;
	unsigned int s, uname;

	if (!plat_mt_rqm.plat_rqm ||
	    plat_mt_rqm.user_num >= MT_LP_RQ_USER_MAX || !user) {
		ru->uid = MT_LP_RQ_USER_INVALID;
		ru->umask = 0;
		ru->request = NULL;
		ru->release = NULL;
		ERROR("rqm register user invalid\n");
		return MT_LP_RQ_STA_BAD;
	}

	len = strnlen(user, MT_LP_RQ_USER_NAME_LEN);
	for (i = 0, uname = 0, s = 0; i < len;
	     i++, s += MT_LP_RQ_USER_CHAR_U) {
		uname |= user[i] << s;
	}

	spin_lock(&mt_lp_rq_lock);
	i = plat_mt_rqm.user_num;
	plat_mt_rqm.user_num += 1;
	plat_mt_rqm.uname[i] = uname;
	plat_mt_rqm.user_valid |= (1 << i);
	spin_unlock(&mt_lp_rq_lock);

	ru->umask = (1 << i);
	ru->uid = i;
	ru->request = mt_lp_resource_request;
	ru->release = mt_lp_resource_release;
	INFO("%s register by %s, uid = %d\n", __func__, user, ru->uid);

	return MT_LP_RQ_STA_OK;
}

int mt_lp_rq_get_status(int type, void *p)
{
	int i;
	unsigned int update_sta;
	struct mt_lp_res_req *const *rs;
	struct resource_req_status *rq_sta = (struct resource_req_status *)p;

	if (plat_mt_rqm.flag) {
		spin_lock(&mt_lp_rq_lock);
		for (i = 0, update_sta = 0, rs = (plat_mt_rqm.plat_rqm)->res;
		     i < plat_mt_rqm.resource_num; i++) {
			update_sta |=
				((rs[i]->res_usage & plat_mt_rqm.user_valid) ?
				 rs[i]->res_rq : 0);
		}

		plat_mt_rqm.generic_resource_req = update_sta;
		plat_mt_rqm.flag = MT_LP_RQ_FLAG_DONE;
		spin_unlock(&mt_lp_rq_lock);
	}

	if (type == PLAT_RQ_REQ_USAGE) {
		rs = (plat_mt_rqm.plat_rqm)->res;
		rq_sta->val = (rq_sta->id < plat_mt_rqm.resource_num) ?
			      rs[rq_sta->id]->res_usage :
			      plat_mt_rqm.generic_resource_req;
	} else if (type == PLAT_RQ_USER_NUM) {
		rq_sta->val = plat_mt_rqm.user_num;
	} else if (type == PLAT_RQ_USER_VALID) {
		rq_sta->val = plat_mt_rqm.user_valid;
	} else if (type == PLAT_RQ_PER_USER_NAME) {
		rq_sta->val = (rq_sta->id < plat_mt_rqm.user_num) ?
			      plat_mt_rqm.uname[rq_sta->id] : 0;
	} else if (type == PLAT_RQ_REQ_NUM) {
		rq_sta->val = plat_mt_rqm.resource_num;
	}

	return MT_LP_RQ_STA_OK;
}

int mt_lp_rq_update_status(int type, void *p)
{
	unsigned int user_mask;
	struct resource_req_status *rq_sta = (struct resource_req_status *)p;

	if (type == PLAT_RQ_USER_VALID) {
		if (rq_sta->id < plat_mt_rqm.user_num) {
			user_mask = 1 << rq_sta->id;
			spin_lock(&mt_lp_rq_lock);
			plat_mt_rqm.user_valid = (!rq_sta->val) ?
				(plat_mt_rqm.user_valid & ~(user_mask)) :
				(plat_mt_rqm.user_valid | user_mask);
			plat_mt_rqm.flag = MT_LP_RQ_FLAG_NEED_UPDATE;
			spin_unlock(&mt_lp_rq_lock);
		}
	} else if (type == PLAT_RQ_USER_REQ) {
		if (spm_res_user.request &&
		    (rq_sta->val < plat_mt_rqm.resource_num)) {
			spm_res_user.request(&spm_res_user, 1 << rq_sta->val);
		}
	} else if (type == PLAT_RQ_USER_REL) {
		if (spm_res_user.release) {
			spm_res_user.release(&spm_res_user);
		}
	}

	return MT_LP_RQ_STA_OK;
}
