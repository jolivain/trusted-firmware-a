...
#define SMC_HANDLER_DEBUG_START_MSG \
	"%s[0x%x] smc handler start, smc desc. index:%d\n"
...
#define SMC_ID_EXPAND_AS_SMC_OPERATION(_smc_id, _smc_num)                       \
	...
	case _smc_id##_AARCH64: {                                               \
		if (_smc_id##_descriptor_index < 0) {                           \
			SMC_HANDLER_DEBUG(SMC_HANDLER_DEBUG_NOT_IMP_MSG,        \
					  #_smc_id, smc_id);                    \
			break;                                                  \
		}                                                               \
		if (_smc_id##_descriptor_index >= smc_id_descriptor_max) {      \
			SMC_HANDLER_DEBUG(                                      \
				"smc descriptor index[%d] exceed max[%d]\n",    \
				_smc_id##_descriptor_index,                     \
				smc_id_descriptor_max);                         \
			break;                                                  \
		}                                                               \
		SMC_HANDLER_DEBUG(SMC_HANDLER_DEBUG_START_MSG, #_smc_id,        \
				  smc_id, _smc_id##_descriptor_index);          \
		ret = smc_handler_pool[_smc_id##_descriptor_index].smc_handler( \
			x1, x2, x3, x4, handle, &smc_ret);                      \
		SMC_HANDLER_DEBUG(SMC_HANDLER_DEBUG_END_MSG, #_smc_id,          \
				  smc_id);                                      \
		break;                                                          \
	}
...
IMPORT_SYM(uintptr_t, __MTK_SMC_POOL_END_UNALIGNED__,
	   MTK_SMC_POOL_END_UNALIGNED);
...
static uintptr_t mtk_smc_handler_bl33(uint32_t smc_id, u_register_t x1,
...
static uintptr_t mtk_smc_handler_nsel1(uint32_t smc_id, u_register_t x1,
				       u_register_t x2, u_register_t x3,
				       u_register_t x4, void *cookie,
				       void *handle, u_register_t flags)
{
...
static uintptr_t mtk_smc_handler(uint32_t smc_id, u_register_t x1,
				 u_register_t x2, u_register_t x3,
				 ...
				 u_register_t flags)
{
	...
	if (is_from_bl33(smc_ori)) {
		...
		return mtk_smc_handler_bl33(smc_num, x1, x2, x3, x4, cookie,
					    handle, flags);
	} else if (is_from_nsel1(smc_ori)) {
		/* SiP SMC service kernel's call */
		return mtk_smc_handler_nsel1(smc_num, x1, x2, x3, x4, cookie,
					     handle, flags);
	}
...
DECLARE_RT_SVC(mtk_smc_handler, OEN_SIP_START, OEN_SIP_END, SMC_TYPE_FAST, NULL,
	       mtk_smc_handler);
