...
extern struct smc_descriptor __MTK_SMC_POOL_START__[];
extern struct smc_descriptor __MTK_SMC_POOL_END_UNALIGNED__[];
struct smc_descriptor *smc_handler_pool;
static short smc_id_descriptor_max;

static void print_smc_descriptor(struct smc_descriptor pool[])
{
	struct smc_descriptor *p_smc_desc;

	INFO("print smc descriptor pool\n");
	for (p_smc_desc = &pool[0];
		(char *)p_smc_desc < (char *)__MTK_SMC_POOL_END_UNALIGNED__;
		p_smc_desc++) {
...
{
	struct smc_descriptor *iter;
	short index_cnt;
	int ret = 0;

	smc_handler_pool = __MTK_SMC_POOL_START__;
	/* Designate descriptor index point to smc_handler_pool */
	for (index_cnt = 0, iter = &smc_handler_pool[0];
		(char *)iter < (char *)__MTK_SMC_POOL_END_UNALIGNED__;
		iter++, index_cnt++) {
		if (index_cnt < 0) {
			...
			u_register_t flags)
{
	...
	default:
		INFO("BL33 SMC ID:0x%x not support\n", smc_id);
		ret = SMC_UNK;
	}
	SMC_RET4(handle, ret, smc_ret.a1, smc_ret.a2, smc_ret.a3);
}
...
static u_register_t mtk_smc_handler_nsel1(uint32_t smc_id,
			u_register_t x1,
			u_register_t x2,
			u_register_t x3,
			u_register_t x4,
			...
			u_register_t flags)
{
	...
	default:
		INFO("NSEL1 SMC ID:0x%x not support\n", smc_id);
		ret = SMC_UNK;
	}
	SMC_RET4(handle, ret, smc_ret.a1, smc_ret.a2, smc_ret.a3);
}

static u_register_t mtk_smc_handler(uint32_t smc_id,
			u_register_t x1,
			u_register_t x2,
			u_register_t x3,
			u_register_t x4,
