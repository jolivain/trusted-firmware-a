...
static io_mmc_dev_spec_t emmc_device_spec = {
	.use_boot_part = false,
};
...
#if STM32MP_EMMC_BOOT
static uint32_t get_boot_part_ssbl_header()
{
	uint32_t magic = 0;
	int io_result;
	size_t bytes_read;
		     ...
		     uint16_t boot_interface_instance)
{
	...
	if (magic == BOOT_API_IMAGE_HEADER_MAGIC_NB) {
		...
		part->bkp_offset = 0U;
		emmc_device_spec.use_boot_part = true;
...
emmc_boot:
	...
	io_result = io_dev_open(mmc_dev_con, (uintptr_t)&emmc_device_spec,
				&storage_dev_handle);
	assert(io_result == 0);
