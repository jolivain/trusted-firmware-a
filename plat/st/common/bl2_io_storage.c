...
static void *stm32_fip_get_spec(const uuid_t *img_type_uuid)
{
	...
	for (i = 0U; i < npolicies; i++) {
		if ((guidcmp(&policies[i].img_type_guid, &null_guid)) == 0) {
			continue;
		}
...
void plat_fwu_set_images_source(const struct fwu_metadata *metadata)
{
	...
	for (i = 0U; i < NR_OF_IMAGES_IN_FW_BANK; i++) {
		...
		if (entry == NULL) {
			INFO("Unable to find the partition with the uuid mentioned in metadata\n");
			panic();
		}
				       ...
				       uintptr_t *image_spec)
{
	...
	if (image_id == FWU_METADATA_IMAGE_ID) {
		result = plat_set_image_source(image_id, handle, image_spec,
					       METADATA_PART_1);
	} else {
		result = plat_set_image_source(image_id, handle, image_spec,
					       METADATA_PART_2);
...
#endif /* GPT_PART && PSA_FWU_SUPPORT */
