...
static void usb_dfu_download(struct usb_handle *pdev, struct usb_setup_req *req)
{
	...
	if (req->length > 0) {
		...
		/* End of DNLOAD operation*/
		hdfu->dev_state = STATE_DFU_MANIFEST_SYNC;
		ret = hdfu->callback->manifestation(hdfu->alt_setting,
						    pdev->user_data);
		if (ret == 0U) {
			hdfu->dev_state = STATE_DFU_MANIFEST_SYNC;
		} else {
...
static void usb_dfu_upload(struct usb_handle *pdev, struct usb_setup_req *req)
{
	...
	length = req->length;
	ret = hdfu->callback->upload(hdfu->alt_setting, &data_ptr, &length,
				     pdev->user_data);
	if (ret == 0U) {
		/* Short frame */
		hdfu->dev_state = (req->length > length) ?
					  STATE_DFU_IDLE :
...
static void usb_dfu_get_status(struct usb_handle *pdev)
{
	...
	/* Start the transfer */
	usb_core_transmit_ep0(pdev, (uint8_t *)&hdfu->status[0],
			      sizeof(hdfu->status));
}
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_INTERFACE:
			...
			usb_core_transmit_ep0(
				pdev, (uint8_t *)&hdfu->alt_setting, 1U);
			break;
