...
/* USB Standard Device Descriptor */
static const uint8_t usbd_stm32mp1_qualifier_desc[USB_LEN_DEV_QUALIFIER_DESC] = {
...
static const uint8_t usb_stm32mp1_config_desc[USB_DFU_CONFIG_DESC_SIZ] = {
	...
	USBD_DFU_IF_DESC(0), USBD_DFU_IF_DESC(1),
...
const char *const if_desc_string[USB_DFU_ITF_NUM] = {
	"@SSBL /0x03/1*16Me", "@virtual /0xF1/1*512Ba"
};
#endif
#if STM32MP15
const char *const if_desc_string[USB_DFU_ITF_NUM] = {
	"@Partition0 /0x00/1*256Ke", "@FSBL /0x01/1*1Me",
	"@Partition2 /0x02/1*1Me",   "@Partition3 /0x03/1*16Me",
	"@Partition4 /0x04/1*16Me",  "@virtual /0xF1/1*512Ba"
};
#endif
 ...
 */
static void stm32mp1_get_string(const char *desc, uint8_t *unicode,
				uint16_t *len)
{
	uint8_t idx = 0U;
