...
static uintptr_t gxm_sec_entrypoint;
static volatile uint32_t gxm_cpu0_go;

static void gxm_pm_set_reset_addr(u_register_t mpidr, uint64_t value)
{
...
static void gxm_pm_reset(u_register_t mpidr)
{
	unsigned int core = plat_calc_core_pos(mpidr);
...
static void __dead2 gxm_system_reset(void)
{
...
static void __dead2 gxm_system_off(void)
{
...
static int32_t gxm_pwr_domain_on(u_register_t mpidr)
{
	unsigned int core = plat_calc_core_pos(mpidr);
	...
	if (core == AML_PRIMARY_CPU) {
		...
		gxm_cpu0_go = 1;
		flush_dcache_range((uintptr_t)&gxm_cpu0_go,
				sizeof(gxm_cpu0_go));
		dsb();
		isb();
	...
	gxm_pm_set_reset_addr(mpidr, gxm_sec_entrypoint);
...
static void gxm_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
...
static void gxm_pwr_domain_off(const psci_power_state_t *target_state)
{
	u_register_t mpidr = read_mpidr_el1();
	unsigned int core = plat_calc_core_pos(mpidr);
...
static void __dead2 gxm_pwr_domain_pwr_down_wfi(const psci_power_state_t
						 *target_state)
{
	u_register_t mpidr = read_mpidr_el1();
	unsigned int core = plat_calc_core_pos(mpidr);
 ...
 ******************************************************************************/
static const plat_psci_ops_t gxm_ops = {
	.pwr_domain_on			= gxm_pwr_domain_on,
	.pwr_domain_on_finish		= gxm_pwr_domain_on_finish,
	.pwr_domain_off			= gxm_pwr_domain_off,
	.pwr_domain_pwr_down_wfi	= gxm_pwr_domain_pwr_down_wfi,
	.system_off			= gxm_system_off,
	.system_reset			= gxm_system_reset,
};

int plat_setup_psci_ops(uintptr_t sec_entrypoint,
			const plat_psci_ops_t **psci_ops)
{
	gxm_sec_entrypoint = sec_entrypoint;
	*psci_ops = &gxm_ops;
	gxm_cpu0_go = 0;
	return 0;
}
