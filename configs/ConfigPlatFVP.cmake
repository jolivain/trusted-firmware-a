...
# -------------------------------------------------------------------------------
group_add(NAME hw_plat TYPE INCLUDE KEY
          ${PROJECT_SOURCE_DIR}/plat/arm/board/fvp/include)
...
group_add(
    ...
    DTS_HW_CONFIG
    VAL
    ${PROJECT_SOURCE_DIR}/fdts/fvp-base-gicv3-psci.dts)
group_add(
    NAME
    hw_plat
    TYPE
    ...
    DTS_TB_FW_CONFIG
    VAL
    ${PROJECT_SOURCE_DIR}/plat/arm/board/fvp/fdts/fvp_tb_fw_config.dts)
group_add(
    NAME
    hw_plat
    TYPE
    ...
    DTS_SOC_FW_CONFIG
    VAL
    ${PROJECT_SOURCE_DIR}/plat/arm/board/fvp/fdts/fvp_soc_fw_config.dts)
group_add(
    NAME
    hw_plat
    TYPE
    ...
    DTS_NT_FW_CONFIG
    VAL
    ${PROJECT_SOURCE_DIR}/plat/arm/board/fvp/fdts/fvp_nt_fw_config.dts)
group_add(
    NAME
    hw_plat
    TYPE
    ...
    DTS_FW_CONFIG
    VAL
    ${PROJECT_SOURCE_DIR}/plat/arm/board/fvp/fdts/fvp_fw_config.dts)
group_add(
    NAME
    hw_plat
    TYPE
    ...
    DTS_TOS_FW_CONFIG
    VAL
    ${PROJECT_SOURCE_DIR}/plat/arm/board/fvp/fdts/fvp_tsp_fw_config.dts)
...
# cores stack Enable reclaiming of BL31 initialisation code for secondary cores
# stacks for FVP. We cannot enable PIE for this case because the overlayed init
# section creates some dynamic relocations which cannot be handled by the fixup
# logic currently.
group_add(
    NAME
    hw_plat
...
# On ARM standard platorms, the TSP can execute from Trusted SRAM, Trusted DRAM
# (if available) or the TZC secured area of DRAM. TZC secured DRAM is the
# default.
group_add(
    NAME
    hw_plat
...
# Whether code and read-only data should be put on separate memory pages. On ARM
# platforms, separate the code and read-only data sections to allow mapping the
# former as executable and the latter as execute-never.
group_add(
    NAME
    hw_plat
    TYPE
...
group_add(
    ...
    VAL
    1)
# group_add(NAME hw_plat TYPE CONFIG DEFINE KEY DYNAMIC_WORKAROUND_CVE_2018_3639
# VAL 0) TODO: enabled in fvp

# Cortex A57 specific optimisation to skip L1 cache flush when cluster is
# powered down.
group_add(
    NAME
    hw_plat
...
# Flag to disable the cache non-temporal hint. It is enabled by default.
group_add(
    NAME
    hw_plat
    TYPE
...
# CPU Errata Build flags. These should be enabled by the platform if the erratum
...
# Flag to apply erratum 843419 workaround at link time. This erratum applies to
# revision <= r0p4 of the Cortex-A53. Enabling this workaround could lead the
# linker to emit "*.stub" sections which are 4kB aligned.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply errata 855873 during reset. This errata applies to all revisions
# of the Cortex-A53, but this firmware workaround only works for revisions r0p3
# and higher. Earlier revisions are taken care of by the rich OS.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply erratum 1319537 workaround during reset. This erratum applies to
# all versions of the Cortex-A57.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply erratum 1319367 workaround during reset. This erratum applies to
# all revisions of the Cortex-A72.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply T32 CLREX workaround during reset. This erratum applies only to
# r0p0 and r1p0 of the Neoverse N1.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply erratum 1315703 workaround during reset. This erratum applies to
# revisions before r3p1 of the Neoverse N1.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply erratum 1542419 workaround during reset. This erratum applies to
# revisions r3p0 - r4p0 of the Neoverse N1.
group_add(
    NAME
    hw_plat
    TYPE
...
# Flag to apply DSU erratum 936184. This erratum applies to DSUs containing the
# ACP interface and revision < r2p0. Applying the workaround results in higher
# DSU power consumption on idle.
group_add(
    NAME
    hw_plat
    TYPE
