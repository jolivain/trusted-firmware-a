...
void inject_undef64(cpu_context_t *ctx)
{
	...
	if (to_el == MODE_EL2) {
		/* Set Exception Level to EL2 in AArch64 mode */
		...
		/* Set Exception Level to EL1 in AArch64 mode */
	...
	/* If FEAT_GCS is supported, update PSTATE.EXLOCK */
	...
	/* If FEAT_EBEP is present set PSTATE.PM bit */
	new_spsr |= old_spsr & SPSR_PM_BIT_AARCH64;
	if (is_feat_ebep_present()) {
		new_spsr |= SPSR_PM_BIT_AARCH64;
	}

	/* If FEAT_SEBEP is present clear PSTATE.PPEND bit */
