...
static unsigned int target_el(void)
{
	...
	if (level == MODE_EL0) {
		if ((read_feat_vhe_id_field() != 0U &&
		    (read_hcr_el2() & HCR_TGE_BIT) &&
		    (read_hcr_el2() & HCR_E2H_BIT))) {
			level = MODE_EL2;
		} else {
			level = MODE_EL1;
		}
...
void inject_undef64(cpu_context_t *ctx)
{
	...
	if (to_el == MODE_EL2) {
		...
		modify_pan_bit = (read_feat_vhe_id_field() != 0U) &&
				 (read_hcr_el2() & HCR_TGE_BIT) &&
				 (read_hcr_el2() & HCR_E2H_BIT);
	} else {
	...
	if (is_feat_pan_supported() && modify_pan_bit && ((sctlr & SCTLR_SPAN_BIT) == 0U)) {
