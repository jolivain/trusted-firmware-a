...
static u_register_t get_elr_el3(u_register_t spsr_el3, u_register_t vbar, unsigned int target_el)
{
	...
	if (outgoing_el == target_el) {
		/*
		 * Valid for EL1 or EL2 only, lsb can tell us the SPsel
		 *  Thread mode  : 0
		 *  Handler mode : 1
		 */
		if (spsr_el3 & 0x1) {
...
/*
 * Explictily create all bits of SPSR to get PSTATE at exception return.
 * Most of the code is based on AArch64.TakeException mentioned in arm ARM
 * DDI0487 J.a, J1-12012. New architechtural features which are not part
 * of this document are taken from DDI 0602 e.g. (FEAT_GCS, FEAT_EBEP and
 * FEAT_SEBEP).
 *
 * NOTE: This piece of code must be reviewed every year to ensure that
 * we keep up with new ARCH features.
 */
