...
static uint32_t read_amcntenset0_px(void)
{
	return (read_amcntenset0() >> AMCNTENSET0_Pn_SHIFT) &
		AMCNTENSET0_Pn_MASK;
}

static uint32_t read_amcntenset1_px(void)
{
	return (read_amcntenset1() >> AMCNTENSET1_Pn_SHIFT) &
		AMCNTENSET1_Pn_MASK;
}

static void write_amcntenset0_px(uint32_t px)
{
...
static void write_amcntenset1_px(uint32_t px)
{
...
static void write_amcntenclr0_px(uint32_t px)
{
...
static void write_amcntenclr1_px(uint32_t px)
{
...
void amu_enable(bool el2_unused)
{
	...
	amcntenset0_px = (1 << (amcgcr_cg0nc)) - 1;

	assert(amcgcr_cg0nc <= 16); /* Maximum of 16 counters */
...
static void *amu_context_save(const void *arg)
{
	...
	amcfgr_ncg = read_amcfgr_ncg();
	amcgcr_cg0nc = read_amcgcr_cg0nc();
	amcgcr_cg1nc = (amcfgr_ncg > 0) ? read_amcgcr_cg1nc() : 0U;
