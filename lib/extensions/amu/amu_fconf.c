 ...
 */
static int amu_fconf_populate_cpu_amu(
	const void *fdt,
	int off,
	struct amu_fconf_core *amu)
{
	...
	for (
		off = fdt_first_subnode(fdt, off); off >= 0;
		...
		ret = fdt_get_reg_props_by_index(fdt, off, 0, &idx, NULL);
		if (ret < 0)
			break;

		value = fdt_getprop(fdt, off, "enable-at-el3", &len);
 ...
 * Returns `0` on success, or a negative integer representing an error code.
 */
static int amu_fconf_populate_cpu(const void *fdt, int off, uintptr_t mpidr)
{
	int ret;
	int idx;
	...
	ret = fdt_read_uint32(fdt, off, "amu", &amu_phandle);
	if (ret < 0)
		return ret;

	off = fdt_node_offset_by_phandle(fdt, amu_phandle);
	if (off < 0)
		return off;
	...
	return amu_fconf_populate_cpu_amu(fdt, off, amu);
}
 ...
 * Returns `0` on success, or a negative integer representing an error code.
 */
static int amu_fconf_foreach_cpu(
	const void *fdt,
	int (*callback)(const void *, int, uintptr_t))
{
	...
	for (
		...
		ret = fdt_get_reg_props_by_index(fdt, off, 0, &mpidr, NULL);
		if (ret < 0)
			break;

		ret = callback(fdt, off, mpidr);
		if (ret < 0)
			break;
	}
