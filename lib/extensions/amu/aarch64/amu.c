...
static inline __unused void ctx_write_cptr_el3_tam(global_context_t *ctx, uint64_t tam)
{
	uint64_t value = read_ctx_reg(get_global_el3state_ctx_from_global_ctx(ctx), CTX_CPTR_EL3);

	value &= ~TAM_BIT;
	value |= (tam << TAM_SHIFT) & TAM_BIT;

	write_ctx_reg(get_global_el3state_ctx_from_global_ctx(ctx), CTX_CPTR_EL3, value);
}
