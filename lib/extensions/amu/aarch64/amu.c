...
void amu_enable_8_6 (bool el2_unused)
{
...
#if AMU_GROUP1_NR_COUNTERS
		u_register_t amcg1idr = amu_read_amcg1idr_el0() >> 16;
	...
	amu_write_amcr_el0(amu_read_amcr_el0() | AMCR_CG1RZ_BIT);
#else
	amu_write_amcr_el0(amu_read_amcr_el0() & ~AMCR_CG1RZ_BIT);
#endif
}
...
uint64_t amu_group1_voffset_read(unsigned int idx)
{
	...
	assert(amu_group1_supported());
	assert(idx < AMU_GROUP1_NR_COUNTERS);
	assert(((amu_read_amcg1idr_el0() >> 16U) & (1U << idx)) != 0);
