...
static uint64_t read_id_aa64pfr0_el1_amu(void)
{
	return (read_id_aa64pfr0_el1() >> ID_AA64PFR0_AMU_SHIFT) &
		ID_AA64PFR0_AMU_MASK;
}

static uint64_t read_hcr_el2_amvoffen(void)
{
	return (read_hcr_el2() & HCR_AMVOFFEN_BIT) >>
		HCR_AMVOFFEN_SHIFT;
}

static void write_cptr_el2_tam(uint64_t value)
{
	write_cptr_el2((read_cptr_el2() & ~CPTR_EL2_TAM_BIT) |
		((value << CPTR_EL2_TAM_SHIFT) & CPTR_EL2_TAM_BIT));
}

static void write_cptr_el3_tam(uint64_t value)
{
	write_cptr_el3((read_cptr_el3() & ~TAM_BIT) |
		((value << TAM_SHIFT) & TAM_BIT));
}

static void write_hcr_el2_amvoffen(uint64_t value)
{
	write_hcr_el2((read_hcr_el2() & ~HCR_AMVOFFEN_BIT) |
		((value << HCR_AMVOFFEN_SHIFT) & HCR_AMVOFFEN_BIT));
}

static void write_amcr_el0_cg1rz(uint64_t value)
{
	write_amcr_el0((read_amcr_el0() & ~AMCR_CG1RZ_BIT) |
		((value << AMCR_CG1RZ_SHIFT) & AMCR_CG1RZ_BIT));
}

static uint64_t read_amcfgr_el0_ncg(void)
{
	return (read_amcfgr_el0() >> AMCFGR_EL0_NCG_SHIFT) &
		AMCFGR_EL0_NCG_MASK;
}

static uint64_t read_amcgcr_el0_cg0nc(void)
{
	return (read_amcgcr_el0() >> AMCGCR_EL0_CG0NC_SHIFT) &
		AMCGCR_EL0_CG0NC_MASK;
}

static uint64_t read_amcg1idr_el0_voff(void)
{
	return (read_amcg1idr_el0() >> AMCG1IDR_VOFF_SHIFT) &
		AMCG1IDR_VOFF_MASK;
}

static uint64_t read_amcgcr_el0_cg1nc(void)
{
...
static uint64_t read_amcntenset0_el0_px(void)
{
	return (read_amcntenset0_el0() >> AMCNTENSET0_EL0_Pn_SHIFT) &
		AMCNTENSET0_EL0_Pn_MASK;
}

static uint64_t read_amcntenset1_el0_px(void)
{
...
static void write_amcntenset0_el0_px(uint64_t px)
{
...
static void write_amcntenset1_el0_px(uint64_t px)
{
...
static void write_amcntenclr0_el0_px(uint64_t px)
{
...
static void write_amcntenclr1_el0_px(uint64_t px)
{
...
{
	if (read_id_aa64pfr0_el1_amu() == ID_AA64PFR0_AMU_NOT_SUPPORTED) {
		return;
	}
	 ...
	 */
	write_cptr_el3_tam(0);

	/* Enable group 0 counters */
	write_amcntenset0_el0_px((1 << read_amcgcr_el0_cg0nc()) - 1);

	if (read_amcfgr_el0_ncg() > 0U) {
		/* Enable group 1 counters */
		write_amcntenset1_el0_px(AMU_GROUP1_COUNTERS_MASK);
	}

	/* Initialize FEAT_AMUv1p1 features if present. */
	if (read_id_aa64pfr0_el1_amu() < ID_AA64PFR0_AMU_V1P1) {
		return;
	}

	if (el2_unused) {
		/* Make sure virtual offsets are disabled if EL2 not used. */
		write_hcr_el2_amvoffen(0);
	}
...
static uint64_t amu_group1_voffset_read(unsigned int idx)
{
	...
	assert(read_amcfgr_el0_ncg() > 0);
	assert(idx < read_amcgcr_el0_cg1nc());
	assert((read_amcg1idr_el0_voff() & (1ULL << idx)) != 0ULL);
...
static void *amu_context_save(const void *arg)
{
	...
	assert(read_amcntenset0_el0_px() == ((1 << read_amcgcr_el0_cg0nc()) - 1));
	 ...
	 */
	write_amcntenclr0_el0_px((1 << read_amcgcr_el0_cg0nc()) - 1);
