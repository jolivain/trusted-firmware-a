 ...
 * Return values
 *   0: not supported
 *   ID_AA64PFR0_AMU_8_4: ARM v8.4 AMU features
 *   ID_AA64PFR0_AMU_8_6: ARM v8.6 AMU features
 */
unsigned int amu_supported(void)
{
...
void amu_enable(bool el2_unused)
{
	...
	if (amu_supported() < ID_AA64PFR0_AMU_8_6) {
		return;
	}
	...
	if (el2_unused) {
		...
		for (unsigned int i = 0U; i < AMU_GROUP1_NR_COUNTERS; i++) {
			if (((amcg1idr >> i) & 1U) != 0U) {
...
{
	assert(amu_supported() >= ID_AA64PFR0_AMU_8_6);
	assert(idx < AMU_GROUP0_NR_COUNTERS);
	assert(idx != 1U);
...
static void *amu_context_save(const void *arg)
{
	...
	if ((amu_supported() >= ID_AA64PFR0_AMU_8_6) &&
		((read_hcr_el2() & HCR_AMVOFFEN_BIT) != 0U)) {
