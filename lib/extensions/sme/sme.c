...
void sme_enable_ns(cpu_context_t *context)
{
	...
	/*
	 * Set the max LEN value and FA64 bit. Since we are only enabling SME
	 * for NS, this register does not need to be saved/restored on world
	 * changes so we set it globally. CPTR_EL3.ESM will ensure that
	 * unauthorized accesses are trapped.
	 */
	...
	VERBOSE("[SME] Enabled SME in non-secure context.\n");
}

void sme_disable_s(cpu_context_t *context)
{
	u_register_t reg;
	el3_state_t *state;
	...
	/* Get the secure context state. */
	state = get_el3state_ctx(context);

	/* Disable functionality in secure world that overlaps with SME. */
	reg = read_ctx_reg(state, CTX_CPTR_EL3);
	...
	/* Disable access to TPIDR2_EL0 in secure world. */
	reg = read_ctx_reg(state, CTX_SCR_EL3);
	reg &= ~SCR_ENTP2_BIT;
	write_ctx_reg(state, CTX_SCR_EL3, reg);

	VERBOSE("[SME] Disabled SME/SVE/FPU in secure context.\n");
}
