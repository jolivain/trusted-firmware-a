...
void sys_reg_trace_enable(global_context_t *global_ctx)
{
	 ...
	 */
	uint64_t val = read_ctx_reg(get_global_el3state_ctx_from_global_ctx(global_ctx), CTX_CPTR_EL3);

	val &= ~(TTA_BIT);
	write_ctx_reg(get_global_el3state_ctx_from_global_ctx(global_ctx), CTX_CPTR_EL3, val);
}
