...
/* array of auth buffer which holds hashes and pk */
static unsigned char auth_bufs[MAX_AUTH_BUF];
static OBJECT_POOL_ARRAY(auth_buf_pool, auth_bufs);
				...
				unsigned int auth_buf_size)
{
	...
	unsigned char *auth_buf = pool_alloc_n(&auth_buf_pool, auth_buf_size);
			...
			auth_method_desc_t *auth_method)
{
	...
	assert(auth_param_type_desc != NULL);
...
static int get_oid(const void *dtb, int node, const char *prop, char **oid)
{
	...
	*oid = pool_alloc_n(&oid_pool, MAX_OID_NAME_LEN);
...
static int populate_and_set_auth_methods(const void *dtb, int node,
				unsigned int img_id,
				img_type_t type, bool root_certificate)
{
...
static int set_desc_data(const void *dtb, int node, img_type_t type)
{
	...
	if (type == IMG_CERT) {
		...
		pool_alloc_n(&auth_params_pool, COT_MAX_VERIFIED_PARAMS);
...
static int populate_manifest_descs(const void *dtb)
{
	...
	/*
	 * Assert the node offset point to "arm, cert-descs"
	 * compatible property
	 */
...
static int populate_image_descs(const void *dtb)
{
	...
	/*
	 * Assert the node offset point to "arm, img-descs"
	 * compatible property
	 */
...
static int fconf_populate_cot_descs(uintptr_t config)
{
	...
	if (rc < 0) {
		ERROR("FCONF: population of manifest descs failed %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		ERROR("FCONF: population of image descs failed %d\n", rc);
		return rc;
	}

	/* update parent's authentication data */
	for (unsigned int i = 0; i < MAX_NUMBER_IDS; i++) {
		if (auth_img_descs[i].parent != NULL) {
			get_auth_param_type_desc(i,
						&type_desc,
						&auth_buf_size);
