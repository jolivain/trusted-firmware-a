...
/*
 * The L1 descriptors are protected by a global spinlock to ensure
 * that multiple CPUs do not attempt to change the descriptors at once.
 */
static spinlock_t gpt_lock;
#else

/* Bitlocks base address for each region of 512MB blocks of PPS */
static bitlock_t *gpt_bitlock_base;
#endif

/* Lock/unlock macros for L1 table entries */
#if (RME_GPT_BITLOCK_BLOCK == 0)
	/*
	 ...
	 * given time.
	 */
#define L1_LOCK		spin_lock(&gpt_lock)
#define L1_UNLOCK	spin_unlock(&gpt_lock)
#else
	/*
	 * Access to a region of 512MB blocks in L1 tables is controlled
	 * by a bitlock.
	 */
#define L1_LOCK		bit_lock(gpi_info.lock, gpi_info.mask)
#define L1_UNLOCK	bit_unlock(gpi_info.lock, gpi_info.mask)
#endif
...
static int validate_l0_params(gpccr_pps_e pps, uintptr_t l0_mem_base,
				size_t l0_mem_size)
{
	size_t l0_alignment, __unused locks_size;
