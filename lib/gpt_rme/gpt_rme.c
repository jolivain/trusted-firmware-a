...
{
	unsigned int idx = GPT_L1_IDX(gpt_config.p, ALIGN_2MB(base));
...
{
	unsigned int idx = GPT_L1_IDX(gpt_config.p, ALIGN_32MB(base));
				...
				uint64_t l1_desc)
{
	...
	idx = GPT_L1_IDX(gpt_config.p, ALIGN_32MB(base));
...
__unused static void fuse_512mb(uintptr_t base, gpi_info_t *gpi_info,
				uint64_t l1_cont_desc)
{
	unsigned int idx = GPT_L1_IDX(gpt_config.p, ALIGN_512MB(base));
				...
				uint64_t l1_desc)
{
	...
	idx = GPT_L1_IDX(gpt_config.p, ALIGN_512MB(base));
	l1 = &gpi_info->gpt_l1_addr[idx];
				   ...
				   uint64_t l1_desc)
{
	 ...
	 */
	if (length >= SZ_2M) {
		new_last = round_down(first + SZ_2M, SZ_2M) -
				GPT_PGS_ACTUAL_SIZE(gpt_config.p);
		/*
...
{
	uint64_t l1_cont_desc;
	unsigned int idx_2 = GPT_L1_IDX(gpt_config.p, ALIGN_2MB(base));
	...
	idx_32 = GPT_L1_IDX(gpt_config.p, ALIGN_32MB(base));
	j = idx_32;
	...
	j = GPT_L1_IDX(gpt_config.p, ALIGN_512MB(base));
