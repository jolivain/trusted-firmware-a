...
static uint64_t *shatter_2mb(uintptr_t base, gpi_info_t *gpi_info,
				uint64_t l1_desc)
{
	unsigned int idx = (unsigned int)GPT_L1_INDEX(ALIGN_2MB(base));
...
static uint64_t *shatter_512mb(uintptr_t base, gpi_info_t *gpi_info,
				uint64_t l1_desc)
{
	unsigned int idx = (unsigned int)GPT_L1_INDEX(ALIGN_32MB(base));
	...
	idx = (unsigned int)GPT_L1_INDEX(ALIGN_512MB(base));
	l1 = &gpi_info->gpt_l1_addr[idx];
...
static void generate_l0_blk_desc(pas_region_t *pas)
{
	...
	idx = (unsigned int)GPT_L0_IDX(pas->base_pa);
	 ...
	 * descriptors.
	 */
	end_idx = (unsigned int)GPT_L0_IDX(pas->base_pa + pas->size);
				   ...
				   size_t length, unsigned int gpi)
{
	...
	for (unsigned long i = 0UL; i < ARRAY_SIZE(gpt_fill_lookup); ) {
		/* Calculate index */
		unsigned int idx = (unsigned int)GPT_L1_INDEX(first);
...
static void flush_l0_for_pas_array(pas_region_t *pas, unsigned int pas_count)
{
	...
	start_idx = (unsigned int)GPT_L0_IDX(pas[0].base_pa);
	end_idx = (unsigned int)GPT_L0_IDX(pas[0].base_pa + pas[0].size - 1UL);
				...
				uint64_t l1_desc)
{
	...
	unsigned int idx_2 = (unsigned int)GPT_L1_INDEX(ALIGN_2MB(base));
				...
				uint64_t l1_desc)
{
	...
	unsigned int idx_32 = (unsigned int)GPT_L1_INDEX(ALIGN_32MB(base));
					...
					uint64_t l1_desc)
{
	...
	unsigned int idx = (unsigned int)GPT_L1_INDEX(ALIGN_512MB(base)) +
					(15U * 16U * gpt_l1_cnt_2mb);
				...
				uint64_t l1_desc)
{
	...
	unsigned int idx_512 = (unsigned int)GPT_L1_INDEX(ALIGN_512MB(base));
