...
/* Invalidate TLBs for 4KB, 16KB and 64KB pages */
...
/* Invalidate TLBs for 2MB, 32MB and 512MB blocks */
...
static uint64_t *fill_desc(uint64_t *l1, uint64_t l1_desc, unsigned int cnt)
{
	...
	for (unsigned int i = 0U; i < cnt; i++) {
		*l1_quad++ = l1_desc | ((uint128_t)l1_desc << 64);
	}
	return (uint64_t *)l1_quad;
}
				   ...
				   size_t length, unsigned int gpi)
{
	...
	for (unsigned long i = 0UL; i < ARRAY_SIZE(gpt_fill_lookup); ) {
		...
		if (GPT_REGION_IS_CONT(length, first, cont_size)) {
			...
			(void)fill_desc(&l1[idx], l1_desc, cnt);
			first += cont_size;
			length -= cont_size;
...
{
	uint64_t l1_desc = (uint64_t)(gpi | (gpi << 4));
		       ...
		       size_t l0_mem_size)
{
	...
	flush_dcache_range((uintptr_t)l0_mem_base,
			   (size_t)GPT_L0_TABLE_SIZE(gpt_config.t) + locks_size);
...
static int get_gpi_params(uint64_t base, gpi_info_t *gpi_info)
{
	...
	/* 512MB block index */
	idx_512 = base / SZ_512M;

	/* Bitlock address and mask */
	gpi_info->bitlock = &gpt_bitlock_base[idx_512 / LOCK_BITS];
 ...
 */
__unused static bool check_fuse_2mb(uint64_t base, gpi_info_t *gpi_info,
					uint64_t l1_desc)
{
	/* L1 entry index of the start of 2MB block */
	unsigned int idx_2 = GPT_L1_INDEX(ALIGN_2MB(base));

	/* Index offset of the current L1 entry from the start of 2MB block */
	unsigned int idx_off = gpi_info->idx - idx_2;

	/* Maximum index of L1 entry in 2MB */
	unsigned int cnt = gpt_l1_cnt_2mb - 1U;

	/*
	 * Start check from the next L1 entry and continue until the first
	 * non-matching to the passed Granules descriptor value is found.
	 * The current L1 entry is already set to 'l1_desc', so only the number
	 * of L1 entries in 2MB minus 1 needs to be checked.
	 * 2MB block of L1 entries is processed as a cyclic buffer with an entry
	 * offset masked with the maximum index.
	 */
...
__unused static void fuse_2mb(uint64_t base, gpi_info_t *gpi_info,
				uint64_t l1_desc)
{
	/* L1 entry index of the start of 2MB block */
	unsigned int idx_2 = GPT_L1_INDEX(ALIGN_2MB(base));
...
/*
 * Helper function to check if L1 entries for 2MB aligned addresses
 * in 32MB block have the same 2MB Contiguous descriptor value.
 *
 * Parameters
 *   base		Base address of the region to be checked
 *   gpi_info		Pointer to 'gpt_config_t' structure
 ...
 *
 * Return
 *   true if L1 entries have the same descriptor value, false otherwise.
 */
__unused static bool check_fuse_32mb(uint64_t base, gpi_info_t *gpi_info,
					uint64_t l1_desc)
{
	/* L1 entry index of the start of 32MB block */
	unsigned int idx_32 = GPT_L1_INDEX(ALIGN_32MB(base));

	/* Number of the current 2MB block in 32MB block */
	unsigned int n_2 = GET_2MB_NUM(base);

	/* 2MB Contiguous descriptor */
	uint64_t l1_cont_desc = GPT_L1_CONT_DESC(l1_desc, 2MB);

	/*
	 * Start check from the next 2MB aligned L1 entry and continue until
	 * the first non-matching to 2MB Contiguous descriptor value is found.
	 * The current L1 entry is not yet set to 'l1_cont_desc', so only 15
	 * L1 entries needs to be checked.
	 * 32MB block of L1 entries is processed as a cyclic buffer with an
	 * entry offset masked with the maximum index of 15.
	 */
	for (unsigned int i = 1U; i <= 15U; i++) {
		/* L1 entry index offset of 2MB block */
		unsigned int idx_off = gpt_l1_cnt_2mb * ((n_2 + i) & 15U);

		if (gpi_info->gpt_l1_addr[idx_32 + idx_off] != l1_cont_desc) {
			/* Non-matching L1 entry found */
			return false;
		}
	}
...
__unused static void fuse_32mb(uint64_t base, gpi_info_t *gpi_info,
				uint64_t l1_desc)
{
	/* L1 entry index of the start of 32MB block */
	unsigned int idx_32 = GPT_L1_INDEX(ALIGN_32MB(base));
...
/*
 * Helper function to check if L1 entries for 32MB aligned addresses
 * in 512MB block have the same 32MB Contiguous descriptor value.
 *
 * Parameters
 *   base		Base address of the region to be checked
 *   gpi_info		Pointer to 'gpt_config_t' structure
 ...
 *   true if L1 entries have the same descriptor value, false otherwise.
 */
__unused static bool check_fuse_512mb(uint64_t base, gpi_info_t *gpi_info,
					uint64_t l1_desc)
{
	/* L1 entry index of the start of 512MB block */
	unsigned int idx_512 = GPT_L1_INDEX(ALIGN_512MB(base));

	/* Number of the current 32MB block in 512MB block */
	unsigned int n_32 = GET_32MB_NUM(base);

	/* 32MB Contiguous descriptor */
	uint64_t l1_cont_desc = GPT_L1_CONT_DESC(l1_desc, 2MB);

	/*
	 * Start check from the next 32MB aligned L1 entry and continue until
	 * the first non-matching to 32MB Contiguous descriptor value is found.
	 * The current L1 entry is not yet set to 'l1_cont_desc', so only 15
	 * L1 entries needs to be checked.
	 * 512MB block of L1 entries is processed as a cyclic buffer with an
	 * entry offset masked with the maximum index of 15.
	 */
	for (unsigned int i = 1U; i <= 15U; i++) {
		/* L1 entry index offset of 32MB block */
		unsigned int idx_off = 16U * gpt_l1_cnt_2mb * ((n_32 + i) & 15U);

		if (gpi_info->gpt_l1_addr[idx_512 + idx_off] != l1_cont_desc) {
			/* Non-matching L1 entry found */
			return false;
		}
	}
...
__unused static void fuse_512mb(uint64_t base, gpi_info_t *gpi_info,
				uint64_t l1_desc)
{
	/* L1 entry index of the start of 512MB block */
	unsigned int idx_512 = GPT_L1_INDEX(ALIGN_512MB(base));
