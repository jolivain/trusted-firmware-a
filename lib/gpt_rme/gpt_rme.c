...
static uint64_t gpt_l1_index_mask;	/* Mask for the L1 index field */
...
{
	unsigned int idx = GPT_L1_INDEX(base);

	VERBOSE_("GPT: %s(0x%"PRIxPTR" 0x%"PRIx64")\n",
				...
				unsigned int l1_gpt_cnt)
{
	...
	VERBOSE_("GPT: Requested 0x%lx bytes for L1 GPTs\n", l1_gpt_mem_sz);
	return 0;
}
...
static void generate_l0_blk_desc(pas_region_t *pas)
{
	...
	for (; idx < end_idx; idx++) {
		...
		VERBOSE_("GPT: L0 entry (BLOCK) index %u [%p]: GPI = 0x%"PRIx64" (0x%"PRIx64")\n",
			idx, &l0_gpt_arr[idx],
				   ...
				   size_t length, uint64_t l1_desc)
{
	for (unsigned long i = 0UL; i < ARRAY_SIZE(gpt_fill_cont_lookup); ) {
		...
		if (GPT_REGION_IS_CONT(length, first, cont_size)) {
			...
			VERBOSE_("GPT: Contiguous descriptor 0x%"PRIxPTR" %luMB\n",
...
static void generate_l0_tbl_desc(pas_region_t *pas)
{
	...
	for (l0_idx = GPT_L0_IDX(pas->base_pa);
		...
		VERBOSE_("GPT: L0 entry (TABLE) index %u [%p] ==> L1 Addr %p (0x%"PRIx64")\n",
			   ...
			   unsigned int pas_count)
{
	...
	VERBOSE_("GPT: %i L1 GPTs requested\n", l1_gpt_cnt);
	...
	for (unsigned int idx = 0U; idx < pas_count; idx++) {
		VERBOSE_("GPT: PAS[%u]: base 0x%"PRIxPTR"\tsize 0x%lx\tGPI 0x%x\ttype 0x%x\n",
...
int gpt_runtime_init(void)
{
	...
	VERBOSE_("GPT: Runtime Configuration\n");
	VERBOSE_("  PPS/T:     0x%x/%u\n", gpt_config.pps, gpt_config.t);
	VERBOSE_("  PGS/P:     0x%x/%u\n", gpt_config.pgs, gpt_config.p);
	VERBOSE_("  L0GPTSZ/S: 0x%x/%u\n", GPT_L0GPTSZ, GPT_S_VAL);
	VERBOSE_("  L0 base:   0x%lx\n", gpt_config.plat_gpt_l0_base);
...
static int get_gpi_params(uint64_t base, gpi_info_t *gpi_info)
{
	...
	if (GPT_L0_TYPE(gpt_l0_desc) != GPT_L0_TYPE_TBL_DESC) {
		VERBOSE_("GPT: Granule is not covered by a table descriptor!\n");
		VERBOSE_("      Base=0x%"PRIx64"\n", base);
		return -EINVAL;
	}
				...
				uint64_t l1_desc)
{
	...
	j = GPT_L1_INDEX(ALIGN_512MB(base));
