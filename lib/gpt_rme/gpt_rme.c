...
typedef uint64_t *(*gpt_fill_func)(uint64_t *l1, uint64_t l1_desc);

/* Fills 2MB of L1 GPTs with Granules or Contiguous descriptors */
static uint64_t *fill_2mb(uint64_t *l1, uint64_t l1_desc);

/* Fills 32MB of L1 GPTs with Contiguous descriptors */
static uint64_t *fill_cont_32mb(uint64_t *l1, uint64_t l1_desc);

/* Fills 32MB of L1 GPTs with Contiguous descriptors */
static uint64_t *fill_cont_512mb(uint64_t *l1, uint64_t l1_desc);

typedef struct {
	size_t size;
	unsigned int contig;
	gpt_fill_func function;
} gpt_fill_lookup_t;
...
static void fuse_2mb(uintptr_t base, gpi_info_t *gpi_info, uint64_t l1_desc)
{
	...
	(void)fill_2mb(&gpi_info->gpt_l1_addr[idx], l1_desc);
}
				...
				uint64_t l1_desc)
{
	...
	(void)fill_cont_32mb(&gpi_info->gpt_l1_addr[idx], l1_desc);
}
				...
				uint64_t l1_desc)
{
	...
	(void)fill_cont_512mb(&gpi_info->gpt_l1_addr[idx], l1_desc);
}
				   ...
				   size_t length, uint64_t l1_desc)
{
	...
	for (unsigned long i = 0UL; i < ARRAY_SIZE(gpt_fill_lookup); ) {
		...
		if (GPT_REGION_IS_CONT(length, first, cont_size)) {
			...
			(void)gpt_fill_lookup[i].function(&l1[idx], l1_cont_desc);
			first += cont_size;
			length -= cont_size;
 ...
 */
static void get_gpi_info(gpi_info_t *gpi_info)
{
