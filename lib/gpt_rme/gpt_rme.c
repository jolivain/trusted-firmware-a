 ...
 */
static uint64_t *fill_nmb(uint64_t *l1, uint64_t l1_desc, unsigned int cnt)
{
				...
				uint64_t l1_desc)
{
	...
	return fill_nmb(&gpi_info->gpt_l1_addr[idx], l1_desc, QWORDS_2MB);
}
				...
				uint64_t l1_desc)
{
	...
	for (unsigned int i = 0U; i < 16U; i++) {
		...
		l1 = fill_nmb(l1, (l1 == l1_gran) ? l1_desc : l1_cont_desc,
								QWORDS_2MB);
	}
				...
				uint64_t l1_desc)
{
	...
	for (unsigned int i = 0U; i < 16U; i++) {
		if (l1 == l1_32mb) {
			...
			l1 = fill_nmb(l1, l1_cont_desc, QWORDS_32MB);
		}
	}
				   ...
				   size_t length, uint64_t l1_desc)
{
	...
	for (unsigned long i = 0UL; i < ARRAY_SIZE(gpt_fill_lookup); ) {
		...
		if (GPT_REGION_IS_CONT(length, first, cont_size)) {
			...
			(void)fill_nmb(&l1[idx], l1_cont_desc, cnt);
			first += cont_size;
			length -= cont_size;
				...
				uint64_t l1_desc)
{
...
#if (GPT_MAX_BLOCK == 2)
	(void)fill_nmb(&gpi_info->gpt_l1_addr[idx_2], l1_cont_desc, QWORDS_2MB);
	tlbi_2mb(base);
#else
	...
	for (unsigned int i = 0U; i < 16U; i++, j += gpt_l1_cnt_2mb) {
		if (gpi_info->gpt_l1_addr[j] != l1_cont_desc) {
			(void)fill_nmb(&gpi_info->gpt_l1_addr[idx_2],
...
#if (GPT_MAX_BLOCK == 32)
	(void)fill_nmb(&gpi_info->gpt_l1_addr[idx_32], l1_cont_desc, QWORDS_32MB);
	...
	for (unsigned int i = 0U; i < 16U; i++, j += gpt_l1_cnt_2mb * 16U) {
		if (gpi_info->gpt_l1_addr[j] != l1_cont_desc) {
			(void)fill_nmb(&gpi_info->gpt_l1_addr[idx_32],
	...
	(void)fill_nmb(&gpi_info->gpt_l1_addr[idx_512], l1_cont_desc, QWORDS_512MB);
