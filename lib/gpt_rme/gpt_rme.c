...
int gpt_runtime_init(void)
{
	...
	VERBOSE_("  L0 base:   0x%"PRIxPTR"\n", gpt_config.plat_gpt_l0_base);
	VERBOSE_("  Bitlocks:  0x%"PRIxPTR"\n", (uintptr_t)gpt_locks);
				...
				uint64_t l1_desc)
{
	...
	tlbirpalos_32m(ALIGN_32MB(base));
	dsbosh();
#else
	...
	tlbirpalos_512m(ALIGN_512MB(base));
	dsbosh();
				   ...
				   unsigned int target_pas)
{
	 ...
	 * Realm and Secure states, if called from undelegate code path.
	 */
	if (target_pas == GPT_GPI_NS) {
		remove_access(base, gpi_info, nse);
	}

	/* Set GPI encoding and transition granule */
	write_gpt(&gpi_info->gpt_l1_desc, gpi_info->gpt_l1_addr,
			  gpi_info->gpi_shift, gpi_info->idx, target_pas);
	dsboshst();

	/* Ensure that all agents observe new configuration */
	tlbi_page(base);
}
...
int gpt_delegate_pas(uint64_t base, size_t size, unsigned int src_sec_state)
{
	...
	if ((ULONG_MAX - base) < size) {
		VERBOSE_("GPT: Transition request address overflow!\n");
		VERBOSE_("      Base=0x%"PRIx64"\n", base);
		VERBOSE_("      Size=0x%lx\n", size);
		return -EINVAL;
	}
	...
	if (((base & (GPT_PGS_ACTUAL_SIZE(gpt_config.p) - 1UL)) != 0UL) ||
		...
		VERBOSE_("GPT: Invalid granule transition address range!\n");
		VERBOSE_("      Base=0x%"PRIx64"\n", base);
		VERBOSE_("      Size=0x%lx\n", size);
		return -EINVAL;
	}
	...
	if (src_sec_state == SMC_FROM_REALM) {
		...
		if (src_sec_state == SMC_FROM_SECURE) {
			...
			VERBOSE_("GPT: Invalid caller security state 0x%x\n",
							src_sec_state);
			return -EINVAL;
		}
	}
	...
	if (gpi_info.gpi != GPT_GPI_NS) {
		VERBOSE_("GPT: Only Granule in NS state can be delegated.\n");
		VERBOSE_("      Caller: %u, Current GPI: %u\n", src_sec_state,
			gpi_info.gpi);
	...
	if ((gpi_info.gpt_l1_desc & GPT_L1_TYPE_CONT_DESC_MASK) ==
					GPT_L1_TYPE_CONT_DESC) {
		shatter_block(base, &gpi_info, GPT_L1_NS_DESC, nse, target_pas);
	} else {
		write_gpt(&gpi_info.gpt_l1_desc, gpi_info.gpt_l1_addr,
			  gpi_info.gpi_shift, gpi_info.idx, target_pas);
		dsboshst();
	 ...
	 */
	VERBOSE_("GPT: Granule 0x%"PRIx64" GPI 0x%x->0x%x\n",
...
int gpt_undelegate_pas(uint64_t base, size_t size, unsigned int src_sec_state)
{
	...
	if ((src_sec_state == SMC_FROM_REALM) &&
		...
		VERBOSE_("GPT: Only Granule in REALM or SECURE state can be undelegated\n");
		VERBOSE_("      Caller: %u Current GPI: %u\n", src_sec_state,
			gpi_info.gpi);
		bit_unlock(gpi_info.bitlock, gpi_info.mask);
		return -EPERM;
	...
	if ((gpi_info.gpt_l1_desc & GPT_L1_TYPE_CONT_DESC_MASK) ==
					GPT_L1_TYPE_CONT_DESC) {
		shatter_block(base, &gpi_info, l1_desc, nse, GPT_GPI_NS);
	} else {
#endif
		remove_access(base, &gpi_info, nse);
