...
static uint64_t *fill_2mb(uint64_t *l1, uint64_t l1_cont_desc);
...
#endif
#if (GPT_MAX_BLOCK != 0)
	{ SZ_2M, GPT_L1_CONT_DESC_2MB, fill_2mb }
#endif
};
...
/* Fill 2MB with Granules or Contiguous descriptors */
static uint64_t *fill_2mb(uint64_t *l1, uint64_t l1_desc)
{
...
static void fuse_2mb(uintptr_t base, gpi_info_t *gpi_info, uint64_t l1_cont_desc)
{
	...
	(void)fill_2mb(&gpi_info->gpt_l1_addr[idx], l1_cont_desc);
}
...
static uint64_t *shatter_2mb(uintptr_t base, gpi_info_t *gpi_info, uint64_t l1_desc)
{
	...
	return fill_2mb(&gpi_info->gpt_l1_addr[idx], l1_desc);
}
				...
				uint64_t l1_desc)
{
	...
	for (unsigned int i = 0U; i < 16U; i++) {
		...
		l1 = fill_2mb(l1, (l1 == l1_gran) ? l1_desc : l1_cont_desc);
	}
