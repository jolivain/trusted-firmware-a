...
#define TLBI_ARG(_addr, _size)	\
		((ALIGNED(_addr, _size) << FOUR_KB_SHIFT) | \
		(TLBI_##_size << 44))
...
/* Get aligned address */
#define ALIGNED(_addr, _size)	((_addr) & ~(_size - 1UL))

/* Determine if address is aligned */
#define IS_ALIGNED(_addr, _size) (((_addr) & ((_size) - UL(1))) == UL(0))
