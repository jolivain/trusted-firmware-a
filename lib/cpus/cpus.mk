...
define cpus-config
        ...
        ifeq ($$($(1)-cpus),)
                $$(error no CPUs were configured (`$(1)-cpus=$$($(1)-cpus)` is empty))
        endif
        ...
        $(1)-cpus-compat = $$(sort \
                $$(filter-out %_helpers cpuamu wa_%, \
                        $$(basename $$(notdir \
                                $$(filter lib/cpus/aarch32/% lib/cpus/aarch64/%,$$($(1)-cpus-compat-sources)) \
                        ) \
                )) \
        )
        ...
        ifeq ($$($(1)-cpus),)
                ifneq ($$($(1)-cpus-compat),)
                        $$(warning this platform has explicitly specified CPU support library source files; generating a best-guess configuration for the CPU support library.)
                        $$(warning platforms should instead prefer to specify CPUs by name via `CPUS` (e.g. `CPUS := cortex_a75 cortex_a55`).)
                endif
        endif

        $(1)-cpus += $$($(1)-cpus-compat)
        $(1)-cpus := $$(sort $$($(1)-cpus))
