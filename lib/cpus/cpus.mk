...
CPUS_MAKEFILE := $(abspath $(lastword $(MAKEFILE_LIST)))
CPUS_SOURCE_DIR := $(patsubst %/,%,$(dir $(CPUS_MAKEFILE)))
...
ifneq ($(CPUS_CPUAMU_ENABLED),0)
        ifeq ($(ENABLE_AMU),0)
                $(error CPUAMU support requires that AMU support be enabled (`CPUS_CPUAMU_ENABLED=$(CPUS_CPUAMU_ENABLED)` requires `ENABLE_AMU=1`, have `ENABLE_AMU=$(ENABLE_AMU)`))
        endif
endif
...
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a65
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a65ae
...
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a76
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a76ae
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a77
...
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a510
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a710
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_a715
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_x1
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_x2
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_x3
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_hayes
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_hunter
CPUS_WITH_AARCH32_EXECUTION_STATE += cortex_hunter_elp_arm
CPUS_WITH_AARCH32_EXECUTION_STATE += denver
CPUS_WITH_AARCH32_EXECUTION_STATE += generic
CPUS_WITH_AARCH32_EXECUTION_STATE += neoverse_n1
CPUS_WITH_AARCH32_EXECUTION_STATE += neoverse_e1
CPUS_WITH_AARCH32_EXECUTION_STATE += neoverse_n2
CPUS_WITH_AARCH32_EXECUTION_STATE += neoverse_v1
CPUS_WITH_AARCH32_EXECUTION_STATE += neoverse_v2
CPUS_WITH_AARCH32_EXECUTION_STATE += neoverse_poseidon
CPUS_WITH_AARCH32_EXECUTION_STATE += qemu_max
CPUS_WITH_AARCH32_EXECUTION_STATE += rainier
...
CPUS_SOURCES := $(addprefix $(CPUS_SOURCE_DIR)/$(ARCH)/,$(CPUS_SUPPORTED))
CPUS_SOURCES := $(addsuffix .S,$(CPUS_SOURCES))

ifneq ($(filter neoverse_%,$(CPUS_SUPPORTED)),)
        CPUS_SUPPORTED += $(CPUS_SOURCE_DIR)/$(ARCH)/neoverse_n_common.S
endif

ifneq ($(CPUS_CPU_HELPERS_ENABLED),0)
        CPUS_SOURCES += $(CPUS_SOURCE_DIR)/$(ARCH)/cpu_helpers.S
endif

ifneq ($(CPUS_DSU_HELPERS_ENABLED),0)
        CPUS_SOURCES += $(CPUS_SOURCE_DIR)/aarch64/dsu_helpers.S
endif
...
ifneq ($(CPUS_CPUAMU_ENABLED),0)
        ...
        ifneq ($(CPUS_SUPPORTED_WITH_CPUAMU),)
                CPUS_SOURCES += $(CPUS_SOURCE_DIR)/$(ARCH)/cpuamu.c
                CPUS_SOURCES += $(CPUS_SOURCE_DIR)/$(ARCH)/cpuamu_helpers.S

                ifeq ($(HW_ASSISTED_COHERENCY),1)
                        CPUS_SOURCES += $(patsubst %,$(CPUS_SOURCE_DIR)/$(ARCH)/%_pubsub.c,$(CPUS_SUPPORTED_WITH_CPUAMU))
                endif
        endif
endif

ifneq ($(CPUS_ERRATA_REPORT_ENABLED),0)
        CPUS_SOURCES += $(CPUS_SOURCE_DIR)/errata_report.c
endif

ifneq ($(CPUS_CVE_2017_5715_WORKAROUND_ENABLED),0)
        CPUS_SOURCES += $(CPUS_SOURCE_DIR)/aarch64/wa_cve_2017_5715_bpiall.S
        CPUS_SOURCES += $(CPUS_SOURCE_DIR)/aarch64/wa_cve_2017_5715_mmu.S
endif
