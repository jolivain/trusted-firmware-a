...
ifneq ($(CPUS_SUPPORTED_NEED_HW_ASSISTED_COHERENCY),)
        ifeq ($(HW_ASSISTED_COHERENCY),0)
                $(error hardware-assisted coherency support must be enabled (have `HW_ASSISTED_COHERENCY=0`, need `HW_ASSISTED_COHERENCY=1`) to support these CPUs: $(CPUS_SUPPORTED_NEED_HW_ASSISTED_COHERENCY))
        endif
endif
...
ifeq ($(ARCH),aarch64)
        ifneq ($(CTX_INCLUDE_AARCH32_REGS),0)
                ifneq ($(CPUS_SUPPORTED_WITHOUT_AARCH32_EXECUTION_STATE),)
                        $(error there are AArch64 CPUs enabled (via `CPUS_SUPPORTED`) which do not support the AArch32 execution state (required by `CTX_INCLUDE_AARCH32_REGS=$(CTX_INCLUDE_AARCH32_REGS)`): $(CPUS_SUPPORTED_WITHOUT_AARCH32_EXECUTION_STATE))
                endif
        endif
endif
