...
#
# Automatically derive CPUs from the bootloader image sources.
#
# This is a backwards-compatibility workaround only, and should disappear once
# all platforms have moved over to using the CPU support library.
#

CPUS_ENABLE_COMPAT := $(filter lib/cpus/aarch32/% lib/cpus/aarch64/%, \
        $(BL1_SOURCES) $(BL2_SOURCES) $(BL31_SOURCES) $(BL32_SOURCES))
CPUS_ENABLE_COMPAT := $(basename $(notdir $(CPUS_ENABLE_COMPAT)))
CPUS_ENABLE_COMPAT := $(filter-out %_helpers cpuamu wa_%, $(CPUS_ENABLE_COMPAT))
CPUS_ENABLE_COMPAT := $(sort $(CPUS_ENABLE_COMPAT))

ifeq ($(CPUS_ENABLE),)
        ifneq ($(CPUS_ENABLE_COMPAT),)
                # If you are a platform maintainer whose platform is receiving
                # this warning, remove any `lib/cpus` sources from your platform
                # Makefiles and configure `CPUS_ENABLED` with the list of CPUs
                # enabled by your platform.
                $(warning warning: this platform has explicitly specified CPU support library source files; generating a best-guess configuration for the CPU support library.)
        endif
endif
...
CPUS_ENABLE := $(sort $(CPUS_ENABLE) $(CPUS_ENABLE_COMPAT))
...
ifneq ($(CPUS_ENABLE_IMPDEF_AMU),0)
        ifeq ($(ENABLE_FEAT_AMU),0)
                $(error the CPU AMU component requires that AMU support be enabled (`CPUS_ENABLE_IMPDEF_AMU=$(CPUS_ENABLE_IMPDEF_AMU)` requires `ENABLE_FEAT_AMU=1`, have `ENABLE_FEAT_AMU=0`))
        endif
endif
...
CPUS_NEED_HW_ASSISTED_COHERENCY += cortex_a720
...
CPUS_NEED_HW_ASSISTED_COHERENCY += neoverse_hermes
CPUS_NEED_HW_ASSISTED_COHERENCY += neoverse_poseidon
CPUS_NEED_HW_ASSISTED_COHERENCY += nevis
...
CPUS_HAVE_IMPDEF_AMU := 0

CPUS_WITH_CPUAMU := cortex_a75
CPUS_WITH_CPUAMU += neoverse_n1

CPUS_ENABLED_WITH_IMPDEF_AMU := $(filter $(CPUS_WITH_CPUAMU),$(CPUS_ENABLE))

ifneq ($(CPUS_ENABLE_IMPDEF_AMU),0)
        ifneq ($(CPUS_ENABLED_WITH_IMPDEF_AMU),)
                CPUS_HAVE_IMPDEF_AMU := 1
        endif
endif
...
CPUS_NEED_DSU_HELPERS := cortex_a55
CPUS_NEED_DSU_HELPERS += cortex_a65
CPUS_NEED_DSU_HELPERS += cortex_a65ae
CPUS_NEED_DSU_HELPERS += cortex_a75
CPUS_NEED_DSU_HELPERS += cortex_a76
CPUS_NEED_DSU_HELPERS += cortex_a510
CPUS_NEED_DSU_HELPERS += cortex_a710
CPUS_NEED_DSU_HELPERS += cortex_x2
CPUS_NEED_DSU_HELPERS += neoverse_n1
CPUS_NEED_DSU_HELPERS += neoverse_n2
CPUS_NEED_DSU_HELPERS += neoverse_e1

CPUS_ENABLED_WITH_DSU_SUPPORT := \
        $(filter $(CPUS_NEED_DSU_HELPERS),$(CPUS_ENABLE))
...
ifneq ($(CPUS_HAVE_IMPDEF_AMU),0)
        CPUS_DEFINES += HAVE_CPU_IMPDEF_AMU=1

        CPUS_SOURCES += lib/cpus/$(ARCH)/cpuamu.c
        CPUS_SOURCES += lib/cpus/$(ARCH)/cpuamu_helpers.S
        CPUS_SOURCES += $(patsubst %,lib/cpus/$(ARCH)/%_pubsub.c,$(CPUS_ENABLED_WITH_IMPDEF_AMU))
endif
...
undefine CPUS_ENABLE_IMPDEF_AMU
