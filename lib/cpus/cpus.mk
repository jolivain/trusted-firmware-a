...
CPUS_WITH_HW_ASSISTED_COHERENCY := aem_generic
...
CPUS_WITH_HW_ASSISTED_COHERENCY += qemu_max

CPUS_SUPPORTED_WITHOUT_HW_ASSISTED_COHERENCY := \
        $(filter-out $(CPUS_WITH_HW_ASSISTED_COHERENCY),$(CPUS_SUPPORTED))

ifeq ($(CPUS_SUPPORTED_WITHOUT_HW_ASSISTED_COHERENCY),)
        ifeq ($(HW_ASSISTED_COHERENCY),0)
                $(warning all of the CPUs in this system support hardware coherency, but it is disabled (have `HW_ASSISTED_COHERENCY=$(HW_ASSISTED_COHERENCY)`, but could use `HW_ASSISTED_COHERENCY=1`))
        endif
else
        ifneq ($(HW_ASSISTED_COHERENCY),0)
                $(error hardware coherency support must be disabled (have `HW_ASSISTED_COHERENCY=$(HW_ASSISTED_COHERENCY)`, need `HW_ASSISTED_COHERENCY=0`) to support these CPUs: $(CPUS_SUPPORTED_WITHOUT_HW_ASSISTED_COHERENCY))
        endif
endif
