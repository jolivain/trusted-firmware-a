...
#define CVE_FORMAT	"%s: %s: CPU workaround for CVE %d_%d was %s\n"
#define ERRATUM_FORMAT	"%s: %s: CPU workaround for erratum %d was %s\n"
...
static void generic_errata_report(void)
{
	...
	for (; entry != end; entry += 1) {
		...
		if (entry->cve) {
			PRINT_STATUS_DISPATCH(status, CVE_FORMAT, BL_STRING,
				cpu_ops->cpu_str, entry->cve, entry->erratum_id);

			if (check_cve && last_erratum_id >= entry->erratum_id) {
				ERROR("CVE %d_%d was out of order!\n",
				      entry->cve, entry->erratum_id);
				failed = true;
			}
			check_cve = true;
		} else {
			PRINT_STATUS_DISPATCH(status, ERRATUM_FORMAT, BL_STRING,
				cpu_ops->cpu_str, entry->erratum_id);

			if (last_erratum_id >= entry->erratum_id || check_cve) {
				ERROR("Erratum %d was out of order!\n",
				      entry->erratum_id);
				failed = true;
			}
		}
		last_erratum_id = entry->erratum_id;
	}
 ...
 * a CPU type.
 */
static __unused int errata_needs_reporting(spinlock_t *lock, long *reported)
{
	bool report_now;
...
void print_errata_status(void)
{
	...
	/*
	 * Unconverted cpus need to have their custom function called
	 * TODO: remove once all have been converted (same below)
	 */
	...
	if (!errata_needs_reporting(
	    cpu_ops->errata_lock, cpu_ops->errata_reported)) {
		return;
	}
