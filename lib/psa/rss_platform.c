...
psa_status_t rss_platform_nv_counter_increment(uint32_t counter_id)
{
	...
	return psa_call(RSS_PLATFORM_SERVICE_HANDLE,
			RSS_PLATFORM_API_ID_NV_INCREMENT, in_vec, 1, NULL, 0);
}

psa_status_t rss_platform_nv_counter_read(uint32_t counter_id, uint32_t size,
					  uint8_t *val)
{
	struct psa_invec in_vec[1];
	...
	return psa_call(RSS_PLATFORM_SERVICE_HANDLE,
			RSS_PLATFORM_API_ID_NV_READ, in_vec, 1, out_vec, 1);
}
