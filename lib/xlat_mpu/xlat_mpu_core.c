...
uint64_t prbar_attr_value (uint32_t attr)
{
	...
	extract = (uint64_t) (  (attr >> MT_PERM_SHIFT)
	                      & MT_PERM_MASK);
	if (extract == 0) {
...
/*
 * region_attr() performs the reverse of prbar_attr_value() and
 * prlar_attr_value() above, combined.  That is, it reconstructs the "attr"
 * from the attributes in PRBAR and PRLAR for the MPU region of interest.
 */
uint32_t region_attr (uint64_t prbar_attr, uint64_t prlar_attr)
{
	...
	ERROR("region_attr() is not yet implemented for xlat_mpu library.");
	panic();
}
...
static void mpu_map_region(mmap_region_t *mm)
{
	...
	/* Find and select first-available MPU region: */
...
void clear_all_mpu_regions(void)
{
	...
	/* MPU should already be disabled, but explicitly disable it
           nevertheless: */
	sctlr_el2_value = read_sctlr_el2() & ~((uint64_t) 1UL);
