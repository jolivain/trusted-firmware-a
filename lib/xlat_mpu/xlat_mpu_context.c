...
int xlat_make_tables_readonly(void)
{
	...
	/* Note:  Expound upon this, if EL3 is supported in future v8-R64. */
	...
	if (tf_xlat_ctx.xlat_regime == EL1_EL0_REGIME) {
		...
		assert(tf_xlat_ctx.xlat_regime == EL2_REGIME);
		enable_mpu_el2(0U);
		/* Note:  Change to EL3 if EL3 is added in future v8-R64. */
	}
...
void enable_mmu(unsigned int flags)
{
	...
	case 1:
		...
		break;
	case 2:
		enable_mpu_el2(flags);
		break;
	/* Note:  Add case 3 if EL3 is added to future v8-R64 cores. */
	default:
		panic();
	}
}
