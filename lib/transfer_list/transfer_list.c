...
{
	uintptr_t tl_old_ev, new_ev, old_ev, ru_new_ev;
	...
	if (__builtin_add_overflow((uintptr_t)te,
		te->hdr_size + te->data_size, &old_ev) ||
		ROUNDUP_OVERFLOW(old_ev, TRANSFER_LIST_GRANULE, &old_ev) ||
		__builtin_add_overflow((uintptr_t)te,
		te->hdr_size + new_data_size, &new_ev) ||
 ...
 ******************************************************************************/
void *transfer_list_data(struct transfer_list_entry *entry)
{
	if (!entry) {
		return NULL;
	}
