					       ...
					       struct transfer_list_entry *last)
{
	...
	if (va + sizeof(*te) > tl_ev || te->hdr_size < sizeof(*te) ||
		__builtin_add_overflow(te->hdr_size, te->data_size, &sz) ||
		__builtin_add_overflow(va, sz, &ev) ||
		ev > tl_ev) {
		return NULL;
	}
				 ...
				 uint32_t new_data_size)
{
	...
	if (__builtin_add_overflow(te->hdr_size, te->data_size, &sz) ||
		add_with_round_up_overflow((uintptr_t)te, sz,
		TRANSFER_LIST_GRANULE, &old_ev)) {
		return false;
	}
	if (__builtin_add_overflow(te->hdr_size, new_data_size, &sz) ||
		add_with_round_up_overflow((uintptr_t)te, sz,
					      ...
					      const void *data)
{
	...
	if (__builtin_add_overflow(sizeof(*te), data_size, &sz) ||
