					       ...
					       struct transfer_list_entry *last)
{
	...
	if (last) {
		va = (uintptr_t)last;
		if (ADD_WITH_ROUNDUP_OVERFLOW(va, last->hdr_size + last->data_size,
	...
	if (va + sizeof(*te) > tl_ev || te->hdr_size < sizeof(*te) ||
		__builtin_add_overflow(va, te->hdr_size + te->data_size, &ev) ||
		ev > tl_ev) {
		return NULL;
	}
				 ...
				 uint32_t new_data_size)
{
	...
	if (ADD_WITH_ROUNDUP_OVERFLOW((uintptr_t)te, te->hdr_size + te->data_size,
		TRANSFER_LIST_GRANULE, &old_ev) ||
		ADD_WITH_ROUNDUP_OVERFLOW((uintptr_t)te, te->hdr_size + new_data_size,
	...
	if (new_ev > old_ev) {
		...
		if (ROUNDUP_OVERFLOW(mov_dis, 1 << tl->alignment,
					      ...
					      const void *data)
{
	...
	if (ADD_WITH_ROUNDUP_OVERFLOW(ev, sizeof(*te) + data_size,
