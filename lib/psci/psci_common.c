...
void psci_save_dsu_state(pmu_state_t *pmu_state)
{
	...
	for (idx = 0U ; idx < read_counter_sel() ; idx++) {
		write_clusterpmselr(idx);
		pmu_state->counter_val[idx] = read_clusterpmxevcntr();
		pmu_state->counter_type[idx] = read_clusterpmxevtyper();
	}
}
...
void psci_restore_dsu_state(pmu_state_t *pmu_state)
{
	...
	for (idx = 0U ; idx < read_counter_sel() ; idx++){
		write_clusterpmselr(idx);
		RESTORE_PMU_REG(clusterpmxevcntr, pmu_state->counter_val[idx]);
		RESTORE_PMU_REG(clusterpmxevtyper, pmu_state->counter_type[idx]);
	}
