...
unsigned int psci_is_last_on_cpu(void)
{
	...
	for (cpu_idx = 0; cpu_idx < plat_core_count;
			cpu_idx++) {
		if (cpu_idx == my_idx) {
					 ...
					 plat_local_state_t req_pwr_state)
{
	...
	if ((pwrlvl > PSCI_CPU_PWR_LVL) && (pwrlvl <= PLAT_MAX_PWR_LVL) &&
			(cpu_idx < (unsigned int) plat_core_count)) {
...
void __init psci_init_req_local_pwr_states(void)
{
	...
	for (pwrlvl = 0U; pwrlvl < PLAT_MAX_PWR_LVL; pwrlvl++) {
		for (core = 0; core < plat_core_count; core++) {
...
void psci_print_power_domain_map(void)
{
	...
	for (idx = 0; idx < (PSCI_NUM_PWR_DOMAINS - plat_core_count);
							idx++) {
	...
	for (idx = 0; idx < plat_core_count; idx++) {
