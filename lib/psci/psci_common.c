 ...
 * Returns 1 (true) if the current CPU is the last ON CPU or 0 (false)
 * otherwise.
 * As a difference from psci_is_last_on_cpu, PSCI states are locked when this
 * function checks whether the current core is the last running core in the
 * system to avoid false positive check.
 ******************************************************************************/
		...
		for (unsigned int core = 0U;
			...
			if (psci_get_aff_info_state_by_idx(core) !=
				...
				psci_release_pwr_domain_locks(PLAT_MAX_PWR_LVL,
							      parent_nodes);
				ERROR("core=%u other than boot core=%u %s\n",
