...
static uint64_t base_xlation_table[NUM_BASE_LEVEL_ENTRIES] __aligned(
...
void enable_mmu_svc_mon(unsigned int flags)
{
	...
	mair0 |= MAIR0_ATTR_SET(ATTR_NON_CACHEABLE, ATTR_NON_CACHEABLE_INDEX);
	write_mair0(mair0);
	...
	if ((flags & XLAT_TABLE_NC) != 0U) {
		...
		ttbcr = TTBCR_EAE_BIT | TTBCR_SH0_NON_SHAREABLE |
			TTBCR_RGN0_OUTER_NC | TTBCR_RGN0_INNER_NC |
			(uint32_t)t0sz;
	} else {
		/* Inner & outer WBWA & shareable. */
		ttbcr = TTBCR_EAE_BIT | TTBCR_SH0_INNER_SHAREABLE |
			TTBCR_RGN0_OUTER_WBA | TTBCR_RGN0_INNER_WBA |
			(uint32_t)t0sz;
	}
