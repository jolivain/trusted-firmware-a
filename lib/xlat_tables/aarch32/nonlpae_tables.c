 ...
 * Copyright (c) 2014-2023, Arm Limited. All rights reserved.
...
#define MMU32B_DEFAULT_ATTRS                                    \
	(MMU32B_TTB_S | MMU32B_TTB_NOS | MMU32B_TTB_IRGN_WBWA | \
	 MMU32B_TTB_RNG_WBWA)
...
#define SMALL_PAGE_TEX(tex) \
	((((tex) >> 2) << 6) | ((((tex) >> 1) & 0x1) << 3) | (((tex)&0x1) << 2))
#define SMALL_PAGE_DEVICE SMALL_PAGE_TEX(MMU32B_ATTR_DEVICE_INDEX)
#define SMALL_PAGE_NORMAL SMALL_PAGE_TEX(MMU32B_ATTR_DEVICE_INDEX)
#define SMALL_PAGE_NORMAL_CACHED SMALL_PAGE_TEX(MMU32B_ATTR_IWBWA_OWBWA_INDEX)
...
#define MMU32B_PRRR_IDX(idx, tr, nos) \
	(((tr) << (2 * (idx))) | ((uint32_t)(nos) << ((idx) + 24)))
#define MMU32B_NMRR_IDX(idx, ir, or) \
	(((ir) << (2 * (idx))) | ((uint32_t)(or) << (2 * (idx) + 16)))
...
static uint32_t mmu_l1_base[NUM_1MB_IN_4GB] __aligned(MMU32B_L1_TABLE_ALIGN)
	__attribute__((section(".xlat_table")));

static uint32_t mmu_l2_base[MAX_XLAT_TABLES][NUM_4K_IN_1MB] __aligned(
	MMU32B_L2_TABLE_ALIGN) __attribute__((section(".xlat_table")));
...
void print_mmap(void)
{
	...
	printf("init xlat - l1:%p  l2:%p (%d)\n", (void *)mmu_l1_base,
	       (void *)mmu_l2_base, MAX_XLAT_TABLES);
	printf("mmap:\n");
	while (mm->size) {
			  ...
			  unsigned int level)
{
	...
	case 1U:
		...
		desc |= (attr & MT_MEMORY) != 0U ? SECTION_NORMAL_CACHED :
						   SECTION_DEVICE;
	...
	case 2U:
		...
		desc |= (attr & MT_MEMORY) != 0U ? SMALL_PAGE_NORMAL_CACHED :
						   SMALL_PAGE_DEVICE;
...
static mmap_region_t *init_xlation_table_inner(mmap_region_t *mm,
					       ...
					       unsigned int level)
{
	unsigned int level_size_shift = (level == 1U) ? ONE_MB_SHIFT :
							FOUR_KB_SHIFT;
	unsigned int level_size = 1U << level_size_shift;
	unsigned int level_index_mask =
		(level == 1U) ? (NUM_1MB_IN_4GB - 1) << ONE_MB_SHIFT :
	...
	do {
		...
		if (level == 2U) {
			printf("      0x%lx %x " + 6 - 2 * level, base_va,
			       level_size);
		}
#endif
		...
		if (desc == MMU32B_UNSET_DESC) {
			...
			mm = init_xlation_table_inner(
				mm, base_va, (uint32_t *)xlat_table, level + 1);
		}
#if LOG_LEVEL >= LOG_LEVEL_VERBOSE
		/* dump only non-lpae level 2 tables content */
		if (level == 2U) {
