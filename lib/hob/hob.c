		...
		uint16_t hob_type, uint16_t hob_length)
{
   ...
   */
	...
	hob_end = (struct efi_hob_generic_header *) ((efi_physical_address_t) new_hob + hob_length);
...
struct efi_hob_handoff_info_table *create_hob_list(
		uintptr_t efi_memory_begin, size_t efi_memory_length,
		uintptr_t efi_free_memory_bottom, size_t efi_free_memory_length)
{
	...
	hob_table->efi_memory_top = (efi_physical_address_t) efi_memory_begin +
		efi_memory_length;
	hob_table->efi_memory_bottom = (efi_physical_address_t) efi_memory_begin;
	hob_table->efi_free_memory_top = (efi_physical_address_t) efi_memory_begin +
		...
		uint64_t resource_length)
{
	...
	memset(&rd_hop->owner, 0x00, sizeof(struct efi_guid));
