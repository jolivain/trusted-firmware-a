...
static int gpt_validate_pas_mappings(pas_region_t *pas_regions, unsigned int pas_region_cnt)
{
	...
	for (idx = 0U; idx < pas_region_cnt; idx++) {
		...
		if (GPT_PAS_ATTR_MAP_TYPE(pas_regions[idx].attrs) ==
			...
			/*
			 * TODO: This counts the number of PAS that need L1
			 * tables but not the actual number of L1 tables needed
			 * this should be changed to fix this, as this value is
			 * used to decide if there is enough L1 table memory
			 * available.
			 */
