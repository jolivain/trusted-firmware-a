...
/* ---------------------------------------------------------------------------
 * GIC registers are common across NS and Realm world, so saving and restoring
 * of these registers only required if S-EL2 is present. For a system with
 * S-EL2, the saving/restoring will happen even when switching happens between
 * NS and Realm. This is because of current desing of context managment.
 *
 ...
 * ICH_LR<n>_EL2
 *
 * TODO: The ideal place for this is SPM Dispatcher but the current design of
 * context management does not allow it.
 * ---------------------------------------------------------------------------
 */
