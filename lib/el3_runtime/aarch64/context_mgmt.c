...
/*
 * Global place holder to capture endianness value, to configure SCTLR_EL2
 * register for systems without S-EL2.
 */
static u_register_t sctlr_el2_endian_val;
	...
	if (security_state == NON_SECURE) {
		...
		if (el2_implemented != EL_IMPL_NONE) {
			...
			if ((scr_el3 & SCR_HCE_BIT) != 0U) {
				/* Use saved endianness value to initialise sctlr_el2 */
				sctlr_el2 = sctlr_el2_endian_val;
...
static void el1_sysregs_context_save(el1_sysregs_t *ctx)
{
	...
	write_el1_ctx_aarch32(ctx, ifscr32_el2, read_ifsr32_el2());
	...
	if (is_feat_csv2_2_supported()) {
		write_el1_ctx_csv2_2(ctx, sxtnum_el0, read_scxtnum_el0());
		write_el1_ctx_csv2_2(ctx, sxtnum_el1, read_scxtnum_el1());
	}
...
static void el1_sysregs_context_restore(el1_sysregs_t *ctx)
{
	...
	write_ifsr32_el2(read_el1_ctx_aarch32(ctx, ifscr32_el2));
