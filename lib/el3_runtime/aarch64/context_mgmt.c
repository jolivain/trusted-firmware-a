...
static void setup_ns_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
	...
	write_el2_ctx_common(get_el2_sysregs_ctx(ctx), cm, ctx_sctlr_el2, sctlr_el2);
	...
	if (is_feat_hcx_supported()) {
		 ...
		 */
		write_el2_ctx_hcx(get_el2_sysregs_ctx(ctx), hcx, ctx_hcrx_el2,
			HCRX_EL2_INIT_VAL);
	}
	...
	if (is_feat_fgt_supported()) {
		 ...
		 */
		write_el2_ctx_fgt(get_el2_sysregs_ctx(ctx), fgt, ctx_hfgitr_el2,
			HFGITR_EL2_INIT_VAL);
		write_el2_ctx_fgt(get_el2_sysregs_ctx(ctx), fgt, ctx_hfgrtr_el2,
			HFGRTR_EL2_INIT_VAL);
		write_el2_ctx_fgt(get_el2_sysregs_ctx(ctx), fgt, ctx_hfgwtr_el2,
			HFGWTR_EL2_INIT_VAL);
	}
...
static void setup_context_common(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	write_el2_ctx_common(el2_ctx, cm, ctx_icc_sre_el2, icc_sre_el2);
#endif /* CTX_INCLUDE_EL2_REGS */
...
{
	write_el2_ctx_fgt(ctx, fgt, ctx_hdfgrtr_el2, read_hdfgrtr_el2());
	if (is_feat_amu_supported()) {
		write_el2_ctx_fgt(ctx, fgt, ctx_hafgrtr_el2, read_hafgrtr_el2());
	}
	write_el2_ctx_fgt(ctx, fgt, ctx_hdfgwtr_el2, read_hdfgwtr_el2());
	write_el2_ctx_fgt(ctx, fgt, ctx_hfgitr_el2, read_hfgitr_el2());
	write_el2_ctx_fgt(ctx, fgt, ctx_hfgrtr_el2, read_hfgrtr_el2());
	write_el2_ctx_fgt(ctx, fgt, ctx_hfgwtr_el2, read_hfgwtr_el2());
}

static void el2_sysregs_context_restore_fgt(el2_sysregs_t *ctx)
{
	write_hdfgrtr_el2(read_el2_ctx_fgt(ctx, fgt, ctx_hdfgrtr_el2));
	if (is_feat_amu_supported()) {
		write_hafgrtr_el2(read_el2_ctx_fgt(ctx, fgt, ctx_hafgrtr_el2));
	}
	write_hdfgwtr_el2(read_el2_ctx_fgt(ctx, fgt, ctx_hdfgwtr_el2));
	write_hfgitr_el2(read_el2_ctx_fgt(ctx, fgt, ctx_hfgitr_el2));
	write_hfgrtr_el2(read_el2_ctx_fgt(ctx, fgt, ctx_hfgrtr_el2));
	write_hfgwtr_el2(read_el2_ctx_fgt(ctx, fgt, ctx_hfgwtr_el2));
}
...
{
	write_el2_ctx_common(ctx, cm, ctx_actlr_el2, read_actlr_el2());
	write_el2_ctx_common(ctx, cm, ctx_afsr0_el2, read_afsr0_el2());
	write_el2_ctx_common(ctx, cm, ctx_afsr1_el2, read_afsr1_el2());
	write_el2_ctx_common(ctx, cm, ctx_amair_el2, read_amair_el2());
	write_el2_ctx_common(ctx, cm, ctx_cnthctl_el2, read_cnthctl_el2());
	write_el2_ctx_common(ctx, cm, ctx_cntvoff_el2, read_cntvoff_el2());
	write_el2_ctx_common(ctx, cm, ctx_cptr_el2, read_cptr_el2());
	if (CTX_INCLUDE_AARCH32_REGS) {
		write_el2_ctx_common(ctx, cm, ctx_dbgvcr32_el2, read_dbgvcr32_el2());
	}
	write_el2_ctx_common(ctx, cm, ctx_elr_el2, read_elr_el2());
	write_el2_ctx_common(ctx, cm, ctx_esr_el2, read_esr_el2());
	write_el2_ctx_common(ctx, cm, ctx_far_el2, read_far_el2());
	write_el2_ctx_common(ctx, cm, ctx_hacr_el2, read_hacr_el2());
	write_el2_ctx_common(ctx, cm, ctx_hcr_el2, read_hcr_el2());
	write_el2_ctx_common(ctx, cm, ctx_hpfar_el2, read_hpfar_el2());
	write_el2_ctx_common(ctx, cm, ctx_hstr_el2, read_hstr_el2());
	...
	isb();
	write_el2_ctx_common(ctx, cm, ctx_icc_sre_el2, read_icc_sre_el2());

	write_scr_el3(scr_el3);
	isb();

	write_el2_ctx_common(ctx, cm, ctx_ich_hcr_el2, read_ich_hcr_el2());
	write_el2_ctx_common(ctx, cm, ctx_ich_vmcr_el2, read_ich_vmcr_el2());
	write_el2_ctx_common(ctx, cm, ctx_mair_el2, read_mair_el2());
	write_el2_ctx_common(ctx, cm, ctx_mdcr_el2, read_mdcr_el2());
	write_el2_ctx_common(ctx, cm, ctx_sctlr_el2, read_sctlr_el2());
	write_el2_ctx_common(ctx, cm, ctx_spsr_el2, read_spsr_el2());
	write_el2_ctx_common(ctx, cm, ctx_sp_el2, read_sp_el2());
	write_el2_ctx_common(ctx, cm, ctx_tcr_el2, read_tcr_el2());
	write_el2_ctx_common(ctx, cm, ctx_tpidr_el2, read_tpidr_el2());
	write_el2_ctx_common(ctx, cm, ctx_ttbr0_el2, read_ttbr0_el2());
	write_el2_ctx_common(ctx, cm, ctx_vbar_el2, read_vbar_el2());
	write_el2_ctx_common(ctx, cm, ctx_vmpidr_el2, read_vmpidr_el2());
	write_el2_ctx_common(ctx, cm, ctx_vpidr_el2, read_vpidr_el2());
	write_el2_ctx_common(ctx, cm, ctx_vtcr_el2, read_vtcr_el2());
	write_el2_ctx_common(ctx, cm, ctx_vttbr_el2, read_vttbr_el2());
}

static void el2_sysregs_context_restore_common(el2_sysregs_t *ctx)
{
	write_actlr_el2(read_el2_ctx_common(ctx, cm, ctx_actlr_el2));
	write_afsr0_el2(read_el2_ctx_common(ctx, cm, ctx_afsr0_el2));
	write_afsr1_el2(read_el2_ctx_common(ctx, cm, ctx_afsr1_el2));
	write_amair_el2(read_el2_ctx_common(ctx, cm, ctx_amair_el2));
	write_cnthctl_el2(read_el2_ctx_common(ctx, cm, ctx_cnthctl_el2));
	write_cntvoff_el2(read_el2_ctx_common(ctx, cm, ctx_cntvoff_el2));
	write_cptr_el2(read_el2_ctx_common(ctx, cm, ctx_cptr_el2));
	if (CTX_INCLUDE_AARCH32_REGS) {
		write_dbgvcr32_el2(read_el2_ctx_common(ctx, cm, ctx_dbgvcr32_el2));
	}
	write_elr_el2(read_el2_ctx_common(ctx, cm, ctx_elr_el2));
	write_esr_el2(read_el2_ctx_common(ctx, cm, ctx_esr_el2));
	write_far_el2(read_el2_ctx_common(ctx, cm, ctx_far_el2));
	write_hacr_el2(read_el2_ctx_common(ctx, cm, ctx_hacr_el2));
	write_hcr_el2(read_el2_ctx_common(ctx, cm, ctx_hcr_el2));
	write_hpfar_el2(read_el2_ctx_common(ctx, cm, ctx_hpfar_el2));
	write_hstr_el2(read_el2_ctx_common(ctx, cm, ctx_hstr_el2));
	...
	write_scr_el3(scr_el3 | SCR_NS_BIT);
	isb();
	write_icc_sre_el2(read_el2_ctx_common(ctx, cm, ctx_icc_sre_el2));

	write_scr_el3(scr_el3);
	isb();

	write_ich_hcr_el2(read_el2_ctx_common(ctx, cm, ctx_ich_hcr_el2));
	write_ich_vmcr_el2(read_el2_ctx_common(ctx, cm, ctx_ich_vmcr_el2));
	write_mair_el2(read_el2_ctx_common(ctx, cm, ctx_mair_el2));
	write_mdcr_el2(read_el2_ctx_common(ctx, cm, ctx_mdcr_el2));
	write_sctlr_el2(read_el2_ctx_common(ctx, cm, ctx_sctlr_el2));
	write_spsr_el2(read_el2_ctx_common(ctx, cm, ctx_spsr_el2));
	write_sp_el2(read_el2_ctx_common(ctx, cm, ctx_sp_el2));
	write_tcr_el2(read_el2_ctx_common(ctx, cm, ctx_tcr_el2));
	write_tpidr_el2(read_el2_ctx_common(ctx, cm, ctx_tpidr_el2));
	write_ttbr0_el2(read_el2_ctx_common(ctx, cm, ctx_ttbr0_el2));
	write_vbar_el2(read_el2_ctx_common(ctx, cm, ctx_vbar_el2));
	write_vmpidr_el2(read_el2_ctx_common(ctx, cm, ctx_vmpidr_el2));
	write_vpidr_el2(read_el2_ctx_common(ctx, cm, ctx_vpidr_el2));
	write_vtcr_el2(read_el2_ctx_common(ctx, cm, ctx_vtcr_el2));
	write_vttbr_el2(read_el2_ctx_common(ctx, cm, ctx_vttbr_el2));
}
...
void cm_el2_sysregs_context_save(uint32_t security_state)
{
	...
	if (is_feat_mte_supported()) {
		write_el2_ctx_mte(el2_sysregs_ctx, mte, ctx_tfsr_el2,
					read_tfsr_el2());
	}
	...
	if (is_feat_ecv_v2_supported()) {
		write_el2_ctx_ecv(el2_sysregs_ctx, ecv, ctx_cntpoff_el2,
					read_cntpoff_el2());
	}

	if (is_feat_vhe_supported()) {
		write_el2_ctx_vhe(el2_sysregs_ctx, vhe, ctx_contextidr_el2,
					read_contextidr_el2());
		write_el2_ctx_vhe(el2_sysregs_ctx, vhe, ctx_ttbr1_el2,
					read_ttbr1_el2());
	}

	if (is_feat_ras_supported()) {
		write_el2_ctx_ras(el2_sysregs_ctx, ras, ctx_vdisr_el2,
					read_vdisr_el2());
		write_el2_ctx_ras(el2_sysregs_ctx, ras, ctx_vsesr_el2,
					read_vsesr_el2());
	}

	if (is_feat_nv2_supported()) {
		write_el2_ctx_neve(el2_sysregs_ctx, neve, ctx_vncr_el2,
					read_vncr_el2());
	}

	if (is_feat_trf_supported()) {
		write_el2_ctx_trf(el2_sysregs_ctx, trf, ctx_trfcr_el2,
					read_trfcr_el2());
	}

	if (is_feat_csv2_2_supported()) {
		write_el2_ctx_csv2_2(el2_sysregs_ctx, csv2, ctx_scxtnum_el2,
					read_scxtnum_el2());
	}

	if (is_feat_hcx_supported()) {
		write_el2_ctx_hcx(el2_sysregs_ctx, hcx, ctx_hcrx_el2,
					read_hcrx_el2());
	}

	if (is_feat_tcr2_supported()) {
		write_el2_ctx_tcr(el2_sysregs_ctx, tcr2, ctx_tcr2_el2,
					read_tcr2_el2());
	}

	if (is_feat_sxpie_supported()) {
		write_el2_ctx_sxpie(el2_sysregs_ctx, sxpie, ctx_pire0_el2, read_pire0_el2());
		write_el2_ctx_sxpie(el2_sysregs_ctx, sxpie, ctx_pir_el2, read_pir_el2());
	}

	if (is_feat_sxpoe_supported()) {
		write_el2_ctx_sxpoe(el2_sysregs_ctx, sxpoe, ctx_por_el2, read_por_el2());
	}

	if (is_feat_s2pie_supported()) {
		write_el2_ctx_s2pie(el2_sysregs_ctx, s2pie, ctx_s2pir_el2,
					read_s2pir_el2());
	}

	if (is_feat_gcs_supported()) {
		write_el2_ctx_gcs(el2_sysregs_ctx, gcs, ctx_gcscr_el2,
					read_gcspr_el2());
		write_el2_ctx_gcs(el2_sysregs_ctx, gcs, ctx_gcspr_el2,
					read_gcscr_el2());
	}
}
...
void cm_el2_sysregs_context_restore(uint32_t security_state)
{
	...
	if (is_feat_mte_supported()) {
		write_tfsr_el2(read_el2_ctx_mte(el2_sysregs_ctx, mte, ctx_tfsr_el2));
	}
	...
	if (is_feat_ecv_v2_supported()) {
		write_cntpoff_el2(read_el2_ctx_ecv(el2_sysregs_ctx, ecv,
					ctx_cntpoff_el2));
	}

	if (is_feat_vhe_supported()) {
		write_contextidr_el2(read_el2_ctx_vhe(el2_sysregs_ctx, vhe,
					ctx_contextidr_el2));
		write_ttbr1_el2(read_el2_ctx_vhe(el2_sysregs_ctx, vhe,
					ctx_ttbr1_el2));
	}

	if (is_feat_ras_supported()) {
		write_vdisr_el2(read_el2_ctx_ras(el2_sysregs_ctx, ras,
					ctx_vdisr_el2));
		write_vsesr_el2(read_el2_ctx_ras(el2_sysregs_ctx, ras,
					ctx_vsesr_el2));
	}

	if (is_feat_nv2_supported()) {
		write_vncr_el2(read_el2_ctx_neve(el2_sysregs_ctx, neve,
					ctx_vncr_el2));
	}

	if (is_feat_trf_supported()) {
		write_trfcr_el2(read_el2_ctx_trf(el2_sysregs_ctx, trf,
					ctx_trfcr_el2));
	}

	if (is_feat_csv2_2_supported()) {
		write_scxtnum_el2(read_el2_ctx_csv2_2(el2_sysregs_ctx, csv2,
					ctx_scxtnum_el2));
	}

	if (is_feat_hcx_supported()) {
		write_hcrx_el2(read_el2_ctx_hcx(el2_sysregs_ctx, hcx,
					ctx_hcrx_el2));
	}

	if (is_feat_tcr2_supported()) {
		write_tcr2_el2(read_el2_ctx_tcr(el2_sysregs_ctx, tcr2,
					ctx_tcr2_el2));
	}

	if (is_feat_sxpie_supported()) {
		write_pire0_el2(read_el2_ctx_sxpie(el2_sysregs_ctx, sxpie, ctx_pire0_el2));
		write_pir_el2(read_el2_ctx_sxpie(el2_sysregs_ctx, sxpie, ctx_pir_el2));
	}

	if (is_feat_sxpoe_supported()) {
		write_por_el2(read_el2_ctx_sxpoe(el2_sysregs_ctx, sxpoe, ctx_por_el2));
	}

	if (is_feat_s2pie_supported()) {
		write_s2pir_el2(read_el2_ctx_s2pie(el2_sysregs_ctx, s2pie, ctx_s2pir_el2));
	}

	if (is_feat_gcs_supported()) {
		write_gcscr_el2(read_el2_ctx_gcs(el2_sysregs_ctx, gcs, ctx_gcscr_el2));
		write_gcspr_el2(read_el2_ctx_gcs(el2_sysregs_ctx, gcs, ctx_gcspr_el2));
	}
}
#endif /* CTX_INCLUDE_EL2_REGS */
