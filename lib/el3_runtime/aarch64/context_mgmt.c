...
#if IMAGE_BL31
static void manage_extensions_nonsecure_global(void);
static void manage_extensions_secure_global(void);
#endif /* IMAGE_BL31 */
...
/*******************************************************************************
 * Intialise global_context for Non-Secure world.
...
static void manage_extensions_nonsecure_global(void)
{
	...
	if (is_feat_sve_supported()) {
		sve_enable_global(&global_context[CPU_CONTEXT_NS]);
	}
	...
	if (is_feat_sys_reg_trace_supported()) {
		sys_reg_trace_enable_global(&global_context[CPU_CONTEXT_NS]);
	}
}
#endif /* IMAGE_BL31 */

/*******************************************************************************
 * Intialise global_context for Secure world.
 * This function enables the architecture extensions, which have same value
...
static void manage_extensions_secure_global(void)
{
	...
	if (is_feat_sve_supported()) {
		 ...
		 * managed.
		 */
			sve_enable_global(&global_context[CPU_CONTEXT_SECURE]);
		} else {
		/*
		 * Disable SVE and FPU in secure context so non-secure world
		 * can safely use them.
		 */
			sve_disable_global(&global_context[CPU_CONTEXT_SECURE]);
		}
	}

	/* NS can access this but Secure shouldn't */
	if (is_feat_sys_reg_trace_supported()) {
		sys_reg_trace_disable_global(&global_context[CPU_CONTEXT_SECURE]);
	}
}
#endif /* IMAGE_BL31 */

/*******************************************************************************
 * Enable architecture extensions for features that have to do with the
 * global context.
 ******************************************************************************/
#if IMAGE_BL31
void cm_manage_extensions_global(void)
{
	manage_extensions_nonsecure_global();
	manage_extensions_secure_global();
}
#endif /* IMAGE_BL31 */
