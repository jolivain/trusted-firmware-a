...
void cm_setup_context(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	/*
	 * Check if Secure Cycle Counter is disabled when ARMv8.5-PMU
	 * is implemented
	 */
	if ((read_mdcr_el3() & MDCR_SCCD_BIT) == 0ULL) {
		/*
		 * If Secure Cycle Counter is not disabled in MDCR_EL3,
		 * initialise PMCR_EL0 setting all fields rather than
		 * relying on h/w. Some fields are architecturally UNKNOWN
		 * on reset.
		 *
		 * PMCR_EL0.LP: Set to one so that event counter overflow, that
		 *  is recorded in PMEVCNTR_EL0[0-30], occurs on the increment
		 *  that changes PMEVCNTR_EL0[63] from 1 to 0, when ARMv8.5-PMU
		 *  is implemented. This bit is RES0 in versions of the
		 *  architecture earlier than ARMv8.5, setting it to 1 doesn't
		 ...
		 *
		 * PMCR_EL0.E: Set to zero to disable all event counters
		 * in the range [0..(PMN-1)] and PMCCNTR_EL0.
		 */
		uint32_t pmcr_el0 = ((PMCR_EL0_RESET_VAL | PMCR_EL0_LP_BIT |
				      PMCR_EL0_LC_BIT | PMCR_EL0_DP_BIT) &
				    ~(PMCR_EL0_X_BIT | PMCR_EL0_D_BIT |
				      PMCR_EL0_E_BIT));
		write_ctx_reg(state, CTX_PMCR_EL0, pmcr_el0);
	}
...
void cm_prepare_el3_exit(uint32_t security_state)
{
	...
	if (security_state == NON_SECURE) {
		...
		if ((scr_el3 & SCR_HCE_BIT) != 0U) {
			 ...
			 *  overflow, that is recorded in PMEVCNTR_EL0[0-30],
			 *  occurs on the increment that changes
			 *  PMEVCNTR_EL0[63] from 1 to 0, when ARMv8.5-PMU is
			 *  implemented. This bit is RES0 in versions of the
			 *  architecture earlier than ARMv8.5, setting it to 1
			 *  doesn't have any effect on them.
			 *
			 * MDCR_EL2.HCCD: Set to one so that cycle counting by
			 *  PMCCNTR_EL0 is prohibited at EL2. This bit is RES0
			 *  in versions of the architecture earlier than
			 *  ARMv8.5, setting it to 1 doesn't have any effect
			 *  on them.
			 *
