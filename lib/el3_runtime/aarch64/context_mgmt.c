...
/* Sets up common bits across SCTLR_EL1 and SCTLR_EL2 */
...
static void setup_realm_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
	 ...
	 *  in an EL other than EL3 it will be trapped in EL3. See
	 *  setup_secure_contex() for more details.
	 */
	scr_el3 &= ~(SCR_API_BIT | SCR_APK_BIT);
#endif /* !CTX_INCLUDE_PAUTH_REGS */
}
...
/*
 * The lower-EL context is zeroed so that no stale values leak to a world. It is
 * assumed that an all-zero lower-EL context is good enough for it to boot
 * correctly. However, there are very few registers where this is not true and
 * some values need to be recreated.
 */
static void recreate_lost_el2_values(cpu_context_t *ctx)
{
	 ...
	 * SRE bit) is problematic for all worlds
	 */
...
static void setup_context_common(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	/* But recreate anything we shouldn't lose */
	recreate_lost_el2_values(ctx);
	 ...
	 */
	mdcr_el3 = (MDCR_EL3_RESET_VAL & (MDCR_TDA_BIT | MDCR_TDOSA_BIT)) |
		    MDCR_SDD_BIT | MDCR_SPD32(MDCR_SPD32_DISABLE);
...
static void manage_extensions_nonsecure_el2(void)
{
#if IMAGE_BL31
	if (is_feat_spe_supported()) {
		spe_enable_el2();
...
#if !CTX_INCLUDE_EL2_REGS
static void init_el2_no_hypervisor(cpu_context_t *ctx)
{
	...
	manage_extensions_nonsecure_el2();
}
#endif /* !CTX_INCLUDE_EL2_REGS */
...
void cm_el2_sysregs_context_save(uint32_t security_state)
{
	...
	write_ctx_reg(el2_sysregs_ctx, CTX_SCTLR_EL2, read_sctlr_el2());
...
#if IMAGE_BL31
	/*
	 * Notice, that this will not be saved in the !CTX_INCLUDE_EL2_REGS
	 * case! This is because in that config, this function is only used to
	 * initialize the register on first exit to a world in
	 * cm_prepare_el3_exit() and there will be no SPM to call
