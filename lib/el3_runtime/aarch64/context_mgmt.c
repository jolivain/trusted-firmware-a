...
static void setup_ns_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
	...
	write_el2_ctx_common(get_el2_sysregs_ctx(ctx), sctlr_el2, sctlr_el2);
...
static void setup_context_common(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	write_el2_ctx_common(el2_ctx, icc_sre_el2, icc_sre_el2);
#endif /* CTX_INCLUDE_EL2_REGS */
...
void cm_el2_sysregs_context_save(uint32_t security_state)
{
	...
	if (is_feat_tcr2_supported()) {
		write_el2_ctx_tcr(el2_sysregs_ctx, tcr2_el2, read_tcr2_el2());
	}
...
void cm_el2_sysregs_context_restore(uint32_t security_state)
{
	...
	if (is_feat_tcr2_supported()) {
		write_tcr2_el2(read_el2_ctx_tcr(el2_sysregs_ctx, tcr2_el2));
	}
