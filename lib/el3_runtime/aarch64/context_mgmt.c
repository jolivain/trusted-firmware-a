...
void cm_prepare_el3_exit(uint32_t security_state)
{
	...
	/* Save EL1 system registers, only when CTX_INCLUDE_EL2_REGS=0 */
 ...
 * page table walk
 */
void save_and_update_ptw_el1_sys_regs(cpu_context_t *ctx)
{
	assert(ctx != NULL);
	/* ----------------------------------------------------------
...
void restore_ptw_el1_sys_regs(cpu_context_t *ctx)
{
#if ERRATA_SPECULATIVE_AT
	/* -----------------------------------------------------------
