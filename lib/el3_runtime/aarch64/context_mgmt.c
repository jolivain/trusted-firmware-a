...
static void setup_secure_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
...
#endif
	/*
	 * Enable MTE support when CTX_INCLUDE_MTE_REGS is set,
	 * or when MTE is only implemented at EL0.
	 */
	...
	/* Enable S-EL2 if the next EL is EL2 */
	if (GET_EL(ep->spsr) == MODE_EL2) {
...
static void setup_ns_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
	...
	/* Initialize SCTLR_EL2 context register using SCTLR_EL1.EE value */
	u_register_t sctlr_el2 = read_ctx_reg(get_el1_sysregs_ctx(ctx),
			CTX_SCTLR_EL1);
	sctlr_el2 &= SCTLR_EE_BIT;
...
/*******************************************************************************
 * An alternative function to 'cm_prepare_el3_exit' that restores EL1 and EL2
 * sysreg contexts instead of directly updating EL1 and EL2 registers. Unlike
 * 'cm_prepare_el3_exit' this function can only be used to exit to NS world.
 * It also restores the EL2 sysreg context, therefore CTX_INCLUDE_EL2_REGS
