...
#if PLATFORM_REPORT_CTX_MEM_USE && (LOG_LEVEL >= LOG_LEVEL_INFO)

/********************************************************************************
 * Function that returns the corresponding string constant for a security state
 * index.
 *******************************************************************************/
const char *get_context_name_by_idx(unsigned int security_state_idx)
{
 ...
 *******************************************************************************/
size_t report_allocated_memory(unsigned int security_state_idx)
{
	size_t core_total = 0;
	size_t el3_total = 0;
#if CTX_INCLUDE_EL2_REGS
...
void report_ctx_memory_usage(void)
{
	...
	for (unsigned int i = 0; i < CPU_CONTEXT_NUM; i++) {
