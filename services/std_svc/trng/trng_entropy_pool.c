 ...
 * in a  single call or 3, 64bit words per call. We have 4 words in the pool so
 * that when we have 1-63 bits in the pool, and we have a request for 192 bits
 * of entropy, we don't have to throw out the leftover 1-63 bits of entropy.
 */
#define WORDS_IN_POOL (4)
uint64_t entropy[WORDS_IN_POOL];
/* index in bits of the first bit of usable entropy */
uint32_t entropy_bit_index;
/* then number of valid bits in the entropy pool */
uint32_t entropy_bit_size;
...
/* Pack entropy the out buffer, filling and taking locks as needed.
...
{
	/* TODO: take lock for thread stafty */
	bool success = true;
	...
	for (word_i = 0; word_i < to_fill; word_i++) {
		/* Repack the entropy from the pool into the passed in ent
		 ...
		 * don't want to or in the whole next word of entropy.
		 */
	...
	const uint64_t mask = 0xFFFFFFFFFFFFFFFF >> (BITS_PER_WORD - (nbits % BITS_PER_WORD));
	out[to_fill - 1] &=  mask;
