					...
					void *handle)
{
	...
	cpu_partnum = EXTRACT_PARTNO(midr_val);

	for (uint8_t i = 0; i < size_cpulist; i++) {
		cpu_ptr = &cpu_list[i];
		uint16_t partno_extracted = EXTRACT_PARTNO(cpu_ptr->cpu_partnumber);
...
bool validate_spsr_mode(void)
{
	...
	/* In AArch32, if in system/svc mode, return true */
		...
		if (GET_M32(read_spsr()) == (MODE32_sys || MODE32_svc)) {
			return true;
		}
		return false;
	#endif
