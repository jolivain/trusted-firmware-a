			...
			uint8_t rxpx_val)
{
	...
	high_index = ARRAY_SIZE(ptr->cpu_errata_list);
				  ...
				  void *handle)
{
	...
	cpu_partnum = ((midr_val) >> MIDR_PN_SHIFT) &  MIDR_PN_MASK;

	for (uint8_t i = 0; i < size_cpulist; i++) {
		cpu_ptr = &cpu_list[i];
		uint16_t partno_extracted = (cpu_ptr->cpu_partnumber >> MIDR_PN_SHIFT) \
						& MIDR_PN_MASK;
		if (partno_extracted == cpu_partnum) {
	...
	case ARM_EM_CPU_ERRATUM_FEATURES:
		/* If processor is in AARCH64 state and forward flag != 0
		  and calling EL = EL1, return Invalid Parameters */
			...
			if (((uint32_t)x2 != 0) && (read_el_state == MODE_EL1)) {
			...
			read_el_state = GET_M32(read_spsr());
