...
int32_t binary_search(struct em_cpu_list *ptr, uint32_t erratum_id, uint8_t rxpx_val)
{
	 ...
	 */
	struct em_cpu *errata_list_ptr = &ptr->cpu_errata_list[0];

	while (low_index <= high_index) {
		mid_index = (low_index + high_index) / 2;
		if (erratum_id < (errata_list_ptr + mid_index)->em_errata_id) {
			high_index = mid_index - 1;
		} else if (erratum_id > (errata_list_ptr + mid_index)->em_errata_id) {
			low_index = mid_index + 1;
		} else if (erratum_id == (errata_list_ptr + mid_index)->em_errata_id) {
			if (RXPX_RANGE(rxpx_val, (errata_list_ptr + mid_index)->em_rxpx_lo, \
				(errata_list_ptr + mid_index)->em_rxpx_hi)) {
				if (((errata_list_ptr + mid_index)->errata_enabled) && \
				(!((errata_list_ptr + mid_index)->non_arm_interconnect))) {
