...
#if NEOVERSE_V1_H_INC
{
	...
	.cpu_errata_list = {
		...
		#if ERRATA_NON_ARM_INTERCONNECT
		{2701953, 0x00, 0x11, ERRATA_V1_2701953, 0x12, true, \
...
#if NEOVERSE_N2_H_INC
{
	...
	.cpu_errata_list = {
		...
		#if ERRATA_NON_ARM_INTERCONNECT
		{2728475, 0x00, 0x02, ERRATA_N2_2728475, 0x03, true, \
		ERRATA_NON_ARM_INTERCONNECT},
		 ...
		 */
			...
			if (RXPX_RANGE(rxpx_val, ptr->cpu_errata_list[mid_index].em_rxpx_lo, \
				...
				if (ptr->cpu_errata_list[mid_index].non_arm_interconnect) {
					return (ptr->cpu_errata_list[mid_index]. \
					errata_enabled == true) ? EM_AFFECTED :
					EM_UNKNOWN_ERRATUM;
				}
				if (ptr->cpu_errata_list[mid_index].errata_enabled == true) {
