...
/*
 * Helper function to validate tha the buffer base and length are
 * within range.
 */
static int validate_buffer_params(uint64_t buf_pa, uint64_t buf_len)
{
	...
	(void)plat_rmmd_get_el3_shared_mem(&shared_buf_base);
				   ...
				   uint64_t challenge_hash_len)
{
	...
	err = validate_buffer_params(buf_pa, *buf_len);
	if (err != 0) {
		return err;
	}
	...
	(void)memcpy(temp_buf, (void *)buf_pa, challenge_hash_len);

	print_challenge((uint8_t *)temp_buf, challenge_hash_len);

	/* Get the platform token. */
	err = plat_get_cca_attest_token((uintptr_t)buf_pa,
				...
				uint64_t ecc_curve)
{
	...
	err = plat_get_cca_realm_attest_key((uintptr_t)buf_pa, buf_len,
