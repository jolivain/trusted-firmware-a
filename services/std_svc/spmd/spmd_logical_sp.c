	...
	uint32_t uuid[4], uint16_t *lp_count_out)
{
	...
	lp_array = get_spmd_el3_lp_array();
	for (uint16_t index = 0; index < SPMD_LSP_DESCS_COUNT; ++index) {
...
int32_t spmd_logical_sp_init(void)
{
	...
	assert(SPMD_LSP_DESCS_COUNT <= EL3_SPMD_MAX_NUM_LP);
...
bool spmd_el3_invoke_partition_info_get(const uint32_t target_uuid[4],
				const uint16_t start_index,
				const uint16_t tag,
				struct ffa_value *retval)
{
