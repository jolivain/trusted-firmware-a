...
{
	return (struct el3_spmd_lp_desc *) EL3_SPMD_LP_DESCS_START;
}
...
static int el3_spmd_sp_desc_validate(void)
{
	...
	assert(EL3_SPMD_LP_DESCS_END >= EL3_SPMD_LP_DESCS_START);

	/* If no SPMD logical partitions are implemented then simply bail out. */
	if (EL3_SPMD_LP_DESCS_COUNT == 0U) {
		return 0;
	}

	lp_array = get_spmd_el3_lp_array();

	for (unsigned int index = 0; index < EL3_SPMD_LP_DESCS_COUNT; index++) {
		...
		for (unsigned int inner_idx = index + 1;
		     inner_idx < EL3_SPMD_LP_DESCS_COUNT; inner_idx++) {
	...
	uint32_t uuid[4], uint16_t *lp_count_out)
{
	...
	lp_array = get_spmd_el3_lp_array();
	for (uint16_t index = 0; index < EL3_SPMD_LP_DESCS_COUNT; ++index) {
		struct el3_spmd_lp_desc *lp = &lp_array[index];
...
int32_t spmd_logical_sp_init(void)
{
	...
	assert(EL3_SPMD_LP_DESCS_COUNT <= EL3_SPMD_MAX_NUM_LP);
	...
	INFO("SPMD Logical Secure Partition init start.\n");
	for (unsigned int i = 0U; i < EL3_SPMD_LP_DESCS_COUNT; i++) {
	...
	struct ffa_partition_info_v1_1 *partition_info)
{
	...
	uint64_t info = *arg_ptrs;

	arg_ptrs++;
	uint64_t uuid_lo = *arg_ptrs;

	arg_ptrs++;
	uint64_t uuid_high = *arg_ptrs;

	partition_info->ep_id = info & 0xFFFF;
	partition_info->execution_ctx_count = (info >> 16) & 0xFFFF;
	partition_info->properties = (info >> 32);
	partition_info->uuid[0] = uuid_lo & 0xFFFFFFFF;
	partition_info->uuid[1] = (uuid_lo >> 32) & 0xFFFFFFFF;
	partition_info->uuid[2] = uuid_high & 0xFFFFFFFF;
	partition_info->uuid[3] = (uuid_high >> 32) & 0xFFFFFFFF;
						...
						uint64_t x2, uint64_t x3)
{
	...
	w0 = (uint32_t)(x1 & 0xFFFFFFFF);
	w1 = (uint32_t)(x1 >> 32);
	w2 = (uint32_t)(x2 & 0xFFFFFFFF);
	w3 = (uint32_t)(x2 >> 32);
	...
	ret.arg2 = (sizeof(struct ffa_partition_info_v1_1) & 0xFFFF) << 48;
...
bool spmd_el3_partition_info_get(const uint32_t target_uuid[4],
				const uint16_t start_index,
				const uint16_t tag,
				struct ffa_value *retval)
{
...
inline bool is_spmd_logical_sp_info_regs_req_in_progress(
		spmd_spm_core_context_t *ctx)
{
#if ENABLE_SPMD_EL3_LP
