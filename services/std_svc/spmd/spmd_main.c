			  ...
			  uint64_t flags)
{
	...
	case FFA_VERSION:
		...
		if ((input_version & FFA_VERSION_BIT31_MASK) ||
			 ...
			 * from SPMD to SPMC with FFA_VERSION target function
			 * identifier in X2 and requested version in X3.
			 */
			spmd_build_spmc_message(gpregs, SPMD_DIR_MSG_FFA_VERSION,
						input_version);

			sync_exit_ret = spmd_spm_core_sync_entry(ctx);
			assert(sync_exit_ret == 0);
			if(SMC_GET_GP(gpregs, CTX_GPREG_X0)
				!= FFA_MSG_SEND_DIRECT_RESP_SMC32 ||
			   SMC_GET_GP(gpregs, CTX_GPREG_X2)
				!= (BIT(31) | SPMD_DIR_MSG_FFA_VERSION)) {
			 ...
			 */
			return spmd_smc_forward(SMC_GET_GP(gpregs, CTX_GPREG_X3),
					true, FFA_PARAM_MBZ, FFA_PARAM_MBZ,
					FFA_PARAM_MBZ, FFA_PARAM_MBZ, gpregs);
		} else {
