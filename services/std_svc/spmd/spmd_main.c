...
static int spmd_spmc_init(void *pm_addr)
{
	...
	int rc;
	uintptr_t sec_address __unused;
	uintptr_t root_base_addr __unused, sec_base_addr __unused;
...
#if ENABLE_RME && SPMD_SPM_AT_SEL2 && !RESET_TO_BL31
	/* Get root and secure manifest addrs from the platform */
	rc = plat_get_config_addrs_and_size(TOS_FW_CONFIG_ID, &root_base_addr,
					    &sec_base_addr, &size);
	if ((rc != 0) || (sec_base_addr == 0UL)) {
		return rc;
	}

	/* Copy manifest from root->secure region */
	spmd_do_sec_cpy(root_base_addr, sec_base_addr, size);

	/* Update ep info of BL32 */
	assert(spmc_ep_info != NULL);
	spmc_ep_info->args.arg0 = sec_base_addr;
#endif /* ENABLE_RME && SPMD_SPM_AT_SEL2 && !RESET_TO_BL31 */
