...
static int32_t spmd_init(void)
{
	...
	spmd_logical_sp_set_spmc_inited();
			  ...
			  uint64_t flags)
{
	...
	/*
	 * if there is an ongoing info regs from EL3 SPMD LP, unconditionally
	 * return, we dont expect any other FF-A ABIs to be called between
	 * calls to partition info get.
	 */
	...
	case FFA_MSG_SEND_DIRECT_REQ_SMC64:
		...
		if (is_el3_spmd_lp_id(ffa_endpoint_source(x1)) ||
				is_el3_spmd_lp_id(ffa_endpoint_destination(x1))) {
			return spmd_ffa_error_return(handle,
