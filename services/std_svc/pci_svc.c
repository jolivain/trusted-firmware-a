			     ...
			     u_register_t flags)
{
	...
	case SMC_PCI_READ: {
		...
		if (pci_read_config(x1, x2, x3, &ret) != SMC_PCI_CALL_SUCCESS) {
			SMC_RET2(handle, SMC_PCI_CALL_INVAL_PARAM, 0);
		} else {
	...
	case SMC_PCI_SEG_INFO: {
		...
		if ((x2 != 0U) || (x3 != 0U) || (x4 != 0U)) {
		    SMC_RET3(handle, SMC_PCI_CALL_INVAL_PARAM, 0, 0);
		}
