...
int el3_sp_desc_validate(void)
{
	...
	for (index = 0U; index < EL3_LP_DESCS_COUNT; index++) {
		el3_lp_desc_t *lp_desc = &lp_array[index];
		...
		for (inner_idx = index + 1;
		     inner_idx < EL3_LP_DESCS_COUNT; inner_idx++) {
			el3_lp_desc_t *other_lp_desc = &lp_array[inner_idx];

			if (lp_desc->sp_id == other_lp_desc->sp_id) {
