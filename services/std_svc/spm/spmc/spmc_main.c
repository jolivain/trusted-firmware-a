...
/*
 * Helper function to validate a secure partition ID to ensure it does not conflict
 * with any other FF-A component and follows the convention to indicate it
 * resides within the secure world.
 */
				       ...
				       uint64_t flags)
{
	...
	if (sp == NULL) {
		VERBOSE("Attempted to send direct request to unknown partition ID (0x%x)\n",
			dst_id);
		return spmc_ffa_error_return(handle, FFA_ERROR_INVALID_PARAMETER);
	}
	...
	if (sp->ec[idx].rt_state != RT_STATE_WAITING) {
		VERBOSE("S-EL1 SP context on core%u not handling a direct request (%u)\n",
			get_ec_index(sp), sp->ec[idx].rt_model);
					...
					uint64_t flags)
{
	...
	if (!secure_origin) {
		VERBOSE("Direct Response not supported from the normal world\n");
		return spmc_ffa_error_return(handle, FFA_ERROR_INVALID_PARAMETER);
	}
	...
	if ((dst_id != FFA_SPMC_ID) && ffa_is_secure_world_id(dst_id)) {
		VERBOSE("Attempted to send direct response to invalid partition ID (0x%x)\n",
			dst_id);
		return spmc_ffa_error_return(handle, FFA_ERROR_INVALID_PARAMETER);
	}
	...
	if (sp == NULL) {
		VERBOSE("Attempted to send direct response to unknown partition ID (0x%x)\n",
			dst_id);
		return spmc_ffa_error_return(handle, FFA_ERROR_INVALID_PARAMETER);
	}
	...
	if (sp->ec[idx].rt_model != RT_MODEL_DIR_REQ) {
		VERBOSE("S-EL1 SP context on core%u not handling a direct response (%u)\n",
			get_ec_index(sp), sp->ec[idx].rt_model);
				 ...
				 uint64_t flags)
{
	...
	return spmc_smc_return(smc_fid, secure_origin, x1, x2, x3, x4,
			       handle, cookie, flags, FFA_MSG_WAIT_EP_ID(x1));
}
				    ...
				    uint64_t flags)
{
	...
	if (secure_origin) {
		 ...
		 *       case.
		 */
		assert(requested_version == spmc_get_current_sp_ctx()->ffa_version);
	} else {
		/*
		 * If this is called by the normal world, record this information
		 * in it's descriptor.
		 */
...
/*
 * Add helper function to obtain the ffa version of the calling
 * partition.
 */
				 ...
				 uint64_t flags)
{
	...
	/* Ensure the buffer size is a multiple of the translation granule size in TF-A. */
...
/*
 * Handle populating the required partiion information for a v1.1
						...
						int *partition_count)
{
	...
	(void)memset(mbox->rx_buffer, 0, mbox->rxtx_page_count * FFA_PAGE_SIZE);
	...
	for (index = 0; index < EL3_LP_DESCS_COUNT; index++) {
		if (compare_uuid(uuid, el3_lp_descs[index].uuid) || is_null_uuid) {
			...
			if (is_null_uuid) {
				desc->uuid[0] = el3_lp_descs[index].uuid[0];
				desc->uuid[1] = el3_lp_descs[index].uuid[1];
				desc->uuid[2] = el3_lp_descs[index].uuid[2];
				desc->uuid[3] = el3_lp_descs[index].uuid[3];
			}
			(*partition_count)++;
		}
	}
...
/*
 * Handle populating the required partiion information for a v1.0
 * caller in the appropriate version of the partition information
 * decsriptor.
 */
...
/*
 * Main handler for Partition_Info_Get which support both v1.1 and v1.0
 * implementations. The version of the caller is first determined and the
					   ...
					   uint64_t flags)
{
	...
	count_only = (x5 & FFA_PARTITION_INFO_GET_COUNT_FLAG_MASK);
	 ...
	 * check we have the buffers available and populate the appropriate
	 * structure version.
	 */
	else {
		...
		if (ffa_version == MAKE_FFA_VERSION(U(1), U(0))) {
			...
			ERROR("Unknown FF-A version %x", ffa_version);
				     ...
				     uint64_t flags)
{
	...
	if ((function_id & FFA_FEATURES_BIT31_MASK)) {
		...
		case FFA_MSG_RUN:
			 ...
			 * will be set to 0 as these values align with the currently
			 * implemented features of the SPMC. If this changes this
			 * function must be extended to handle reporting the
			     ...
			     entry_point_info_t *ep_info)
{
	...
	ret = fdt_read_uint32_array(sp_manifest, node, "uuid", 4,
				    sp->uuid);
	if (ret) {
	...
	if (config_32 != PLATFORM_CORE_COUNT) {
		ERROR("SP Execution Context Count (%d) must be %d).\n",
	...
	if (ret) {
		...
		if (!validate_partition_id(config_32)) {
...
static int find_and_prepare_sp_context(void)
{
	...
	ret = fdt_node_offset_by_compatible(sp_manifest, -1, "arm,ffa-manifest-1.0");
	if (ret < 0) {
...
static int32_t sp_init(void)
{
	...
	VERBOSE("S-EL1 SP context on core%u is in %u state.\n",
...
static void initalize_ns_ep_descs(void)
{
	...
	for (int i = 0; i < NS_PARTITION_COUNT; i++) {
		ns_ep = &ns_ep_desc[i];
		/*
		 * Clashes with the Hypervisor ID but wil not be a
		 * problem in practice.
		 */
		ns_ep->ns_ep_id = 0;
		ns_ep->ffa_version = 0;
...
int32_t spmc_setup(void)
{
	...
	/*
	 * Retrieve the information of the datstore for tracking shared memory
			  ...
			  uint64_t flags)
{
	...
	case FFA_VERSION:
		return ffa_version_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					   cookie, handle, flags);

	case FFA_SPM_ID_GET:
		return ffa_spm_id_get_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					      cookie, handle, flags);

	case FFA_ID_GET:
		return ffa_id_get_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					  cookie, handle, flags);
	...
	case FFA_MSG_SEND_DIRECT_REQ_SMC32:
	case FFA_MSG_SEND_DIRECT_REQ_SMC64:
		return direct_req_smc_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					      cookie, handle, flags);

	case FFA_MSG_SEND_DIRECT_RESP_SMC32:
	case FFA_MSG_SEND_DIRECT_RESP_SMC64:
		return direct_resp_smc_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					       cookie, handle, flags);
	...
	case FFA_RXTX_UNMAP:
		return rxtx_unmap_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					  cookie, handle, flags);

	case FFA_PARTITION_INFO_GET:
		return partition_info_get_handler(smc_fid, secure_origin, x1, x2, x3, x4,
						  cookie, handle, flags);

	case FFA_RX_RELEASE:
		return rx_release_handler(smc_fid, secure_origin, x1, x2, x3, x4,
					  cookie, handle, flags);
	...
	case FFA_MEM_FRAG_TX:
		return spmc_ffa_mem_frag_tx(smc_fid, secure_origin, x1, x2, x3, x4,
					    cookie, handle, flags);

	case FFA_MEM_FRAG_RX:
		return spmc_ffa_mem_frag_rx(smc_fid, secure_origin, x1, x2, x3, x4,
					    cookie, handle, flags);

	case FFA_MEM_RETRIEVE_REQ_SMC32:
	case FFA_MEM_RETRIEVE_REQ_SMC64:
		return spmc_ffa_mem_retrieve_req(smc_fid, secure_origin, x1, x2, x3, x4,
						 cookie, handle, flags);

	case FFA_MEM_RELINQUISH:
		return spmc_ffa_mem_relinquish(smc_fid, secure_origin, x1, x2, x3, x4,
					       cookie, handle, flags);

	case FFA_MEM_RECLAIM:
		return spmc_ffa_mem_reclaim(smc_fid, secure_origin, x1, x2, x3, x4,
					    cookie, handle, flags);

	default:
		WARN("Not Supported 0x%x (0x%lx, 0x%lx, 0x%lx, 0x%lx) FFA Request ID\n",
		     smc_fid, x1, x2, x3, x4);
		break;
	}
	return spmc_ffa_error_return(handle, FFA_ERROR_NOT_SUPPORTED);
