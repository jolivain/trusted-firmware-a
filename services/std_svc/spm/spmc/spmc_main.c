					...
					uint64_t flags)
{
	 ...
	 */
	if ((dst_id != FFA_SPMC_ID) && (dst_id & FFA_SWD_ID_MASK)) {
		return spmc_ffa_error_return(handle, FFA_ERROR_INVALID_PARAMETER);
	}
				 ...
				 uint64_t flags)
{
	...
	/*
	 * We only expect FFA ERROR to be received during SP initialisation
						...
						int *partition_count)
{
	...
	/*
	 * If we are populating the descriptor cast the RX buffer to a v1.1 format,
	 * we have already checked that is it not NULL.
	 */
	...
	for (index = 0; index < SECURE_PARTITION_COUNT; index++) {
		if (compare_uuid(uuid, sp_desc[index].uuid) || is_null_uuid) {
			...
			/* Execution context count must match No. cores for S-EL1 SPs. */
			desc->execution_ctx_count = PLATFORM_CORE_COUNT;
						...
						int *partition_count)
{
	...
	/*
	 * If we are populating the descriptor cast the RX buffer to a v1.0 format,
	 * we have already checked that is it not NULL.
	 */
					   ...
					   uint64_t flags)
{
	...
	/*
	 * Handle the case where the partition descriptors are required, check we have the
	 * buffers available and populate the appropriate structure version.
	 */
	else {
