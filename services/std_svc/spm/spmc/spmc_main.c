...
static inline bool direct_msg_validate_arg2(uint64_t x2)
{
	...
	if (x2 & FFA_DIRECT_FRAMEWORK_MSG_MASK) {
		...
		if (x2 & (~FFA_PM_MSG_MASK & ~(FFA_DIRECT_FRAMEWORK_MSG_MASK))) {
		...
		if (x2 != (uint64_t) 0) {
			VERBOSE("Arg2 MBZ for partition messages. (0xlx).\n", x2);
			return false;
		}
	}
	return true;
				       ...
				       uint64_t flags)
{
	...
	/* Check if arg2 has been populated correctly base on message type. */
					  ...
					  void *cookie)
{
	 ...
	 */
	return spmd_smc_switch_state(FFA_INTERRUPT, is_caller_secure(flags), FFA_PARAM_MBZ,
				     FFA_PARAM_MBZ, FFA_PARAM_MBZ, FFA_PARAM_MBZ,
				     handle);
}
