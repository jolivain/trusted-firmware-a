				       ...
				       uint64_t flags)
{
	 ...
	 */
	if (secure_origin) {
		VERBOSE("Direct request not supported to the NWd.\n");
		return spmc_ffa_error_return(handle,
					     FFA_ERROR_INVALID_PARAMETER);
	}
					...
					uint64_t flags)
{
	...
	if (!secure_origin) {
		VERBOSE("Direct Response not supported from NWd.\n");
		return spmc_ffa_error_return(handle,
					     FFA_ERROR_INVALID_PARAMETER);
	}
				 ...
				 uint64_t flags)
{
	...
	if (sp->ec[idx].rt_model == RT_MODEL_INTR) {
		/* We can only be called from the secure world. */
				    ...
				    uint64_t flags)
{
	...
	if (secure_origin) {
		 ...
		 *       case.
		 */
		uint32_t sp_version = spmc_get_current_sp_ctx()->ffa_version;

		assert(requested_version == sp_version);
	} else {
		/*
				   ...
				   uint64_t flags)
{
	...
	/* Unmap RX Buffer */
	ret = mmap_remove_dynamic_region((uintptr_t) mbox->rx_buffer, buf_size);
	assert(ret == 0);
	mbox->rx_buffer = 0;

	/* Unmap TX Buffer */
	ret = mmap_remove_dynamic_region((uintptr_t) mbox->tx_buffer, buf_size);
	assert(ret == 0);
	mbox->tx_buffer = 0;
					   ...
					   int *partition_count)
{
	...
	for (index = 0; index < EL3_LP_DESCS_COUNT; index++) {
		if (is_null_uuid ||
		    compare_uuid(uuid, el3_lp_descs[index].uuid)) {
	...
	for (index = 0; index < SECURE_PARTITION_COUNT; index++) {
		if (is_null_uuid || compare_uuid(uuid, sp_desc[index].uuid)) {
			/* Found a matching UUID, populate appropriately. */
			if (*partition_count >= max_partitions) {
				return FFA_ERROR_NO_MEMORY;
			}
