...
/*
 * This array will be used to identify which SP descriptor to initialize
 * and manage during SP runtime. Default value is '0'.
 */
static uint32_t active_sp_desc_idx_arr[PLATFORM_CORE_COUNT];
...
sp_desc_t* spmc_get_current_sp_ctx(void) {
	return &(sp_desc[spmc_get_active_sp_desc_index()]);
}
...
/*
 * Helper function to set the index number of the active SP descriptor
 */
void spmc_set_active_sp_desc_index(uint32_t idx)
{
	assert(idx < num_secure_partitions);
	active_sp_desc_idx_arr[plat_my_core_pos()] = idx;
}
...
{
	return active_sp_desc_idx_arr[plat_my_core_pos()];
}
...
static int32_t sp_init(void)
{
	...
	for (int i = 0; i < num_secure_partitions; i++) {
		...
		/* Current SP index */
		spmc_set_active_sp_desc_index(i);
