			  ...
			  int32_t boot_info_reg)
{
	...
	if (boot_info_reg >= 0) {
		/*
		 * Create a boot information descriptor, copy the partition
		 * manifest and populate any fields in it inplace.
		 */
		spmc_create_boot_info(ep_info);

		/*
		 * We have consumed what we need from ep args to create the so we can now
		...
		/*
		 * If we're not populating boot information we don't need any
		 * of the information that was populated in ep_args so we can
		 * clear them.
		 */
		zeromem(&ep_info->args, sizeof(ep_info->args));
	}
