			       ...
			       void *smc_handle)
{
	...
	/* We don't currently support any optional flags so ensure none are requested. */
 ...
 *
 * Implements a subset of the FF-A FFA_MEM_SHARE and FFA_MEM_LEND calls needed t
 * share or lend memory from non-secure os to secure os (with no stream endpoints).
 *
