...
{
	uint64_t region1_start, region1_size, region1_end;
	uint64_t region2_start, region2_size, region2_end;
 ...
 *			  and that the descriptor does not overlap with
 *			  previously inflight memory transactions.
 * @obj:    Object containing ffa_memory_region_descriptor.
 *
	...
	for (int emad_num = 0; emad_num < obj->desc.emad_count; emad_num++) {
		...
		if (comp_mrd_offset == 0) {
			...
			if (comp_mrd_offset != offset) {
				NOTICE("%s: mismatching offsets provided, %u != %u\n",
...
static int spmc_shmem_check_state_obj(struct spmc_shmem_obj *obj)
{
	...
	while (inflight_obj != NULL) {
		/* Don't compare the transaction to itself. */
			       ...
			       void *smc_handle)
{
	...
	if (obj->desc_filled != obj->desc_size) {
	 ...
	 /*
	  * The full descriptor has been transmitted, perform any final
	  * actions.
	  */
...

{
	...
	/* Perform validation of descriptor. */
	if (!mbox->rxtx_page_count) {
	...
	for (int i = 0; i < req->emad_count; i++) {
		if (ffa_is_secure_world_id(req->emad[i].mapd.endpoint_id)) {
			ffa_endpoint_id16_t id = req->emad[i].mapd.endpoint_id;

			if (spmc_get_sp_ctx(id) == NULL) {
	...
	for (int i = 0; i < req->emad_count; i++) {
		for (int j = i + 1; j < req->emad_count; j++) {
			if (req->emad[i].mapd.endpoint_id ==
				req->emad[j].mapd.endpoint_id) {
				ret = FFA_ERROR_INVALID_PARAMETER;
				goto err_unlock_all;
			}
		}
			  ...
			  uint64_t flags)
{
	...
	if (req->flags && ((req->flags & FFA_MTD_FLAG_TYPE_MASK) !=
		...
		/*
		 * If the retrieve request specficies the memory transaction
