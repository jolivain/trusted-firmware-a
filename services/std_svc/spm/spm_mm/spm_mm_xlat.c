...
REGISTER_XLAT_CONTEXT2(sp, PLAT_SP_IMAGE_MMAP_REGIONS,
		       PLAT_SP_IMAGE_MAX_XLAT_TABLES, PLAT_VIRT_ADDR_SPACE_SIZE,
		       PLAT_PHY_ADDR_SPACE_SIZE, EL1_EL0_REGIME,
...
static unsigned int smc_attr_to_mmap_attr(unsigned int attributes)
{
	...
	unsigned int access =
		(attributes & MM_SP_MEMORY_ATTRIBUTES_ACCESS_MASK) >>
					      ...
					      uintptr_t base_va)
{
	...
	int rc = xlat_get_mem_attributes_ctx(sp_ctx->xlat_ctx_handle, base_va,
					     &attributes);
					  ...
					  u_register_t smc_attributes)
{
	...
	int ret = xlat_change_mem_attributes_ctx(
		sp_ctx->xlat_ctx_handle, base_va, size,
