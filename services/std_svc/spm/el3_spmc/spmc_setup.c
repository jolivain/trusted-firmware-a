...
#if defined(SPMC_AT_EL3_SEL0_SP)
/*
...
/*
 * We are assuming that the index of the execution context used is the linear
 * index of the current physical cpu.
 * SEL0 SPs run to completion and will only use one context (0) at a time.
 */
...
/* Setup spsr in entry point info for common context mgmt code to use. */
		...
		int node, uint8_t type)
{
	...
	INFO("Mapping SP's %s\n",
	     type == REGION_DEVICE ? "device regions" : "memory regions");

	compatibilty = type == REGION_DEVICE ?
	"arm,ffa-manifest-device-regions" : "arm,ffa-manifest-memory-regions";
	if (fdt_node_check_compatible(sp_manifest, node, compatibilty) != 0) {
	...
	for (offset = fdt_first_subnode(sp_manifest, node), mem_region = 0;
		...
		if ((mem_attr & SP_MEM_SECURE) != SP_MEM_SECURE) {
...
/* Setup context of a EL0 Secure Partition */
		       ...
		       void *sp_manifest)
{
	...
	if (node < 0)
		...
		populate_sp_regions(sp, sp_manifest, node, REGION_DEVICE);
	}
	...
	if (node < 0)
		...
		populate_sp_regions(sp, sp_manifest, node, REGION_MEMORY);
	}
