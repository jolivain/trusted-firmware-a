				  ...
				  struct secure_partition_desc *sp)
{
	 ...
	 */
	assert(sp->ffa_version != MAKE_FFA_VERSION(1, 0));
	...
	/* Set the magic number "0xFFA". */
	boot_header->magic = FFA_INIT_DESC_MAGIC;
	...
	boot_descriptor->type = FFA_BOOT_INFO_TYPE_STD |
				FFA_BOOT_INFO_TYPE_FDT;

	boot_descriptor->flags = FFA_BOOT_INFO_FLAG_NAME_UUID |
				 FFA_BOOT_INFO_FLAG_CONTENT_ADR;

	/* Copy the UUID of the manifest into the boot information. */
	memcpy((void *) boot_descriptor->name,
	       (void *) &manifest_uuid,	sizeof(uuid_t));

	/*
	 * Copy the manifest into boot info region after the boot information
	 * descriptor.
	 */
	boot_descriptor->size_boot_info = ep_info->args.arg1;
