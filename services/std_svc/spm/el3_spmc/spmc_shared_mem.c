			...
			uint32_t ffa_version, size_t *emad_size)
{
	...
	if (index >= desc->emad_count) {
		ERROR("Out-of-bounds access: emad index is %u but only %u emads present.\n",
		      index, desc->emad_count);
		panic();
	}
...
{
	unsigned long long emad_end, emad_size, emad_offset;
	...
	CASSERT(offsetof(struct ffa_mtd, emad_size) ==
		offsetof(struct ffa_mtd_v1_0, reserved_24_27),
		assert_ffa_mtd_v1_0_reserved_offset_not_equal_ffa_mtd_emad_size_offset);
	...
	if (ffa_version == MAKE_FFA_VERSION(1, 0)) {
		...
		if (desc->emad_size != 0) {
			WARN("%s: reserved MBZ field 0x%08" PRIx32 " is not zero.\n",
		...
		emad_size = desc->emad_size;
		if ((desc->reserved_36_39 != 0) || (desc->reserved_40_47 != 0)) {
			WARN("%s: reserved MBZ fields 0x%08" PRIx32 " and 0x%016" PRIx64 " are not zero.",
			     __func__, desc->reserved_36_39, desc->reserved_40_47);
			return FFA_ERROR_INVALID_PARAMETER;
		}
	}

	/*
	 * Overflow is impossible: the aritmentic happens in at least 64-bit
	 * precision, but all of the operans are bounded by UINT32_MAX, and
	 * ((2^32 - 1)^2 + (2^32 - 1) + (2^32 - 1)) = ((2^32 - 1) * (2^32 + 1))
	 * = (2^64 - 1).
	 */
	CASSERT(sizeof(desc->emad_count == 4), assert_emad_count_max_too_large);
	emad_end = (unsigned long long)desc->emad_count * emad_size +
				...
				uint32_t ffa_version)
{
	...
	if (spmc_validate_mtd_start(&obj->desc, ffa_version,
				    obj->desc_filled, obj->desc_size)) {
		ERROR("BUG: spmc_shm_check_obj called on object with corrupt memory region descriptor\n");
		panic();
	}
...

{
	...
	if (ffa_version == MAKE_FFA_VERSION(1, 0)) {
		...
		WARN("%s: bad FFA version.\n", __func__);
		return spmc_ffa_error_return(handle,
					     FFA_ERROR_INVALID_PARAMETER);
	}

	/* Check if the descriptor is too small for the FFA version. */
	if (fragment_length < min_desc_size) {
