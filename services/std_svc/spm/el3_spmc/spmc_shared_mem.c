 ...
 *                descriptor.
 * Return: A pointer to the emad structure corresponding to the supplied FF-A
 *         version.
 */
static void *
...
spmc_shmem_obj_get_comp_mrd(struct spmc_shmem_obj *obj, uint32_t ffa_version)
{
	size_t emad_size;
	/*
	 * The comp_mrd_offset field of the emad descriptor remains consisten
	...
	for (size_t i = 0; i < region1->address_range_count; i++) {
		...
		for (size_t j = 0; j < region2->address_range_count; j++) {
			...
			if ((region1_start >= region2_start &&
				...
				NOTICE("Overlapping mem regions 0x%lx-0x%lx & 0x%lx-0x%lx\n",
...
spmc_shm_get_v1_1_descriptor_size(struct ffa_mtd_v1_0 *orig, size_t desc_size)
{
	...
	/* Add the size of the v1.1 EMAD descriptors. */
	size += orig->emad_count * sizeof(struct ffa_emad);
	...
	if ((uint8_t *) mrd >= ((uint8_t *) orig + desc_size)) {
		return 0;
	}
...
spmc_shm_get_v1_0_descriptor_size(struct ffa_mtd *orig, size_t desc_size)
{
	size_t size = 0;
	struct ffa_comp_mrd *mrd;
	struct ffa_emad *emad_array = (struct ffa_emad *)
				      ((uint8_t *) orig + orig->emad_offset);
				     ...
				     struct spmc_shmem_obj *orig)
{
	...
	struct ffa_emad *emad_array_out;
	...
	out->emad_size = sizeof(struct ffa_emad);
	...
	emad_array_out = (struct ffa_emad *)
			 ((uint8_t *) out + out->emad_offset);

	/*
	 * The v1.1 emad struct has additional reserved space at the end of the
	 * descriptor, copy the remaining fields directly.
	 * We can use the output array directly here as we want to convert an
	 * older data structure to the latest (biggest) format.
	 */
	...
	if (((uint8_t *) mrd_in + mrd_size) >
	     ((uint8_t *) mtd_orig + orig->desc_size) ||
	    (((uint8_t *) mrd_out + mrd_size) >
	     ((uint8_t *) out + out_obj->desc_size))) {
		ERROR("%s: Invalid mrd structure.\n", __func__);
		return false;
	}

	/* The mrd descriptors have not changed sizes so copy them directly. */
			     ...
			     struct spmc_shmem_obj *orig)
{
	...
	struct ffa_emad *emad_in;
	struct ffa_emad *emad_array_in;
	...
	emad_array_in = (struct ffa_emad *)
			((uint8_t *) mtd_orig + mtd_orig->emad_offset);
	emad_array_out = out->emad;

	/*
	 * Copy the v1.0 fields directly as these are the same in both
	 * descriptor versions.
	 */
				...
				uint32_t ffa_version)
{
	...
	for (size_t emad_num = 0; emad_num < obj->desc.emad_count; emad_num++) {
		...
		if (size < sizeof(struct ffa_comp_mrd)) {
			NOTICE("%s: invalid object, offset %u, total size %zu, no header space.\n",
			       __func__, offset, obj->desc_size);
			return -EINVAL;
		}
		size -= sizeof(struct ffa_comp_mrd);
			       ...
			       void *smc_handle)
{
	...
	if (ffa_is_secure_world_id(obj->desc.sender_id)) {
		NOTICE("%s: Invalid sender ID 0x%x.\n",
	...
	if (obj->desc.memory_region_attributes & FFA_MEM_ATTR_NS_BIT) {
		NOTICE("%s: NS mem attributes flags MBZ\n", __func__);
		ret = FFA_ERROR_INVALID_PARAMETER;
		goto err_arg;
	}
	...
	if (obj->desc.flags && mtd_flag && (obj->desc.flags != mtd_flag)) {
		NOTICE("%s: invalid memory transaction flags %u != %u\n",
	...
	for (size_t i = 0; i < obj->desc.emad_count; i++) {
		...
		for (size_t j = i + 1; j < obj->desc.emad_count; j++) {
			...
			if (emad->mapd.endpoint_id ==
				...
				NOTICE("%s: Duplicated endpoint id 0x%x\n",
...
/**
 * spmc_ffa_set_ns_bit - Set the NS bit in the response descriptor if the caller
 *                       is not using FF-A v1.0 or if they have requested the
 *                       functionality.
 *                       TODO: We are assuming that the caller is an SP. To
 *                       support retrieval from the normal world this function
 *                       will need to be expanded accordingly.
 * @resp:       Descriptor populated in callers RX buffer.
 * @sp_ctx:     Context of the calling SP.
 */
void spmc_ffa_set_ns_bit(struct ffa_mtd *resp,
			  ...
			  uint64_t flags)
{
	...
	/*
	 * We are only accessing fields that are the same in both the v1.0 and
	 * v1.1 mtd struct therefore we can use a v1.1 struct directly here.
	 * We only need validate against the appropriate struct size.
	 */
	struct ffa_mtd *resp;
	...
	/* Compare against the v1.0 struct as this is the minimum size. */
	if (total_length < sizeof(struct ffa_mtd_v1_0)) {
	...
	if (req->memory_region_attributes & FFA_MEM_ATTR_NS_BIT) {
		NOTICE("%s: NS mem attributes flags MBZ\n", __func__);
		ret = FFA_ERROR_INVALID_PARAMETER;
		goto err_unlock_all;
	}
	...
	if (req->flags && ((req->flags & FFA_MTD_FLAG_TYPE_MASK) !=
		...
		/*
		 * If the retrieve request specifies the memory transaction
	...
	for (size_t i = 0; i < req->emad_count; i++) {
		...
		for (size_t j = 0; j < req->emad_count; j++) {
	...
	spmc_ffa_set_ns_bit(resp, sp_ctx);
			  ...
			  uint64_t flags)
{
	...
	if (!secure_origin) {
		NOTICE("%s: can only be called from swld.\n",
		       __func__);
		return spmc_ffa_error_return(handle,
					     FFA_ERROR_INVALID_PARAMETER);
	}
			 ...
			 uint64_t flags)
{
	...
	if (secure_origin) {
		NOTICE("%s: unsupported reclaim direction.\n", __func__);
		return spmc_ffa_error_return(handle,
					     FFA_ERROR_INVALID_PARAMETER);
	}
