...
static size_t arg_to_char_helper(const uint64_t src, size_t chars_in_register,
				  size_t to_write)
{
	size_t size = MIN(chars_in_register, to_write);
	size_t written = 0;
...
static uint64_t spmc_ffa_console_log(uint32_t smc_fid,
				     ...
				     uint64_t flags)
{
	size_t chars_in_register = smc_fid == FFA_CONSOLE_LOG_SMC32
				? sizeof(uint32_t)
				: sizeof(uint64_t);
	...
	if ((x1 == 0) || (x1 > chars_in_register * 6)) {
		return spmc_ffa_error_return(handle, FFA_ERROR_INVALID_PARAMETER);
	}

	total_to_write -= arg_to_char_helper(x2, chars_in_register, total_to_write);
	total_to_write -= arg_to_char_helper(x3, chars_in_register, total_to_write);
	total_to_write -= arg_to_char_helper(x4, chars_in_register, total_to_write);
	total_to_write -= arg_to_char_helper(SMC_GET_GP(handle, CTX_GPREG_X5),
			  chars_in_register, total_to_write);
