				...
				unsigned long long psci_event)
{
	 ...
	 * however these are not available for use in the existing TF-A
	 * implementation and therefore will not be used.
	 */
	...
	if ((resp & FFA_DIRECT_FRAMEWORK_MSG_MASK) == 0U ||
...
static void spmc_cpu_suspend_handler(u_register_t unused)
{
	...
	rc = spmc_send_pm_msg(FFA_PM_MSG_PSCI_REQ, PSCI_CPU_SUSPEND_AARCH64);
	if (rc < 0) {
		ERROR("%s failed (%d) on CPU%u\n", __func__, rc, linear_id);
		return;
	}
...
static int32_t spmc_cpu_off_handler(u_register_t unused)
{
	...
	ret = spmc_send_pm_msg(FFA_PM_MSG_PSCI_REQ, PSCI_CPU_OFF);
	if (ret < 0) {
