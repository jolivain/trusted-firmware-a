...
#define iterate_mapping(_mapping, _i, _map)                                   \
	for ((_map) = (_mapping)->map, (_i) = 0; (_i) < (_mapping)->num_maps; \
	     (_i)++, (_map)++)
...
#define FEATURE_BIND_SLOTS(_priv, _shrd)          \
	(((((uint64_t)(_priv)) & BIND_SLOTS_MASK) \
	  << FEATURES_PRIVATE_SLOTS_SHIFT) |      \
	 ((((uint64_t)(_shrd)) & BIND_SLOTS_MASK) \
	  << FEATURES_SHARED_SLOTS_SHIFT))
...
typedef enum { SDEI_NORMAL, SDEI_CRITICAL } sdei_class_t;
