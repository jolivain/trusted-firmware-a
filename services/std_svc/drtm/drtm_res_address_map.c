...
/* Amount of space needed for address map based on PLAT_ARM_MMAP_ENTRIES */
...
static char drtm_address_map[DRTM_ADDRESS_MAP_SIZE];
...
drtm_memory_region_descriptor_table_t *drtm_build_address_map (void)
{
	...
	const mmap_region_t *mmap = plat_arm_get_mmap();

	int i;

	/* Count valid entries in plat_arm_mmap */
	for (i = 0; plat_arm_mmap[i].base_pa != 0; i++);
