...
static inline void invalidate_icache_all()
{
					 ...
					 struct_drtm_dl_args *a_out)
{
	...
	/* Invalidate data passed in args by the DCE Preamble. */
	...
	if (!(a->dlme_img_start < a->dlme_size &&
	...
	dlme_img_start = a->dlme_paddr + a->dlme_img_start;
	...
	/* Map dlme image and flush the cache */
	...
	rc = mmap_add_dynamic_region_alloc_va(dlme_img_start, &va_mapping, va_mapping_size,
					      ...
					      MT_SHAREABILITY_ISH);
	if (rc != 0) {
		WARN("DRTM: %s: mmap_add_dynamic_region_alloc_va() failed rc=%d\n",
		      __func__, rc);
		return INTERNAL_ERROR;
	}
	flush_dcache_range(va_mapping, va_mapping_size);
	...
	if (rc != 0) {
		ERROR("DRTM: DLME data region lies within secure memory\n");
		return INVALID_PARAMETERS;
	}
	...
	if (dlme_data_end - dlme_data_start < dlme_data_min_size) {
		ERROR("DRTM: argument DLME data region is short of %lz bytes\n",
