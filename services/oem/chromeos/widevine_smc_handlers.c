...
struct cros_oem_data cros_oem_widevine_dice = {
	.buffer = cros_oem_widevine_dice_buffer,
	.max_length = sizeof(cros_oem_widevine_dice_buffer),
};

struct cros_oem_data cros_oem_widevine_ta_key = {
	.buffer = cros_oem_widevine_ta_key_len_buffer,
	.max_length = sizeof(cros_oem_widevine_ta_key_len_buffer),
};

static uintptr_t cros_write_data(struct cros_oem_data *data, u_register_t length,
				 u_register_t address, void *handle)
{
	 ...
	 */
	rc = mmap_add_dynamic_region(aligned_address, aligned_address, aligned_size,
...
DECLARE_RT_SVC(cros_oem_svc_smc_handler, OEN_OEM_START, OEN_OEM_END, SMC_TYPE_FAST,
	       NULL, cros_oem_svc_smc_handler);
