...
#define MAX_REGS_FOR_READ 7
#define MAX_REGS_FOR_WRITE 6
				...
				void *handle)
{
	...
	u_register_t results[MAX_REGS_FOR_READ] = {};
	...
	if (length > MAX_REGS_FOR_READ * sizeof(u_register_t)) {
		length = MAX_REGS_FOR_READ * sizeof(u_register_t);
		ret = length;
	} else {
...
static uintptr_t cros_write_data(cros_oem_data *data, u_register_t length,
				 ...
				 u_register_t x6, u_register_t x7, void *handle)
{
	u_register_t value[MAX_REGS_FOR_WRITE] = { x2, x3, x4, x5, x6, x7 };
	...
	if (length < 0 || length > MAX_REGS_FOR_WRITE * sizeof(u_register_t) ||
			       ...
			       u_register_t flags)
{
	...
	case CROS_OEM_SMC_SET_TPM_AUTH_PUB_FUNC_ID:
		return cros_write_data(&cros_oem_tpm_auth_pk, x1, x2, x3, x4,
				       x5, x6, x7, handle);
	...
	case CROS_OEM_SMC_SET_DEVICE_ID_FUNC_ID:
		return cros_write_data(&cros_oem_huk, x1, x2, x3, x4, x5, x6,
				       x7, handle);
	...
	case CROS_OEM_SMC_SET_WIDEVINE_ROT_FUNC_ID:
		return cros_write_data(&cros_oem_widevine_rot, x1, x2, x3, x4,
				       x5, x6, x7, handle);
	default:
				   ...
				   u_register_t flags)
{
	...
	if (is_oem_fid(smc_fid)) {
		return cros_oem_smc_handler(smc_fid, x1, x2, x3, x4, cookie,
					    handle, flags);
	}
	...
	case CROS_OEM_SVC_VERSION:
		...
		SMC_RET2(handle, CROS_OEM_VERSION_MAJOR,
			 CROS_OEM_VERSION_MINOR);
