...
#define clr_tsp_pstate(state) (state &= ~(TSP_PSTATE_MASK << TSP_PSTATE_SHIFT))
#define set_tsp_pstate(st, pst)                                    \
	do {                                                       \
		clr_tsp_pstate(st);                                \
		st |= (pst & TSP_PSTATE_MASK) << TSP_PSTATE_SHIFT; \
	} while (0);
...
#define get_yield_smc_active_flag(state) \
	((state >> YIELD_SMC_ACTIVE_FLAG_SHIFT) & YIELD_SMC_ACTIVE_FLAG_MASK)
#define set_yield_smc_active_flag(state) \
	(state |= 1 << YIELD_SMC_ACTIVE_FLAG_SHIFT)
#define clr_yield_smc_active_flag(state) \
	(state &= ~(YIELD_SMC_ACTIVE_FLAG_MASK << YIELD_SMC_ACTIVE_FLAG_SHIFT))
...
#define store_tsp_args(_tsp_ctx, _x1, _x2)         \
	do {                                       \
		_tsp_ctx->saved_tsp_args[0] = _x1; \
		_tsp_ctx->saved_tsp_args[1] = _x2; \
	} while (0)

#define get_tsp_args(_tsp_ctx, _x1, _x2)           \
	do {                                       \
