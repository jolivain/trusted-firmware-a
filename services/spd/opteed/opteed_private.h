...
#define get_optee_pstate(state) \
	((state >> OPTEE_PSTATE_SHIFT) & OPTEE_PSTATE_MASK)
#define clr_optee_pstate(state) \
	(state &= ~(OPTEE_PSTATE_MASK << OPTEE_PSTATE_SHIFT))
#define set_optee_pstate(st, pst)                                      \
	do {                                                           \
		clr_optee_pstate(st);                                  \
		st |= (pst & OPTEE_PSTATE_MASK) << OPTEE_PSTATE_SHIFT; \
	} while (0)
...
void __dead2 opteed_synchronous_sp_exit(optee_context_t *optee_ctx,
					uint64_t ret);
