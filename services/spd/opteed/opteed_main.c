...
static int32_t opteed_setup(void)
{
	...
	/*
	 * Register an interrupt handler for S-EL1 interrupts when generated during
	 * code executing in the non-secure state. We must do this now so that the
	 * interrupt priorities are not changed after starting the linux kernel.
	 */
	flags = 0;
