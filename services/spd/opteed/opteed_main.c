...
static int32_t opteed_handle_smc_load_elf(uint64_t data_size, uint32_t data_pa)
{
	...
	for (i = 0; i < image_info->num_segments; ++i) {
	...
	INFO("Loaded OP-TEE via SMC of size %ld at addr 0x%lx\n", target_size,
	     target_pa);

	for (i = 0; i < image_info->num_segments; ++i) {
		segment_pa = image_info->segments[i].segment_pa;
		segment_data_size = image_info->segments[i].segment_data_size;
		segment_mem_size = image_info->segments[i].segment_mem_size;
