...
static int add_options_widevine_node(void *fdt)
{
	...
	ret = fdt_property_string(fdt, "compatible", "google,widevine");
	if (ret)
		return ret;

	if (cros_oem_tpm_auth_pk.length) {
		ret = fdt_property(fdt, "tpm-auth-public-key",
	...
	if (cros_oem_huk.length) {
		ret = fdt_property(fdt, "hardware-unique-key",
	...
	if (cros_oem_rot.length) {
		ret = fdt_property(fdt, "root-of-trust", cros_oem_rot.buffer,
				   cros_oem_rot.length);
		if (ret)
			return ret;
	}
