 ...
 */
static int create_opteed_dt(void* fdt_buf, size_t size)
{
	int ret;
	uint8_t addresses[sizeof(uint64_t) + sizeof(uint32_t)];
#if COREBOOT
	uint64_t coreboot_table_addr;
	uint32_t coreboot_table_size;
#endif /* COREBOOT */

	ret = fdt_create(fdt_buf, size);
	if (ret)
		return ret;
...
#if COREBOOT
	get_coreboot_table_location(&coreboot_table_addr, &coreboot_table_size);
	...
	coreboot_table_addr = cpu_to_fdt64(coreboot_table_addr);
	coreboot_table_size = cpu_to_fdt32(coreboot_table_size);
	memcpy(addresses, &coreboot_table_addr, sizeof(uint64_t));
	memcpy(addresses + sizeof(uint64_t), &coreboot_table_size,
	       sizeof(uint32_t));
	ret = fdt_property(fdt_buf, "reg", addresses,
...
static int32_t opteed_handle_smc_load(uint64_t data_size, uint32_t data_pa)
{
	...
	uint32_t linear_id = plat_my_core_pos();
	uint64_t dt_addr = 0;
	uint8_t fdt_buf[OPTEED_FDT_SIZE];
	...
	rc = create_opteed_dt(fdt_buf, OPTEED_FDT_SIZE);
	if (rc) {
		WARN("Failed with device tree creation %d\n", rc);
		return rc;
	}
