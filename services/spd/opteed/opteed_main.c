...
static int add_options_widevine_node(void *fdt)
{
	...
	if (cros_oem_tpm_auth_pk.length) {
		ret = fdt_property(fdt, "google,tcg-tpm-auth-public-key",
	...
	if (cros_oem_huk.length) {
		ret = fdt_property(fdt, "google,optee-hardware-unique-key",
	...
	if (cros_oem_rot.length) {
		ret = fdt_property(fdt, "google,widevine-root-of-trust",
