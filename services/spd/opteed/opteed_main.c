...
static int32_t opteed_setup(void)
{
	...
	if (TRANSFER_LIST &&
		...
		te = transfer_list_find(tl, TL_TAG_OPTEED_AARCH);
		if (!te) {
			return 1;
		}

		opteed_rw = *(uint32_t *)transfer_list_entry_data(te);
		if (opteed_rw != OPTEE_AARCH64 && opteed_rw != OPTEE_AARCH32) {
			return 1;
		}

		te = transfer_list_find(tl, TL_TAG_FDT);
		if (opteed_rw == OPTEE_AARCH64) {
			arg0 = te? (uint64_t)transfer_list_entry_data(te): 0;
			arg2 = 0;
		} else {
			arg2 = te? (uint64_t)transfer_list_entry_data(te): 0;
			arg0 = 0;
		}
...
static int32_t opteed_handle_smc_load(uint64_t data_size, uint32_t data_pa)
{
	...
	if (TRANSFER_LIST && !create_smc_tl(dt_addr, OPTEED_FDT_SIZE)) {
		te = transfer_list_find(bl31_tl, TL_TAG_FDT);
		if (opteed_rw == OPTEE_AARCH64) {
			arg0 = te? transfer_list_entry_data(te): 0;
			arg2 = 0;
		} else {
			arg2 = te? transfer_list_entry_data(te): 0;
			arg0 = 0;
		}
