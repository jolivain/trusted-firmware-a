...
static void read_feat_mte(void)
{
	...
	feat_panic((mte != MTE_UNIMPLEMENTED),"MTE");
#endif
...
static void read_feat_bti(void)
{
	 ...
	 * not supported.
	 */
    feat_panic(is_armv8_5_bti_present(),"BTI");
#endif
...
static void read_feat_rme(void)
{
	 ...
	 * not supported.
	 */
    feat_panic((get_armv9_2_feat_rme_support()!= 
 ...
 * either in BL1 or BL31 depending on the platform and assists in identifying and
 * notifying the features which are enabled but not supported by the PE.
 *
 * It reads all the enabled features ID-registers and ensures the feature
 ...
 *
 * These three states are wrapped in helper macros for each individual feature
