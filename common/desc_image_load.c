...
/*******************************************************************************
 * This function returns the index of the first executable in
 * bl_mem_params_node_t array or -1 if none found.
 ******************************************************************************/
int get_bl_first_exe_img_index(const bl_mem_params_node_t *mem_params, size_t len)
{
	/* Get the list HEAD */
	for (size_t count = 0U; count < len; count++) {
		if ((EP_GET_EXE(mem_params[count].ep_info.h.attr) ==
		     EXECUTABLE) &&
		    (EP_GET_FIRST_EXE(mem_params[count].ep_info.h.attr) ==
		     EP_FIRST_EXE)) {
			return count;
		}
	}
