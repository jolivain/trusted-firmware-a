...
static int fdt_add_cpu(void *dtb, int parent, u_register_t mpidr)
{
	...
	reg_prop = mpidr & MPID_MASK;

	snprintf(snode_name, sizeof(snode_name), "cpu@%x", (uint32_t)reg_prop);
 ...
 * @dtb:		pointer to the device tree blob in memory
 * @afflv0:		Maximum value for the affinity level 0.
 * @afflv1:		Maximum value for the affinity level 1.
 * @afflv2:		Maximum value for the affinity level 2.
 *
 ...
 *
 * A system with two CPUs would generate a node equivalent to:
 *
 *	cpus {
 *		#address-cells = <2>;
 *		#size-cells = <0>;
