				      ...
				      uint32_t nl_phandle, char *compatible)
{
	...
	if (node_offs < 0) {
		ERROR ("FDT: add subnode %s to node with offset %d failed\n",
			node_name, parent);
		return node_offs;
	}
	...
	if (err < 0) {
		ERROR ("FDT: write to \"%s\" property of node at offset %d failed\n",
			   ...
			   uint32_t cache_ph)
{
	...
	tmp_offs = fdt_add_cluster_cache(dtb, cpus_offs, id,
					 cache_ph, "cache");
	if (tmp_offs < 0) {
		ERROR ("FDT: add new cache node to node at offset %d failed\n",
			cpus_offs);
		return tmp_offs;
	}
	cc_phandle = fdt_get_phandle(dtb, tmp_offs);
	if (cc_phandle == 0) {
		ERROR ("FDT: error getting phandle for node at offset %d\n",
			tmp_offs);
		return -1;
	}
	...
	if (tmp_offs < 0) {
		ERROR ("FDT: error getting offset for phandle %d\n",
			map_ph);
		return tmp_offs;
	}
	...
	if (tmp_offs < 0) {
		ERROR ("FDT: add subnode %s to node at offset %d failed\n",
			snode_name, tmp_offs);
		return tmp_offs;
	}
	...
	for (; i < (id + 1) * ncpus; i++) {
		...
		tmp_offs = fdt_add_cpu_node(dtb, cpus_offs, id, i, mpidr,
					    "arm,armv8", cc_phandle,
					     cpu_phandle);
		if (tmp_offs < 0) {
			ERROR ("FDT: add new cpu node to node at offset %d failed\n",
				cpus_offs);
			return tmp_offs;
		}
		...
		tmp_offs = fdt_add_subnode(dtb, tmp_offs, snode_name);
		if (tmp_offs < 0) {
			ERROR ("FDT: add new subnode %s to node at offset %d failed\n",
...
int fdt_add_topology_tree(void *dtb, const unsigned char *pd_tree)
{
	...
	if (offs < 0) {
		ERROR ("FDT: create subnode \"%s\" at offset %d failed\n",
			"cpu_map", offs);
		return offs;
	}
