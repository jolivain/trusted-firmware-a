 ...
 ******************************************************************************/
int fdt_get_stdout_node_offset(const void *fdt)
{
	int node;
	...
	prop = fdt_getprop(fdt, node, "stdout-path", NULL);
	if (prop == NULL) {
		return -FDT_ERR_NOTFOUND;
	}
	...
	if (prop[0] == '/') {
		return fdt_path_offset_namelen(fdt, prop, len);
	}

	/* Lookup the alias, as this contains the actual path. */
	path = fdt_get_alias_namelen(fdt, prop, len);
	if (path == NULL) {
		return -FDT_ERR_NOTFOUND;
	}
