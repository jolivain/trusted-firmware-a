		...
		uint64_t *translated_addr)
{
	...
	parent_address = fdt_read_prop_cells(value + child_addr_size,
				parent_addr_size);
	addr_range = fdt_read_prop_cells(value + child_addr_size + parent_addr_size,
				range_size);
...
static uint64_t fdtw_search_all_xlat_entries(const void *dtb,
				const uint32_t *ranges, int local_bus,
				uint64_t base_address, int length)
{
	bool found;
	uint64_t translated_addr;
	const uint32_t *next_entry = ranges;
 	int parent_bus_node, nxlat_entries;
	...
	for (int i = 0; i < nxlat_entries; i++) {
		found = fdtw_xlat_hit(next_entry, self_addr_cells,
				...
				uint64_t base_address)
{
	...
	global_address = fdtw_search_all_xlat_entries(dtb, ptr, local_bus_node,
					base_address, length);
