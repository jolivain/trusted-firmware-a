...
static bool fdtw_find_mapping(const uint32_t *value, int child_addr_size,
		...
		uint64_t *translated_addr)
{
	...
	/* Found mapping for the addr that needs to be translated */
...
static bool fdtw_search_all_xlat_entries(const uint32_t *value, int child_addr_size,
				int parent_addr_size, uint64_t base_address,
				...
				int num_entries)
{
	...
	for (int i = 0; i < num_entries; i++) {
		...
		found = fdtw_find_mapping(next_entry, child_addr_size,
				parent_addr_size, base_address, translated_addr);
		if (found) {
			return true;
		}
...
{
	int length, local_bus_node, parent_bus_node, nxlat_entries;
	bool found;
	...
	const uint32_t *ptr = fdt_getprop(dtb, local_bus_node, "ranges",
					&length);
	...
	found = fdtw_search_all_xlat_entries(ptr, self_addr_cells,
			parent_addr_cells, base_address, &global_address,
			ncells_xlat, nxlat_entries);
	if (!found) {
