 ...
 * The API fdtw_translate_address() performs address translation of a local address to
			...
			uint64_t base_address, uint64_t *translated_addr)
{
	...
	parent_address = fdt_read_prop_cells(value + child_addr_size, parent_addr_size);
...
uint64_t fdtw_translate_address(const void *dtb, int node, uint64_t base_address)
{
	...
	/* In the example given above, starting from the lead node:
	...
	/* Find the number of translations(mappings) specified in the current ranges
	 * property. Note that length represents number of bytes.
	 */
