...
class TransferList:
    ...
    def add_transfer_entry(self, tag_id: int, data: bytes) -> "TransferEntry":
        ...
        assert tag_id >= 0 and tag_id <= 0xFFFFFF and isinstance(data, bytes)
        assert self.total_size >= self.size + TransferEntry.hdr_size + len(data)
    ...
    def write_to_file(self, file: Path) -> None:
        ...
        with open(file, "wb") as f:
            ...
            for te in self.entries:
                ...
                f.write(te.data)
                # Ensure the next TE has the correct alignement
                f.write(
                    bytes(
                        (
                            align(
