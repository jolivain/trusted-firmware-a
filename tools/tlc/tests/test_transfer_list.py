...
def test_add_transfer_entry(tag_id, data):
    ...
    assert tl.size == TransferList.tl_hdr_size + te.size
...
def test_single_te_transfer_list(tag_id, data, tmpdir):
    ...
    with open(test_file, "rb") as f:
        assert f.read(tl.hdr_size) == tl.header_to_bytes()
        assert int.from_bytes(f.read(3), TransferList.endianness) == te.id
        assert int.from_bytes(f.read(1), TransferList.endianness) == te.hdr_size
        assert int.from_bytes(f.read(4), TransferList.endianness) == te.data_size
...
def test_multiple_te_transfer_list(tmpdir):
    ...
    with open(test_file, "rb") as f:
        ...
        for tag_id, data in test_entries:
            ...
            print(f.tell())
            assert int.from_bytes(f.read(3), TransferList.endianness) == tag_id
            assert (
                int.from_bytes(f.read(1), TransferList.endianness)
                == TransferEntry.hdr_size
            )
            # Make sure the data in the TE matches the data in the original case
            data_size = int.from_bytes(f.read(4), TransferList.endianness)
