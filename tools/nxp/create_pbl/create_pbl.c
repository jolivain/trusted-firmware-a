...
uint32_t base_addr_ch3[MAX_BOOT] = {
	BASE_ADDR_UNDEFINED, BASE_ADDR_IFC_NOR,
	BASE_ADDR_UNDEFINED, /*IFC NAND */
	BASE_ADDR_QSPI,	     BASE_ADDR_SD,
	BASE_ADDR_EMMC,	     BASE_ADDR_UNDEFINED, /*FLXSPI NOR */
	BASE_ADDR_UNDEFINED, /*FLXSPI NAND 2K */
	BASE_ADDR_UNDEFINED /*FLXSPI NAND 4K */
};

uint32_t base_addr_ch32[MAX_BOOT] = {
	BASE_ADDR_UNDEFINED, BASE_ADDR_UNDEFINED, /* IFC NOR */
	BASE_ADDR_UNDEFINED, /* IFC NAND */
	BASE_ADDR_UNDEFINED, /* QSPI */
	BASE_ADDR_SD,	     BASE_ADDR_EMMC,
	BASE_ADDR_FLX_NOR,   BASE_ADDR_UNDEFINED, /*FLXSPI NAND 2K */
	BASE_ADDR_UNDEFINED /*FLXSPI NAND 4K */
};
...
char *boot_src_string[] = {
	"UNKNOWN_BOOT",	   "IFC_NOR_BOOT",     "IFC_NAND_BOOT",
	"QSPI_BOOT",	   "SD_BOOT",	       "EMMC_BOOT",
	"FLXSPI_NOR_BOOT", "FLXSPI_NAND_BOOT", "FLXSPI_NAND4K_BOOT",
};

enum stop_command { STOP_COMMAND = 0, CRC_STOP_COMMAND };
...
#define BYTE_SWAP_32(word)                                          \
	((((word)&0xff000000) >> 24) | (((word)&0x00ff0000) >> 8) | \
	...
	while ((fread(&data, sizeof(data), NUM_MEM_BLOCK, fp_rcw_pbi_op)) ==
		...
		if (flag == CRC_STOP_COMMAND) {
			if (pblimg.chassis == CHASSIS_2) {
				pbi_crc = crc_table[((pbi_crc >> 24) ^ (data)) &
						    0xff] ^
					  (pbi_crc << 8);
			} else {
				pbi_crc = (pbi_crc >> 8) ^
					  crc32_lookup[((pbi_crc) ^ (data)) &
						       0xff];
			}
		}
	}
...
int add_blk_cpy_cmd(FILE *fp_rcw_pbi_op, uint16_t args)
{
	...
	if (file_size > 0) {
		...
		if (fwrite(&pblimg.addr, sizeof(pblimg.addr), NUM_MEM_BLOCK,
			   fp_rcw_pbi_op) != NUM_MEM_BLOCK) {
		...
		/* Add size */
		if (fwrite(&new_file_size, sizeof(new_file_size), NUM_MEM_BLOCK,
			   fp_rcw_pbi_op) != NUM_MEM_BLOCK) {
...
int add_cpy_cmd(FILE *fp_rcw_pbi_op)
{
	...
	do {
		...
		ret = fread(&pbi_data, MAX_PBI_DATA_LEN_BYTE, NUM_MEM_BLOCK,
			    fp_img);
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, ":b:f:r:i:e:d:c:o:h:s")) != -1) {
		switch (opt) {
		case 'd':
			pblimg.addr = strtoull(optarg, &ptr, 16);
			if (*ptr != 0) {
				fprintf(stderr,
					"CMD Error: invalid load or destination address %s\n",
					optarg);
				goto exit_main;
			}
	...
	if ((args & MAND_ARG_MASK) != MAND_ARG_MASK || pblimg.rcw_nm == NULL ||
	...
	case CHASSIS_3_2:
		...
		while (word != 0x808f0000 && word != 0x80ff0000) {
			 ...
			 */
			if (pbl_size == 35) {
				word = crypto_calculate_checksum(
					fp_rcw_pbi_op, NUM_RCW_WORD - 1);
				if (word == FAILURE) {
					goto exit_main;
				}
			}
