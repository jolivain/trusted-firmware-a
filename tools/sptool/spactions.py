...
More information in the doc comments bellow.
'''
import bisect
...
def sp_logged_action(action):
    def logged_action(sp_layout, sp, args :dict):
...
class sp_action:
    ...
    """
    def __init__(self, action, exec_order=DEFAULT_ACTION_ORDER, global_action=True):
        self.action = action
    ...
    def __lt__(self, other):
        """
        To allowed for ordered inserts in a list of actions.
        """
...
class SpSetupActions:
    ...
    def sp_action(in_action = None, global_action = False, log_calls=False, exec_order=DEFAULT_ACTION_ORDER):
        ...
        def append_action(action):
            ...
            action = sp_action(action, exec_order, global_action)
