...
static void output_write(const char *path, struct sp_pkg_info *sp, bool header)
{
	...
	/* Now, save partition manifest blob */
	...
	/* Finally, save partition image aligned to Page size */

	file_ptr = align_to((sizeof(struct sp_pkg_header) + sp->pm_size),
						 PAGE_SIZE);
	xfseek(f, file_ptr, SEEK_SET);
...
int main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "hni:o:")) != -1) {
		...
		case 'i':
			if (in_head == NULL) {
				in_head = xzalloc(sizeof(struct arg_list),
				...
				in_list->next = xzalloc(sizeof(struct arg_list),
					"Failed to allocate arg_list struct");
				in_list = in_list->next;
			}
			in_list->usr_input = optarg;
			match_counter++;
			break;
		case 'o':
			if (out_head == NULL) {
				out_head = xzalloc(sizeof(struct arg_list),
					"Failed to allocate arg_list struct");
				out_list = out_head;
			} else {
				out_list = out_head;
				...
				out_list->next = xzalloc(sizeof(struct arg_list),
					"Failed to allocate arg_list struct");
				out_list = out_list->next;
			}
			out_list->usr_input = optarg;
			match_counter--;
			break;
		case 'n':
			need_header = false;
