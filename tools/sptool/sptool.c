...
struct sp_pkg_info {
	...
	void *img_data, *rd_data;

	/* Size of the files. */
	uint64_t img_size, rd_size;

	/* Location of the binary files inside the package output file */
	uint64_t img_offset, rd_offset;
...
static void output_write(const char *path, struct sp_pkg_info *sp)
{
	...
	/* Now, save resource description blob aligned to 8 bytes */
	...
	sp_header_info.rd_offset = sp->rd_offset;
	sp_header_info.rd_size = sp->rd_size;
...
static void usage(void)
{
	...
	printf("This tool takes as input set of image binary files and the\n"
	       "resource description blobs as input and generates set of\n"
