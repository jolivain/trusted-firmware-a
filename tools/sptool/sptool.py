...
class SpPkg:
    def __init__(self, pm_path : str, img_path : str, boot_info: bool, pm_offset: int,
                 img_offset: int):
        ...
        self._SpPkgHeader = namedtuple("SpPkgHeader",
                             ("magic", "version", "boot_info",
                              "pm_offset", "pm_size",
                              "img_offset", "img_size"))

        if pm_offset >= img_offset:
            raise ValueError("pm_offset to be smaller than img_offset")
        ...
        if img_offset - pm_offset < os.path.getsize(pm_path):
    ...
    @property
    def boot_info(self):
        return 0x1 if self._boot_info else 0
    ...
    def generate(self, f_out : str):
        with open(f_out, "wb+") as output:
            ...
            output.seek(self.pm_offset, 0)
            with open(self.pm_path, "rb") as pm:
                copyfileobj(pm, output)
            output.seek(self.img_offset, 0)
...
def Main():
    ...
    parser.add_argument("--boot-info", required=False, default=False,
                        action="store_true", help="Set boot info blob offset.")
    ...
    pkg = SpPkg(manifest_path, image_path, args.boot_info, pm_offset, img_offset)
    pkg.generate(args.o)
