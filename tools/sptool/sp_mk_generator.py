...
def check_physical_load_address(sp_layout, sp, args: dict):
    key = "physical-load-address"
    if key not in sp_layout[sp].keys():
        raise Exception(f"Field `{key}` missing from sp_layout")
...
def gen_fconf_fragment(sp_layout, sp, args: dict):
    ...
    check_physical_load_address(sp_layout, sp, args)
