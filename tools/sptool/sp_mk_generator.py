...
def get_sp_manif_full_path(sp_node, args :dict):
...
def gen_fdt_sources(sp_layout, sp, args :dict):
    ...
    manifest_path = get_sp_manif_full_path(sp_layout[sp], args)
...
def gen_crt_args(sp_layout, sp, args :dict):
    ...
    write_to_sp_mk_gen(f"CRT += --sp-pkg {sp_pkg_idx} {get_sp_pkg(sp, args)}\n", args)
    return args
...
def gen_fiptool_args(sp_layout, sp, args :dict):
    ...
    if "uuid" in sp_layout[sp]:
        ...
        with open(get_sp_manif_full_path(sp_layout[sp], args), "r") as pm_f:
