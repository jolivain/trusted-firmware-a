...
static int gcm_encrypt(int enc_alg, char *key_string, const char *ip_name,
		       const char *op_name)
{
	FILE *ip_file;
	FILE *op_file;
...
int encrypt_file(int enc_alg, char *key_string, const char *ip_name,
		 const char *op_name)
{
	switch (enc_alg) {
	case KEY_ALG_GCM:
		gcm_encrypt(enc_alg, key_string, ip_name, op_name);
		break;
	default:
		return -1;
	}
