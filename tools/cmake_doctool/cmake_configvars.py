...
CMAKE_FILENAMES = ["CMakeLists.txt", "*.cmake"]
...
def parse_cmake_file(path, vprint):
    ...
    with open(path) as f:
        ...
        result = {
            contents_from_func_call(fc, vprint): path
            + ":"
            + str(lineno_from_func_call(fc))
            for fc in list(
                filter(
                    lambda x: filter_func_calls(x, "arm_config_option"), tree
                )
            )
        }
    return result
...
def not_documented(defined, documented, exclusions):
    ...
    not_doc = list(
        set(defined.keys()) - set(documented.keys()) - set(exclusions)
    )
    not_doc.sort()
...
def not_defined(defined, documented, exclusions):
    not_def = list(
        set(documented.keys()) - set(defined.keys()) - set(exclusions)
    )
    not_def.sort()
...
def check_configvars(verbose=False):
    ...
    cmake_search_paths = [TFA_ROOT + "**/" + filename for filename in CMAKE_FILENAMES]
    vprint("Search paths:\n - " + "\n - ".join(cmake_search_paths))

    cmake_files = []
    for search_path in cmake_search_paths:
        cmake_files.extend(glob.glob(search_path, recursive=True))
    ...
    sorted_paths = [
        x.replace(TFA_CMAKE_VARS, "") for x in list(doc_vars.values())
    ]
    sorted_paths.sort()
