...
static int key_create_rsa(local_key_t *key, int key_bits)
{
#if USING_OPENSSL3
...
#if USING_OPENSSL3
static int key_create_ecdsa(local_key_t *key, int key_bits, const char *curve)
{
...
static int key_create_ecdsa_nist(local_key_t *key, int key_bits)
{
	if (key_bits == 384) {
...
static int key_create_ecdsa_brainpool_r(local_key_t *key, int key_bits)
{
	return key_create_ecdsa(key, key_bits, "brainpoolP256r1");
}

static int key_create_ecdsa_brainpool_t(local_key_t *key, int key_bits)
{
	return key_create_ecdsa(key, key_bits, "brainpoolP256t1");
}
#else
static int key_create_ecdsa(local_key_t *key, int key_bits, const int curve_id)
{
	EC_KEY *ec;
...
typedef int (*key_create_fn_t)(local_key_t *key, int key_bits);
...
int key_create(local_key_t *key, int type, int key_bits)
{
...
unsigned int key_load(local_key_t *key)
{
	if (key->fn == NULL) {
...
local_key_t *key_get_by_opt(const char *opt)
{
	local_key_t *key;
	unsigned int i;
