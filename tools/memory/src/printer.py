...
class TfaPrettyPrinter:
    def __init__(self, term_size: int = 120, num_as_hex=True):
    ...
    def print_footprint(
        ...
        assert len(app_mem_usage), "Empty memory usage dictionary!"
        if not fields:
            fields = ["Component", "Start", "End", "Allocated", "Free"]
        ...
        for mem in sorted(set(k for _, v in app_mem_usage.items() for k in v)):
            ...
            for mod, vals in app_mem_usage.items():
                if mem in vals.keys():
                    val = vals[mem]
                    start, end, length = val["start"], val["end"], val["length"]
                    free = length - end

                    table.add_row(
                        [
                            mod.upper(),
                            *self.format_args(start, end, length, free),
                        ]
                    )
            print(table, "\n")
    ...
    def print_symbol_table(
        ...
        assert len(symbols), "The symbol list is empty!"
