...
class DTTree:
    ...
    def dtValidateFile(self, input, printInfo=False):
        ...
        dtbfilename = filename[:filename.rfind(".")] + ".dtb"
        cmd = ["dtc", "-f", "-O", "dtb", "-o", dtbfilename, filename]
        p = subprocess.run(cmd,  capture_output=True, text=True)
        output = p.stderr
        
        p = subprocess.run(["dt-validate", dtbfilename], capture_output=True, text=True)
        output += p.stderr

        if "FATAL ERROR" in output or "Traceback" in output:
            valid = False
    ...
    def dtParseFile(self, input, printInfo=False):
        ...
        try:
            ...
            if printInfo:
                print(bcolors.FAIL + "{} INVALID:\t{}\n".format(input, str(e)) + bcolors.ENDC)
            else:
                f = open(self.logging_file, "a")
                f.write("=====================================================================================\n")
                f.write("{} result:\n".format(input))
