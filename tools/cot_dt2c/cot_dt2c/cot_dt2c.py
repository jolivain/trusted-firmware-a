...
def manifest(filename, braces, ifdefTag):
    ...
    for line in filename:
        ...
        if match != None:
            word1, word2 = match.groups()
            certs.append(extractCert(filename, word2, ifdefTag))
...
def images(filename, braces, ifdefTag):
    ...
    for line in filename:
        ...
        if match != None:
            word = match.groups()[0]
            #print(word)
            allImages.append(extractImage(filename, word, ifdefTag))
...
def generateCot(images, certs, ctrs, pks, outputfileName, includeFile, licence):
...
def generate_main(input, output): 
    ...
    includerex = re.compile(r'#include <([\w\./]*)>')
    includefile = ""
    ifdefregex = re.compile(r'#if defined\(([\w]+)\)')
    ifdefend = "#endif"
    ...
    for line in filename:
        ...
        if "manifests" in line:
            braces.append("{")
            certs = manifest(filename, braces, ifdefTag)
            continue
    ...
    generateCot(allImages, certs, ctrs, pks, outputfileName, includefile, licence)
  
if __name__=="__main__": 
    if (len(sys.argv) < 3):
        print("usage: python3 " + sys.argv[0] + " [dtsi file path] [output c file path]")
        exit()
    generate_main(sys.argv[1], sys.argv[2])