...
class COT:
    ...
    def resolve_lay(self, parent_set, lay, name_idx, root, bounds):
        ...
        for i, c in enumerate(child):
            ...
            start = end
            self.resolve_lay(parent_set, lay, name_idx, c, new_bounds)
    ...
    def tree_visualization(self):
        ...
        self.resolve_lay(parent_set, lay, name_idx, root_name, [-3, 3])
        #lay = G.layout('rt')

        numVertex = len(G.get_vertex_dataframe())
        vertices = G.get_vertex_dataframe()
        v_label = [i for i in vertices['name']]
