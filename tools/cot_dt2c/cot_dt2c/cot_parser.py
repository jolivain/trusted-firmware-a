...
class COT:
    ...
    def cert_to_c(self, node: Node, f):
        ...
        f.write(f"\t.img_id = {node.get_field("image-id")},\n")
        f.write("\t.img_type = IMG_CERT,\n")

        if not self.if_root(node):
            f.write(f"\t.parent = &{node.get_field("parent").label.name},\n")
        else:
            f.write("\t.parent = NULL,\n")
    ...
    def img_to_c(self, node:Node, f):
        ...
        f.write("\t\t\t.param.hash = {\n")
        f.write("\t\t\t\t.data = &raw_data,\n")
        f.write(f"\t\t\t\t.hash = &{node.get_field("hash").label.name}\n")
        f.write("\t\t\t}\n")
    ...
    def nv_to_c(self, f):
        ...
        for nv in nv_ctr:
            f.write(f"static auth_param_type_desc_t {nv.name} = "\
                    f"AUTH_PARAM_TYPE_DESC(AUTH_PARAM_NV_CTR, \"{nv.get_field("oid")}\");\n")
    ...
    def pk_to_c(self, f):
        ...
        for p in pks:
            f.write(f"static auth_param_type_desc_t {p.name} = "\
                    f"AUTH_PARAM_TYPE_DESC(AUTH_PARAM_PUB_KEY, \"{p.get_field("oid")}\");\n")
    ...
    def cot_to_c(self, f):
        ...
        for i, c in enumerate(certs):
            f.write(f"\t[{c.get_field("image-id")}]	=	&{c.name},\n")
