...
static uint32_t rdqdm_dly[DRAM_CH_CNT][CSAB_CNT][SLICE_CNT * 2U][9];
...
static const uint32_t reg_pi_mr11_data_fx_csx[2][CSAB_CNT] = {
	{
...
static const uint32_t reg_pi_mr12_data_fx_csx[2][CSAB_CNT] = {
	{
...
static const uint32_t reg_pi_mr14_data_fx_csx[2][CSAB_CNT] = {
	{
...
static void ddr_config(void)
{
	...
	reg_ddrphy_write_a(ddr_regdef_adr(_reg_PHY_FREQ_SEL_MULTICAST_EN),
			   0x00U);
	foreach_vch(ch) {
		for (i = 0; i < (int32_t)_reg_PHY_CLK_CACS_SLAVE_DELAY_X_NUM - 4; i++) {
		...
		for (i = ((int32_t)_reg_PHY_CLK_CACS_SLAVE_DELAY_X_NUM - 4);
		     i < (int32_t)_reg_PHY_CLK_CACS_SLAVE_DELAY_X_NUM; i++) {
			adj = _f_scale_adj(board_cnf->ch[ch].cacs_adj[i]);
			ddrtbl_setval(_cnf_DDR_PHY_ADR_G_REGSET,
				      _reg_PHY_CLK_CACS_SLAVE_DELAY_X[i],
				      data_l + (uint32_t)adj);
		...
		if (ddr_phycaslice == 1U) {
			for (i = 0; i < 6; i++) {
				adj = _f_scale_adj
					...
					[i + (int32_t)_reg_PHY_CLK_CACS_SLAVE_DELAY_X_NUM]);
				ddrtbl_setval(_cnf_DDR_PHY_ADR_V_REGSET,
					      _reg_PHY_CLK_CACS_SLAVE_DELAY_X
					      [i], data_l + (uint32_t)adj);
				reg_ddrphy_write(ch, ddr_regdef_adr
					(_reg_PHY_CLK_CACS_SLAVE_DELAY_X[i]) +
					0x0100U,
					_cnf_DDR_PHY_ADR_V_REGSET
		...
		foreach_ech(ch) {
			if ((ddr_phyvalid & (1U << ch))) {
				data_l = (data_l & (~(0x000000FFU << (ch * 8U))))
					...
					| ((board_cnf->ch[ch].dqs_swap & 0x0030U)
					   >> 2U)
					| ((board_cnf->ch[ch].dqs_swap & 0x0300U)
					   >> 4U)
					| ((board_cnf->ch[ch].dqs_swap & 0x3000U)
					   >> 6U)) << (ch * 8U));
			}
		}
...
static void dbsc_regset(void)
{
	...
	tmp[3] = ddrtbl_getval(_cnf_DDR_PI_REGSET, _reg_PI_RDLAT_ADJ_F1) - 1U;
...
#define _par_DBRNK_VAL	(0x7007U)
	...
	/* SCFCTST0 */
	/* SCFCTST0 ACT-ACT */
	tmp[3] = 1UL * js2[js2_trcpb] * 800U * ddr_mbpsdiv / ddr_mbps;
	/* SCFCTST0 RDA-ACT */
	tmp[2] =
	    1UL * ((16U / 2U) + js2[js2_trtp] - 8U +
		   js2[js2_trppb]) * 800U * ddr_mbpsdiv / ddr_mbps;
	/* SCFCTST0 WRA-ACT */
	tmp[1] =
	    1UL * (WL + 1U + (16U / 2U) +
		   js1[js1_ind].nwr) * 800U * ddr_mbpsdiv / ddr_mbps;
	/* SCFCTST0 PRE-ACT */
	...
	tmp[2] =
	    ...
	    ddr_mbps;
	/* SCFCTST1 ACT-RD/WR */
	tmp[1] = 1UL * js2[js2_trcd] * 800U * ddr_mbpsdiv / ddr_mbps;
	/* SCFCTST1 ASYNCOFS */
	tmp[0] = 12U;
	...
	/* DBSCHRW1 */
	/* DBSCHRW1 SCTRFCAB */
	tmp[0] = 1UL * js2[js2_trfcab] * 800U * ddr_mbpsdiv / ddr_mbps;
	...
	if ((prr_product == PRR_PRODUCT_M3) && (prr_cut < PRR_PRODUCT_30)) {
		mmio_write_32(DBSC_DBSCHRW1, tmp[0]
			+ ((mmio_read_32(DBSC_DBTR(22)) & 0x0000FFFF)
			* 400U * 2U * ddr_mbpsdiv + (ddr_mbps - 1U)) /
			ddr_mbps - 3U);
	} else {
		mmio_write_32(DBSC_DBSCHRW1, tmp[0]
			+ ((mmio_read_32(DBSC_DBTR(22)) & 0x0000FFFF)
...
static void dbsc_regset_post(void)
{
	...
	if (ddr_backup == DRAM_BOOT_STATUS_WARM) {
		...
		send_dbcmd(0x0A040001U);
#else /* DDR_BACKUPMODE_HALF */		/* for All channels */
		send_dbcmd(0x0A840001U);
...
static uint32_t swlvl1(uint32_t ddr_csn, uint32_t reg_cs, uint32_t reg_kick)
{
	...
	/* kick */
	foreach_vch(ch) {
		if (ch_have_this_cs[ddr_csn % 2U] & (1U << ch)) {
