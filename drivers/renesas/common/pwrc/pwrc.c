...
uint32_t rcar_pwrc_status(uint64_t mpidr)
{
	...
	if (IS_CA53(c)) {
		if (reg & (1U << (STATE_CA53_CPU + cpu))) {
			ret = RCAR_INVALID;
		}
	}
	if (IS_CA57(c)) {
		if (reg & (1U << (STATE_CA57_CPU + cpu))) {
			ret = RCAR_INVALID;
		}
	}
done:
...
static void scu_power_up(uint64_t mpidr)
{
	...
	if ((mmio_read_32(reg_pwrsr) & STATUS_PWRDOWN) == 0U) {
		goto done;
	}

	if (mmio_read_32(reg_cpumcr) != 0U) {
	...
	do {
		while ((mmio_read_32(RCAR_SYSCSR) & REQ_RESUME) == 0U) {
		}
		mmio_write_32(reg_pwron, 1);
	} while (mmio_read_32(reg_pwrer) & 1U);

	while ((mmio_read_32(RCAR_SYSCISR) & sysc_reg_bit) == 0U) {
	}
	mmio_write_32(RCAR_SYSCISR, sysc_reg_bit);
	while ((mmio_read_32(reg_pwrsr) & STATUS_PWRUP) == 0U) {
	}
done:
	return;
}
...
void rcar_pwrc_cpuon(uint64_t mpidr)
{
	...
	limit = IS_CA53(c) ? 0x5A5A0000U : 0xA5A50000U;
	...
	mmio_write_32(res_reg, res_data & (~(1U << (3U - cpu))));
...
void rcar_pwrc_cpuoff(uint64_t mpidr)
{
	...
	mmio_write_32(reg + cpu * 0x0010U, CPU_PWR_OFF);
...
void rcar_pwrc_enable_interrupt_wakeup(uint64_t mpidr)
{
	...
	mmio_write_32(reg, ~((uint32_t) 1U << shift_irq) &
...
void rcar_pwrc_setup(void)
{
	...
	uintptr_t rst_barl;
	uint32_t i, j;
	uint64_t reset = (uint64_t) (&plat_secondary_reset) & 0xFFFFFFFFUL;
	...
	for (i = 0U; i < PLATFORM_CLUSTER_COUNT; i++) {
		rst_barh = reg_barh[i];
		rst_barl = reg_barl[i];
		for (j = 0U; j < rcar_pwrc_get_cpu_num(cluster[i]); j++) {
...
rcar_pwrc_set_self_refresh_e3(void)
{
	...
	mmio_write_32(DBSC4_REG_DBPDRGD0, ddr_md == 0U ?
...
rcar_pwrc_go_suspend_to_ram(void)
{
	...
	for (i = 0U; (i < PMIC_RETRY_MAX) && (qllm != 0); i++) {
		qllm = rcar_iic_dvfs_send(PMIC, PMIC_QLLM_CNT, 0U);
	}

	/* Set trigger of power down to PMIV */
	for (i = 0U; (i < PMIC_RETRY_MAX) && (rc != 0) && (qllm == 0); i++) {
...
uint32_t rcar_pwrc_get_cpu_num(uint32_t c)
{
	...
	if (IS_A53A57(c) || IS_CA53(c)) {
		...
		for (i = 0U; i < RCAR_CA53CPU_NUM_MAX; i++) {
			if (reg & (1U << (STATE_CA53_CPU + i))) {
				continue;
			}
			count++;
		}
...
count_ca57:
	if (IS_A53A57(c) || IS_CA57(c)) {
		...
		for (i = 0U; i < RCAR_CA57CPU_NUM_MAX; i++) {
			if (reg & (1U << (STATE_CA57_CPU + i))) {
				continue;
			}
			count++;
		}
