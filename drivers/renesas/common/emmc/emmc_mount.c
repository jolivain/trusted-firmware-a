...
static EMMC_ERROR_CODE emmc_card_init(void)
{
	...
	if ((mmc_drv_obj.initialize != TRUE)
	    ...
	    || ((GETR_32(SD_INFO2) & SD_INFO2_CBSY) != 0U)
	    ) {
	...
	for (retry = 300; retry > 0; retry--) {
		...
		if ((mmc_drv_obj.r3_ocr & (uint32_t)EMMC_OCR_STATUS_BIT) != 0U) {
	...
	switch (mmc_drv_obj.r3_ocr & (uint32_t)EMMC_OCR_ACCESS_MODE_MASK) {
	 ...
	 */
	resultCalc = emmc_calc_tran_speed(&freq);
	if (resultCalc == 0U) {
		emmc_write_error_info(EMMC_FUNCNO_CARD_INIT,
				      EMMC_ERR_ILLEGAL_CARD);
		return EMMC_ERR_ILLEGAL_CARD;
...
static EMMC_ERROR_CODE emmc_high_speed(void)
{
	...
	/* max frequency */
	card_type = (uint8_t) mmc_drv_obj.ext_csd_data[EMMC_EXT_CSD_CARD_TYPE];
	if ((card_type & (uint8_t)EMMC_EXT_CSD_CARD_TYPE_52MHZ) != 0U) {
		freq = MMC_52MHZ;
	} else if ((card_type & (uint8_t)EMMC_EXT_CSD_CARD_TYPE_26MHZ) != 0U) {
		freq = MMC_26MHZ;
	} else {
		freq = MMC_20MHZ;
	}
...
static EMMC_ERROR_CODE emmc_clock_ctrl(uint8_t mode)
{
	...
	/* busy check */
	if ((GETR_32(SD_INFO2) & SD_INFO2_CBSY) != 0U) {
...
static EMMC_ERROR_CODE emmc_bus_width(uint32_t width)
{
	...
	emmc_make_nontrans_cmd(CMD6_SWITCH,
			       ((uint32_t)EMMC_SWITCH_BUS_WIDTH_1 |
				((uint32_t)mmc_drv_obj.bus_width << 8)));
	result = emmc_exec_cmd(EMMC_R1_ERROR_MASK, mmc_drv_obj.response);
	if (result != EMMC_SUCCESS) {
		/* occurred error */
...
{
	EMMC_ERROR_CODE result = EMMC_SUCCESS;
	uint32_t arg;
	...
	if ((partition_config & (uint32_t)PARTITION_ID_MASK) == (uint32_t)id) {
		...
		partition_config =
		    ((partition_config & ~((uint32_t)PARTITION_ID_MASK)) | (uint32_t)id);
...
static uint32_t emmc_calc_tran_speed(uint32_t *freq)
{
	...
	if (maxFreq == 0U) {
		result = 0U;
	} else if (maxFreq >= MMC_FREQ_52MHZ) {
		*freq = MMC_52MHZ;
	} else if (maxFreq >= MMC_FREQ_26MHZ) {
		*freq = MMC_26MHZ;
	} else if (maxFreq >= MMC_FREQ_20MHZ) {
		*freq = MMC_20MHZ;
	} else {
		*freq = MMC_400KHZ;
	}
...
EMMC_ERROR_CODE emmc_set_request_mmc_clock(uint32_t *freq)
{
	...
	set_sd_clk(*freq);
	mmc_drv_obj.clock_enable = FALSE;
	result = emmc_clock_ctrl(TRUE); /* clock on */
done:
	return result;
}
