...
uint32_t emmc_bit_field(uint8_t *data, uint32_t top, uint32_t bottom)
{
	...
	uint32_t index_top = (15U - (top >> 3));
	uint32_t index_bottom = (15U - (bottom >> 3));
	...
	if (index_top == index_bottom) {
		...
		value =
		    (uint32_t) ((data[index_top] << 16) |
				(data[index_top + 1U] << 8) | data[index_top +
								  2U]);
	} else {
		value =
		    (uint32_t) ((data[index_top] << 24) |
				(data[index_top + 1U] << 16) |
				(data[index_top + 2U] << 8) |
				data[index_top + 3U]);
	}
...
void emmc_make_nontrans_cmd(HAL_MEMCARD_COMMAND cmd, uint32_t arg)
{
	...
	mmc_drv_obj.cmd_info.hw =
		cmd_reg_hw[cmd & (HAL_MEMCARD_COMMAND)HAL_MEMCARD_COMMAND_INDEX_MASK];
	...
	switch (mmc_drv_obj.cmd_info.cmd &
		(HAL_MEMCARD_COMMAND)HAL_MEMCARD_RESPONSE_TYPE_MASK) {
