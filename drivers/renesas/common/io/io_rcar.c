...
#define RCAR_ATTR_SET_CALCADDR(a)	((a) & 0xFU)
#define RCAR_ATTR_SET_ISNOLOAD(a)	(((a) & 0x1U) << 16U)
#define RCAR_ATTR_SET_CERTOFF(a)	(((a) & 0xFU) << 8U)
...
static const plat_rcar_name_offset_t name_offset[] = {
	{BL31_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(0U, 0U, 0U)},

	/* BL3-2 is optional in the platform */
	{BL32_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(1U, 0U, 1U)},
	{BL33_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(2U, 0U, 2U)},
	{BL332_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(3U, 0U, 3U)},
	{BL333_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(4U, 0U, 4U)},
	{BL334_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(5U, 0U, 5U)},
	{BL335_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(6U, 0U, 6U)},
	{BL336_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(7U, 0U, 7U)},
	{BL337_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(8U, 0U, 8U)},
	{BL338_IMAGE_ID, 0U, RCAR_ATTR_SET_ALL(9U, 0U, 9U)},
};

#if TRUSTED_BOARD_BOOT
static const plat_rcar_name_offset_t cert_offset[] = {
	/* Certificates */
	{TRUSTED_KEY_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 0U)},
	{SOC_FW_KEY_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 0U)},
	{TRUSTED_OS_FW_KEY_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 0U)},
	{NON_TRUSTED_FW_KEY_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 0U)},
	{SOC_FW_CONTENT_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 0U)},
	{TRUSTED_OS_FW_CONTENT_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 1U)},
	{NON_TRUSTED_FW_CONTENT_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 2U)},
	{BL332_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 3U)},
	{BL333_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 4U)},
	{BL334_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 5U)},
	{BL335_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 6U)},
	{BL336_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 7U)},
	{BL337_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 8U)},
	{BL338_CERT_ID, 0U, RCAR_ATTR_SET_ALL(0U, 1U, 9U)},
};
...
void rcar_read_certificate(uint64_t cert, uint32_t *len, uintptr_t *dst)
{
	...
	val = mmio_read_32(RCAR_BOOT_KEY_CERT_NEW + 0xCU);
	info_1 = (val >> 18) & 0x3U;
	val = mmio_read_32(cert + 0xCU);
	info_2 = (val >> 21) & 0x3U;
...
{
	uint32_t legacy = dst + len <= UINT32_MAX - 1U ? 1U : 0U;
...
{
	uint64_t header[64] __aligned(FLASH_TRANS_SIZE_UNIT) = {0};
	uintptr_t handle;
	ssize_t offset;
	uint32_t i;
	int32_t rc;
	...
	for (i = 0U; i < rcar_image_number + 2U; i++) {
		rcar_image_header[i] = header[i * 2U + 1U];
		rcar_image_header_prttn[i] = header[i * 2U + 2U];
	}

	if (rcar_image_number == 0U || rcar_image_number > RCAR_MAX_BL3X_IMAGE) {
	...
	inv_dcache_range(RCAR_SDRAM_certESS,
			 RCAR_CERT_SIZE * (2U + rcar_image_number));
#endif
	rc = io_read(handle, RCAR_SDRAM_certESS,
		     RCAR_CERT_SIZE * (2U + rcar_image_number), &cnt);
	if (rc != IO_SUCCESS) {
			      ...
			      io_entity_t *entity)
{
	...
	if (noload) {
		current_file.offset = 1U;
		current_file.dst = 0U;
		current_file.size = 1U;
		current_file.position = 0U;
