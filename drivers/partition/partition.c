...
static int load_mbr_header(uintptr_t image_handle, mbr_entry_t *mbr_entry)
{
	...
	if ((mbr_entry->sector_nums == 0) || (mbr_entry->sector_nums == UINT32_MAX)) {
			   ...
			   unsigned long long *part_lba)
{
	...
	if ((result != 0) || (sizeof(gpt_header_t) != bytes_read)) {
		VERBOSE("GPT header read error(%i) or read mismatch occurred expected(%zu) and actual(%zu)\n",
...
static int load_gpt_entry(uintptr_t image_handle, gpt_entry_t *entry)
{
	...
	if ((result != 0) || (sizeof(gpt_entry_t) != bytes_read)) {
		VERBOSE("GPT Entry read error(%i) or read mismatch occurred expected(%zu) and actual(%zu)\n",
 ...
 */
static int load_partition_gpt(uintptr_t image_handle, const unsigned long long part_lba)
{
	...
	if (result != 0) {
		VERBOSE("Failed to seek (%i), Failed loading GPT partition table entries\n",
			result);
		return result;
	}
...
static int load_backup_gpt(unsigned int image_id, unsigned int sector_nums)
{
	 ...
	 * try mapping only last 33 last blocks from the image to read the Backup-GPT
	 * header and its entries.
	 */
	...
	/* Note we mapped last 33 blocks(LBA-33), first block here starts with entries
	 * while last block was header.
	 */
...
static int load_primary_gpt(uintptr_t image_handle, unsigned int first_lba)
{
	...
	result = load_gpt_header(image_handle, gpt_header_offset, &part_lba);
	if ((result != 0) || (part_lba == 0)) {
		VERBOSE("Failed to retrieve Primary GPT header, trying to retrieve back-up GPT header\n");
		return result;
	}

	result = load_partition_gpt(image_handle, part_lba);
	return result;
}
...
int load_partition_table(unsigned int image_id)
{
	...
	if (mbr_entry.type == PARTITION_TYPE_GPT) {
		...
		if (result != 0) {
			io_close(image_handle);
			result = load_backup_gpt(BKUP_GPT_IMAGE_ID, mbr_entry.sector_nums);
			return result;
		}
	} else {
