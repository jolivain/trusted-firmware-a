...
static int load_partition_gpt(uintptr_t image_handle, gpt_header_t header)
{
	 ...
	 * partition table.
	 */
	if (i != (unsigned int)list.entry_count) {
		/*
		 * Loop break due to last entry not being parsable, decrement valid entry count
		 */
		list.entry_count = i - 1;
	}
	dump_entries(list.entry_count);

	/*
	 * If there are less entries than the possible number of entries from the header,
	 * continue to load the partition entry table to calculate the full CRC in order
	 * to check against the partition CRC from the header for validation.
	 */
