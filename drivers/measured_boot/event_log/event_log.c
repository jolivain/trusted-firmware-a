...
/* Pointer to the end of the Event Log Buffer */
static uintptr_t event_log_end;
 ...
 *
 * @param[in] event_log_base		Base address of Event Log buffer
 * @param[in] event_log_max_size	Maximum Size of Event Log buffer
 * @param[in] event_log_offset		Event Log buffer offset
 */
void event_log_init(uint8_t *event_log_base, size_t event_log_max_size,
		    size_t event_log_offset)
{
	assert(event_log_base != NULL);

	log_ptr = event_log_base + event_log_offset;
	event_log_end = (uintptr_t)event_log_base + event_log_max_size;

	/* Get pointer to platform's event_log_metadata_t structure */
	plat_metadata_ptr = plat_get_event_log_metadata();
}
 ...
 *    < 0 = error
 */
int event_log_measure_record(uintptr_t data_base, uint32_t data_size,
			     uint32_t data_id)
{
...
/*
 * Get current Event Log buffer size
 *
 * @param[in]  event_log_base		Base Pointer to Event Log buffer
 *
 * @return: current Size of Event Log buffer
 */
size_t event_log_get_cur_size(uint8_t *event_log_base)
{
	assert(event_log_base != NULL);
	assert(log_ptr != NULL);

	return ((uintptr_t)log_ptr - (uintptr_t)event_log_base);
}
