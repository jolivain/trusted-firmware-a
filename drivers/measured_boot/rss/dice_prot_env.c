...
{
	assert(DICE_HASH_SIZE >= DPE_DIGEST_SIZE);
	...
	dice_inputs->code_descriptor = metadata->sw_type;
	 /* Terminating 0 character is not need due to CBOR encoding */
	dice_inputs->code_descriptor_size = metadata->sw_type_size - 1;
}
...
void dpe_init(struct dpe_metadata *metadata)
{
	...
	while (metadata->id != DPE_INVALID_ID) {
		metadata->sw_type_size =
			strlen((const char *)&metadata->sw_type) + 1;
		metadata++;
	}
