...
static const image_data_t images_data[] = {
	...
	{ STM32_IMAGE_ID, STM32_IMAGE_STRING },
 ...
 * Might be overridden in the platform code.
 */
			...
			uint32_t image_id)
{
	...
	while ((image_id != images_data[i].id) &&
		(++i < ARRAY_SIZE(images_data))) {
	}

	if (i == ARRAY_SIZE(images_data)) {
	...
	return add_event2(hash_data, images_data[i].name, image_id);
}
...
int event_log_finalise(uint8_t **log_addr, size_t *log_size)
{
	...
	*log_size = 0U;
	 ...
	 */
	rc = plat_set_nt_fw_info((void *)nt_fw_config_base,
#ifdef SPD_opteed
				event_log,
#endif
	...
	if (tos_fw_config_base != 0UL) {
		...
		rc = plat_set_tos_fw_info((void *)tos_fw_config_base,
						event_log, num_bytes);
		if (rc != 0) {
