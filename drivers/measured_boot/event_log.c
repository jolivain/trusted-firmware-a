...
static int add_event2(const uint8_t *hash, const image_data_t *image_ptr)
{
	...
	/* Check for space in Event Log buffer */
	if (((uintptr_t)((uint8_t *)ptr - event_log) +
		(unsigned long)TCG_DIGEST_SIZE) > sizeof(event_log)) {
		ERROR("%s(): Event Log is short of memory for %lu bytes",
			__func__, (unsigned long)TCG_DIGEST_SIZE -
			(sizeof(event_log) -
			(uintptr_t)((uint8_t *)ptr - event_log)));
		return -ENOMEM;
	}
...
void event_log_init(void)
{
	...
	if ((uintptr_t)((uint8_t *)ptr - event_log) != (unsigned long)(ID_EVENT_SIZE)) {
		panic();
	}
	...
	if ((uintptr_t)((uint8_t *)ptr - start_ptr) !=
		(unsigned long)(LOC_EVENT_SIZE)) {
		panic();
	}
