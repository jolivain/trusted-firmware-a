...
static const measured_boot_data_t *plat_get_measured_boot_data(void)
{
...
static int add_event2(const uint8_t *hash, const image_data_t *image_ptr)
{
	...
	size_of_event = (EVENT2_HDR_SIZE + name_len);

	/* Check for space in Event Log buffer */
	if (((uintptr_t)((uint8_t *)ptr - event_log) + size_of_event) >
							sizeof(event_log)) {
		ERROR("%s(): Event Log is short of memory for %lu bytes",
			__func__, size_of_event - (sizeof(event_log) -
			(uintptr_t)((uint8_t *)ptr - event_log)));
		return -ENOMEM;
	}
	...
	/* Check for space in Event Log buffer */
	if (((uintptr_t)((uint8_t *)ptr - event_log) + TCG_DIGEST_SIZE) >
							sizeof(event_log)) {
		ERROR("%s(): Event Log is short of memory for %lu bytes",
			__func__, TCG_DIGEST_SIZE - (sizeof(event_log) -
			(uintptr_t)((uint8_t *)ptr - event_log)));
		return -ENOMEM;
	}
...
void event_log_init(void)
{
	...
	if ((uintptr_t)((uint8_t *)ptr - event_log) != (ID_EVENT_SIZE)) {
		panic();
	}
	...
	(void)memcpy(ptr, (const void *)TCG_STARTUP_LOCALITY_SIGNATURE,
	...
	if ((uintptr_t)((uint8_t *)ptr - start_ptr) != (LOC_EVENT_SIZE)) {
		panic();
	}
...
{
	/* Event Log size */
	size_t num_bytes = (size_t)(log_ptr - event_log);
	int rc;
