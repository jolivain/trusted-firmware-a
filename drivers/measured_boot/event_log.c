...
static int add_event2(const uint8_t *hash, const image_data_t *image_ptr)
{
	...
	/* Check for space in Event Log buffer */
	if (((uintptr_t)ptr + (unsigned int)TCG_DIGEST_SIZE) > EVENT_LOG_END) {
		ERROR("%s(): Event Log is short of memory", __func__);
		return -ENOMEM;
	}
...
{
	const void *locality = TCG_STARTUP_LOCALITY_SIGNATURE;
	...
	(void)memcpy(ptr, locality, sizeof(TCG_STARTUP_LOCALITY_SIGNATURE));
	/*
