 ...
 */
static void id_event_print(void **log_addr, size_t *log_size)
{
	unsigned int i;
	...
	assert(event->header.pcr_index == PCR_0);
 ...
 * @param[in/out] log_size	Pointer to Event Log size
 */
void event2_print(uint8_t **log_addr, size_t *log_size)
{
	uint32_t event_size, count;
	size_t sha_size, digests_size = 0U;
	uint8_t *ptr = *log_addr;
#if ENABLE_ASSERTIONS
	uint8_t *end_ptr = *log_addr + *log_size;
#endif
	...
	assert((ptr <= end_ptr) && (count != 0U));

	for (unsigned int i = 0U; i < count; ++i) {
		/* Check AlgorithmId address */
		assert(ptr + offsetof(tpmt_ha, digest) <= end_ptr);
	...
	assert(ptr + offsetof(event2_data_t, event) <= end_ptr);
...
void dump_event_log(uint8_t *log_addr, size_t log_size)
{
	...
	id_event_print((void *)&log_addr, &log_size);
