...
static void id_event_print(uint8_t **log_addr, size_t *log_size)
{
	...
	const id_event_headers_t *event = (const id_event_headers_t *)*log_addr;
	...
	alg_ptr = (id_event_algorithm_size_t *)event->struct_header.digest_size;
	...
	*log_size -= (size_t)(info_size_ptr - *log_addr);
...
static void event2_print(uint8_t **log_addr, size_t *log_size)
{
	...
	assert(((uint8_t *)ptr <= end_ptr) && (count != 0U));
	...
	*log_size -= (size_t)((uint8_t *)ptr + event_size - *log_addr);
