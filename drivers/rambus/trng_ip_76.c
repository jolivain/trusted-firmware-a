...
static int eip76_rng_init(struct eip76_rng_dev *dev)
{
	uint32_t val;
	 ...
	 *   This field can only be modified while ‘enable_trng’ in TRNG_CONTROL
...
int32_t eip76_rng_probe(uintptr_t base_addr)
{
	...
	ret = eip76_rng_init(&eip76_dev);
	if (ret == 0) {
		uint32_t ver = eip76_rng_read(&eip76_dev, RNG_REV_REG);

		INFO("%s Random Number Generator HW ver. %01x.%01x.%01x\n",
		     (ver & 0xff) == 0x4C ? "TRNG-IP-76" : "Unknown",
		     (ver & (0xf << 24)) >> 24,  /* HW ver. major */
		     (ver & (0xf << 20)) >> 20,  /* HW ver. minor */
		     (ver & (0xf << 16)) >> 16); /* HW patch level */
	}
