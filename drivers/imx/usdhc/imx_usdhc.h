...
#define EMMC_INTSTATEN_BITS                                                \
	(INTSTATEN_CC | INTSTATEN_TC | INTSTATEN_DINT | INTSTATEN_BWR |    \
	 INTSTATEN_BRR | INTSTATEN_CINT | INTSTATEN_CTOE | INTSTATEN_CCE | \
...
#define VENDSPEC_INIT                                                      \
	(VENDSPEC_RSRV1 | VENDSPEC_CARD_CLKEN | VENDSPEC_PER_CLKEN |       \
	 VENDSPEC_AHB_CLKEN | VENDSPEC_IPG_CLKEN | VENDSPEC_AC12_CHKBUSY | \
	 VENDSPEC_EXTDMA)

#define MMCBOOT 0x0c4

#define mmio_clrsetbits32(addr, clear, set) \
	mmio_write_32(addr, (mmio_read_32(addr) & ~(clear)) | (set))
#define mmio_clrbits32(addr, clear) \
	mmio_write_32(addr, mmio_read_32(addr) & ~(clear))
#define mmio_setbits32(addr, set) \
