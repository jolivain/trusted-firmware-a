					   ...
					   psa_key_type_t *psa_key_type)
{
	...
	case MBEDTLS_PK_ECDSA:
		*psa_alg = PSA_ALG_ECDSA(psa_md_alg);
		/*
		 * TODO: ECC family is hardcoded as of now, need to parse the PK
		 * to get the proper family
		 */
 ...
 *
 */
static inline void get_pksize_from_rfc5280_pk(uint8_t **p, size_t *size)
{
	uint8_t *pk_start;
			    ...
			    void *pk_ptr, unsigned int pk_len)
{
	...
	if (pk_alg == MBEDTLS_PK_ECDSA) {
		...
		get_pksize_from_rfc5280_pk((uint8_t **)&pk_ptr, (size_t *)&pk_len);
