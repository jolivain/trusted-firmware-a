...
/* Maxmum length of R_S pair in the ECDSA signature in bytes */
				      ...
				      psa_ecc_family_t *psa_ecc_family)
{
	...
	pk_end = pk_ptr + len;
	mbedtls_asn1_get_alg(&pk_ptr, pk_end, &alg_oid, &alg_params);
	if (rc != 0) {
		return CRYPTO_ERR_SIGNATURE;
	}
	...
	*psa_ecc_family = mbedtls_ecc_group_to_psa(grp_id, &curve_bits);
	...
	pk_end = pk_ptr + *pk_len - alg_oid.len - alg_params.len;
...
TF_MBEDTLS_KEY_ALG_ID == TF_MBEDTLS_RSA_AND_ECDSA
	if (pk_alg == MBEDTLS_PK_ECDSA) {
		...
		rc = get_ecdsa_pkinfo_from_asn1((unsigned char **)&pk_ptr, &pk_len,
						&psa_ecc_family);
		if (rc != 0) {
			goto end2;
		}
