...
static int aes_gcm_decrypt(void *data_ptr, unsigned int len, const void *key,
			   ...
			   unsigned int tag_len)
{
	...
	while (len > 0) {
		dec_len = MIN((unsigned int)sizeof(buf), len);
...
static int auth_decrypt(enum crypto_dec_algo dec_algo, void *data_ptr,
			unsigned int len, const void *key, unsigned int key_len,
