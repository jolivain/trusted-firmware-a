			  ...
			  void *img, unsigned int img_len)
{
	...
	if (img_desc->parent != NULL) {
		...
		/*
		 * Set Zero-OID for ROTPK(subject key) as authentication params
		 * does not hold Key-OID information for ROTPK.
		 */
		if (param->pk->cookie == NULL) {
			param->pk->cookie = ZERO_OID;
		}

		/*
		 * Public key is verified at this stage, notify platform
		 * to measured and publish it
		 */
		rc = plat_mboot_measure_key(param->pk->cookie, pk_ptr, pk_len);
		if (rc != 0) {
