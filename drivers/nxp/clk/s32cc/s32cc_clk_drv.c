...
static void enable_pll_hw(uintptr_t pll_addr)
{
	...
	while ((mmio_read_32(PLLDIG_PLLSR(pll_addr)) & PLLDIG_PLLSR_LOCK) == U(0)) {
	}
}
...
static void config_pll_out_div(uintptr_t pll_addr, uint32_t div_index, uint32_t dc)
{
	...
	if (((pdiv + U(1)) == dc) && ((pllodiv & PLLDIG_PLLODIV_DE) != U(0))) {
		return;
	}

	if ((pllodiv & PLLDIG_PLLODIV_DE) != U(0)) {
		disable_odiv(pll_addr, div_index);
	}

	pllodiv = PLLDIG_PLLODIV_DIV_SET(dc - U(1));
			      ...
			      bool safe_clk)
{
	...
	if (!safe_clk) {
		...
		if (((MC_CGM_MUXn_CSS_SWTRG(css) == MC_CGM_MUXn_CSS_SWTRG_SAFE_CLK) ||
		     ...
		     ((MC_CGM_MUXn_CSS_SAFE_SW & css) != U(0))) {
			return 0;
		}
