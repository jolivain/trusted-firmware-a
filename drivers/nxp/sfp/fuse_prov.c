			...
			uint32_t mask)
{
	...
	if (sfp_read32(fuse_addr) !=
	    (last_stored_val | (*fuse_hdr_val & mask))) {
		return ERROR_WRITE;
	}
		     ...
		     struct sfp_ccsr_regs_t *sfp_ccsr_regs)
{
	...
	if (ret != 0) {
		ret = (ret == ERROR_ALREADY_BLOWN) ? ERROR_SRKH_ALREADY_BLOWN :
						     ERROR_SRKH_WRITE;
	}
		      ...
		      struct sfp_ccsr_regs_t *sfp_ccsr_regs)
{
	...
	if (ret != 0) {
		ret = (ret == ERROR_ALREADY_BLOWN) ? ERROR_OTPMK_ALREADY_BLOWN :
						     ERROR_OTPMK_WRITE;
	} else {
		/* Check for DRV hamming error */
		      ...
		      struct sfp_ccsr_regs_t *sfp_ccsr_regs)
{
	...
	if (ret != 0) {
		ret = (ret == ERROR_ALREADY_BLOWN) ? ERROR_OSPR1_ALREADY_BLOWN :
						     ERROR_OSPR1_WRITE;
	}
		       ...
		       struct sfp_ccsr_regs_t *sfp_ccsr_regs)
{
	...
	if (ret != 0) {
		ret = (ret == ERROR_ALREADY_BLOWN) ? ERROR_SC_ALREADY_BLOWN :
						     ERROR_SC_WRITE;
	}
 ...
 */
int provision_fuses(unsigned long long fuse_scr_addr, bool en_povdd_status)
{
