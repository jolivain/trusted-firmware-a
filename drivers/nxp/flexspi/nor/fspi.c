...
static void fspi_op_setup(uint32_t fspi_op_seq_id, bool ignore_flash_sz)
{
	...
	case fspiBE_SEQ_ID:
		...
		break;
	case fspiRDSR_SEQ_ID:
		cmd_id1 = FSPI_NOR_CMD_RDSR;
		cmd_id2 = FSPI_NOR_CMD_RDSR;
		break;
	}

	xAddr = FSPI_LUTREG_OFFSET + (uint32_t)(0x10 * fspi_op_seq_id);
	if ((F_FLASH_SIZE_BYTES <= SZ_16M_BYTES) || (ignore_flash_sz)) {
		xInstr0 = FSPI_INSTR_OPRND0(cmd_id1);
		xInstr1 = FSPI_INSTR_OPRND1(FSPI_LUT_ADDR24BIT);
		VERBOSE("CMD_ID = %x offset = 0x%x\n", cmd_id1, xAddr);
	} else {
		xInstr0 = FSPI_INSTR_OPRND0(cmd_id2);
		xInstr1 = FSPI_INSTR_OPRND1(FSPI_LUT_ADDR32BIT);
		VERBOSE("CMD_ID = %x offset = 0x%x\n", cmd_id2, xAddr);
	}
	xInstr0 |= FSPI_INSTR_PAD0(FSPI_LUT_PAD1)
		| FSPI_INSTR_OPCODE0(FSPI_LUT_CMD);

	xInstr1 |= FSPI_INSTR_PAD1(FSPI_LUT_PAD1)
		| FSPI_INSTR_OPCODE1(FSPI_LUT_ADDR);

	if (fspi_op_seq_id == fspiRDSR_SEQ_ID) {
		xInstr0 |= FSPI_INSTR_OPRND1(1) | FSPI_INSTR_PAD1(FSPI_LUT_PAD1)
					| FSPI_INSTR_OPCODE1(FSPI_LUT_READ);
	} else if ((fspi_op_seq_id != fspiBE_SEQ_ID)
			&& (fspi_op_seq_id != fspiWREN_SEQ_ID)) {
		xInstr0 |= xInstr1;
	}

	fspi_writel((xAddr), xInstr0);
	fspi_writel((xAddr + 0x4), xInstr2);
...
static void fspi_setup_LUT(void)
{
	...
	fspi_op_setup(fspiREAD_SEQ_ID, false);

	/* LUT Setup for FAST READ Command 3-Byte/4-Byte high Frequency */
	fspi_op_setup(fspiFASTREAD_SEQ_ID, false);

	/* LUT Setup for Page Program */
	fspi_op_setup(fspiWRITE_SEQ_ID, false);

	/* LUT Setup for WREN */
	fspi_op_setup(fspiWREN_SEQ_ID, true);

	/* LUT Setup for Sector_Erase */
	fspi_op_setup(fspiSE_SEQ_ID, false);

	/* LUT Setup for Sub Sector 4K Erase */
	fspi_op_setup(fspiSE_4K_SEQ_ID, false);

	/* LUT Setup for Bulk_Erase */
	fspi_op_setup(fspiBE_SEQ_ID, true);

	/* Read Status */
	fspi_op_setup(fspiRDSR_SEQ_ID, true);
...
int xspi_ahb_read(uint32_t pcRxAddr, uint32_t *pcRxBuf, uint32_t xSize_Bytes)
{
	...
	if (((pcRxAddr % 4) != 0) || (((uintptr_t)pcRxBuf % 4) != 0)) {
...
int xspi_write(uint32_t pcWrAddr, void *pvWrBuf, uint32_t uiLen)
{
	...
	xAddr = (uint32_t)(pcWrAddr);
	if ((uiLen <= F_PAGE_256) && ((xAddr % F_PAGE_256) == 0)) {
		xPage1_Len = uiLen;
		INFO("%d ---> xPage1_Len=0x%x xPageL_Len =0x%x j=0x%x\n", __LINE__, xPage1_Len, xPageL_Len, j);
	} else if ((uiLen <= F_PAGE_256) && ((xAddr % F_PAGE_256) != 0)) {
