...
static void fspi_op_setup(uint32_t fspi_op_seq_id, bool ignore_flash_sz)
{
	...
	fspi_writel((x_addr + 0x4), x_instr2);
	fspi_writel((x_addr + 0x8), (uint32_t) 0x0);	/* STOP command */
	fspi_writel((x_addr + 0xc), (uint32_t) 0x0);	/* STOP command */
}
...
int xspi_ip_read(uint32_t pc_rx_addr, uint32_t *pv_rx_buf, uint32_t ui_len)
{
	...
	uint32_t i = 0, j = 0, x_rem = 0;
	uint32_t x_iteration = 0, x_size_rx = 0, x_size_wm, temp_size;
	uint32_t data = 0;
...
void xspi_ip_write(uint32_t pc_wr_addr, uint32_t *pv_wr_buf, uint32_t ui_len)
{
	...
	uint32_t x_iteration = 0, x_rem = 0;
	uint32_t x_size_tx = 0, x_size_wm, temp_size;
	uint32_t i = 0, j = 0;
	uint32_t ui_data = 0;
	uint32_t x_addr, x_len_bytes;


	x_size_wm = 8;	/* Default TX WaterMark level: 8 Bytes. */
...
int xspi_write(uint32_t pc_wr_addr, void *pv_wr_buf, uint32_t ui_len)
{
	...
	uint32_t x_addr;
	uint32_t x_page1_len = 0, x_page_l_len = 0;
	uint32_t i, j = 0;
	void *buf = pv_wr_buf;
...
{
	uint32_t x_addr, x_len_bytes, i, num_sector = 0;
	...
	num_sector += x_len_bytes % F_SECTOR_ERASE_SZ ? 1 : 0;
...
__attribute__((unused)) static void  fspi_delay_ms(uint32_t x)
{
	...
	for (ul_count = 0; ul_count < (30 * x); ul_count++)
		;
...
int fspi_init(uint32_t base_reg_addr, uint32_t flash_start_addr)
{
	...
	/* Disable SameDeviceEnable,
	 * to configure each chip select device indivisualy
	 */
