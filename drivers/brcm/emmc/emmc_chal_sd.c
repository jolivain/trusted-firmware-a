...
int32_t chal_sd_stop(CHAL_HANDLE *sd_handle)
{
 ...
 */
uint32_t chal_sd_check_cap(CHAL_HANDLE *sd_handle, uint32_t caps)
{
	struct sd_dev *handle;
...
int32_t chal_sd_start(CHAL_HANDLE *sd_handle,
		      int32_t mode, uint32_t sd_base, uint32_t host_base)
{
 ...
 */
int32_t chal_sd_get_present_status(CHAL_HANDLE *sd_handle)
{
	struct sd_dev *handle;
 ...
 */
int32_t chal_sd_set_dma(CHAL_HANDLE *sd_handle, int32_t mode)
{
	uint32_t val;
	struct sd_dev *handle;
	...
	if (mode) {
		if (chal_sd_check_cap(sd_handle,
