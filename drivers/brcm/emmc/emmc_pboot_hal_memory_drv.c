...
static uint32_t sdio_read(struct sd_handle *p_sdhandle, uintptr_t mem_addr,
			  uintptr_t storage_addr, size_t storage_size,
		   ...
		   size_t storage_size, size_t bytes_to_read)
{
	...
	return sdio_read(sdio_gethandle(), mem_addr, storage_addr, storage_size,
			 bytes_to_read);
}
...
int emmc_erase(uintptr_t mem_addr, size_t num_of_blocks, uint32_t partition)
{
	...
	erase_group =
		emmc_global_buf_ptr->u.Ext_CSD_storage[EXT_CSD_ERASE_GRP_SIZE] *
		1024;
 ...
 */
uint32_t send_sdio_cmd(uint32_t cmdIndex, uint32_t argument, uint32_t options,
		       struct sd_resp *resp)
{
	struct sd_handle *sd_handle = sdio_gethandle();
...
struct sd_handle *sdio_init(void)
{
	...
	if (chal_sd_start((CHAL_HANDLE *)p_sdhandle->device, SD_PIO_MODE,
			  SDIO_base, SDIO_base) != SD_OK) {
		return NULL;
	}
 ...
 */
uint32_t sdio_read(struct sd_handle *p_sdhandle, uintptr_t mem_addr,
		   uintptr_t storage_addr, size_t storage_size,
		   size_t bytes_to_read)
{
	...
	if (offset > 0) {
		if (!read_block(p_sdhandle, emmc_global_buf_ptr->u.tempbuf,
			...
			memcpy(outputBuf,
			       (void *)((uintptr_t)(emmc_global_buf_ptr->u
							    .tempbuf +
						    offset)),
			       manual_copy_size);
	...
	if (remSize > 0) {
		if (!read_block(p_sdhandle, emmc_global_buf_ptr->u.tempbuf,
			...
			memcpy(outputBuf, emmc_global_buf_ptr->u.tempbuf,
			       remSize);
			   ...
			   uintptr_t data_addr, size_t bytes_to_write)
{
	...
	if (p_sdhandle->device->ctrl.ocr & SD_CARD_HIGH_CAPACITY) {
		blockAddr = (uint32_t)(mem_addr /
				       p_sdhandle->device->cfg.blockSize);
		offset = (uint32_t)(mem_addr -
				    blockAddr *
					    p_sdhandle->device->cfg.blockSize);
	} else {
		blockAddr = ((uint32_t)mem_addr /
	...
	if (offset > 0) {
		if (!read_block(p_sdhandle, emmc_global_buf_ptr->u.tempbuf,
			...
			memcpy((void *)((uintptr_t)(emmc_global_buf_ptr->u
							    .tempbuf +
						    offset)),
			...
			if (!write_block(p_sdhandle,
				...
				if (remSize <
					...
					remSize -= (p_sdhandle->device->cfg
							    .blockSize -
						    offset);
					wtCount += (p_sdhandle->device->cfg
							    .blockSize -
						    offset);
					manual_copy_size =
						p_sdhandle->device->cfg
							.blockSize -
						offset;
				}
				...
				if (p_sdhandle->device->ctrl.ocr &
					...
					blockAddr += p_sdhandle->device->cfg
							     .blockSize;
				}
			} else
				return 0;
	...
	if (remSize > 0) {
		if (!read_block(p_sdhandle, emmc_global_buf_ptr->u.tempbuf,
				blockAddr, p_sdhandle->device->cfg.blockSize)) {
			memcpy(emmc_global_buf_ptr->u.tempbuf, inputBuf,
			       remSize);
