...
const struct spi_flash_info spi_flash_ids[] = {
	{ "W25Q64CV",
	  FINFO(0xef4017, 0x0, 64 * 1024, 128, 256, WR_QPP | SECT_4K) },
	{ "W25Q64DW",
	  FINFO(0xef6017, 0x0, 64 * 1024, 128, 256, WR_QPP | SECT_4K) },
...
static int spi_flash_read_common(const uint8_t *cmd, size_t cmd_len, void *data,
				 size_t data_len)
{
	int ret;
		    ...
		    void *buf)
{
	...
	for (actual = 0; actual < len; actual += chunk_len) {
		...
		chunk_len =
			MIN(len - actual, (uint32_t)(page_size - byte_addr));
...
int spi_flash_probe(struct spi_flash *flash)
{
	...
	INFO("Flash Name: %s sectors %x, sec size %x\n", info->name,
	     info->n_sectors, info->sector_size);
