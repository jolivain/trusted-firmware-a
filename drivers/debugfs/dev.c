 ...
 * it to fdset.
 */
static chan_t *newchan(unsigned char index)
{
	chan_t *c = NULL;
	int i;
	    ...
	    const dirtab_t *tab, int ntab, devgen_t *gen)
{
	...
	for (i = 0; ; i++) {
		...
		case -1:
			...
			goto leave;
		case 1:
			mnt = mntpoint(dir->index, dir->qid);
			if (mnt != NULL) {
				dir->qid = mnt->qid;
				dir->index = mnt->index;
			}
...
int stat(const char *path, dir_t *dir)
{
	...
	r = devtab[c->index]->stat(c, path, dir);
	chanclose(c);
...
int read(int fd, void *buf, int n)
{
	...
	return devtab[c->index]->read(c, buf, n);
}
...
int write(int fd, void *buf, int n)
{
	...
	return devtab[c->index]->write(c, buf, n);
}
...
int seek(int fd, long off, int whence)
{
	...
	return devtab[c->index]->seek(c, off, whence);
}
