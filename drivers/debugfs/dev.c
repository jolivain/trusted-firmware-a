 ...
 ******************************************************************************/
static const char *next(const char *path, char *elem)
{
	int n;
	const char *t;
...
static int get_device_index(int id)
{
	...
	for (i = 0, dp = devtab; *dp && (*dp)->id != id; ++dp) {
		i++;
	}
...
/*******************************************************************************
 * This function checks whether a channel (identified by its device index and qid)
 * is registered as a mount point.
...
chan_t *path_to_channel(const char *path, int mode)
{
	...
	for (s = next(s, elem); *elem; s = next(s, elem)) {
		...
		if (devtab[c->index]->walk(c, elem) < 0) {
			channel_close(c);
			goto notfound;
		}

		mnt = mount_point_to_channel(c->index, c->qid);
		if (mnt != NULL) {
			clone(mnt, c);
		}
	}
 ...
 ******************************************************************************/
void mkentry(chan_t *channel, dir_t *dir,
...
int devgen(chan_t *channel, const dirtab_t *tab, int ntab, int n, dir_t *dir)
{
	...
	dp = &tab[n];
	mkentry(channel, dir, dp->name, dp->length, dp->qid, dp->perm);
	return 1;
}
 ...
 ******************************************************************************/
static int addmntpoint(chan_t *channel, const char *new)
{
	int i;
	chan_t *cn;
	struct mount_point *mp;
...
int mount(const char *srv, const char *where, const char *spec)
{
	...
	cs = path_to_channel(srv, O_RDWR);
	if (cs == NULL) {
		goto err0;
	}

	c = devtab[cs->index]->mount(cs, spec);
	if (c == NULL) {
		goto err1;
	}
