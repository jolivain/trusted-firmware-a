...
struct chan {
	long		offset;
	qid_t		qid;
	unsigned char	type;	/* FIXME: in effect this is the device index */
				/* in devtab, so we might rename this field  */
...
void mkentry(chan_t *c, dir_t *dir, const char *name, long length, qid_t qid,
		    unsigned int mode);
void devlink(void);
