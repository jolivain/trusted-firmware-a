 ...
 * It avoids code breakage if the crc_32 field gets moved due to
 * the structure member(s) addition in the future.
 */
...
/*******************************************************************************
 * Load verified copy of FWU metadata image present lying in the platform NV
 * storage into local FWU metadata structure.
 * Also, update platform I/O policies with the offset address and length of
 * firmware-updated images lying in the platform NV storage.
 ******************************************************************************/
void fwu_init(void)
{
	...
	plat_fwu_set_images_source(metadata.active_index);
