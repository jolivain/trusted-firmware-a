...
static inline int is_valid_header(fw_enc_hdr_t *header)
{
...
static int enc_file_len(io_entity_t *entity, size_t *length)
{
	 ...
	 */
	if (*length < sizeof(fw_enc_hdr_t))
		return -EIO;

	*length -= sizeof(fw_enc_hdr_t);
			 ...
			 size_t *length_read)
{
	int result;
	fw_enc_hdr_t header;
	enum fw_enc_status fw_enc_status = FW_ENC_WITH_SSK;
	size_t bytes_read;
