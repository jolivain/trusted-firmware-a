...
static unsigned int fip_enc_status = FIP_ENC_DISABLED;
...
static int fip_dev_init(io_dev_info_t *dev_info, const uintptr_t init_params)
{
	...
	fip_enc_status = header.flags & FIP_ENC_STATUS_FLAG_MASK;
			 ...
			 io_entity_t *entity)
{
	...
	if (found_file == 1) {
		...
		if ((fip_enc_status == FIP_ENC_WITH_SSK ||
		     fip_enc_status == FIP_ENC_WITH_BSSK) &&
			  ...
			  size_t *length_read)
{
	...
	if (fp->file_encrypted) {
		...
		assert(dec_data.iv_size <= FIP_MAX_IV_SIZE);
		assert(dec_data.tag_size <= FIP_MAX_TAG_SIZE);
	...
	if (fp->file_encrypted) {
		...
		result = plat_get_fip_encryption_key(fip_enc_status, key,
						     &key_len);
		if (result != 0) {
		...
		result = crypto_mod_auth_decrypt(dec_data.dec_algo,
						 ...
						 key, key_len, dec_data.iv,
						 dec_data.iv_len, dec_data.tag,
						 dec_data.tag_len);
		if (result != 0) {
