...
static enum fw_enc_status fw_enc_status = FW_ENC_DISABLED;
...
static int fip_dev_init(io_dev_info_t *dev_info, const uintptr_t init_params)
{
	...
	assert(fw_enc_status != FW_ENC_RESERVED);
			 ...
			 io_entity_t *entity)
{
	...
	if (found_file == 1) {
		...
		if ((fw_enc_status == FW_ENC_WITH_SSK ||
		     fw_enc_status == FW_ENC_WITH_BSSK) &&
		    (current_file.entry.flags & FIP_ENTRY_ENC_FLAG_MASK)) {
#if TRUSTED_BOARD_BOOT
			current_file.file_encrypted = true;
#else
			/* File encryption only supported with TBB enabled */
			assert(fw_enc_status == 0);
...
#if TRUSTED_BOARD_BOOT
	/*
	 * Decryption data prepend the file and are not considered
	 * part of the payload.
	 */
			  ...
			  size_t *length_read)
{
	...
	if (fp->file_encrypted) {
		...
		assert(dec_data.iv_len <= FIP_MAX_IV_SIZE);
		assert(dec_data.tag_len <= FIP_MAX_TAG_SIZE);
	...
	if (fp->file_encrypted) {
		...
		size_t key_len = FIP_MAX_ENC_KEY_SIZE;
		unsigned int key_flags = 0;

		result = plat_get_fip_enc_key_info(fw_enc_status, key,
						   &key_len, &key_flags);
		if (result != 0) {
