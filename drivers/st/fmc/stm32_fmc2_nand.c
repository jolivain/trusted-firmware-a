...
static void stm32_fmc2_nand_setup_timing(void)
{
	...
	tims.tset_mem = MIN(MAX(timing, 1UL),
			    (unsigned long)FMC2_PMEM_PATT_TIMING_MASK);
	...
	tims.thold_mem = MIN(MAX(timing, 1UL),
			     (unsigned long)FMC2_PMEM_PATT_TIMING_MASK);
	...
	tims.tset_att = MIN(MAX(timing, 1UL),
			    (unsigned long)FMC2_PMEM_PATT_TIMING_MASK);
	...
	tims.thold_att =  MIN(MAX(timing, 1UL),
			      (unsigned long)FMC2_PMEM_PATT_TIMING_MASK);
