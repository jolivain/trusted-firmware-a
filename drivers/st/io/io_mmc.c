...
static size_t (*emmc_read_blocks)(int lba, uintptr_t buf, size_t size);
...
static int mmc_dev_open(const uintptr_t init_params, io_dev_info_t **dev_info)
{
	unsigned char use_boot_part = init_params;

	assert(dev_info != NULL);
	*dev_info = (io_dev_info_t *)&mmc_dev_info;

	emmc_read_blocks = use_boot_part == 0 ?
			  ...
			  size_t length, size_t *length_read)
{
	...
	for (retries = 0U; retries < 3U; retries++) {
		*length_read = emmc_read_blocks(seek_offset / MMC_BLOCK_SIZE,
				buffer, length);
