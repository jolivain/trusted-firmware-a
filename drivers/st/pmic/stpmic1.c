...
static const uint16_t buck3_voltage_table[] = {
	...
	1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1100, 1100, 1100, 1100,
	1200, 1200, 1200, 1200, 1300, 1300, 1300, 1300, 1400, 1400, 1400, 1400,
...
static const uint16_t buck4_voltage_table[] = {
	...
	875,  900,  925,  950,	975,  1000, 1025, 1050, 1075, 1100, 1125,
	1150, 1175, 1200, 1225, 1250, 1275, 1300, 1300, 1350, 1350, 1400,
	1400, 1450, 1450, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200,
...
static const uint16_t ldo3_voltage_table[] = {
	1700, 1700, 1700, 1700, 1700, 1700, 1700, 1700, 1700, 1800, 1900,
	2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000,
	3100, 3200, 3300, 3300, 3300, 3300, 3300, 3300, 3300,
};
...
int stpmic1_regulator_pull_down_set(const char *name)
{
	...
	if (regul->pull_down_reg != 0) {
		return stpmic1_register_update(
			regul->pull_down_reg, BIT(regul->pull_down),
			LDO_BUCK_PULL_DOWN_MASK << regul->pull_down);
	}
...
int stpmic1_regulator_mask_reset_set(const char *name)
{
	...
	return stpmic1_register_update(
		regul->mask_reset_reg, BIT(regul->mask_reset),
...
int stpmic1_regulator_icc_set(const char *name)
{
	...
	return stpmic1_register_update(regul->icc_reg, BIT(regul->icc_mask),
				       BIT(regul->icc_mask));
}
...
int stpmic1_regulator_bypass_mode_set(const char *name)
{
	...
	return stpmic1_register_update(LDO3_CONTROL_REG, LDO3_BYPASS,
				       LDO3_BYPASS | LDO_VOLTAGE_MASK);
}
...
int stpmic1_active_discharge_mode_set(const char *name)
{
	...
	if (strncmp(name, "pwr_sw2", 8) == 0) {
		return stpmic1_register_update(
			USB_CONTROL_REG, SW_OUT_DISCHARGE, SW_OUT_DISCHARGE);
	}
...
void stpmic1_dump_regulators(void)
{
	...
	for (i = 0U; i < MAX_REGUL; i++) {
		...
		VERBOSE("PMIC regul %s: %sable, %dmV", name,
