...
#define SDMMC_DCTRLR_CLEAR_MASK                                         \
	(SDMMC_DCTRLR_DTEN | SDMMC_DCTRLR_DTDIR | SDMMC_DCTRLR_DTMODE | \
...
#define SDMMC_STATIC_FLAGS                                                 \
	(SDMMC_STAR_CCRCFAIL | SDMMC_STAR_DCRCFAIL | SDMMC_STAR_CTIMEOUT | \
	 SDMMC_STAR_DTIMEOUT | SDMMC_STAR_TXUNDERR | SDMMC_STAR_RXOVERR |  \
	 SDMMC_STAR_CMDREND | SDMMC_STAR_CMDSENT | SDMMC_STAR_DATAEND |    \
	 SDMMC_STAR_DBCKEND | SDMMC_STAR_IDMATE | SDMMC_STAR_IDMABTC)
...
static int stm32_sdmmc2_send_cmd_req(struct mmc_cmd *cmd)
{
	...
	case MMC_CMD(18):
		...
		if (sdmmc2_params.use_dma) {
			flags_data |= SDMMC_STAR_DCRCFAIL |
				      SDMMC_STAR_DTIMEOUT | SDMMC_STAR_DATAEND |
				      SDMMC_STAR_RXOVERR | SDMMC_STAR_IDMATE;
		}
		break;
	case MMC_ACMD(41):
	...
	if ((status &
	     (SDMMC_STAR_DTIMEOUT | SDMMC_STAR_DCRCFAIL | SDMMC_STAR_TXUNDERR |
	      SDMMC_STAR_RXOVERR | SDMMC_STAR_IDMATE)) != 0U) {
...
static int stm32_sdmmc2_set_ios(unsigned int clk, unsigned int width)
{
	...
	mmio_write_32(base + SDMMC_CLKCR,
		      SDMMC_CLKCR_HWFC_EN | clock_div | bus_cfg |
			      sdmmc2_params.negedge | sdmmc2_params.pin_ckin);
...
static int stm32_sdmmc2_prepare(int lba, uintptr_t buf, size_t size)
{
	...
	if (sdmmc2_params.use_dma) {
		mmio_write_32(base + SDMMC_IDMACTRLR, SDMMC_IDMACTRLR_IDMAEN);
	...
	mmio_clrsetbits_32(base + SDMMC_DCTRLR, SDMMC_DCTRLR_CLEAR_MASK,
			   data_ctrl);
...
static int stm32_sdmmc2_dt_get_config(void)
{
	...
	sdmmc2_params.vmmc_regu =
		regulator_get_by_supply_name(fdt, sdmmc_node, "vmmc");
...
int stm32_sdmmc2_mmc_init(struct stm32_sdmmc2_params *params)
{
	...
	if ((int)sdmmc2_params.reset_id >= 0) {
		...
		rc = stm32mp_reset_assert(sdmmc2_params.reset_id,
					  TIMEOUT_US_1_MS);
		if (rc != 0) {
			panic();
		}
		udelay(2);
		rc = stm32mp_reset_deassert(sdmmc2_params.reset_id,
					    TIMEOUT_US_1_MS);
		if (rc != 0) {
			panic();
		}
