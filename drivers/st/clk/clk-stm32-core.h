...
struct stm32_clk_ops {
	unsigned long (*recalc_rate)(struct stm32_clk_priv *priv, int id,
				     unsigned long rate);
...
int clk_stm32_divider_set_rate(struct stm32_clk_priv *priv, int id,
...
#define STM32_DIV(idx, _binding, _parent, _flags, _div_id)                     \
	...
	{                                                                      \
		.binding = (_binding), .parent = (_parent), .flags = (_flags), \
		.clock_cfg =                                                   \
			&(struct clk_stm32_div_cfg){                           \
				.id = (_div_id),                               \
			},                                                     \
...
unsigned long fixed_factor_recalc_rate(struct stm32_clk_priv *priv, int _idx,
				       unsigned long prate);

#define FIXED_FACTOR(idx, _idx, _parent, _mult, _div)   \
	[(idx)] = (struct clk_stm32)                    \
	{                                               \
		.binding = (_idx), .parent = (_parent), \
		.clock_cfg =                            \
			&(struct fixed_factor_cfg){     \
				.mult = (_mult),        \
				.div = (_div),          \
...
#define STM32_MUX(idx, _binding, _mux_id, _flags)                            \
	[(idx)] = (struct clk_stm32)                                         \
	{                                                                    \
		.binding = (_binding), .parent = (MUX(_mux_id)),             \
		.flags = (_flags), .clock_cfg = NULL, .ops = (&clk_mux_ops), \
	}
...
#define CLK_FIXED_RATE(idx, _binding, _rate)                    \
	[(idx)] = (struct clk_stm32)                            \
	{                                                       \
		.binding = (_binding), .parent = (CLK_IS_ROOT), \
		.clock_cfg =                                    \
			&(struct clk_stm32_fixed_rate_cfg){     \
				.rate = (_rate),                \
			},                                      \
...
#define BYPASS(_offset, _bit_byp, _bit_digbyp)              \
	&(struct stm32_clk_bypass)                          \
	{                                                   \
		.offset = (_offset), .bit_byp = (_bit_byp), \
...
#define CSS(_offset, _bit_css)                              \
	&(struct stm32_clk_css)                             \
	{                                                   \
		.offset = (_offset), .bit_css = (_bit_css), \
	}

#define DRIVE(_offset, _shift, _width, _default)                  \
	&(struct stm32_clk_drive)                                 \
	{                                                         \
		.offset = (_offset), .drv_shift = (_shift),       \
		.drv_width = (_width), .drv_default = (_default), \
	}
...
#define OSCILLATOR(idx_osc, _id, _name, _gate_id, _gate_rdy_id, _bypass, _css, \
	...
	{                                                                      \
		.name = (_name), .id_clk = (_id), .gate_id = (_gate_id),       \
		.gate_rdy_id = (_gate_rdy_id), .bypass = (_bypass),            \
		.css = (_css), .drive = (_drive),                              \
	}
