...
static void _tzc400_clear_it(long base, uint32_t filter)
{
	mmio_write_32(base + INT_CLEAR, BIT(filter));
}

static uint32_t _tzc400_get_int_by_filter(long base, uint32_t filter)
{
	return mmio_read_32(base + INT_STATUS) & BIT(filter);
}

#if DEBUG
static unsigned long _tzc400_get_fail_address(long base, uint32_t filter)
{
...
static uint32_t _tzc400_get_fail_id(long base, uint32_t filter)
{
	return mmio_read_32(base + FAIL_ID + (filter * FILTER_OFFSET));
}

static uint32_t _tzc400_get_fail_control(long base, uint32_t filter)
{
	return mmio_read_32(base + FAIL_CONTROL_OFF + (filter * FILTER_OFFSET));
}

static void _tzc400_dump_fail_filter(long base, uint32_t filter)
{
...
void tzc400_it_handler(void)
{
	...
	for (filter = 0U; (filter < tzc400.num_filters); filter++) {
		if (_tzc400_get_int_by_filter(tzc400.base, filter)) {
