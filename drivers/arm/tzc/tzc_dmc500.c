...
_tzc_dmc500_read_region_attr_0(uintptr_t dmc_si_base, unsigned int region_no)
{
	return mmio_read_32(
		dmc_si_base +
	...
	for (dmc_inst = 0; dmc_inst < g_driver_data->dmc_count; dmc_inst++) {
		...
		for (sys_if = 0; sys_if < g_sys_if_count; sys_if++)
			_tzc_dmc500_configure_region0(
				DMC_INST_SI_BASE(dmc_inst, sys_if), sec_attr,
				nsaid_permissions);
	}
	...
	for (dmc_inst = 0; dmc_inst < g_driver_data->dmc_count; dmc_inst++) {
		...
		for (sys_if = 0; sys_if < g_sys_if_count; sys_if++)
			_tzc_dmc500_configure_region(
				DMC_INST_SI_BASE(dmc_inst, sys_if),
				TZC_DMC500_REGION_ATTR_F_EN_MASK, region_no,
				region_base, region_top, sec_attr,
				nsaid_permissions);
	}
 ...
 */
static void
validate_plat_driver_data(const tzc_dmc500_driver_data_t *plat_driver_data)
{
