...
#define DEFINE_TZC_COMMON_WRITE_ACTION(fn_name, macro_name)                  \
	static inline void _tzc##fn_name##_write_action(uintptr_t base,      \
							unsigned int action) \
	{                                                                    \
		mmio_write_32(base + TZC_##macro_name##_ACTION_OFF, action); \
	}
...
#define DEFINE_TZC_COMMON_WRITE_REGION_ATTRIBUTES(fn_name, macro_name)         \
	static inline void _tzc##fn_name##_write_region_attributes(            \
		uintptr_t base, unsigned int region_no, unsigned int attr)     \
	{                                                                      \
		mmio_write_32(base +                                           \
				      TZC_REGION_OFFSET(                       \
					      TZC_##macro_name##_REGION_SIZE,  \
...
#define DEFINE_TZC_COMMON_WRITE_REGION_ID_ACCESS(fn_name, macro_name)         \
	static inline void _tzc##fn_name##_write_region_id_access(            \
		uintptr_t base, unsigned int region_no, unsigned int val)     \
	{                                                                     \
		mmio_write_32(                                                \
			base +                                                \
				TZC_REGION_OFFSET(                            \
...
#define DEFINE_TZC_COMMON_UPDATE_FILTERS(fn_name, macro_name)            \
	static inline void _tzc##fn_name##_update_filters(               \
		uintptr_t base, unsigned int region_no,                  \
		unsigned int nbfilters, unsigned int filters)            \
	{                                                                \
...
#define DEFINE_TZC_COMMON_CONFIGURE_REGION0(fn_name)                      \
	static void _tzc##fn_name##_configure_region0(                    \
		uintptr_t base, unsigned int sec_attr,                    \
	...
	{                                                                 \
		...
		VERBOSE("TrustZone : Configuring region 0 "               \
			...
			" ns_devs=0x%x)\n",                               \
			base, sec_attr, ns_device_access);                \
                                                                          \
		/* Set secure attributes on region 0 */                   \
		_tzc##fn_name##_write_region_attributes(                  \
		...
		/* to access region 0.				   */                           \
		/***************************************************/     \
		_tzc##fn_name##_write_region_id_access(base, 0,           \
						       ns_device_access); \
	}
...
#define DEFINE_TZC_COMMON_CONFIGURE_REGION(fn_name)                            \
	...
	{                                                                      \
		...
		/* 64bit.					   */                                            \
		/***************************************************/          \
		_tzc##fn_name##_write_region_base(base, region_no,             \
						  region_base);                \
		_tzc##fn_name##_write_region_top(base, region_no, region_top); \
                                                                               \
		...
		/***************************************************/          \
		_tzc##fn_name##_write_region_id_access(base, region_no,        \
						       nsaid_permissions);     \
	}
