...
#if defined(PLAT_ARM_SDS_MEM_BASE_2)
#define SDS_REGION_CNT	2
#elif defined(PLAT_ARM_SDS_MEM_BASE)
#define SDS_REGION_CNT	1
#else
#error "PLAT_ARM_SDS_MEM_BASE is not defined"
#endif

/*
 * Variables used to track and maintain the state of the memory regions reserved
...
static struct sds_region regions[SDS_REGION_CNT] = {
	{ .base = PLAT_ARM_SDS_MEM_BASE },
#if SDS_REGION_CNT == 2
	{ .base = PLAT_ARM_SDS_MEM_BASE_2 },
#endif
};
...
static int sds_struct_is_valid(unsigned int region_id, uintptr_t header)
{
	...
	if (struct_size > regions[region_id].size) {
		return SDS_ERR_FAIL;
	}
...
static int validate_sds_struct_headers(unsigned int region_id)
{
	...
	uintptr_t header;
	uintptr_t sds_mem_base = regions[region_id].base;
...
int sds_init(unsigned int region_id)
{
	...
	regions[region_id].size = GET_SDS_REGION_SIZE(sds_mem_base);
	if (regions[region_id].size > PLAT_ARM_SDS_MEM_SIZE_MAX) {
