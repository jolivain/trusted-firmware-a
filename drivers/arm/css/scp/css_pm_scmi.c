		...
		unsigned int *scmi_domain_id, unsigned int *scmi_channel_id)
{
	...
	*scmi_domain_id = composite_id & SCMI_DOMAIN_ID_MASK;
	*scmi_channel_id = ((composite_id & SCMI_CHANNEL_ID_MASK)
...
void css_scp_suspend(const struct psci_power_state *target_state)
{
	...
	if (css_system_pwr_state(target_state) == ARM_LOCAL_STATE_OFF) {
		...
		for (i = PLAT_ARM_SCMI_CHANNEL_COUNT; i > 0; i--) {
			ret = scmi_sys_pwr_state_set(scmi_handles[i-1],
					SCMI_SYS_PWR_FORCEFUL_REQ, SCMI_SYS_PWR_SUSPEND);
			if (ret != SCMI_E_SUCCESS) {
				ERROR("SCMI system power domain suspend return 0x%x \
						unexpected\n", ret);
				panic();
			}
		}
...
int css_scp_get_power_state(u_register_t mpidr, unsigned int power_level)
{
	...
	unsigned int channel_id, domain_id;
...
void __dead2 css_scp_system_off(int state)
{
	 ...
	 */
	ret = scmi_sys_pwr_state_set(scmi_handles[PLAT_ARM_SCMI_DEFAULT_CHANNEL_ID],
...
void __init plat_arm_pwrc_setup(void)
{
	...
	for (idx = 0; idx < PLAT_ARM_SCMI_CHANNEL_COUNT; idx++) {
		...
		scmi_channels[idx].channel_id = idx;
...
const plat_psci_ops_t *css_scmi_override_pm_ops(plat_psci_ops_t *ops)
{
	uint32_t msg_attr;
	int ret;
	void *scmi_handle = scmi_handles[PLAT_ARM_SCMI_DEFAULT_CHANNEL_ID];
