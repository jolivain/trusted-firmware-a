...
#define get_node_id(id_reg) \
	(((id_reg) >> REGION_NODE_ID_SHIFT) & REGION_NODE_ID_MASK)

#define REGION_NODE_TYPE_SHIFT 0
#define REGION_NODE_TYPE_MASK 0x1f
#define get_node_type(id_reg) \
	(((id_reg) >> REGION_NODE_TYPE_SHIFT) & REGION_NODE_TYPE_MASK)
...
#define MAKE_HNF_SAM_CTRL_VALUE(sn0, sn1, sn2, tab0, tab1, three_sn_en)   \
	((((sn0)&HNF_SAM_CTRL_SN_ID_MASK) << HNF_SAM_CTRL_SN0_ID_SHIFT) | \
	 ...
	 (((three_sn_en)&HNF_SAM_CTRL_3SN_ENB_MASK)                       \
...
#define FOR_EACH_BIT(bit_pos, bit_map)                    \
	for (bit_pos = __builtin_ctzll(bit_map); bit_map; \
...
#define FOR_EACH_PRESENT_REGION_ID(region_id, bit_map)    \
	for (unsigned long long region_id_limit =         \
		     count_set_bits(bit_map) + region_id; \
	     region_id < region_id_limit; region_id++)
...
#define CCN_GET_MN_NODEID_MAP(periphbase) \
	(1 << get_node_id(                \
		 ccn_reg_read(periphbase, MN_REGION_ID, REGION_ID_OFFSET)))
