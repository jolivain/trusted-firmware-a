...
#define ETHOSN_CORE_SEC_REG(core_addr, reg_offset) (core_addr + reg_offset)
					  ...
					  uint32_t asset_alloc_idx)
{
	...
	const uint32_t streams[9] = { main_alloc->firmware.stream_id,
	...
	for (i = 0U; i < ARRAY_SIZE(streams); ++i) {
		const uintptr_t reg_addr =
			SEC_MMUSID_REG_BASE + (SEC_MMUSID_OFFSET * i);
...
uintptr_t ethosn_smc_handler(uint32_t smc_fid, u_register_t core_addr,
			     u_register_t asset_alloc_idx, u_register_t x3,
