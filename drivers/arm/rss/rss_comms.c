					  ...
					  psa_outvec *out_vec, size_t out_len)
{
	...
	VERBOSE("protocol_ver=%d\n", msg.header.protocol_ver);
	VERBOSE("seq_num=%d\n", msg.header.seq_num);
	VERBOSE("client_id=%d\n", msg.header.client_id);
	for (idx = 0; idx < in_len; idx++) {
		VERBOSE("in_vec[%d].len=%d\n", idx, in_vec[idx].len);
		VERBOSE("in_vec[%d].buf=%p\n", idx, in_vec[idx].base);
	}
	...
	VERBOSE("protocol_ver=%d\n", reply.header.protocol_ver);
	VERBOSE("seq_num=%d\n", reply.header.seq_num);
	VERBOSE("client_id=%d\n", reply.header.client_id);
	...
	for (idx = 0; idx < out_len; idx++) {
		VERBOSE("out_vec[%d].len=%d\n", idx, out_vec[idx].len);
		VERBOSE("out_vec[%d].buf=%p\n", idx, out_vec[idx].base);
	}
