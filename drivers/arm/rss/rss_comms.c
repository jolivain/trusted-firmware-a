 ...
 * if functions not being reentrant becomes a problem. msg and reply are never used at the same time
 * so a union is used to save memory.
 */

union __packed rss_comms_io_buffer_t {
				       ...
				       const psa_outvec *out_vec, size_t out_len)
{
	...
	/* Use v0 if we can pack into one message and reply, else use v1. A v0 message has 20 bytes
	 * of header with 4 bytes of MHU layer overhead, and a reply has 16 bytes of header. The
	 * largest RSS MHU can transmit 64 bytes in a single transaction, hence 40 and 44 being the
	 * amount of payload that will fit in a single message.
	 *
	 * TODO tune this with real performance numbers, it's possible a v1 message is less
	 * performant than multiple v0 messages due to ATU configuration costs to allow access to
	 * the pointers.
	 */
