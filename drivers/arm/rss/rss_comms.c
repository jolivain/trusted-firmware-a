				       ...
				       const psa_outvec *out_vec, size_t out_len)
{
	...
	comms_embed_msg_min_size = sizeof(struct serialized_rss_comms_header_t) +
				   sizeof(struct serialized_rss_comms_msg_embed_t) -
				   RSS_COMMS_PAYLOAD_MAX_SIZE;

	comms_embed_reply_min_size = sizeof(struct serialized_rss_comms_header_t) +
				     sizeof(struct serialized_rss_comms_reply_embed_t) -
				     RSS_COMMS_PAYLOAD_MAX_SIZE;

	/* Use embed if we can pack into one message and reply, else use
	 * pointer_access. A embed message has 20 bytes of header with 4 bytes
	 * of MHU layer overhead, and a reply has 16 bytes of header. The
	 * largest RSS MHU can transmit 64 bytes in a single transaction, hence
	 * 40 and 44 being the amount of payload that will fit in a single
	 * message.
	 *
