...
static bool is_spi_interrupt(unsigned int id);
...
/*
 * Helper macros to save and restore GICD registers corresponding to
 * their numbers to and from the context
 */
#define SAVE_GICD_REG(REG, name)	\
	dist_ctx->gicd_##name[i] =	\
			mmio_read_32(gicd_base + GICD_##REG + (i << 2))

#define SAVE_GICD_EREG(REG, name)			\
	dist_ctx->gicd_##name[i + spi_regs_num] =	\
			mmio_read_32(gicd_base + GICD_##REG##E + (i << 2))

#define RESTORE_GICD_REG(REG, name)				\
	mmio_write_32(gicd_base + GICD_##REG + (i << 2),	\
			dist_ctx->gicd_##name[i])

#define RESTORE_GICD_EREG(REG, name)				\
	mmio_write_32(gicd_base + GICD_##REG##E + (i << 2),	\
			dist_ctx->gicd_##name[i + spi_regs_num])
...
void __init gicv3x_driver_init(const gicv3x_driver_data_t *plat_driver_data)
{
	 ...
	 */
	bool gicv2_compat = ((gicd_read_ctlr(plat_driver_data->gicd_base) >>
				CTLR_ARE_S_SHIFT) & CTLR_ARE_S_MASK) == 0;
#endif
	...
	gicd_typer_reg = gicd_read_typer(plat_driver_data->gicd_base);

	/*
	 * Set Redistributor base offset.
	 * Check whether Direct Virtual LPI injection is supported.
	 * For GICv3 DVIS[18] field is RES0.
	 */
	gicr_base_offset = (gicd_typer_reg & TYPER_DVIS) == 0 ?
				1U << GICR_PCPUBASE_SHIFT_V3 :
				1U << GICR_PCPUBASE_SHIFT_V4;
	/*
...
/******************************************************************************
 * This function checks the interrupt id and returns true for (E)SPIs and false
 * for SGIs and (E)PPIs IDs.
 *****************************************************************************/
static bool is_spi_interrupt(unsigned int id)
{
	/* SGIs, PPIs and EPPIs: 0-31, 1056-1119 */
	if ((id <= MAX_PPI_ID) ||
	   ((id >= MIN_EPPI_ID) && (id <= max_ppi_intid))) {
		return false;
	}

	/* SPIs and ESPIs: 32-1019, 4096-5119 */
	if ((id <= max_spi_intid) ||
	   ((id >= MIN_ESPI_ID) && (id <= max_espi_intid))) {
		return true;
	}
