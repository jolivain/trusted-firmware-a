...
/*******************************************************************************
 * Helper function to get the exclusive limit of SPI INTID.
 ******************************************************************************/
unsigned int gicv3_get_spi_limit(uintptr_t gicd_base)
{
	...
	/* The exclusive SPI INTID limit is 32 * (GICD_TYPER.ITLinesNumber+1) */
	spi_limit = ((typer_reg & TYPER_IT_LINES_NO_MASK) + 1U) << 5;

	/*
	 * The GICv3 archetecture allows GICD_TYPER.ITLinesNumber to be 31, so
...
#if GIC_EXT_INTID
/*******************************************************************************
 * Helper function to get the exclusive limit of ESPI INTID.
 ******************************************************************************/
unsigned int gicv3_get_espi_limit(uintptr_t gicd_base)
{
	unsigned int espi_limit = 0U;
	unsigned int typer_reg = gicd_read_typer(gicd_base);

	/* Check if extended SPI range is implemented */
	if ((typer_reg & TYPER_ESPI) != 0U) {
		/*
		 * The exclusive ESPI INTID limit is
		 * 32 * (GICD_TYPER.ESPI_range + 1) + 4096
		 */
		espi_limit = ((((typer_reg >> TYPER_ESPI_RANGE_SHIFT) &
