...
uintptr_t gicv3_derive_gicd_base(uint32_t spi_id, uintptr_t gicd_base)
{
...
void gicv3_spis_config_defaults(uintptr_t gicd_base)
{
	...
	for (i = MIN_SPI_ID; i < num_ints; i += (1U << IGROUPR_SHIFT)) {
		gicd_write_igroupr(gicv3_derive_gicd_base(i, gicd_base), i, ~0U);
	}
	...
	for (i = MIN_SPI_ID; i < num_ints; i += (1U << IPRIORITYR_SHIFT)) {
		gicd_write_ipriorityr(gicv3_derive_gicd_base(i, gicd_base), i, GICD_IPRIORITYR_DEF_VAL);
	}
	...
	for (i = MIN_SPI_ID; i < num_ints; i += (1U << ICFGR_SHIFT)) {
		gicd_write_icfgr(gicv3_derive_gicd_base(i, gicd_base), i, 0U);
	}
		...
		unsigned int interrupt_props_num)
{
	...
	for (i = 0U; i < interrupt_props_num; i++) {
		...
		gicd_clr_igroupr(gicv3_derive_gicd_base(intr_num, gicd_base), intr_num);
		...
		if (current_prop->intr_grp == INTR_GROUP1S) {
			gicd_set_igrpmodr(gicv3_derive_gicd_base(intr_num, gicd_base), intr_num);
			ctlr_enable |= CTLR_ENABLE_G1S_BIT;
		} else {
			gicd_clr_igrpmodr(gicv3_derive_gicd_base(intr_num, gicd_base), intr_num);
			ctlr_enable |= CTLR_ENABLE_G0_BIT;
		}

		/* Set interrupt configuration */
		gicd_set_icfgr(gicv3_derive_gicd_base(intr_num, gicd_base),
			intr_num, current_prop->intr_cfg);

		/* Set the priority of this interrupt */
		gicd_set_ipriorityr(gicv3_derive_gicd_base(intr_num, gicd_base),
			intr_num, current_prop->intr_pri);

		/* Target (E)SPIs to the primary CPU */
		gic_affinity_val =
			gicd_irouter_val_from_mpidr(read_mpidr(), 0U);
		gicd_write_irouter(gicv3_derive_gicd_base(intr_num, gicd_base),
			intr_num, gic_affinity_val);

		/* Enable this interrupt */
		gicd_set_isenabler(gicv3_derive_gicd_base(intr_num, gicd_base),
			intr_num);
	}
