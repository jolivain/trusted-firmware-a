...
#else	/* GICv3 */
#define	GICD_OFFSET_8(REG, id)	\
	(uintptr_t)(GICD_##REG + (id))

#define	GICD_OFFSET(REG, id)	\
	(uintptr_t)(GICD_##REG + (((id) >> REG##_SHIFT) << 2))

#define	GICD_OFFSET_64(REG, id)	\
	(uintptr_t)(GICD_##REG + (((id) >> REG##_SHIFT) << 3))
...
#define	GICR_OFFSET_8(REG, id)				\
	(uintptr_t)(((id) <= MAX_PPI_ID) ?		\
	GICR_##REG + (id) :				\
	GICR_##REG + (id) - (MIN_EPPI_ID - MIN_SPI_ID))

#define GICR_OFFSET(REG, id)				\
	(uintptr_t)(((id) <= MAX_PPI_ID) ?		\
	GICR_##REG + (((id) >> REG##_SHIFT) << 2) :	\
	GICR_##REG + ((((id) - (MIN_EPPI_ID - MIN_SPI_ID)) >> REG##_SHIFT) \
								<< 2))
#else	/* GICv3 */
#define	GICR_OFFSET_8(REG, id)	\
	(uintptr_t)(GICR_##REG + (id))

#define GICR_OFFSET(REG, id)	\
	(uintptr_t)(GICR_##REG + (((id) >> REG##_SHIFT) << 2))
#endif
...
{
	return mmio_read_32(base + GICR_ICENABLER0 + (reg_num << 2));
}

static inline void gicr_write_icenabler(uintptr_t base, unsigned int reg_num,
					unsigned int val)
{
	mmio_write_32(base + GICR_ICENABLER0 + (reg_num << 2), val);
}
...
{
	return mmio_read_32(base + GICR_ICFGR0 + (reg_num << 2));
}

static inline void gicr_write_icfgr(uintptr_t base, unsigned int reg_num,
					unsigned int val)
{
	mmio_write_32(base + GICR_ICFGR0 + (reg_num << 2), val);
}
...
static inline unsigned int gicr_read_igroupr(uintptr_t base,
						unsigned int reg_num)
{
	return mmio_read_32(base + GICR_IGROUPR0 + (reg_num << 2));
}

static inline void gicr_write_igroupr(uintptr_t base, unsigned int reg_num,
						unsigned int val)
{
	mmio_write_32(base + GICR_IGROUPR0 + (reg_num << 2), val);
}
...
static inline unsigned int gicr_read_igrpmodr(uintptr_t base,
						unsigned int reg_num)
{
	return mmio_read_32(base + GICR_IGRPMODR0 + (reg_num << 2));
}

static inline void gicr_write_igrpmodr(uintptr_t base, unsigned int reg_num,
				       unsigned int val)
{
	mmio_write_32(base + GICR_IGRPMODR0 + (reg_num << 2), val);
}
...
static inline unsigned int gicr_read_isactiver(uintptr_t base,
						unsigned int reg_num)
{
	return mmio_read_32(base + GICR_ISACTIVER0 + (reg_num << 2));
}

static inline void gicr_write_isactiver(uintptr_t base, unsigned int reg_num,
					unsigned int val)
{
	mmio_write_32(base + GICR_ISACTIVER0 + (reg_num << 2), val);
}
...
static inline unsigned int gicr_read_isenabler(uintptr_t base,
						unsigned int reg_num)
{
	return mmio_read_32(base + GICR_ISENABLER0 + (reg_num << 2));
}

static inline void gicr_write_isenabler(uintptr_t base, unsigned int reg_num,
					unsigned int val)
{
	mmio_write_32(base + GICR_ISENABLER0 + (reg_num << 2), val);
}
...
static inline unsigned int gicr_read_ispendr(uintptr_t base,
						unsigned int reg_num)
{
	return mmio_read_32(base + GICR_ISPENDR0 + (reg_num << 2));
}

static inline void gicr_write_ispendr(uintptr_t base, unsigned int reg_num,
						unsigned int val)
{
	mmio_write_32(base + GICR_ISPENDR0 + (reg_num << 2), val);
}
