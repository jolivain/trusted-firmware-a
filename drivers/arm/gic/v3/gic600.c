 ...
 * for operating through the core
 */
#define PWRR_ON		(0 << PWRR_RDPD_SHIFT) | (0 << PWRR_RDAG_SHIFT)
#define PWRR_OFF	(1 << PWRR_RDPD_SHIFT) | (0 << PWRR_RDAG_SHIFT)
...
static void gic600_pwr_on(uintptr_t base)
{
	do {	/* Wait until group not transitioning */
		 ...
		 */
	} while ((gicr_read_pwrr(base) & PWRR_RDPD) != 0);
}
