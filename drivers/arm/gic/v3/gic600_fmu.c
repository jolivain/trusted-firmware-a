...
/*
 * Driver for GIC-600 Fault Management Unit
 */
...
#include <drivers/arm/gic600_fmu.h>

/* GIC-600 FMU specific register offsets */

/* GIC-600 FMU specific macros */
 ...
 *
 * 1. enable error detection for error records that are present on the
 *    platform
 * 2. enable MBIST REQ safety mechanism for error records that are present
 *    on the platform
 */
void gic600_fmu_init(uint64_t base, uint64_t blk_present_mask)
{
	...
	for (unsigned int i = 0; i < num_blk; i++) {
		...
		/* Enable error detection, if it is disabled */
		if ((errctlr & FMU_ERRCTLR_ED_BIT) == 0U) {
			errctlr |= FMU_ERRCTLR_ED_BIT;
		}

		/* Enable CE and UE to be reported separately */
		errctlr |= (FMU_ERRCTLR_CI_BIT | FMU_ERRCTLR_UI_BIT |
	...
	/*
	 * Enable MBIST REQ error safety mechanism for all blocks
	 *
	 * GICD, SMID 23
	 * PPI, SMID 10
	 * ITS, SMID 13
	 */
	if (blk_present_mask & BIT(FMU_BLK_GICD)) {
