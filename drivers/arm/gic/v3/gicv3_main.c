...
#define SAVE_GICD_EREGS(base, ctx, intr_num, reg, REG)			\
	do {								\
		for (unsigned int int_id = MIN_ESPI_ID; int_id < (intr_num);\
				int_id += (1U << REG##_SHIFT)) {	\
			ctx->gicd_##reg[(int_id - (MIN_ESPI_ID - MIN_SPI_ID))\
			>> REG##_SHIFT] = gicd_read_##reg((base), int_id);\
		}							\
	} while (false)
#else
#define SAVE_GICD_EREGS(base, ctx, intr_num, reg, REG)
#define RESTORE_GICD_EREGS(base, ctx, intr_num, reg, REG)
#endif /* GICV3_EXT_INTID */
...
void gicv3_distif_save(gicv3_dist_ctx_t * const dist_ctx)
{
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, igroupr, IGROUPR);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, isenabler, ISENABLER);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints,	ispendr, ISPENDR);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, isactiver, ISACTIVER);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, ipriorityr, IPRIORITYR);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, icfgr, ICFGR);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, igrpmodr, IGRPMODR);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, nsacr, NSACR);
	...
	SAVE_GICD_EREGS(gicd_base, dist_ctx, num_eints, irouter, IROUTER);
...
void gicv3_distif_init_restore(const gicv3_dist_ctx_t * const dist_ctx)
{
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, igroupr, IGROUPR);

	/* Restore GICD_IPRIORITYR for INTIDs 32 - 1019 */
	RESTORE_GICD_REGS(gicd_base, dist_ctx, num_ints, ipriorityr,
								IPRIORITYR);

	/* Restore GICD_IPRIORITYRE for INTIDs 4096 - 5119 */
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, ipriorityr,
								IPRIORITYR);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, icfgr, ICFGR);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, igrpmodr, IGRPMODR);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, nsacr, NSACR);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, irouter, IROUTER);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, isenabler,
								ISENABLER);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, ispendr, ISPENDR);
	...
	RESTORE_GICD_EREGS(gicd_base, dist_ctx, num_eints, isactiver,
								ISACTIVER);
