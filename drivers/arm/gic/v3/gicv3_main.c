				...
				unsigned int proc_self)
{
	...
	/*
	 * Each CPU discovers the base address of its corresponding
	 * Redistributor Interface frame. The memory for saving these addresses
	 * has to be allocated by the platform port.
	 */
	gicv3_rdistif_probe(plat_driver_data, gicr_frames_base_addrs,
				ARRAY_SIZE(gicr_frames_base_addrs), proc_self);
