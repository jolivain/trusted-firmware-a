...
#warning "GIC-600 Multichip driver is currently experimental and doesn't support full features."
				...
				unsigned int spi_id_max)
{
	...
	/* SPI interrupt id ranges from 32 to 960 for GIC600 */
	assert((spi_id_min >= GIC600_SPI_ID_MIN) &&
			(spi_id_min < GIC600_SPI_ID_MAX));
	assert((spi_id_max >= GIC600_SPI_ID_MIN) &&
			(spi_id_max < GIC600_SPI_ID_MAX));

	/* SPI interrupt ids should be a group of 32 */
	assert((spi_id_min % 32) == 0);
	assert((((spi_id_max + 1) % 32) == 0) ||
			(spi_id_max == GIC600_SPI_ID_MIN));
	...
	assert(((read_gicd_chipsr(base) & GICD_CHIPSR_RTS_MASK) >>
...
void gic600_multichip_init(struct gic600_multichip_data *multichip_data)
{
	 ...
	 */
	gicd_clr_ctlr(multichip_data->rt_owner_base,
		      CTLR_ENABLE_G0_BIT |
		      CTLR_ENABLE_G1S_BIT |
	...
	assert(((read_gicd_chipsr(multichip_data->rt_owner_base) &
	...
	/*
	 * Before setting GICD_CHIPRn register, ensure that SPIs are not
	 * overlapping between chips.
	 */
	for (i = 0; i < multichip_data->chip_count; i++)
		for (j = i+1; j < multichip_data->chip_count; j++)
			if ((multichip_data->spi_ids[i][0] ==
			     multichip_data->spi_ids[j][0]) ||
			     (multichip_data->spi_ids[i][1] ==
			      multichip_data->spi_ids[j][1])) {
				ERROR("Found overlapping SPIs while intializing"
						" GIC-600 Multichip\n");
				panic();
			}
