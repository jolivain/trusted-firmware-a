...
static void set_gicd_dchipr_rt_owner(uintptr_t base, unsigned int rt_owner)
{
	...
	if ((gicd_read_ctlr(base) &
	     (CTLR_ENABLE_G0_BIT | CTLR_ENABLE_G1S_BIT | CTLR_ENABLE_G1NS_BIT |
	      GICD_CTLR_RWP_BIT)) != 0) {
	...
	write_gicd_dchipr(base,
			  read_gicd_dchipr(base) |
 ...
 ******************************************************************************/
static void set_gicd_chipr_n(uintptr_t base, unsigned int chip_id,
			     uint64_t chip_addr, unsigned int spi_id_min,
			     unsigned int spi_id_max)
{
	...
	case IIDR_MODEL_ARM_GIC_600:
		...
		chipr_n_val = GICD_CHIPR_VALUE_GIC_600(chip_addr, spi_block_min,
						       spi_blocks);
		break;
	case IIDR_MODEL_ARM_GIC_700:
		...
		chipr_n_val = GICD_CHIPR_VALUE_GIC_700(chip_addr, spi_block_min,
						       spi_blocks);
		break;
	default:
	...
	if (((read_gicd_chipsr(base) & GICD_CHIPSR_RTS_MASK) >>
	     GICD_CHIPSR_RTS_SHIFT) != GICD_CHIPSR_RTS_STATE_CONSISTENT) {
 ...
 ******************************************************************************/
static void
gic600_multichip_validate_data(struct gic600_multichip_data *multichip_data)
{
 ...
 ******************************************************************************/
static void
gic700_multichip_validate_data(struct gic600_multichip_data *multichip_data)
{
	unsigned int i, spi_id_min, spi_id_max, blocks_of_32;
	...
	for (i = 0U; i < multichip_data->chip_count; i++) {
		...
		if (spi_id_max > GIC700_ESPI_ID_MIN) {
			blocks_of_32 =
				BLOCKS_OF_32(spi_id_min - GIC700_ESPI_ID_MIN,
...
void gic600_multichip_init(struct gic600_multichip_data *multichip_data)
{
	...
	if (((read_gicd_chipsr(multichip_data->rt_owner_base) &
	     ...
	     GICD_CHIPSR_RTS_SHIFT) != GICD_CHIPSR_RTS_STATE_DISCONNECTED) {
	...
	set_gicd_chipr_n(
		multichip_data->rt_owner_base, multichip_data->rt_owner,
		multichip_data->chip_addrs[multichip_data->rt_owner],
		multichip_data->spi_ids[multichip_data->rt_owner][SPI_MIN_INDEX],
		multichip_data
