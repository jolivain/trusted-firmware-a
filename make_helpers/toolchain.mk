...
# compiler and GCC as your linker, yet we would consider these to be part of the
# same AArch64 toolchain.
#

toolchains := host # Native architecture
toolchains += aarch32 # Arm AArch32
toolchains += aarch64 # Arm AArch64

toolchain-name-host := Host
...
#
# These variables define the default tools that we will search for in the
# absence of user-specified values (e.g. if `HOSTCC` is not specified, then the
# default host C compiler takes on the value of `host-cc-default`).
#
# We use a typical GNU toolchain by default, where the target triplet for the
# AArch32 and AArch64 target architectures is derived from the value of
# `CROSS_COMPILE` to maintain legacy behaviour:
#
#      CROSS_COMPILE=arm-none-eabi- make # arm-none-eabi-gcc
#      CROSS_COMPILE=aarch64-none-elf- make # aarch64-none-elf-gcc
#
...
# with the `HOST<TOOL-CLASS>` variables (e.g. `HOSTCC`), while the AArch32 and
# AArch64 toolchains are configured via the implicit variables, e.g. `CC`,
# `CPP`, `LD`, etc.
#
# We need to ensure that the non-host toolchains only pick up these implicit
# variables when they have not been defined automatically by Make, because
# otherwise they point to the default host toolchain. We do this by filtering
# out variables with an origin of `default`.
#
