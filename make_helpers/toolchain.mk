...
# solution to this, do not try to detect the toolchain if we are building
# exclusively with targets which do not use any toolchain tools.
#

toolchain-available := $(or $(MAKECMDGOALS),all)
toolchain-available := $(filter-out check% %clean %tool,$(toolchain-available))
toolchain-available := $(if $(toolchain-available),1)
...
ifneq ($(if $(toolchain-mk),,$(toolchain-available)),)
