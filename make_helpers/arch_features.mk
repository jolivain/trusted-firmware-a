...
# This file lists all the Architectural features and initializes
# and enables them based on Arch version.
...
# keep it enabled by default for arch64
ifeq (${ARCH},aarch64)
	ENABLE_SPE_FOR_NS		?=	2
# SPE is only supported on AArch64 so disable it on AArch32.
else ifeq (${ARCH},aarch32)
	ifdef ENABLE_SPE_FOR_NS
		$(error Error: ENABLE_SPE_FOR_NS is not supported for AArch32)
	else
		ENABLE_SPE_FOR_NS 	:=	0
	endif
endif
...
# SVE is only supported on AArch64 so disable it on AArch32.
	...
	ifdef ENABLE_SVE_FOR_NS
		$(error Error: ENABLE_SVE_FOR_NS is not supported for AArch32)
	else
		ENABLE_SVE_FOR_NS 	:=	0
	endif
endif
...
ifeq (${ARCH},aarch64)
	...
	ifdef ENABLE_TRBE_FOR_NS
		$(error Error: ENABLE_TRBE_FOR_NS is not supported for AArch32)
	else
		ENABLE_TRBE_FOR_NS 	:=	0
	endif
endif
