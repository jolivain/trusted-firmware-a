...
# [...]
#
# Similarly on gcc 11.3 it would return armv8.6-a if platform requested armv9.0-a
#
...
# so here we try to find best supported arch value and use that for compilation.
#
define best_avail_march
$(shell if $(CC) --target-help -march=foo 2>&1 | grep -o -P 'armv...-a|armv.-a' | grep armv$(ARM_ARCH_MAJOR) | tail -1; then echo $(1); fi)
endef

# This function is used to just return last march value supported by the given compiler.
#
# Example:
...
# is not found it should be ok to try with last known supported march value from compiler.
define last_match_march
...
ifeq (,$(findstring $(provided-march), $(call march_option, -march=${provided-march})))
    available-march	:= $(call best_avail_march)

ifeq (, $(available-march))
    available-march	:= $(call last_match_march)
endif
