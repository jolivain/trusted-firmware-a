			...
			unsigned int flags)
{
	...
	case FWU_SMC_IMAGE_COPY:
		SMC_RET1(handle, bl1_fwu_image_copy((uint32_t)x1, x2, (uint32_t)x3, (uint32_t)x4, flags));

	case FWU_SMC_IMAGE_AUTH:
		SMC_RET1(handle, bl1_fwu_image_auth((uint32_t)x1, x2, (uint32_t)x3, flags));

	case FWU_SMC_IMAGE_EXECUTE:
		SMC_RET1(handle, bl1_fwu_image_execute((uint32_t)x1, &handle, flags));
			...
			unsigned int flags)
{
	...
	if ((image_src == NULL) || (block_size == 0) ||
			...
			unsigned int flags)
{
	...
	if (desc->state == IMAGE_STATE_COPIED) {
		...
		if ((image_src == NULL) || (image_size == 0) ||
