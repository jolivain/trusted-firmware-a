			...
			unsigned int flags)
{
	...
	/* Save NS-EL1 system registers, only when CTX_INCLUDE_EL2_REGS=0 */
...
#if (!CTX_INCLUDE_EL2_REGS)
	/*
	 * Save the EL1 system registers of calling world, only when
	 * CTX_INCLUDE_EL2_REGS=0.
	 */
	cm_el1_sysregs_context_save(caller_sec_state);

	/*
	 * Restore the EL1 system registers of resuming world, only when
	 * CTX_INCLUDE_EL2_REGS=0.
	 */
...
static int bl1_fwu_sec_image_done(void **handle, unsigned int flags)
{
	 ...
	 * Just restore the Non-Secure context, when CTX_INCLUDE_EL2_REGS=0.
	 */
