...
.. command:: tfa_preprocess_source

Preprocess a file with the C preprocessor.

.. code-block:: cmake

  tfa_preprocess_source(<target> <source>)
...
.. code-block:: cmake
    ...
    tfa_preprocess_source(foo "bar.ld.S")
...
:command:`tfa_target_linker_script` command instead.
#]=======================================================================]

include_guard()

macro(tfa_preprocess_source target source)
    #
    ...
    get_filename_component(preprocessed_source "${source}" ABSOLUTE)
    file(RELATIVE_PATH preprocessed_source "${CMAKE_CURRENT_SOURCE_DIR}"
        "${preprocessed_source}")

    #
    # Now we know where the intermediate file is going, we strip the last
    # extension.
    #

    get_filename_component(preprocessed_source "${preprocessed_source}"
        NAME_WLE)
    ...
    #
    # Make the source path absolute so that we don't need need to care which
    ...
    foreach(config IN LISTS CMAKE_CONFIGURATION_TYPES CMAKE_BUILD_TYPE)
        ...
        separate_arguments(FLAGS UNIX_COMMAND
            "${CMAKE_C_FLAGS} ${CMAKE_C_FLAGS_${config}} -P")
