...
Creates a configuration option with more flexibility than offered by berd.
...
the :cmake:prop_cache:`TYPE <prop_cache:TYPE>` cache variable property for a
list of valid types. If ``<depends>`` is false, the type is forced to
``INTERNAL`` in order to hide it from the GUI.

``<default>`` represents the initial value of the created cache variable, unless
it has been overridden by :ref:`TFAConfigOptionOverride`. If ``<depends>``
becomes false at any point and ``<else>`` is unspecified, the value of the
configuration option is reset to this value. If ``<else>`` is specified then
...
function(tfa_config_option)
    ...
    set(_multi_args "STRINGS;DEPENDS;ELSE;FORCE")
