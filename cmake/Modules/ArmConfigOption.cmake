...
.. code:: cmake
    ...
    arm_config_option(
        ...
        [DEFAULT <default>] [DEPENDS <depends> [ELSE <else>]]
        [STRINGS <strings>... [FREEFORM]]
        [FORCE <force>] [ADVANCED] [HIDDEN])
...
1. If the ``TYPE`` argument has been provided, the type is ``<type>``.
2. Otherwise, if the ``STRINGS`` argument has been provided, the type is
   ``STRING``.
3. Otherwise, if the ``DEFAULT`` argument has been provided, the type is
   derived from ``<default>`` according to the following rules:

   1. If ``<default>`` matches the case-insensitive regular expression
      ``^(N|Y|NO|YES|OFF|ON|FALSE|TRUE)$``, the type is ``BOOL``.
...
against the string list is disabled, and the user may enter any value.
...
Note that if the ``STRINGS`` argument has been provided and there is only one
value in ``<strings>``, **and** the ``FREEFORM`` flag has *not* been provided,
...
.. code:: cmake
    ...
    arm_config_option(NAME XYZ DEFAULT Y ... STRINGS X) # "X"
...
A different default value can be provided using the ``DEFAULT`` argument:

.. code:: cmake

    arm_config_option(NAME XYZ ... DEFAULT TRUE) # TRUE
    arm_config_option(NAME XYZ ... DEFAULT "X") # "X"
...
predominantly useful for *internal* options - options where some part of the
build system may wish to override the value.

Hidden options cannot be modified from the UI, and so in that sense they are
similar to configuration options whose dependency always evaluates ``FALSE``.

.. code:: cmake

    arm_config_option(NAME XYZ ... HIDDEN) # Not visible in the UI
...
whether the option will be shown in :manual:`cmake-gui <manual:cmake-gui(1)>`
and :manual:`ccmake <manual:ccmake(1)>` or not.

For example, if you have a feature flag ``foo``, and you have a feature flag
``bar`` that only makes sense wher ``foo`` is enabled, you might use:
...
In the following advanced example, the size of the combined stackheap is
user-configurable, but it can be disabled by giving it a value of 0. When the
stackheap is disabled, it enables a separate heap and stack both with their own
sizes. Likewise, when either the individual heap or stacks are given a value of
0, the stack-heap is re-enabled:
...
.. code:: cmake
    ...
    arm_config_option(
        ...
        HELP "Stackheap size."
        DEFAULT 65536
        DEPENDS ((NOT DEFINED STACK_SIZE) OR (NOT STACK_SIZE GREATER 0)) OR
                ((NOT DEFINED HEAP_SIZE) OR (NOT HEAP_SIZE GREATER 0))
        ELSE 0)
    ...
    arm_config_option(
        ...
        DEFAULT 512
        DEPENDS (NOT STACKHEAP_SIZE GREATER 0)
        ELSE 0)
...
# Assert an invariant.
#
# Asserts an invariant, and fails the build if it's not met. This is used
# predominantly for parameter checking.
#

function(_arm_config_option_assert)
    set(options "")
    set(single-args "")
    set(multi-args "CONDITION;MESSAGE")

    cmake_parse_arguments(PARSE_ARGV 0 _ARM_CONFIG_OPTION_EXPECT
        "${options}" "${single-args}" "${multi-args}")

    if(NOT (${_ARM_CONFIG_OPTION_EXPECT_CONDITION}))
        message(FATAL_ERROR ${_ARM_CONFIG_OPTION_EXPECT_MESSAGE})
    endif()
endfunction()
...
function(arm_config_option)
    ...
    # the following rules:
    #
    # 1. If the `DEFAULT` argument was passed, infer the type from it.
    # 2. Otherwise, if the `ELSE` argument was passed, infer the type from it.
    # 3. Otherwise, the type is `BOOL`.
    #
    # The rules for inferring the type from the `DEFAULT` and `ELSE` arguments
    # are as follows:
    #
