...
.. code:: cmake
    ...
    arm_preprocess_source([INHIBIT_LINEMARKERS]
        OUTPUT <output> TARGET <target> SOURCE <source> LANGUAGE <language>)
...
.. note::
    ...
    The created target automatically inherits :variable:`CMAKE_<LANG>_FLAGS
...
function(arm_preprocess_source)
    ...
    cmake_parse_arguments(PARSE_ARGV 0 _ARM_PREPROCESS_SOURCE
        "${options}" "${single-args}" "${multi-args}")

    arm_assert(
        CONDITION DEFINED _ARM_PREPROCESS_SOURCE_TARGET
        MESSAGE "No value was given for the `TARGET` argument.")

    arm_assert(
        CONDITION DEFINED _ARM_PREPROCESS_SOURCE_SOURCE
        MESSAGE "No value was given for the `SOURCE` argument.")

    arm_assert(
        CONDITION DEFINED _ARM_PREPROCESS_SOURCE_OUTPUT
        MESSAGE "No value was given for the `OUTPUT` argument.")

    arm_assert(
        CONDITION DEFINED _ARM_PREPROCESS_SOURCE_LANGUAGE
        MESSAGE "No value was given for the `LANGUAGE` argument.")

    set(target "${_ARM_PREPROCESS_SOURCE_TARGET}")
    set(source "${_ARM_PREPROCESS_SOURCE_SOURCE}")
    set(output "${_ARM_PREPROCESS_SOURCE_OUTPUT}")
    set(language "${_ARM_PREPROCESS_SOURCE_LANGUAGE}")

    set(inhibit-linemarkers "${_ARM_PREPROCESS_SOURCE_INHIBIT_LINEMARKERS}")
    ...
    # is replaced with `${DEFINES}` (literally). We then need to do some string
    ...
    arm_assert(
        CONDITION DEFINED CMAKE_${language}_CREATE_PREPROCESSED_SOURCE
        MESSAGE "Unable to determine the preprocessor command line for the "
                "${language} language. Please report this to the developer.")

    set(command "${CMAKE_${language}_CREATE_PREPROCESSED_SOURCE}")
    ...
    cmake_path(NATIVE_PATH source NORMALIZE SOURCE)
    cmake_path(NATIVE_PATH output NORMALIZE PREPROCESSED_SOURCE)
    ...
    set(FLAGS "$<TARGET_PROPERTY:${target},COMPILE_OPTIONS>")
